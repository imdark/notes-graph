...
const {
  ...
  STATIC_IP_NAME,
  NOTESGRAPH_INDEXER_SEARCH_PROVIDER,
  NOTESGRAPH_INDEXER_SEARCH_ENDPOINT,
  NOTESGRAPH_INDEXER_SEARCH_API_KEY,
} = process.env;
...
const createHelmCommand = ({ isDryRun }) => {
  ...
  const indexerOptions = [
    `--set-string global.indexer.provider="${NOTESGRAPH_INDEXER_SEARCH_PROVIDER}"`,
    `--set-string global.indexer.endpoint="${NOTESGRAPH_INDEXER_SEARCH_ENDPOINT}"`,
    `--set-string global.indexer.apiKey="${NOTESGRAPH_INDEXER_SEARCH_API_KEY}"`,
  ];
  ...
  const deployCommand = [
    `helm upgrade --install notesgraph .github/helm/notesgraph`,
    `--namespace  ${namespace}`,
    `--set-string global.deployment.type="notesgraph"`,
