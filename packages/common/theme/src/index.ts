...
export const themeToVar = (themeName: keyof NotesGraphTheme) =>
  `--notesgraph-${camelToKebab(themeName)}` as keyof NotesGraphCssVariables;
...
export const baseTheme = {
  ...
  zoom: '1',
  scale: 'calc(1 / var(--notesgraph-zoom))',
...
export function cssVar(
  ...
  const varName = key.startsWith('--')
    ? key
    : themeToVar(key as keyof NotesGraphTheme);
