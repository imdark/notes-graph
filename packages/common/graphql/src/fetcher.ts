...
import { GraphQLError } from '@notesgraph/error';
...
export const gqlFetcherFactory = (
  ...
  const gqlFetch = async <Query extends GraphQLQuery>(
    ...
    // Surface real network failures (fetch rejected / request timed out) to the
    // console so they're visible and captured by the in-app error reporter.
    // Left untouched: GraphQL application errors (the server responded) and
    // user-cancelled requests. Telemetry uses a separate transport + debug-gated
    // logger, so it never reaches here.
    return ret.catch((err: unknown) => {
      const isGraphQLError = err instanceof GraphQLError;
      const isUserAbort =
        (err as { name?: string })?.name === 'AbortError' &&
        !!options.signal?.aborted;
      if (!isGraphQLError && !isUserAbort) {
        // oxlint-disable-next-line no-console
        console.error(
          `Network request failed: ${options.query.op} @ ${endpoint} — ${
