import type { ColumnDataType } from '@blocksuite/notesgraph/model';
...
export function parseBlock(
  ...
  try {
    switch (flavour) {
      ...
      case 'notesgraph:divider': {
        result.content = '\n---\n\n';
        break;
      }
      case 'notesgraph:list': {
      ...
      case 'notesgraph:embed-linked-doc':
      case 'notesgraph:embed-synced-doc': {
      ...
      case 'notesgraph:surface':
      case 'notesgraph:page':
      case 'notesgraph:note':
      case 'notesgraph:frame': {
        result.content = '';
        break;
      }
      case 'notesgraph:database': {
        ...
        const optionToTagHtml = (option: any) => {
          return `<span data-notesgraph-option data-value="${option.id}" data-option-color="${option.color}">${option.value}</span>`;
        };
  ...
  if (result.content && aiEditable && blockLevel === 2) {
    ...
    if (flavour === 'notesgraph:database' || placeholder) {
...
export const parsePageDoc = (ctx: ParserContext): ParsedDoc => {
  ...
  const maybePageBlock = Object.entries(yBlocks.toJSON()).findLast(
    ([_, b]) => b['sys:flavour'] === 'notesgraph:page'
  );
  ...
  if (!maybePageBlock) {
    ...
    rootBlock.children = rootBlock.children.filter(
      (block): block is BaseParsedBlock => block.flavour === 'notesgraph:note'
    );
