...
/// Block flavours used in NotesGraph documents.
...
pub enum BlockFlavour {
  ...
  EmbedIframe,
  Callout,
  /// A block markdown cannot represent (database/board, synced embed, …).
  /// Never produced by the markdown parser; the update differ preserves
  /// such blocks in place instead of deleting or failing on them.
  Opaque,
}
...
impl BlockFlavour {
  pub fn as_str(&self) -> &'static str {
    match self {
      BlockFlavour::Paragraph => "notesgraph:paragraph",
      BlockFlavour::List => "notesgraph:list",
      BlockFlavour::Code => "notesgraph:code",
      BlockFlavour::Divider => "notesgraph:divider",
      BlockFlavour::Image => "notesgraph:image",
      BlockFlavour::Table => "notesgraph:table",
      BlockFlavour::Bookmark => "notesgraph:bookmark",
      BlockFlavour::EmbedYoutube => "notesgraph:embed-youtube",
      BlockFlavour::EmbedIframe => "notesgraph:embed-iframe",
      BlockFlavour::Callout => "notesgraph:callout",
      BlockFlavour::Opaque => "opaque",
    }
  }

  pub fn from_str(value: &str) -> Option<Self> {
    match value {
      "notesgraph:paragraph" => Some(BlockFlavour::Paragraph),
      "notesgraph:list" => Some(BlockFlavour::List),
      "notesgraph:code" => Some(BlockFlavour::Code),
      "notesgraph:divider" => Some(BlockFlavour::Divider),
      "notesgraph:image" => Some(BlockFlavour::Image),
      "notesgraph:table" => Some(BlockFlavour::Table),
      "notesgraph:bookmark" => Some(BlockFlavour::Bookmark),
      "notesgraph:embed-youtube" => Some(BlockFlavour::EmbedYoutube),
      "notesgraph:embed-iframe" => Some(BlockFlavour::EmbedIframe),
      "notesgraph:callout" => Some(BlockFlavour::Callout),
      _ => None,
    }
  }
}
...
impl BlockSpec {
  /// Placeholder spec for a block the markdown pipeline can't represent —
  /// carried through the diff purely so the block is kept in place.
  pub(super) fn opaque() -> Self {
    BlockSpec {
      flavour: BlockFlavour::Opaque,
      block_type: None,
      text: Vec::new(),
      checked: None,
      language: None,
...
mod tests {
  ...
  fn test_from_block_map_paragraph() {
    let spec = spec_from_markdown("Plain paragraph.", "block-spec-paragraph", "notesgraph:paragraph");
  ...
  fn test_from_block_map_list_checked() {
    let spec = spec_from_markdown("- [x] Done", "block-spec-list", "notesgraph:list");
  ...
  fn test_from_block_map_image() {
    let spec = spec_from_markdown("![Alt](blob://image-id)", "block-spec-image", "notesgraph:image");
