...
use self::org_task::{collect_hashtags, collect_org_task_props};
use super::{
  ParseError,
...
fn is_known_unsupported_markdown_flavour(flavour: &str) -> bool {
  KNOWN_UNSUPPORTED_MARKDOWN_FLAVOURS.contains(&flavour) || flavour.starts_with("notesgraph:edgeless-")
}

fn is_edgeless_markdown_flavour(flavour: &str) -> bool {
  matches!(
    flavour,
    "notesgraph:surface" | "notesgraph:frame" | "notesgraph:surface-ref"
  ) || flavour.starts_with("notesgraph:edgeless-")
}
...
pub fn parse_page_doc(
  ...
  while let Some((_parent_block_id, block_id)) = walker.next() {
    ...
    match flavour.as_str() {
      "notesgraph:page" | "notesgraph:note" => {
        walker.enqueue_children(&block_id, block);
      }
      "notesgraph:attachment" | "notesgraph:transcription" | "notesgraph:callout" => {
      ...
      "notesgraph:paragraph" | "notesgraph:list" | "notesgraph:code" => {
        walker.enqueue_children(&block_id, block);
...
pub fn parse_doc_to_markdown(
  ...
  while let Some((_parent_block_id, block_id)) = walker.next() {
    ...
    if parent_flavour.as_deref() == Some("notesgraph:database") {
      continue;
    }
    ...
    if BlockFlavour::from_str(flavour.as_str()).is_none() && flavour.as_str() != "notesgraph:database" {
  ...
  while let Some((parent_block_id, block_id)) = walker.next() {
    ...
    if matches!(
      ...
      if let Some(text) = block.get("prop:text").and_then(|value| value.to_text()) {
        let database_name =
          if flavour == "notesgraph:paragraph" && parent_flavour.as_deref() == Some("notesgraph:database") {
        ...
        // task-entity fields, mirroring the TS reader
        // (packages/common/reader/src/reader.ts)
        let (tags, props) = collect_hashtags(&content);
        if !tags.is_empty() {
          info.tags = Some(tags);
        }
        if !props.is_empty() {
        ...
        info.created_at = block_created_at_iso(block);

        if flavour == "notesgraph:list" {
          let prop_type = get_string(block, "prop:type");
          let checked = block.get("prop:checked").and_then(|v| v.to_any()).and_then(|a| match a {
            Any::True => Some(true),
            Any::False => Some(false),
            _ => None,
          });

          if prop_type.as_deref() == Some("todo") {
            info.todo_status = Some(if checked == Some(true) { "done" } else { "todo" }.to_string());
            info.todo_trail = todo_trail_for(&block_id, &context.parent_lookup, &context.block_pool);
          }

          let org = collect_org_task_props(&content, &delta, prop_type.as_deref(), checked);
          info.org_status = org.org_status;
          info.scheduled_at = org.scheduled_at;
          info.deadline_at = org.deadline_at;
          info.started_at = org.started_at;
          info.closed_at = org.closed_at;
        }
    ...
    if matches!(
      flavour.as_str(),
      "notesgraph:embed-linked-doc" | "notesgraph:embed-synced-doc"
    ) {
    ...
    if flavour == "notesgraph:attachment" {
    ...
    if flavour == "notesgraph:database" {
...
/// Breadcrumb of ancestor block text for a nested todo, mirroring the TS
/// reader: walk up to 6 ancestors, stop at note/page/surface containers,
/// join with ' › ', truncate to 80 chars keeping the tail.
fn todo_trail_for(
  block_id: &str,
  parent_lookup: &HashMap<String, String>,
  ...
  let mut ancestor_id = parent_lookup.get(block_id).cloned();
  let mut guard = 0;
  while let Some(id) = ancestor_id {
    ...
    let Some(ancestor) = block_pool.get(&id) else {
      break;
    };
    let ancestor_flavour = get_flavour(ancestor).unwrap_or_default();
    if matches!(
      ancestor_flavour.as_str(),
      "notesgraph:note" | "notesgraph:page" | "notesgraph:surface"
    ) {
      break;
    }
    if let Some(text) = ancestor.get("prop:text").and_then(|value| value.to_text()) {
      let trimmed = text.to_string().trim().to_string();
      if !trimmed.is_empty() {
        parts.insert(0, trimmed);
      }
    }
    ancestor_id = parent_lookup.get(&id).cloned();
  }
  if parts.is_empty() {
    return None;
  }
  let mut trail = parts.join(" › ");
  let char_count = trail.chars().count();
  if char_count > 80 {
    let tail: String = trail.chars().skip(char_count - 79).collect();
    trail = format!("…{tail}");
  }
  Some(trail)
}

/// Block creation time (`meta:createdAt` epoch ms) as UTC ISO-8601 with
/// millisecond precision, byte-identical to JS `Date#toISOString`.
fn block_created_at_iso(block: &Map) -> Option<String> {
  let epoch_ms = block.get("prop:meta:createdAt").and_then(|v| v.to_any()).and_then(|a| match a {
  ...
  let datetime = chrono::DateTime::<chrono::Utc>::from_timestamp_millis(epoch_ms)?;
...
mod tests {
  ...
  fn test_database_cell_references() {
    ...
    page.insert("sys:flavour".into(), "notesgraph:page").unwrap();
    ...
    note.insert("sys:flavour".into(), "notesgraph:note").unwrap();
    ...
    db.insert("sys:flavour".into(), "notesgraph:database").unwrap();
  ...
  fn test_parse_doc_to_markdown_ai_editable_image_table() {
    ...
    assert!(md.contains("flavour=notesgraph:image"));
    ...
    assert!(
      result
        .known_unsupported_blocks
        .iter()
        .any(|block| block.ends_with(":notesgraph:note"))
    );
  ...
  fn test_parse_doc_to_markdown_with_edgeless_text_container() {
    ...
    surface.insert("sys:flavour".into(), "notesgraph:surface").unwrap();
    ...
    edgeless_text
      .insert("sys:flavour".into(), "notesgraph:edgeless-text")
      .unwrap();
    ...
    paragraph.insert("sys:flavour".into(), "notesgraph:paragraph").unwrap();
    paragraph
    ...
    assert!(
      result
        .known_unsupported_blocks
        .contains(&"surface:notesgraph:surface".to_string())
    );
    assert!(
      result
        .known_unsupported_blocks
        .contains(&"edgeless-text:notesgraph:edgeless-text".to_string())
    );
    assert!(result.unknown_blocks.is_empty());
  }
  ...
  fn test_parse_doc_to_markdown_collects_unknown_blocks() {
    ...
    mystery
      .insert("sys:flavour".into(), "notesgraph:custom-unknown")
      .unwrap();
    ...
    assert_eq!(
      ...
      vec!["mystery:notesgraph:custom-unknown".to_string()]
    );
  }
}
