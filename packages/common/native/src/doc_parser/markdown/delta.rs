...
fn delta_op_from_any(value: &Any) -> Option<DeltaOp> {
  ...
  if let Some(annotation) = org_annotation_text(&attributes) {
    return Some(DeltaOp {
      insert: DeltaInsert::Text(annotation),
      attributes: TextAttributes::new(),
    });
  }
...
/// Org-mode annotation chips (`orgStatus` / `orgTimestamp`) carry their
/// plain-text form (`[-]`, `DONE`, `SCHEDULED: <…>`) in the attribute while
/// the document text holds a single placeholder character. Serialize the
/// attribute's value so markdown readers (export, MCP agents) see the real
/// org annotation instead of a stray space.
fn org_annotation_text(attrs: &TextAttributes) -> Option<String> {
  for key in ["orgStatus", "orgTimestamp"] {
    if let Some(Any::String(value)) = attrs.get(key) {
...
fn build_delta_ops(delta: &[TextDeltaOp]) -> Vec<DeltaOp> {
  ...
  for op in delta {
    ...
    if let Some(annotation) = org_annotation_text(&attrs) {
      ops.push(DeltaOp {
        insert: DeltaInsert::Text(annotation),
        // the chip's own attribute must not leak into inline styling
        attributes: TextAttributes::new(),
      });
      continue;
    }
...
mod tests {
  ...
  #[test]
  fn test_org_annotation_chips_serialize_as_plain_org_text() {
    let mut status_attrs = TextAttributes::new();
    status_attrs.insert("orgStatus".into(), Any::String("[-]".into()));
    let mut stamp_attrs = TextAttributes::new();
    stamp_attrs.insert(
      "orgTimestamp".into(),
      Any::String("STARTED: [2026-07-17 Fri 09:30]".into()),
    );

    let delta = vec![
      TextDeltaOp::Insert {
        insert: TextInsert::Text(" ".into()),
        format: Some(status_attrs),
      },
      TextDeltaOp::Insert {
        insert: TextInsert::Text(" Fix the bug ".into()),
        format: None,
      },
      TextDeltaOp::Insert {
        insert: TextInsert::Text(" ".into()),
        format: Some(stamp_attrs),
      },
    ];

    let options = DeltaToMdOptions::new(None);
    let rendered = delta_to_markdown_with_options(&delta, &options, false);
    assert_eq!(rendered, "[-] Fix the bug STARTED: [2026-07-17 Fri 09:30]");
  }
  ...
  fn test_delta_to_inline_markdown_link() {
    ...
    let delta = vec![TextDeltaOp::Insert {
      insert: TextInsert::Text("NotesGraph".into()),
      format: Some(attrs),
    }];

    let options = DeltaToMdOptions::new(None);
    let rendered = delta_to_markdown_with_options(&delta, &options, false);
    assert_eq!(rendered, "[NotesGraph](https://example.com)");
  }
