...
import { DocRecordCache } from './doc-cache';
...
class StoreConsumer {
  ...
  private readonly docCache: DocRecordCache;
  ...
  constructor(
    ...
    this.docCache = new DocRecordCache(this.docStorage);
  ...
  private registerHandlers(consumer: OpConsumer<WorkerOps>) {
    ...
    consumer.registerAll({
      'docStorage.getDoc': (docId: string) => this.docCache.getDoc(docId),
      ...
      'docStorage.deleteDoc': (docId: string) => {
        // deletes don't emit doc-update events, so evict explicitly
        this.docCache.invalidate(docId);
        return this.docStorage.deleteDoc(docId);
      },
      ...
      'indexerSync.reindex': () => this.indexerSync.reindex(),
...
// Keep a store alive for a while after its last client disconnects. A page
// reload drops the refcount to zero for a moment before the new page reopens
// the same store; destroying it in between throws away connected storages,
// sync state, and the doc snapshot cache, forcing the reloaded page to pay
// the full cold-open latency again.
const STORE_CLOSE_GRACE_MS = 30_000;
  ...
  private registerHandlers(consumer: OpConsumer<WorkerManagerOps>) {
    consumer.registerAll({
      open: ({ port, key, closeKey, options }) => {
        ...
        storeRef.refCount++;
        if (storeRef.destroyTimeout) {
          clearTimeout(storeRef.destroyTimeout);
          storeRef.destroyTimeout = undefined;
        }
        ...
        this.storeDisposers.set(closeKey, () => {
          ...
          if (storeRef.refCount === 0) {
            storeRef.destroyTimeout = setTimeout(() => {
              if (storeRef.refCount === 0) {
