...
export interface IndexerSync {
  ...
  /** Force a full re-crawl of every doc (e.g. after indexing logic changes). */
...
export class IndexerSyncImpl implements IndexerSync {
  ...
  async reindex() {
    // Drop every known doc's indexed clock so the next crawl re-processes all
    // docs from scratch (clocks no longer match -> not skipped), then restart
    // the crawl loop.
    const docIds = new Set<string>([
      this.rootDocId,
      ...this.status.docsInRootDoc.keys(),
      ...this.status.docsInIndexer.keys(),
    ]);
