...
export class DocFrontend {
  ...
  /**
   * Emits whether the doc has been loaded from storage and is ready to use.
   *
   * Unlike `docState$` this is NOT throttled: docState$'s 1s leading+trailing
   * throttle swallows the leading `ready: false` window, holding the
   * `ready: true` transition back for up to a second. UI that gates first
   * render on readiness (e.g. the workspace skeleton) must use this instead.
   * It also deliberately avoids combining with the sync state, so the first
   * emission doesn't wait on a sync-engine round trip.
   */
  docReady$(docId: string): Observable<boolean> {
    return new Observable<boolean>(subscriber => {
      const next = () => {
        subscriber.next(this.status.readyDocs.has(docId));
      };
      next();
      return this.statusUpdatedSubject$.subscribe(updatedId => {
        if (updatedId === docId) next();
