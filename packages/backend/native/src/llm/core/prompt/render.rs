...
fn render_prompt_messages(
  ...
  render_context.retain(|key, _| !key.starts_with("notesgraph::"));
...
pub(super) fn create_prompt_builtins(params: &Map<String, Value>) -> Map<String, Value> {
  ...
  Map::from_iter([
    (
      "notesgraph::date".to_string(),
      Value::String(Local::now().format("%-m/%-d/%Y").to_string()),
    ),
    (
      "notesgraph::language".to_string(),
      Value::String(
        params
          .get("language")
          .and_then(Value::as_str)
    ...
    ("notesgraph::hasDocsRef".to_string(), Value::Bool(has_docs)),
    ("notesgraph::hasFilesRef".to_string(), Value::Bool(has_files)),
    ("notesgraph::hasSelected".to_string(), Value::Bool(has_selected)),
    ("notesgraph::hasCurrentDoc".to_string(), Value::Bool(has_current_doc)),
  ])
}
