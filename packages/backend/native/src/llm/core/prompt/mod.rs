...
mod tests {
  ...
  fn should_render_sections_and_current_item() {
    ...
    let rendered = render_tokens(
      &tokens,
      &[&json!({
        "links": ["https://notesgraph.com", "https://github.com/notesgraph/notesgraph"]
      })],
    );
  ...
  fn should_render_prompt_with_normalized_params_and_attachments() {
    let response = llm_render_prompt(
      serde_json::from_value::<PromptRenderContract>(json!({
        ...
        "renderParams": {
          "attachments": ["https://notesgraph.com/example.jpg"],
          "content": "hello world"
        }
      }))
      .unwrap(),
    ...
    let response = llm_render_prompt(
      serde_json::from_value::<PromptRenderContract>(json!({
        "messages": [
          {
            "role": "system",
            "content": "{{notesgraph::language}}|{{tags}}|{{obj}}|{{#links}}- {{.}}\n{{/links}}"
          }
        ],
        "templateParams": {},
        "renderParams": {
          "language": "French",
          "notesgraph::language": "ignored",
          "links": ["https://notesgraph.com", "https://github.com/notesgraph/notesgraph"],
          "obj": { "hello": "world" },
          "tags": ["a", "b"]
        }
    ...
    assert_eq!(
      ...
      json!({
        "messages": [
          {
            "role": "system",
            "content": "French|a,b|[object Object]|- https://notesgraph.com\n- https://github.com/notesgraph/notesgraph\n",
            "params": {
              "language": "French",
              "notesgraph::language": "ignored",
              "links": ["https://notesgraph.com", "https://github.com/notesgraph/notesgraph"],
  ...
  fn should_render_session_prompt_by_merging_latest_user_content() {
    let response = llm_render_session_prompt(
      serde_json::from_value::<PromptSessionContract>(json!({
        ...
        "turns": [
          { "role": "user", "content": "hello", "attachments": ["https://notesgraph.com/hello.png"] }
        ],
        "renderParams": {},
        "maxTokenSize": 1000
      }))
      ...
      json!({
        "messages": [
          { "role": "system", "content": "answer briefly", "params": { "content": "hello" } },
          {
            "role": "user",
            "content": "hello",
            "attachments": ["https://notesgraph.com/hello.png"],
