...
impl Runtime {
  ...
  fn run(&mut self) -> Result<ActionRuntimeOutput> {
    ...
    let mut executor = NotesGraphActionStepExecutor::new(&self.control, attachments.clone());
...
struct NotesGraphActionStepExecutor<'a> {
  #[cfg(test)]
...
impl<'a> NotesGraphActionStepExecutor<'a> {
...
impl RecipeStepExecutor for NotesGraphActionStepExecutor<'_> {
  fn execute_step(
    &mut self,
    step: &RecipeStepExecution,
    input: Option<Value>,
