...
// - NotesGraph-native-owned: model-registry projection and transcript/action
//   product contracts.
...
fn schema_by_name(name: &str) -> Option<Value> {
  match name {
    ...
    >()),
    // NotesGraph-native-owned N-API projection over adapter model registry/matcher
    ...
    // NotesGraph-native-owned prompt facade over adapter prompt DTOs/catalog
    ...
    // NotesGraph-native-owned product transcript contracts
