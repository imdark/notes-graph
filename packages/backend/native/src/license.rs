...
const NOTESGRAPH_PRO_ENDPOINT: &str = "https://app.notesgraph.com";
const NOTESGRAPH_PRO_HOST: &str = "app.notesgraph.pro";
const NOTESGRAPH_PRO_REQUEST_TIMEOUT_MS: u32 = 10_000;
const NOTESGRAPH_PRO_MAX_BYTES: u32 = 1024 * 1024;
const ECH_DNS_QUERY_TIMEOUT_MS: u32 = 5_000;

static NOTESGRAPH_PRO_ECH_CONFIG: OnceLock<Mutex<Option<Vec<u8>>>> = OnceLock::new();
...
impl Task for AsyncCreateCustomerPortalTask {
  ...
  fn compute(&mut self) -> Result<Self::Output> {
    let response = match notesgraph_pro_request(
      ...
      Err(_) => {
        return Ok(PortalResponse {
          url: None,
          error: Some(internal_notesgraph_pro_error()),
        });
      }
    };
    if let Some(error) = notesgraph_pro_error(&response) {
      return Ok(PortalResponse {
        url: None,
        error: Some(error),
      });
...
fn license_info(
  ...
  let response = match notesgraph_pro_request(path, method, headers, body) {
    Ok(response) => response,
    Err(_) => {
...
fn command(
  ...
  Ok(CommandResponse {
    error: notesgraph_pro_error(&response),
  })
}

fn notesgraph_pro_request(
  path: &str,
  method: safefetch::SafeFetchMethod,
  headers: Option<HashMap<String, String>>,
  body: Option<Vec<u8>>,
) -> AnyResult<safefetch::SafeFetchResponse> {
  let url = Url::parse(NOTESGRAPH_PRO_ENDPOINT)
    .context("invalid notesgraph pro endpoint")?
    .join(path)
    .context("invalid notesgraph pro path")?;
  ...
  safefetch::safe_fetch(&safefetch::SafeFetchRequest {
    ...
    body,
    timeout_ms: Some(NOTESGRAPH_PRO_REQUEST_TIMEOUT_MS),
    max_redirects: Some(3),
    max_bytes: Some(NOTESGRAPH_PRO_MAX_BYTES),
    allowed_headers: Some(vec![
      "authorization".to_string(),
      "content-type".to_string(),
      "x-validate-key".to_string(),
    ]),
    allowed_hosts: Some(vec![NOTESGRAPH_PRO_HOST.to_string()]),
    allow_http: Some(false),
    allow_private_target_origin: None,
    ech_config_list: Some(notesgraph_pro_ech_config()?),
  })
}
...
fn notesgraph_pro_error(response: &safefetch::SafeFetchResponse) -> Option<LicenseError> {
  if (200..300).contains(&response.status) {
    return None;
  }
  let body = String::from_utf8_lossy(&response.body).to_string();
  if serde_json::from_str::<serde_json::Value>(&body).is_err() {
    return Some(internal_notesgraph_pro_error());
  }
  Some(LicenseError {
...
fn internal_notesgraph_pro_error() -> LicenseError {
  LicenseError {
    status: 500,
    body: serde_json::json!({
      ...
      "message": "Failed to contact with https://app.notesgraph.com",
      "data": null,
    })
    .to_string(),
  }
...
fn parse_body<T: DeserializeOwned>(response: &safefetch::SafeFetchResponse) -> AnyResult<T> {
  serde_json::from_slice(&response.body).context("invalid notesgraph pro response")
}
...
fn notesgraph_pro_ech_config() -> AnyResult<Vec<u8>> {
  let cache = NOTESGRAPH_PRO_ECH_CONFIG.get_or_init(|| Mutex::new(None));
  {
