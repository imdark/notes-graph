...
CREATE OR REPLACE FUNCTION notesgraph_permission_legacy_workspace_role(role_value INTEGER)
RETURNS TEXT
LANGUAGE SQL
IMMUTABLE
AS $$
...
CREATE OR REPLACE FUNCTION notesgraph_permission_legacy_doc_role(role_value INTEGER)
RETURNS TEXT
LANGUAGE SQL
IMMUTABLE
AS $$
...
CREATE OR REPLACE FUNCTION notesgraph_permission_legacy_default_doc_role(role_value INTEGER)
RETURNS TEXT
LANGUAGE SQL
IMMUTABLE
AS $$
...
CREATE OR REPLACE FUNCTION notesgraph_permission_workspace_invitation_state(status_value "WorkspaceMemberStatus")
RETURNS TEXT
LANGUAGE SQL
IMMUTABLE
AS $$
...
CREATE OR REPLACE FUNCTION notesgraph_permission_projection_enabled()
RETURNS BOOLEAN
LANGUAGE SQL
STABLE
AS $$
  SELECT COALESCE(current_setting('notesgraph.permission_projection.enabled', true), 'on') = 'on'
$$;

CREATE OR REPLACE FUNCTION notesgraph_permission_sync_origin()
RETURNS TEXT
LANGUAGE SQL
STABLE
AS $$
  SELECT NULLIF(current_setting('notesgraph.permission_sync_origin', true), '')
$$;

CREATE OR REPLACE FUNCTION notesgraph_permission_should_project_from_legacy()
RETURNS BOOLEAN
LANGUAGE plpgsql
VOLATILE
AS $$
DECLARE
  origin TEXT;
BEGIN
  IF NOT notesgraph_permission_projection_enabled() THEN
    RETURN FALSE;
  END IF;

  origin := notesgraph_permission_sync_origin();
  IF origin IS NULL THEN
    PERFORM set_config('notesgraph.permission_sync_origin', 'legacy', true);
    RETURN TRUE;
  END IF;
  ...
  RAISE EXCEPTION 'Invalid notesgraph.permission_sync_origin %', origin;
END
$$;

CREATE OR REPLACE FUNCTION notesgraph_permission_should_project_from_new()
RETURNS BOOLEAN
LANGUAGE plpgsql
VOLATILE
AS $$
...
BEGIN
  ...
  origin := notesgraph_permission_sync_origin();
  IF origin IS NULL THEN
    PERFORM set_config('notesgraph.permission_sync_origin', 'new', true);
    RETURN TRUE;
  END IF;
...
CREATE OR REPLACE FUNCTION notesgraph_permission_new_workspace_role(role_value TEXT)
RETURNS SMALLINT
LANGUAGE SQL
IMMUTABLE
AS $$
...
CREATE OR REPLACE FUNCTION notesgraph_permission_new_workspace_source(source_value TEXT)
...
CREATE OR REPLACE FUNCTION notesgraph_permission_new_invitation_status(state_value TEXT)
...
CREATE OR REPLACE FUNCTION notesgraph_permission_new_doc_role(role_value TEXT)
RETURNS SMALLINT
LANGUAGE SQL
IMMUTABLE
AS $$
...
CREATE OR REPLACE FUNCTION notesgraph_permission_projection_error_category(
  sql_state TEXT,
  message TEXT
)
RETURNS TEXT
...
AS $$
  SELECT CASE
    ...
    WHEN message LIKE '%notesgraph.permission_sync_origin%' THEN 'projection_recursion_guard_missing'
    ELSE 'unknown'
  END
$$;

CREATE OR REPLACE FUNCTION notesgraph_permission_lock_workspace(workspace_id VARCHAR)
RETURNS VOID
LANGUAGE SQL
VOLATILE
AS $$
...
CREATE OR REPLACE FUNCTION notesgraph_permission_lock_doc(workspace_id VARCHAR, doc_id VARCHAR)
RETURNS VOID
LANGUAGE SQL
VOLATILE
AS $$
...
CREATE OR REPLACE FUNCTION notesgraph_permission_project_workspace_user_permission()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  IF NOT notesgraph_permission_should_project_from_legacy() THEN
  ...
  projected_role := notesgraph_permission_legacy_workspace_role(NEW.type);
  ...
  projected_state := notesgraph_permission_workspace_invitation_state(NEW.status);
...
EXCEPTION WHEN OTHERS THEN
  RAISE EXCEPTION 'permission_projection_error:%:%', notesgraph_permission_projection_error_category(SQLSTATE, SQLERRM), SQLERRM;
END
$$;

CREATE OR REPLACE FUNCTION notesgraph_permission_project_workspace_page()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  default_role := notesgraph_permission_legacy_default_doc_role(NEW."defaultRole");
...
CREATE OR REPLACE FUNCTION notesgraph_permission_project_workspace_page_user_permission()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  projected_role := notesgraph_permission_legacy_doc_role(NEW.type);
  IF projected_role IS NULL THEN
...
CREATE OR REPLACE FUNCTION notesgraph_permission_project_workspace_policy()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION notesgraph_permission_project_new_workspace_member()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  IF NOT notesgraph_permission_should_project_from_new() THEN
    IF TG_OP = 'DELETE' THEN
      RETURN OLD;
    END IF;
    RETURN NEW;
  ...
  legacy_role := notesgraph_permission_new_workspace_role(NEW.role);
  ...
  VALUES (
    ...
    notesgraph_permission_new_workspace_source(NEW.source),
    NEW.created_at,
    NEW.updated_at
  )
...
CREATE OR REPLACE FUNCTION notesgraph_permission_project_new_workspace_invitation()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  legacy_status := notesgraph_permission_new_invitation_status(NEW.status);
...
CREATE OR REPLACE FUNCTION notesgraph_permission_project_new_workspace_access_policy()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
...
CREATE OR REPLACE FUNCTION notesgraph_permission_project_new_doc_access_policy()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  IF TG_OP = 'DELETE' THEN
    legacy_default_role := notesgraph_permission_new_doc_role(
      COALESCE(
...
CREATE OR REPLACE FUNCTION notesgraph_permission_project_new_doc_grant()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
  ...
  legacy_role := notesgraph_permission_new_doc_role(NEW.role);
...
CREATE TRIGGER "notesgraph_permission_project_workspace_user_permission"
AFTER INSERT OR UPDATE OR DELETE ON "workspace_user_permissions"
FOR EACH ROW EXECUTE FUNCTION notesgraph_permission_project_workspace_user_permission();

CREATE TRIGGER "notesgraph_permission_project_workspace_page"
AFTER INSERT OR UPDATE OR DELETE ON "workspace_pages"
FOR EACH ROW EXECUTE FUNCTION notesgraph_permission_project_workspace_page();

CREATE TRIGGER "notesgraph_permission_project_workspace_page_user_permission"
AFTER INSERT OR UPDATE OR DELETE ON "workspace_page_user_permissions"
FOR EACH ROW EXECUTE FUNCTION notesgraph_permission_project_workspace_page_user_permission();

CREATE TRIGGER "notesgraph_permission_project_workspace_policy"
AFTER INSERT OR UPDATE OR DELETE ON "workspaces"
FOR EACH ROW EXECUTE FUNCTION notesgraph_permission_project_workspace_policy();

CREATE TRIGGER "notesgraph_permission_project_new_workspace_member"
AFTER INSERT OR UPDATE OR DELETE ON "workspace_members"
FOR EACH ROW EXECUTE FUNCTION notesgraph_permission_project_new_workspace_member();

CREATE TRIGGER "notesgraph_permission_project_new_workspace_invitation"
AFTER INSERT OR UPDATE OR DELETE ON "workspace_invitations"
FOR EACH ROW EXECUTE FUNCTION notesgraph_permission_project_new_workspace_invitation();

CREATE TRIGGER "notesgraph_permission_project_new_workspace_access_policy"
AFTER INSERT OR UPDATE OR DELETE ON "workspace_access_policies"
FOR EACH ROW EXECUTE FUNCTION notesgraph_permission_project_new_workspace_access_policy();

CREATE TRIGGER "notesgraph_permission_project_new_doc_access_policy"
AFTER INSERT OR UPDATE OR DELETE ON "doc_access_policies"
FOR EACH ROW EXECUTE FUNCTION notesgraph_permission_project_new_doc_access_policy();

CREATE TRIGGER "notesgraph_permission_project_new_doc_grant"
AFTER INSERT OR UPDATE OR DELETE ON "doc_grants"
FOR EACH ROW EXECUTE FUNCTION notesgraph_permission_project_new_doc_grant();
