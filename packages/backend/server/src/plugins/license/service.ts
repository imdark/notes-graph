...
export class LicenseService {
  ...
  private remoteLicense(response: LicenseResponse) {
    ...
    if (!response.license) {
      throw new InternalServerError('Invalid NotesGraph Pro license response.');
    }
  ...
  private remotePortal(response: PortalResponse) {
    this.throwRemoteLicenseError(response.error);
    if (!response.url) {
      throw new InternalServerError('Invalid NotesGraph Pro portal response.');
    }
  ...
  private throwRemoteLicenseError(
    ...
    try {
      ...
      throw new InternalServerError(
        'Failed to contact with https://app.notesgraph.com'
      );
    }
  }
  ...
  private resolveWorkspaceTeamLicense(workspaceId: string | null, buf: Buffer) {
    if (!this.crypto.NotesGraphProPublicKey) {
      throw new InternalServerError(
        'License public key is not loaded. Please contact with NotesGraph support.'
      );
    }

    if (!this.crypto.NotesGraphProLicenseAESKey) {
      throw new InternalServerError(
        'License AES key is not loaded. Please contact with NotesGraph support.'
      );
    }
    ...
    const resolved = resolveEntitlementV1({
      ...
      signedPayload: buf,
      publicKey: this.crypto.NotesGraphProPublicKey.toString(),
      licenseAesKey: this.crypto.NotesGraphProLicenseAESKey.toString('hex'),
    ...
    if (!resolved.valid && resolved.status === 'expired') {
      throw new InvalidLicenseToActivate({
        reason:
          'License file has expired. Please contact with NotesGraph support to fetch a latest one.',
      });
    }
