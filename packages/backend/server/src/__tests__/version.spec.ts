...
test('should passthrough if version check is not enabled', async t => {
  ...
  res = await app.GET('/guarded/test').set('x-notesgraph-version', '0.20.0');

  t.is(res.status, 200);

  res = await app.GET('/guarded/test').set('x-notesgraph-version', 'invalid');
...
test('should enforce hard required version when version range is invalid', async t => {
  ...
  let res = await app
    .GET('/guarded/test')
    .set('x-notesgraph-version', '0.25.0');
...
test('should pass if client version is allowed', async t => {
  ...
  res = await app.GET('/guarded/test').set('x-notesgraph-version', '0.26.0');
  ...
  res = await app.GET('/guarded/test').set('x-notesgraph-version', '0.25.0');
...
test('should tell downgrade if client version is higher than allowed', async t => {
  ...
  let res = await app
    .GET('/guarded/test')
    .set('x-notesgraph-version', '0.27.0');
...
test('should test prerelease version', async t => {
  ...
  let res = await app
    .GET('/guarded/test')
    .set('x-notesgraph-version', '0.25.0-canary.1');
  ...
  res = await app
    .GET('/guarded/test')
    .set('x-notesgraph-version', '0.26.0-canary.1');

  t.is(res.status, 200);

  res = await app
    .GET('/guarded/test')
    .set('x-notesgraph-version', '0.26.0-beta.2');
...
test('should allow recent canary date version in canary namespace', async t => {
  ...
  try {
    const res = await app
      .GET('/guarded/test')
      .set('x-notesgraph-version', makeCanaryDateVersion(new Date(), '015'));
...
test('should reject old canary date version in canary namespace', async t => {
  ...
  try {
    ...
    const res = await app
      .GET('/guarded/test')
      .set('x-notesgraph-version', oldVersion);
