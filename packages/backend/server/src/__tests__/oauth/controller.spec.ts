  ...
  const app = await createTestingApp({
    imports: [
      ConfigModule.override({
        ...
        server: {
          hosts: ['localhost', 'test.notesgraph.dev'],
          https: true,
        },
      }),
      AppModule,
...
test.beforeEach(async t => {
  ...
  t.context.u1 = await t.context.auth.signUp('u1@notesgraph.com', '1');
});
...
test('should be able to redirect to oauth provider with multiple hosts', async t => {
  ...
  t.is(
    redirect.searchParams.get('redirect_uri'),
    'https://test.notesgraph.dev/oauth/callback'
  );
  t.is(redirect.searchParams.get('response_type'), 'code');
  t.is(redirect.searchParams.get('prompt'), 'select_account');
  t.truthy(redirect.searchParams.get('state'));
...
test('should record sign in client version from oauth preflight state', async t => {
  ...
  const preflightRes = await app
    .POST('/api/oauth/preflight')
    .set('x-notesgraph-version', '0.25.3')
    .send({ provider: 'Google', client_nonce: 'test-nonce' })
    .expect(HttpStatus.OK);
  ...
  Sinon.stub(provider, 'getUser').resolves({
    id: '1',
    email: 'oauth-version@notesgraph.com',
    avatarUrl: 'avatar',
  });
...
function mockOidcProvider(
  ...
  Sinon.stub(
    ...
    userinfo_endpoint: 'https://oidc.notesgraph.dev/userinfo',
  }));
  Sinon.stub(
...
function createOidcRegistrationHarness(config?: {
  ...
  const notesgraphConfig = {
    server: {
      externalUrl: 'https://notesgraph.example',
      host: 'localhost',
      path: '',
      https: true,
      hosts: [],
    ...
    oauth: {
      providers: {
        oidc: {
          ...
          issuer: config?.issuer ?? 'https://issuer.notesgraph.dev',
          args: {},
        },
      },
    },
  };
  const provider = new OIDCProvider(new URLHelper(notesgraphConfig as any));

  (provider as any).factory = factory;
  (provider as any).NotesGraphConfig = notesgraphConfig;
...
test('should be able to sign up with oauth', async t => {
  ...
  const clientNonce = mockOAuthProvider(app, 'u2@notesgraph.com');
  ...
  t.is(sessionUser!.email, 'u2@notesgraph.com');
  ...
  t.truthy(user);
  t.is(user!.email, 'u2@notesgraph.com');
...
test('should be able to sign up with oauth and client_nonce', async t => {
  ...
  const userEmail = `${clientNonce}@notesgraph.com`;
...
test('should be able to fullfil user with oauth sign in', async t => {
  ...
  const u3 = await app.createUser('u3@notesgraph.com');
...
test('github oauth should resolve private email from emails api', async t => {
  ...
  const email = 'github-private@notesgraph.com';
  ...
  fetchJson.onSecondCall().resolves([
    { email: 'unverified@notesgraph.com', primary: true, verified: false },
...
test('github oauth should reject responses without a verified email', async t => {
  ...
  fetchJson
    .onSecondCall()
    .resolves([
      { email: 'private@notesgraph.com', primary: true, verified: false },
    ]);
...
test('oidc should accept email from id token when userinfo email is missing', async t => {
  ...
  mockOidcProvider(provider, {
    idTokenClaims: {
      sub: 'oidc-user',
      email: 'oidc-id-token@notesgraph.com',
      name: 'OIDC User',
    },
    userinfo: {
      sub: 'oidc-user',
  ...
  t.is(user.email, 'oidc-id-token@notesgraph.com');
...
test('oidc should resolve custom email claim from userinfo', async t => {
  ...
  mockOidcProvider(provider, {
    ...
    userinfo: {
      sub: 'oidc-user',
      mail: 'oidc-userinfo@notesgraph.com',
  ...
  t.is(user.id, 'oidc-user');
  t.is(user.email, 'oidc-userinfo@notesgraph.com');
...
test('oidc should resolve custom email claim from id token', async t => {
  ...
  mockOidcProvider(provider, {
    ...
    idTokenClaims: {
      sub: 'oidc-user',
      mail: 'oidc-custom-id-token@notesgraph.com',
      mail_verified: 'true',
    },
    userinfo: {
      sub: 'oidc-user',
  ...
  t.is(user.id, 'oidc-user');
  t.is(user.email, 'oidc-custom-id-token@notesgraph.com');
});
...
test('oidc should not fall back to default email claim when custom claim is configured', async t => {
  ...
  mockOidcProvider(provider, {
    ...
    idTokenClaims: {
      sub: 'oidc-user',
      email: 'fallback@notesgraph.com',
    },
    userinfo: {
      sub: 'oidc-user',
      email: 'userinfo-fallback@notesgraph.com',
    },
  });
...
test('oidc discovery should remove oauth feature on failure and restore it after backoff retry succeeds', async t => {
  ...
  fetchStub
    ...
    .resolves(
      new Response(
        JSON.stringify({
          authorization_endpoint: 'https://issuer.notesgraph.dev/auth',
          token_endpoint: 'https://issuer.notesgraph.dev/token',
          userinfo_endpoint: 'https://issuer.notesgraph.dev/userinfo',
          issuer: 'https://issuer.notesgraph.dev',
          jwks_uri: 'https://issuer.notesgraph.dev/jwks',
        }),
        {
          status: 200,
