  ...
  const actual = {
    whitelist: {
      proMonthly: resolveProductMapping({
        productId: 'app.notesgraph.pro.Monthly',
      }),
      proAnnual: resolveProductMapping({
        productId: 'app.notesgraph.pro.Annual',
      }),
      aiAnnual: resolveProductMapping({
        productId: 'app.notesgraph.pro.ai.Annual',
      }),
    },
    override: {
...
test('should map via entitlement+duration when productId not whitelisted (P1M/P1Y only)', async t => {
  ...
  const Unsupported = {
    ...
    productId: 'app.notesgraph.pro.Quarterly',
    store: 'app_store',
    willRenew: true,
...
test('user subscriptions preserve provider trialing status', async t => {
  ...
  const trialUser = await models.user.create({
    email: `${Date.now()}-trial-status@notesgraph.com`,
  });
