...
function initTestStaticFiles(staticPath: string) {
  const files = {
    'selfhost.html': `<!DOCTYPE html><html><body>NotesGraph</body><script src="main.a.js"/></html>`,
    'main.a.js': `const name = 'notesgraph'`,
    'admin/selfhost.html': `<!DOCTYPE html><html><body>NotesGraph Admin</body><script src="/admin/main.b.js"/></html>`,
    'admin/main.b.js': `const name = 'notesgraph-admin'`,
    'mobile/selfhost.html': `<!DOCTYPE html><html><body>NotesGraph mobile</body><script src="/mobile/main.c.js"/></html>`,
    'mobile/main.c.js': `const name = 'notesgraph-mobile'`,
  };
...
test('should always return static asset files', async t => {
  let res = await request(t.context.app.getHttpServer())
    .get('/main.a.js')
    .expect(200);
  t.is(res.text, "const name = 'notesgraph'");

  res = await request(t.context.app.getHttpServer())
    .get('/admin/main.b.js')
    .expect(200);
  t.is(res.text, "const name = 'notesgraph-admin'");

  res = await request(t.context.app.getHttpServer())
    .get('/main.c.js')
    .expect(200);
  t.is(res.text, "const name = 'notesgraph-mobile'");

  await t.context.db.user.create({
    data: {
      name: 'test',
      email: 'test@notesgraph.com',
    },
  });
...
test('should allow visiting setup page if not initialized', async t => {
  ...
  t.true(res.text.includes('NotesGraph Admin'));
});
...
test.skip('should return web assets if visited by mobile', async t => {
  ...
  t.true(res.text.includes('NotesGraph mobile'));
});
