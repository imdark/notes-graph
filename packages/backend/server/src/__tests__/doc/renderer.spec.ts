...
function initTestStaticFiles(staticPath: string) {
  const files = {
    ...
    'assets-manifest.json': JSON.stringify({
      js: ['main.a.js'],
      css: [],
      publicPath: 'https://app.notesgraph.com/',
      gitHash: '',
      description: '',
    }),
...
test.before(async t => {
  const staticPath = new Package('@notesgraph/server').join('static').value;
...
test('should render correct html', async t => {
  ...
  t.true(
    res.text.includes(
      `<script src="https://app.notesgraph.com/main.a.js" crossorigin></script>`
    )
  );
});
...
test.skip('should render correct mobile html', async t => {
  ...
  t.true(
    res.text.includes(
      `<script src="https://app.notesgraph.com/main.c.js" crossorigin></script>`
    )
  );
});
