...
import { ProjectRoot } from '@notesgraph-tools/utils/path';
...
test.beforeEach(async t => {
  ...
  const user = await auth.signUp(
    `test-${randomUUID()}@notesgraph.com`,
    '123456'
  );
  userId = user.id;
...
test('capability runtime should require explicit structured schema contract', async t => {
  ...
  const error = await t.throwsAsync(() =>
    runtime.generateStructuredValue(
      { modelId: 'gpt-5-mini' },
      singleUserPromptMessages('Summarize NotesGraph.'),
      {
        responseSchemaJson: {
          type: 'object',
          properties: { summary: { type: 'string' } },
...
test('should be able to fork chat session', async t => {
  ...
  const newUser = await auth.signUp('darksky.1@notesgraph.com', '123456');
...
test('should merge latest user turn content and attachments into prompt', async t => {
  ...
  for (const testCase of [
    ...
    },
    {
      ...
      message: {
        attachments: ['https://notesgraph.com/example.jpg'] as string[],
      },
      project: (messages: { attachments?: unknown }[]) =>
        messages.map(({ attachments }) => attachments),
      expected: [undefined, ['https://notesgraph.com/example.jpg']],
    },
    {
      title: 'empty message',
      message: {},
