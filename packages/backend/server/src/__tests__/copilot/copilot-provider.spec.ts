...
const actions = [
  {
    ...
    promptName: ['Chat With NotesGraph AI'],
...
The term **“CRDT”** was first introduced by Marc Shapiro, Nuno Preguiça, Carlos Baquero, and Marek Zawirski in their 2011 paper titled “Conflict-free Replicated Data Types” (published in the _Stabilization, Safety, and Security of Distributed Systems (SSS)_ conference). They formalized two families of CRDTs—state-based (“Convergent Replicated Data Types” or CvRDTs) and operation-based (“Commutative Replicated Data Types” or CmRDTs)—and proved their convergence properties under asynchronous, unreliable networks.
  ...
  },
  {
    ...
    promptName: ['Chat With NotesGraph AI'],
    messages: singleUserPromptMessages('what is NotesGraph AI?', {
      params: {
        files: [
          {
            blobId: 'todo_md',
  ...
  },
  {
    ...
    messages: singleUserPromptMessages(
      'In one short sentence, explain what NotesGraph AI is and mention NotesGraph by name.'
    ),
    config: { model: 'gemini-2.5-flash' },
    verifier: (t: ExecutionContext<Tester>, result: string) => {
      assertNotWrappedInCodeBlock(t, result);
      t.assert(
        result.toLowerCase().includes('notesgraph'),
        'should mention NotesGraph'
      );
    },
  ...
  },
  {
    ...
    promptName: ['Chat With NotesGraph AI'],
    messages: singleUserPromptMessages(
      'Respond with one short sentence about NotesGraph AI and mention NotesGraph by name.'
    ),
    config: { model: 'gemini-2.5-flash' },
    verifier: (t: ExecutionContext<Tester>, result: string) => {
      t.truthy(checkStreamObjects(result), 'should be valid stream objects');
      ...
      t.assert(
        assembledText.toLowerCase().includes('notesgraph'),
        'should mention NotesGraph'
      );
    },
    prefer: CopilotProviderType.Gemini,
  ...
  },
  {
    ...
    messages: singleUserPromptMessages(TestAssets.NotesGraph),
    verifier: (t: ExecutionContext<Tester>, result: string) => {
      assertNotWrappedInCodeBlock(t, result);
  ...
  },
  {
    ...
    messages: singleUserPromptMessages('', {
      attachments: [
        'https://cdn.notesgraph.com/copilot-test/Qgqy9qZT3VGIEuMIotJYoCCH.jpg',
      ],
    }),
    verifier: (t: ExecutionContext<Tester>, result: string) => {
      assertNotWrappedInCodeBlock(t, result);
  ...
  },
  {
    ...
    messages: singleUserPromptMessages('', {
      attachments: [
        'https://cdn.notesgraph.com/copilot-test/Zkas098lkjdf-908231.jpg',
      ],
    }),
...
async function installActionSessionMock(
  ...
  const user = await models.user.create({
    email: `copilot-provider-user-${randomUUID()}@notesgraph.test`,
  });
...
const TRANSCRIPT_AUDIO_CASES = [
  {
    name: 'short audio',
    url: 'https://cdn.notesgraph.com/copilot-test/MP9qDGuYgnY+ILoEAmHpp3h9Npuw2403EAYMEA.mp3',
    mimeType: 'audio/mpeg',
    modelId: 'gemini-2.5-flash',
  },
  {
    name: 'middle audio',
    url: 'https://cdn.notesgraph.com/copilot-test/2ed05eo1KvZ2tWB_BAjFo67EAPZZY-w4LylUAw.m4a',
    mimeType: 'audio/m4a',
    modelId: 'gemini-2.5-flash',
  },
  {
    name: 'long audio',
    url: 'https://cdn.notesgraph.com/copilot-test/nC9-e7P85PPI2rU29QWwf8slBNRMy92teLIIMw.opus',
    mimeType: 'audio/opus',
...
for (const testCase of TRANSCRIPT_AUDIO_CASES) {
  test(
    ...
    async t => {
      ...
      const user = await models.user.create({
        email: `copilot-provider-transcript-${randomUUID()}@notesgraph.com`,
      });
