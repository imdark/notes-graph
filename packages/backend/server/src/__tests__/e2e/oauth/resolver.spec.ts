import { oauthProvidersQuery } from '@notesgraph/graphql';
...
e2e('should return apple oauth provider in version >= 0.22.0', async t => {
  ...
  const res2 = await app.gql({
    ...
    context: {
      headers: {
        'x-notesgraph-version': '0.23.0-beta.1',
      },
    },
  });
  ...
  const res3 = await app.gql({
    ...
    context: {
      headers: {
        'x-notesgraph-version': '2025.6.29-canary.93',
      },
    },
  });
