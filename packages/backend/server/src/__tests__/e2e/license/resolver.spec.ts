...
e2e.before(async () => {
  ...
  process.env.NotesGraph_PRO_PUBLIC_KEY = testPublicKey;
  process.env.NotesGraph_PRO_LICENSE_AES_KEY = testTestLicenseAESKey;
  refreshEnv();
...
e2e('should not install expired license', async t => {
  await t.throwsAsync(
    ...
    {
      message:
        'Invalid license to activate. License file has expired. Please contact with NotesGraph support to fetch a latest one.',
    }
  );
});
