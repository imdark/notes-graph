...
async function applyPermissionProjectionTriggerFunctionUpdates() {
  ...
  for (const name of [
    'notesgraph_permission_project_new_workspace_member',
    'notesgraph_permission_project_new_workspace_invitation',
    'notesgraph_permission_project_new_doc_access_policy',
    'notesgraph_permission_project_new_doc_grant',
  ]) {
...
test('PermissionProjectionModel backfill runs with legacy origin in a long transaction', async t => {
  ...
  t.regex(String(executed[0]), /notesgraph\.permission_sync_origin/);
});
...
test('permission projection migration uses non-recursive origin guard', t => {
  ...
  const guardBody = migration.match(
    /CREATE OR REPLACE FUNCTION notesgraph_permission_should_project_from_legacy\(\)[\s\S]*?END\n\$\$;/
  )?.[0];
  ...
  t.false(
    guardBody?.includes(
      'IF NOT notesgraph_permission_should_project_from_legacy()'
    )
  );
  t.truthy(
    migration.match(
      /CREATE OR REPLACE FUNCTION notesgraph_permission_should_project_from_new\(\)[\s\S]*?IF NOT notesgraph_permission_projection_enabled\(\) THEN[\s\S]*?END\n\$\$;/
    )
  );
});
