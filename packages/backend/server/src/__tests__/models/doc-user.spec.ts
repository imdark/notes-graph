...
test('should set doc owner', async t => {
  ...
  const user = await models.user.create({ email: 'u1@notesgraph.com' });
...
test('should transfer doc owner', async t => {
  const user = await models.user.create({ email: 'u1@notesgraph.com' });
  const user2 = await models.user.create({ email: 'u2@notesgraph.com' });
  const workspace = await create();
  const docId = 'fake-doc-id';
...
test('should count doc user roles', async t => {
  ...
  const users = await Promise.all([
    models.user.create({ email: 'u1@notesgraph.com' }),
    models.user.create({ email: 'u2@notesgraph.com' }),
  ]);
