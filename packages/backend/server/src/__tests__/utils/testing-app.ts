...
import { transformToForm } from '@notesgraph/graphql';
...
export async function createTestingApp(
  ...
  const logger = new NotesGraphLogger();
...
export class TestingApp extends ApplyType<INestApplication>() {
  ...
  request(
    ...
    if (this.csrfCookie) {
      req.set('x-notesgraph-csrf-token', this.csrfCookie);
    }
  ...
  // TODO(@forehalo): directly make proxy for graphql queries defined in `@notesgraph/graphql`
  ...
  private randomEmail() {
    return `test-${randomUUID()}@notesgraph.com`;
  }
