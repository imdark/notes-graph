...
test('should read NAMESPACE', t => {
  t.deepEqual(
    ['dev', 'beta', 'production'].map(envVal => {
      process.env.NOTESGRAPH_ENV = envVal;
      const env = new Env();
  ...
  t.throws(() => {
    process.env.NOTESGRAPH_ENV = 'unknown';
    new Env();
  });
});
...
test('should tell selfhosted correctly', t => {
  ...
  process.env.DEPLOYMENT_TYPE = 'notesgraph';
  t.false(new Env().selfhosted);
});

test('should tell namespaces correctly', t => {
  process.env.NOTESGRAPH_ENV = 'dev';
  ...
  process.env.NOTESGRAPH_ENV = 'beta';
  t.deepEqual(new Env().namespaces, {
    canary: false,
    beta: true,
    production: false,
  ...
  process.env.NOTESGRAPH_ENV = 'production';
  t.deepEqual(new Env().namespaces, {
    canary: false,
    beta: false,
    production: true,
