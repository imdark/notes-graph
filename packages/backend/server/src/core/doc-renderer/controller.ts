...
export class DocRendererController {
  ...
  _render(opts: RenderOptions | null, assets: HtmlAssets): string {
    // TODO(@forehalo): how can we enable the type reference to @notesgraph/env
    ...
    const title = opts?.title
      ? htmlSanitize(`${opts.title} | NotesGraph`)
      : 'NotesGraph';
    const summary = opts ? htmlSanitize(opts.summary) : assets.description;
    const image = opts?.avatar ?? 'https://notesgraph.com/og.jpeg';
...
<html>
  <head>
    ...
    <meta name="twitter:site" content="@NotesGraphOfficial" />
