    ...
    const samples = await this.db.$queryRaw<ProjectionDecisionSample[]>`
      (
        ...
        WHERE old_member.status = 'Accepted'::"WorkspaceMemberStatus"
          AND notesgraph_permission_legacy_workspace_role(old_member.type) IS NOT NULL
          AND notesgraph_permission_legacy_default_doc_role(old_doc."defaultRole") IS NOT NULL
      ...
      (
        ...
        FROM workspace_user_permissions old_member
        WHERE old_member.status <> 'Accepted'::"WorkspaceMemberStatus"
          AND notesgraph_permission_workspace_invitation_state(old_member.status) IS NOT NULL
          AND notesgraph_permission_legacy_workspace_role(old_member.type) IS NOT NULL
      ...
      (
        ...
        WHERE notesgraph_permission_legacy_doc_role(old_grant.type) IS NOT NULL
