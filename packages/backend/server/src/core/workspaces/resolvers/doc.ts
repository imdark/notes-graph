...
export class WorkspaceDocResolver {
  ...
  async publishDoc(
    ...
    @Args({
      ...
      type: () => DocRole,
      nullable: true,
      defaultValue: DocRole.Reader,
      description:
        'Role granted to anyone opening the share link: Reader (read only) or Commenter (read and comment)',
    })
    publicRole: DocRole
  ) {
    ...
    if (publicRole !== DocRole.Reader && publicRole !== DocRole.Commenter) {
      throw new ExpectToPublishDoc();
    }
    ...
    const doc = await this.models.doc.publish(
      workspaceId,
      docId,
      mode,
      publicRole === DocRole.Commenter ? 'commenter' : 'external'
    );
...
export class DocResolver {
  ...
  async publicRole(@Parent() doc: DocType): Promise<DocRole | null> {
    const policy = await this.models.docAccessPolicy.get(
      doc.workspaceId,
      doc.docId
    );
    ...
    return policy.publicRole === 'commenter' ? DocRole.Commenter : DocRole.Reader;
  }
