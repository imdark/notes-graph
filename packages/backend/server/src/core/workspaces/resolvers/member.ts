...
export class WorkspaceMemberResolver {
  ...
  async inviteMembers(
    ...
    @Args('role', { type: () => WorkspaceRoleInput, nullable: true })
    role?: WorkspaceRole
  ): Promise<InviteResult[]> {
    // `External` is the restricted guest tier — real access comes from
    // explicit per-doc/project grants, not the workspace's default doc role.
    const inviteRole = role ?? WorkspaceRole.Collaborator;
    await this.ac
      .user(me.id)
