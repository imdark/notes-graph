...
export class WorkspacesController {
  ...
  /** 'commenter' when the share link grants read + comment, else 'external'. */
  private async getPublicRoleHeader(workspaceId: string, docId: string) {
    const policy = await this.models.docAccessPolicy.get(workspaceId, docId);
    return policy?.publicRole === 'commenter' ? 'commenter' : 'external';
  }
  ...
  async doc(
    ...
    if (!docId.isWorkspace) {
      ...
      res.setHeader(
        'public-role',
        await this.getPublicRoleHeader(docId.workspace, docId.guid)
      );
    }
  ...
  async headPublicDoc(
    ...
    // A published doc changes whenever its owner edits it, so it must never be
    // served from a shared/heuristic cache. `no-cache` still allows cheap ETag
    // revalidation; `private` keeps intermediary proxies from storing it.
    res.setHeader('cache-control', 'private, no-cache, must-revalidate');
    res.setHeader('publish-mode', publishPageMode);
    res.setHeader(
      'public-role',
      await this.getPublicRoleHeader(workspaceId, docId)
    );
    res.status(200).end();
  }
  ...
  async publicDoc(
    ...
    // Cache with version-keyed invalidation: the ETag is the doc's own version
    // (its snapshot timestamp), so a shared viewer revalidates cheaply — 304
    // while the doc is unchanged, a full refetch the moment the owner edits it.
    // `no-cache` forces that revalidation; `private` blocks stale proxy storage.
    res.setHeader('etag', `"${binResponse.timestamp}"`);
    res.setHeader('cache-control', 'private, no-cache, must-revalidate');
    res.setHeader('content-type', 'application/octet-stream');
    res.send(binResponse.bin);
  }
  ...
  async publicRootDoc(
    ...
    // Small composite (root state + public-doc titles) — let Express derive a
    // content ETag and force revalidation so the published-docs list a shared
    // viewer sees stays current instead of being served stale from a proxy.
    res.setHeader('cache-control', 'private, no-cache, must-revalidate');
    res.setHeader('content-type', 'application/octet-stream');
