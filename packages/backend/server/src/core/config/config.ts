...
defineModuleConfig('server', {
  name: {
    desc: 'A recognizable name for the server. Will be shown when connected with NotesGraph Desktop.',
    default: undefined,
    shape: z.string().optional(),
  },
  externalUrl: {
    desc: `Base url of NotesGraph server, used for generating external urls.
Default to be \`[server.protocol]://[server.host][:server.port]\` if not specified.
    `,
    default: '',
    env: 'NOTESGRAPH_SERVER_EXTERNAL_URL',
    validate: val => {
  ...
  https: {
    ...
    default: false,
    env: ['NOTESGRAPH_SERVER_HTTPS', 'boolean'],
    shape: z.boolean(),
  },
  host: {
    desc: 'Where the server get deployed(FQDN).',
    default: 'localhost',
    env: 'NOTESGRAPH_SERVER_HOST',
  },
  hosts: {
  ...
  port: {
    ...
    default: 3010,
    env: ['NOTESGRAPH_SERVER_PORT', 'integer'],
  },
  path: {
    desc: 'Subpath where the server get deployed if there is one.(e.g. /notesgraph)',
    default: '',
    env: 'NOTESGRAPH_SERVER_SUB_PATH',
  },
});
