...
export class ServerConfigResolver {
  ...
  serverConfig(): ServerConfigType {
    return {
      name:
        ...
        (env.selfhosted
          ? 'NotesGraph SelfHosted Cloud'
          : env.namespaces.canary
            ? 'NotesGraph Canary Cloud'
            : env.namespaces.beta
              ? 'NotesGraph Beta Cloud'
              : 'NotesGraph Cloud'),
      version: env.version,
  ...
  companionUrl(): string | null {
    return process.env.NOTESGRAPH_COMPANION_URL || null;
  }
  ...
  async availableUpgrade(): Promise<ReleaseVersionType | null> {
    ...
    const url = `https://notesgraph.com/api/worker/releases?channel=${channel}`;
    ...
    try {
      ...
      if (!response.ok) {
        this.logger.error(
          'failed to fetch notesgraph releases',
          await response.text()
        );
        return null;
      }
      ...
      this.logger.error('failed to fetch notesgraph releases', e);
      return null;
    }
  }
}
