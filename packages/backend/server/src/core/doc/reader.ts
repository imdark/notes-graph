...
export abstract class DocReader {
  ...
  parseDocContent(bin: Uint8Array, docId: string, maxSummaryLength = 150) {
    return buildDocSummary(docId, bin, maxSummaryLength);
  }
...
export class DatabaseDocReader extends DocReader {
  ...
  protected override async getDocContentWithoutCache(
    ...
    return this.parseDocContent(docBinary, guid, fullContent ? -1 : 150);
  }
