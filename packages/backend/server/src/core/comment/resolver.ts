...
export class CommentResolver {
  ...
  /**
   * Post a comment on a public doc whose share link grants commenting,
   * without an account. Attributed to the shared "Anonymous" user.
   */
  @Public()
  @Throttle('strict')
  @Mutation(() => CommentObjectType)
  async createAnonymousComment(
    @Args('input') input: AnonymousCommentCreateInput
  ): Promise<CommentObjectType> {
    // the public link must grant commenting; evaluated as an anonymous
    // visitor so members-only docs can never be reached this way
    await this.ac
      .user('anonymous')
      .doc(input.workspaceId, input.docId)
      .assert('Doc.Comments.Create');

    // unauthenticated input — keep payloads sane
    if (JSON.stringify(input.content).length > 100_000) {
      throw new ValidationError({ errors: 'comment content is too large' });
    }

    const anonymous = await this.getOrCreateAnonymousUser();
    const comment = await this.service.createComment({
  ...
  @Public()
  @Query(() => PaginatedCommentObjectType)
  async publicDocComments(
    ...
    const [totalCount, comments] = await Promise.all([
      this.service.getCommentCount(workspaceId, docId),
      this.service.listComments(workspaceId, docId, {
        sid: cursor.sid,
    ...
    return paginateWithCustomCursor(
      ...
      startCursor,
      endCursor,
      comments.length === (pagination?.first ?? comments.length)
    );
  }

  private async getOrCreateAnonymousUser() {
    const email = 'anonymous@notesgraph.com';
    const existing = await this.models.user.getUserByEmail(email);
    if (existing) {
      return existing;
    }
    // no password is ever set — this account cannot be signed into
