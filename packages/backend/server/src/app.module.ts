...
import { NotesApiModule } from './core/notes-api';
import { UppyRelayModule } from './core/uppy-relay';
...
export class AppModuleBuilder {
  private readonly modules: NotesGraphModule[] = [];

  use(...modules: NotesGraphModule[]): this {
    modules.forEach(m => {
      this.modules.push(m);
    });
  ...
  useIf(predicator: () => boolean, ...modules: NotesGraphModule[]): this {
    if (predicator()) {
      this.use(...modules);
    }
