...
export async function run() {
  ...
  const logger = app.get(NotesGraphLogger);
  ...
  if (env.dev) {
    ...
    const docConfig = new DocumentBuilder()
      .setTitle('NotesGraph API')
      .setDescription(`NotesGraph Server ${env.version} API documentation`)
  ...
  logger.log(`NotesGraph Server is running in [${env.DEPLOYMENT_TYPE}] mode`);
