...
export class PermissionProjectionModel extends BaseModel {
  ...
  async backfillLegacyProjection() {
    ...
    await db.$transaction(
      async tx => {
        await tx.$executeRaw`
        SELECT set_config('notesgraph.permission_sync_origin', 'legacy', true)
      `;
        ...
        WHERE projected.legacy_permission_id IS NOT NULL
          AND NOT EXISTS (
            ...
            WHERE old.id = projected.legacy_permission_id
              ...
              AND notesgraph_permission_legacy_workspace_role(old.type) IS NOT NULL
          )
      `;
        ...
        WHERE projected.legacy_permission_id IS NOT NULL
          AND NOT EXISTS (
            ...
            WHERE old.id = projected.legacy_permission_id
              AND old.status <> 'Accepted'::"WorkspaceMemberStatus"
              AND notesgraph_permission_workspace_invitation_state(old.status) IS NOT NULL
              AND notesgraph_permission_legacy_workspace_role(old.type) IS NOT NULL
          )
      `;
        ...
        WHERE projected.principal_type = 'user'
          AND NOT EXISTS (
            ...
            WHERE old.workspace_id = projected.workspace_id
              ...
              AND notesgraph_permission_legacy_doc_role(old.type) IS NOT NULL
          )
      `;
        ...
        WHERE NOT EXISTS (
          ...
          WHERE old.workspace_id = projected.workspace_id
            AND old.page_id = projected.doc_id
            AND notesgraph_permission_legacy_default_doc_role(old."defaultRole") IS NOT NULL
        )
      `;
      ...
      `;
        ...
        SELECT
          workspace_id,
          user_id,
          notesgraph_permission_legacy_workspace_role(type),
          'active',
          CASE source
        ...
        WHERE status = 'Accepted'::"WorkspaceMemberStatus"
          AND notesgraph_permission_legacy_workspace_role(type) IS NOT NULL
      ...
      `;
        ...
        SELECT
          ...
          user_id,
          inviter_id,
          CASE WHEN notesgraph_permission_legacy_workspace_role(type) = 'admin' THEN 'admin' ELSE 'member' END,
          notesgraph_permission_workspace_invitation_state(status),
          CASE source
            WHEN 'Link'::"WorkspaceMemberSource" THEN 'link'
            ELSE 'email'
          END,
        ...
        FROM workspace_user_permissions
        WHERE status <> 'Accepted'::"WorkspaceMemberStatus"
          AND notesgraph_permission_workspace_invitation_state(status) IS NOT NULL
          AND notesgraph_permission_legacy_workspace_role(type) IS NOT NULL
        ON CONFLICT ("legacy_permission_id") WHERE "legacy_permission_id" IS NOT NULL
        DO UPDATE SET
      ...
      `;
        ...
        SELECT
          ...
          notesgraph_permission_legacy_default_doc_role("defaultRole"),
          published_at,
          now()
        FROM workspace_pages
        WHERE notesgraph_permission_legacy_default_doc_role("defaultRole") IS NOT NULL
      ...
      `;
        ...
        WITH legacy_doc_grants AS (
          SELECT
            ...
            created_at,
            row_number() OVER (
              PARTITION BY workspace_id, page_id, notesgraph_permission_legacy_doc_role(type)
              ORDER BY created_at ASC, user_id ASC
            ) AS role_rank
          FROM workspace_page_user_permissions
          WHERE notesgraph_permission_legacy_doc_role(type) IS NOT NULL
        )
        ...
        SELECT
          ...
          'user',
          user_id,
          notesgraph_permission_legacy_doc_role(type),
          workspace_id,
          page_id,
          user_id,
          created_at,
        ...
        WHERE notesgraph_permission_legacy_doc_role(type) IS NOT NULL
          AND (
            notesgraph_permission_legacy_doc_role(type) <> 'owner'
            OR role_rank = 1
          )
    ...
    const [
      ...
      count(this.db.$queryRaw<CountRow[]>`
        ...
        WHERE old.status = 'Accepted'::"WorkspaceMemberStatus"
          ...
          AND (
            ...
            projected.role <> notesgraph_permission_legacy_workspace_role(old.type) OR
      ...
      count(this.db.$queryRaw<CountRow[]>`
        ...
        WHERE
          ...
          AND (
            ...
            notesgraph_permission_legacy_workspace_role(old.type) IS NULL
          )
      `),
      count(this.db.$queryRaw<CountRow[]>`
        SELECT COUNT(*)::bigint AS count
        ...
        WHERE old.status <> 'Accepted'::"WorkspaceMemberStatus"
          ...
          AND (
            ...
            projected.requested_role <> CASE WHEN notesgraph_permission_legacy_workspace_role(old.type) = 'admin' THEN 'admin' ELSE 'member' END OR
            projected.status <> notesgraph_permission_workspace_invitation_state(old.status)
          )
      `),
      count(this.db.$queryRaw<CountRow[]>`
        SELECT COUNT(*)::bigint AS count
        ...
        WHERE projected.invitee_user_id IS NOT NULL
          AND (
            old.id IS NULL OR
            old.status = 'Accepted'::"WorkspaceMemberStatus" OR
            notesgraph_permission_workspace_invitation_state(old.status) IS NULL OR
            notesgraph_permission_legacy_workspace_role(old.type) IS NULL
          )
      `),
      count(this.db.$queryRaw<CountRow[]>`
        ...
        WHERE notesgraph_permission_legacy_doc_role(old.type) IS NOT NULL
          AND (
            projected.workspace_id IS NULL OR
            projected.role <> notesgraph_permission_legacy_doc_role(old.type)
          )
      `),
      count(this.db.$queryRaw<CountRow[]>`
        SELECT COUNT(*)::bigint AS count
        ...
        WHERE projected.principal_type = 'user'
          AND (
            ...
            notesgraph_permission_legacy_doc_role(old.type) IS NULL
          )
      `),
      count(this.db.$queryRaw<CountRow[]>`
        SELECT COUNT(*)::bigint AS count
        ...
        LEFT JOIN doc_access_policies projected
          ON projected.workspace_id = old.workspace_id
          AND projected.doc_id = old.page_id
        WHERE notesgraph_permission_legacy_default_doc_role(old."defaultRole") IS NOT NULL
          AND (
            projected.workspace_id IS NULL OR
            ...
            projected.member_default_role IS DISTINCT FROM notesgraph_permission_legacy_default_doc_role(old."defaultRole")
          )
      `),
      count(this.db.$queryRaw<CountRow[]>`
        SELECT COUNT(*)::bigint AS count
      ...
      this.db.$queryRaw<ProjectionIssueRow[]>`
        ...
        FROM (
          ...
          FROM workspace_user_permissions
          WHERE notesgraph_permission_legacy_workspace_role(type) IS NULL
            AND type <> -99
          UNION ALL
          SELECT 'unknown_doc_role' AS category
          FROM workspace_page_user_permissions
          WHERE notesgraph_permission_legacy_doc_role(type) IS NULL
  ...
  async markNewWriteOrigin() {
    await this.db.$executeRaw`
      SELECT set_config('notesgraph.permission_sync_origin', 'new', true)
    `;
  }
