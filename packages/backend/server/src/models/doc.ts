...
export type DocMetaUpsertInput = Omit<
  ...
  'workspaceId' | 'docId'
> & {
  /** role granted to anyone opening the public link */
...
export class DocModel extends BaseModel {
  ...
  async upsertMeta(
    ...
    // publicRole lives on docAccessPolicy (handled above), not workspaceDoc
    const { publicRole: _publicRole, ...docData } = data ?? {};
  ...
  async publish(
    ...
    publicRole: 'external' | 'commenter' = 'external'
  ) {
    return await this.upsertMeta(workspaceId, docId, {
      public: true,
      mode,
