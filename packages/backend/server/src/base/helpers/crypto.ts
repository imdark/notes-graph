...
import {
  NOTESGRAPH_PRO_LICENSE_AES_KEY,
  NOTESGRAPH_PRO_PUBLIC_KEY,
} from '../../native';
...
export class CryptoHelper implements OnModuleInit {
  ...
  NotesGraphProPublicKey: Buffer | null = null;
  NotesGraphProLicenseAESKey: Buffer | null = null;

  onModuleInit() {
    if (env.selfhosted) {
      this.NotesGraphProPublicKey = this.loadNotesGraphProPublicKey();
      this.NotesGraphProLicenseAESKey = this.loadNotesGraphProLicenseAESKey();
    }
  }
  ...
  private loadNotesGraphProPublicKey() {
    if (NOTESGRAPH_PRO_PUBLIC_KEY) {
      return Buffer.from(NOTESGRAPH_PRO_PUBLIC_KEY);
    } else {
      this.logger.warn('NOTESGRAPH_PRO_PUBLIC_KEY is not set at compile time.');
    }

    if (!env.prod && process.env.NotesGraph_PRO_PUBLIC_KEY) {
      return Buffer.from(process.env.NotesGraph_PRO_PUBLIC_KEY);
    }

    return null;
  }

  private loadNotesGraphProLicenseAESKey() {
    if (NOTESGRAPH_PRO_LICENSE_AES_KEY) {
      return this.sha256(NOTESGRAPH_PRO_LICENSE_AES_KEY);
    } else {
      this.logger.warn(
        'NOTESGRAPH_PRO_LICENSE_AES_KEY is not set at compile time.'
      );
    }

    if (!env.prod && process.env.NotesGraph_PRO_LICENSE_AES_KEY) {
      return this.sha256(process.env.NotesGraph_PRO_LICENSE_AES_KEY);
    }
