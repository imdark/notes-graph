          ...
          {users.map((user, index) => (
            <tr
              ...
              className={`${user.valid === false ? 'bg-destructive/10' : ''}
                ...
                ${user.importStatus === ImportStatus.Success ? 'bg-[var(--notesgraph-v2-layer-background-success)]' : ''}
                ${user.importStatus === ImportStatus.Processing ? 'bg-[var(--notesgraph-v2-layer-background-warning)]' : ''}`}
            >
              ...
              <td className="py-2 px-4 text-sm">
                {user.importStatus === ImportStatus.Success ? (
                  <span className="text-foreground">
                    <span className="mr-2 inline-block h-2 w-2 rounded-full bg-[var(--notesgraph-v2-status-success)]" />
                    Success
                  </span>
