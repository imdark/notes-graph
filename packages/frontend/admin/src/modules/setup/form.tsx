...
import type { CarouselApi } from '@notesgraph/admin/components/ui/carousel';
import {
  Carousel,
  CarouselContent,
  CarouselItem,
} from '@notesgraph/admin/components/ui/carousel';
import { validateEmailAndPassword } from '@notesgraph/admin/utils';
import { useAsyncCallback } from '@notesgraph/core/components/hooks/notesgraph-async-hooks';
import { useCallback, useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import { toast } from 'sonner';

import { notesgraphFetch } from '../../fetch-utils';
  ...
  return (
    ...
    >
      <h1 className="text-5xl font-extrabold max-lg:text-3xl max-lg:font-bold">
        Welcome to NotesGraph
      </h1>
      <p className="mt-5 font-semibold text-xl max-lg:px-4 max-lg:text-lg">
        Configure your Self Host NotesGraph with a few simple settings.
      </p>
    </div>
  );
};
...
export const Form = () => {
  ...
  const createAdmin = useCallback(async () => {
    try {
      const createResponse = await notesgraphFetch(
    ...
    <div className="flex flex-col justify-between h-full w-full  lg:pl-36 max-lg:items-center ">
      ...
      <div>
        ...
        <Button onClick={onNext} disabled={disableContinue}>
          {current === count ? 'Open NotesGraph' : 'Continue'}
        </Button>
      </div>
