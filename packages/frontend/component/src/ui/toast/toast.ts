...
const createToastContainer = (portal?: HTMLElement) => {
  ...
  const template = html`<div
    style="${styles}"
    data-testid="notesgraph-toast-container"
  ></div>`;
...
const createAndShowNewToast = (
  ...
  const toastStyles = css`
    ...
    background: var(--notesgraph-tooltip);
    box-shadow: var(--notesgraph-float-button-shadow);
    border-radius: 8px;
    opacity: 0;
  ...
  window.dispatchEvent(
    new CustomEvent('notesgraph-toast:emit', { detail: message })
  );
