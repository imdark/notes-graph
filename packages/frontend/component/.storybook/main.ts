...
export default {
  ...
  async viteFinal(config, _options) {
    const { getBuildConfig } = await import('@notesgraph-tools/utils/build-config');
    const { Package } = await import('@notesgraph-tools/utils/workspace');
    ...
    return mergeConfig(config, {
      ...
      define: Object.entries(
        getBuildConfig(new Package('@notesgraph/web'), {
          mode: 'development',
          channel: 'canary',
        })
