...
import type { BlobEngine } from '@blocksuite/notesgraph/sync';
...
const TRANSCRIPTION_BLOCK_FLAVOUR = 'notesgraph:transcription';
...
function ensureNoteId(docStore: Store) {
  const [existingNote] = docStore.getModelsByFlavour('notesgraph:note');
  if (existingNote) {
    return existingNote.id;
  }

  const [page] = docStore.getModelsByFlavour('notesgraph:page');
  if (!page) {
    throw new Error('Recording doc is missing the page block');
  }

  return docStore.addBlock('notesgraph:note', {}, page.id);
}

function findExistingAttachment(docStore: Store, attachmentName: string) {
  return (
    docStore.getModelsByFlavour(
