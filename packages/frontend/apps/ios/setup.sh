...
yarn notesgraph @notesgraph/ios build
yarn notesgraph @notesgraph/ios sync
...
sed -i '' "s/exact: \"[^\"]*\"/exact: \"$LATEST_VERSION\"/g" $SCRIPT_DIR_PATH/App/Packages/NotesGraphGraphQL/Package.swift
...
trap cleanup EXIT
CUSTOM_SCALARS_DIR=$SCRIPT_DIR_PATH/App/Packages/NotesGraphGraphQL/Sources/Schema/CustomScalars
cp -r $CUSTOM_SCALARS_DIR/* $TEMP_DIR/

echo "[*] codegen..."
rm -rf $CUSTOM_SCALARS_DIR/*
yarn notesgraph @notesgraph/ios codegen "$LATEST_VERSION"
cp -r $TEMP_DIR/* $CUSTOM_SCALARS_DIR/

echo "[+] setup complete"
yarn notesgraph @notesgraph/ios xcode
