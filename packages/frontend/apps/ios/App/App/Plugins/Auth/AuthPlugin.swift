...
public class AuthPlugin: CAPPlugin, CAPBridgedPlugin {
  ...
  private let tokenService = "app.notesgraph.pro.auth-token"
  private let authCookieNames = Set(["notesgraph_session", "notesgraph_user_id", "notesgraph_csrf_token"])
    ...
    Task {
      do {
        ...
        let (data, response) = try await self.fetch(
          ...
          headers: [
            "x-notesgraph-client-kind": "native"
  ...
  private func fetch(
    ...
    request.setValue(AppConfigManager.getNotesGraphVersion(), forHTTPHeaderField: "x-notesgraph-version")
