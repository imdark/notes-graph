...
private let unselectedColor: UIColor = .notesgraphIconPrimary
private let selectedColor: UIColor = .notesgraphIconActivated
...
class InputBoxFunctionBar: UIView {
  ...
  lazy var attachmentButton = UIButton(type: .system).then {
    $0.setImage(UIImage.notesgraphPlus, for: .normal)
  ...
  lazy var toolButton = UIButton(type: .system).then {
    $0.setImage(UIImage.notesgraphTools, for: .normal)
    $0.tintColor = unselectedColor
    $0.imageView?.contentMode = .scaleAspectFit
  ...
  lazy var networkButton = UIButton(type: .system).then {
    $0.setImage(UIImage.notesgraphWeb, for: .normal)
    $0.tintColor = unselectedColor
    $0.imageView?.contentMode = .scaleAspectFit
  ...
  lazy var deepThinkingButton = UIButton(type: .system).then {
    $0.setImage(UIImage.notesgraphThink, for: .normal)
    $0.tintColor = unselectedColor
    $0.imageView?.contentMode = .scaleAspectFit
  ...
  lazy var sendButton = UIButton(type: .system).then {
    $0.setImage(UIImage.notesgraphArrowUpBig, for: .normal)
    $0.tintColor = UIColor.notesgraphTextPureWhite
    $0.backgroundColor = UIColor.notesgraphButtonPrimary
    $0.imageView?.contentMode = .scaleAspectFit
  ...
  private func updateColors() {
    attachmentButton.layer.borderColor = UIColor.notesgraphLayerBorder.cgColor
  }

  private func createAttachmentMenu() -> UIMenu {
    let takePhotoAction = UIAction(
      title: "Take Photo or Video",
      image: UIImage.notesgraphCamera
    ) { [weak self] _ in
    ...
    let attachFilesAction = UIAction(
      ...
      image: UIImage.notesgraphUpload
    ) { [weak self] _ in
      guard let self else { return }
    ...
    let embedDocsAction = UIAction(
      title: "Add NotesGraph Docs",
      image: UIImage.notesgraphPage
    ) { [weak self] _ in
      guard let self else { return }
