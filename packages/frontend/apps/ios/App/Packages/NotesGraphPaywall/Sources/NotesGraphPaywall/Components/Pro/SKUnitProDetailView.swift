  ...
  var body: some View {
    VStack(spacing: 24) {
      ...
      ScrollView {
        ...
        .clipped()
        .padding(16)
        .background(NotesGraphColors.layerBackgroundPrimary.color)
        .cornerRadius(16)
        .shadow(color: NotesGraphColors.layerBorder.color.opacity(0.08), radius: 8, y: 2)
        .padding(16)
      }
      .padding(-16)
...
#Preview {
  SKUnitProDetailView(viewModel: .vmPreviewForPro)
    .padding()
    .background(
      NotesGraphColors.layerBackgroundSecondary
        .color
        .ignoresSafeArea()
    )
