...
This is the NotesGraph iOS application built with Capacitor, React, and TypeScript. It's a hybrid mobile app that wraps a React web application in a native iOS shell.
...
1. `BUILD_TYPE=canary PUBLIC_PATH="/" yarn notesgraph @notesgraph/ios build` - Build web assets
2. `yarn notesgraph @notesgraph/ios cap sync` - Sync with iOS project
3. `yarn notesgraph @notesgraph/ios cap open ios` - Open in Xcode

### Live Reload Setup

1. Run `yarn dev` and select `ios` for Distribution option
2. Run `yarn notesgraph @notesgraph/ios sync:dev`
3. Run `yarn notesgraph @notesgraph/ios cap open ios`
...
- **DI Framework** - Dependency injection via `@notesgraph/infra`
...
- Webpack bundling via `@notesgraph-tools/cli`
...
- Workspace packages: `@notesgraph/core`, `@notesgraph/component`, `@notesgraph/env`
