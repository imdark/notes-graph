...
const makers = [
  ...
  !process.env.SKIP_BUNDLE && {
    ...
    config: {
      ...
      setupIcon: icoPath,
      iconUrl: iconUrl,
      loadingGif: './resources/icons/notesgraph_installing.gif',
    },
  },
  !process.env.SKIP_BUNDLE && {
  ...
  !process.env.SKIP_BUNDLE && {
    ...
    config: {
      options: {
        ...
        files: [
          [
            './resources/notesgraph.metainfo.xml',
            '/usr/share/metainfo/notesgraph.metainfo.xml',
          ],
        ],
        modules: [
          {
...
export default {
  ...
  packagerConfig: {
    ...
    icon: icnsPath,
    // Signing needs the "Developer ID Application" cert (present in CI's
    // keychain). Set SKIP_OSX_SIGN=1 to build an unsigned app locally
    // (open it via right-click → Open to get past Gatekeeper).
    osxSign: process.env.SKIP_OSX_SIGN
      ? undefined
      : {
    ...
    extraResource: [
      ...
      ...(platform === 'linux' ? ['./resources/notesgraph.metainfo.xml'] : []),
    ],
    protocols: [
      {
        name: productName,
    ...
    extendInfo: {
      NSAudioCaptureUsageDescription:
        'Please allow access in order to capture audio from other apps by NotesGraph.',
    },
  },
  makers,
  plugins: [
