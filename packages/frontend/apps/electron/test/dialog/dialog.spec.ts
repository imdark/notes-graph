...
vi.doMock('@notesgraph/electron/helper/nbstore', () => ({
  getDocStoragePool: () => ({
    connect,
    checkpoint,
...
vi.doMock('@notesgraph/electron/helper/main-rpc', () => ({
  mainRPC: {
    getPath,
    showItemInFolder,
    showOpenDialog,
...
vi.doMock('@notesgraph/electron/helper/workspace/meta', () => ({
  getSpaceDBPath,
  getWorkspaceDBPath,
  getWorkspacesBasePath,
}));

vi.doMock('@notesgraph/electron/helper/workspace', () => ({
  storeWorkspaceMeta,
}));
...
describe('dialog export', () => {
  test('saveDBFileAs exports a vacuumed backup instead of copying the live db', async () => {
    ...
    const exportPath = '/tmp/export.notesgraph';
    const tempExportPath = '/tmp/export.notesgraph.workspace-1.tmp';
    ...
    const { saveDBFileAs } =
      await import('@notesgraph/electron/helper/dialog/dialog');
  ...
  test('saveDBFileAs rejects exporting to a symlink alias of the live database', async () => {
    const dbPath = '/tmp/workspace/storage.db';
    const exportPath = '/tmp/alias.notesgraph';
    const id = '@peer(local);@type(workspace);@id(workspace-1);';
...
describe('dialog import', () => {
  test('loadDBFile validates schema and vacuums v2 imports into internal storage', async () => {
    const originalPath = '/tmp/import.notesgraph';
  ...
  test('loadDBFile validates schema and vacuums v1 imports into internal storage', async () => {
    const originalPath = '/tmp/import-v1.notesgraph';
  ...
  test('loadDBFile closes v1 connection when schema validation fails', async () => {
    const originalPath = '/tmp/import-v1-invalid.notesgraph';
  ...
  test('loadDBFile rejects normalized paths inside app data', async () => {
    const selectedPath = '/tmp/import.notesgraph';
