...
vi.doMock('@notesgraph/electron/helper/db/ensure-db', () => ({
...
afterAll(() => {
  vi.doUnmock('@notesgraph/electron/helper/main-rpc');
});

describe('workspace db management', () => {
  test('list local workspace ids', async () => {
    const { listLocalWorkspaceIds } =
      await import('@notesgraph/electron/helper/workspace/handlers');
