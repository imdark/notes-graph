...
describe('redirect proxy allowlist', () => {
  ...
  it('allows trusted redirect_uri', async () => {
    ...
    expect(
      validateRedirectProxyUrl(
        'assets://./redirect-proxy?redirect_uri=https%3A%2F%2Fgithub.com%2Ftoeverything%2FNotesGraph'
      )
    ).toEqual({
      allow: true,
      redirectTarget: 'https://github.com/notesgraph/notesgraph',
    });
  });
  ...
  it('allows current hostname (canary)', async () => {
    ...
    expect(
      validateRedirectProxyUrl(
        'assets://./redirect-proxy?redirect_uri=https%3A%2F%2Fcanary.notesgraph.com%2Fpricing'
      )
    ).toEqual({
      allow: true,
      redirectTarget: 'https://canary.notesgraph.com/pricing',
    });
  });
