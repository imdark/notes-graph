...
vi.mock('@notesgraph/core/modules/doc', () => ({
  DocsService: class DocsService {},
}));

vi.mock('@notesgraph/core/modules/media/services/audio-attachment', () => ({
  AudioAttachmentService: class AudioAttachmentService {},
}));

vi.mock('@notesgraph/core/modules/workbench', () => ({
  WorkbenchService: class WorkbenchService {},
}));

vi.mock('@notesgraph/debug', () => ({
  ...
  const blockSuiteDoc = {
    ...
    addBlock: vi.fn(
      (flavour: string, props: Record<string, unknown>, parentId?: string) => {
        ...
        if (flavour === 'notesgraph:transcription') {
