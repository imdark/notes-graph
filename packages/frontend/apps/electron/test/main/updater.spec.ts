...
import {
  ...
  NotesGraphUpdateProvider,
} from '../../src/main/updater/notesgraph-update-provider';
...
describe('testing for client update', () => {
  ...
  const basicRequestHandlers = [
    http.get(
      'https://notesgraph.com/api/worker/releases',
      async ({ request }) => {
    ...
    ...flatten(
      expectReleaseList.map(({ version }) => {
        return [
          http.get(
            `https://github.com/notesgraph/notesgraph/releases/download/v${version}/latest.yml`,
            async req => {
              const buffer = await fs.readFile(
                path.join(
                  __dirname,
  ...
  describe('release api request successfully', () => {
    ...
    for (const { buildType, version } of expectReleaseList) {
      it(`check update for ${buildType} channel successfully`, async () => {
        ...
        updater.setFeedURL(
          NotesGraphUpdateProvider.configFeed({
