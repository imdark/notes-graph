import { MemoryMemento } from '@notesgraph/infra';
import { ipcRenderer } from 'electron';

import {
  NOTESGRAPH_API_CHANNEL_NAME,
...
function invokeWithCatch(key: string, ...args: any[]) {
  ipcRenderer.invoke(NOTESGRAPH_API_CHANNEL_NAME, key, ...args).catch(err => {
...
function createSharedStorageApi(
  ...
  ipcRenderer.on(NOTESGRAPH_EVENT_CHANNEL_NAME, (_event, channel, updates) => {
