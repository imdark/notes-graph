...
let protocol =
  buildType === 'stable' ? 'notesgraph' : `notesgraph-${buildType}`;
if (isDev) {
  protocol = 'notesgraph-dev';
}
...
export function setupDeepLink(app: App) {
  ...
  app.on('open-url', (event, url) => {
    ...
    if (url.startsWith(`${protocol}://`)) {
      ...
      app
        .whenReady()
        .then(() => handleNotesGraphUrl(url))
        .catch(e => {
          logger.error('failed to handle notesgraph url', e);
        });
    }
  });
  ...
  app.on('second-instance', (event, commandLine) => {
    getMainWindow()
      .then(window => {
        ...
        if (url?.startsWith(`${protocol}://`)) {
          event.preventDefault();
          handleNotesGraphUrl(url).catch(e => {
            logger.error('failed to handle notesgraph url', e);
          });
        }
      })
...
async function handleNotesGraphUrl(url: string) {
  await showMainWindow();

  logger.info('open notesgraph url', url);
