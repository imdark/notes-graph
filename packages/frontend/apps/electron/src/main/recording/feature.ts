...
import type * as NativeModuleType from '@notesgraph/native';
...
function getNativeModule(): NativeModule {
  return (
    nativeModuleOverride ?? (require('@notesgraph/native') as NativeModule)
  );
}
...
async function getNativeModuleAsync(): Promise<NativeModule> {
  ...
  return (await import('@notesgraph/native')) as NativeModule;
}
...
function getAllApps(): TappableAppInfo[] {
  ...
  return apps.filter(
    (value): value is TappableAppInfo =>
      ...
      !value.bundleIdentifier.startsWith('pro.notesgraph') &&
