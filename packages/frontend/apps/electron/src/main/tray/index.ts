...
class TrayState implements Disposable {
  ...
  getSecondaryMenuProvider(): TrayMenuProvider {
    return {
      ...
      getConfig: () => [
        {
          label: 'Open NotesGraph',
          click: () => {
            logger.info('User action: Open NotesGraph');
            getMainWindow()
              .then(w => {
                w.show();
              })
              .catch(err => {
                logger.error('Failed to open NotesGraph:', err);
              });
          },
        },
        {
        ...
        'separator',
        {
          label: 'Quit NotesGraph Completely...',
          click: () => {
            logger.info('User action: Quit NotesGraph Completely');
            app.quit();
          },
        },
      ],
