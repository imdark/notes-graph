...
const apiBaseByBuildType: Record<typeof buildType, string> = {
  ...
  canary: 'https://canary.notesgraph.com',
};
...
const defaultReferer = 'https://client.notesgraph.local/';
const notesgraphDomains = [
  /^(?:[a-z0-9-]+\.)*usercontent\.notesgraph\.pro$/i,
  /^(?:[a-z0-9-]+\.)*notesgraph\.pro$/i,
  /^(?:[a-z0-9-]+\.)*notesgraph\.fail$/i,
  /^(?:[a-z0-9-]+\.)*notesgraph\.run$/i,
];
...
function allowCors(
  ...
  // Signed blob URLs redirect to *.usercontent.notesgraph.com without CORS headers.
    ...
    (responseDetails, callback) => {
      ...
      (async () => {
        if (responseHeaders) {
          ...
          // Adjust CORS for assets responses and allow blob redirects on notesgraph domains
          if (protocol === 'assets:') {
            ...
            notesgraphDomains.some(regex => regex.test(hostname))
          ) {
