...
import {
  NOTESGRAPH_EVENT_CHANNEL_NAME,
  NOTESGRAPH_EVENT_SUBSCRIBE_CHANNEL_NAME,
} from '../shared/type';
...
export function registerEvents() {
  ...
  ipcMain.on(NOTESGRAPH_EVENT_SUBSCRIBE_CHANNEL_NAME, onSubscribe);
  unsubs.push(() =>
    ipcMain.removeListener(NOTESGRAPH_EVENT_SUBSCRIBE_CHANNEL_NAME, onSubscribe)
  );
  // register events
    ...
    for (const [key, eventRegister] of Object.entries(namespaceEvents)) {
      const unsubscribe = eventRegister((...args: any[]) => {
        ...
        getTargetContents(chan).forEach(wc => {
          if (!wc.isDestroyed()) {
            wc.send(NOTESGRAPH_EVENT_CHANNEL_NAME, chan, ...args);
          }
        });
      });
