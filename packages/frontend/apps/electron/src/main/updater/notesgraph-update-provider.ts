...
export class NotesGraphUpdateProvider extends Provider<GithubUpdateInfo> {
  static configFeed(options: UpdateProviderOptions): CustomPublishOptions {
    return {
      provider: 'custom',
      feedUrl: 'https://notesgraph.com/api/worker/releases',
      updateProvider: NotesGraphUpdateProvider,
      ...options,
    };
  }
...
type FileParts =
  | ['notesgraph', string, VersionDistribution, VersionPlatform, VersionArch]
  | [
      'notesgraph',
      string,
      `${'canary' | 'beta'}.${number}`,
...
export function availableForMyPlatformAndInstaller(
  ...
  //  in form of:
  //   notesgraph-${build}-${buildSuffix}-${distribution}-${platform}-${arch}.${installer}
