...
import { DocStorage, ValidationResult } from '@notesgraph/native';
import { parseUniversalId } from '@notesgraph/nbstore';
      ...
      filters: [
        {
          ...
          extensions: ['db', 'notesgraph'],
        },
      ],
      message: 'Load Workspace from a NotesGraph file',
    });
...
async function cpV1DBFile(
  ...
  const { SqliteConnection } = await import('@notesgraph/native');
