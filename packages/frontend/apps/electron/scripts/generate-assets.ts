...
const notesgraphWebOutDir = path.join(webDir, 'dist');
const publicNotesGraphOutDir = path.join(publicDistDir, `web-static`);
...
console.log('build with following variables', {
  ...
  electronRootDir,
  publicDistDir,
  notesgraphSrcDir: webDir,
  notesgraphSrcOutDir: notesgraphWebOutDir,
  publicNotesGraphOutDir,
  releaseVersionEnv,
});
...
if (!process.env.SKIP_WEB_BUILD) {
  spawnSync('yarn', ['notesgraph', '@notesgraph/electron-renderer', 'build'], {
    stdio: 'inherit',
    env: process.env,
    cwd,
    shell: true,
  ...
  spawnSync('yarn', ['notesgraph', '@notesgraph/electron', 'build'], {
    stdio: 'inherit',
    env: process.env,
    cwd,
    shell: true,
  ...
  await fs.move(notesgraphWebOutDir, publicNotesGraphOutDir, {
    overwrite: true,
  });
}
