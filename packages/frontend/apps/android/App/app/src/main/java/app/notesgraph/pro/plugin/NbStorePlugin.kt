...
import app.notesgraph.pro.sync.BackgroundSyncWorker
import app.notesgraph.pro.utils.dataStore
import app.notesgraph.pro.utils.del
import app.notesgraph.pro.utils.set
import uniffi.notesgraph_mobile_native.DocRecord
import uniffi.notesgraph_mobile_native.SetBlob
...
class NbStorePlugin : Plugin() {
  ...
  /**
   * Persist the background-sync manifest and enqueue the periodic, network-
   * constrained push worker. The app calls this (while alive + signed in, and
   * only when the "background sync" setting is on) with the current server URL
   * and the list of cloud spaces to push. See [BackgroundSyncWorker].
   *
   * `manifest`: { serverBaseUrl, clientVersion, spaces: [{ universalId,
   * spaceType, spaceId, dbPeer, peerId }] }
   */
  @PluginMethod
    ...
    launch(Dispatchers.IO) {
      try {
        ...
        NotesGraphApp.context().dataStore.set(
          BackgroundSyncWorker.MANIFEST_KEY,
          manifest.toString()
        )
        BackgroundSyncWorker.schedule(NotesGraphApp.context())
        call.resolve(JSObject().put("ok", true))
      } catch (e: Exception) {
        call.reject("Failed to schedule background sync: ${e.message}", null, e)
      }
    }
  }

  /** Clear the manifest and cancel the background push worker (setting off / sign-out). */
  @PluginMethod
  fun cancelBackgroundSync(call: PluginCall) {
    launch(Dispatchers.IO) {
      try {
        NotesGraphApp.context().dataStore.del(BackgroundSyncWorker.MANIFEST_KEY)
        BackgroundSyncWorker.cancel(NotesGraphApp.context())
        call.resolve(JSObject().put("ok", true))
      } catch (e: Exception) {
        call.reject("Failed to cancel background sync: ${e.message}", null, e)
      }
    }
  }
}
