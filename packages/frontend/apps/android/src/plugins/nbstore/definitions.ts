import type { CrawlResult, DocIndexedClock } from '@notesgraph/nbstore';

export interface Blob {
  key: string;
  // base64 encoded data, or "__NOTESGRAPH_BLOB_FILE__:<absolutePath>" for large blobs
  data: string;
  mime: string;
  size: number;
  createdAt: number;
...
export interface NbStorePlugin {
  ...
  /**
   * Persist the background push-sync manifest and enqueue the periodic,
   * connectivity-constrained worker (see BackgroundSyncWorker.kt). The web app
   * calls this while alive + signed in, gated on the "background sync" setting.
   */
  scheduleBackgroundSync: (options: {
    manifest: {
  ...
  /** Clear the manifest and cancel the worker (setting off / signed out). */
  cancelBackgroundSync: () => Promise<{ ok: boolean }>;
}
