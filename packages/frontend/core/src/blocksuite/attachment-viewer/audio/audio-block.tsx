...
import { AudioPlayer } from '@notesgraph/component/ui/audio-player';
...
import { useSeekTime } from '@notesgraph/core/components/hooks/use-seek-time';
import { CurrentServerScopeProvider } from '@notesgraph/core/components/providers/current-server-scope';
import { PublicUserLabel } from '@notesgraph/core/modules/cloud/views/public-user';
import { GlobalDialogService } from '@notesgraph/core/modules/dialogs';
import type { AudioAttachmentBlock } from '@notesgraph/core/modules/media/entities/audio-attachment-block';
import { AudioAttachmentService } from '@notesgraph/core/modules/media/services/audio-attachment';
...
const AttachmentAudioPlayer = ({ block }: { block: AudioAttachmentBlock }) => {
  ...
  const handleNotesClick = useAsyncCallback(async () => {
    ...
    if (result?.error === 'created-by-others') {
      confirmModal.openConfirmModal({
        title: t['com.notesgraph.audio.transcribe.non-owner.confirm.title'](),
        description: (
          <Trans i18nKey="com.notesgraph.audio.transcribe.non-owner.confirm.message">
  ...
  const notesEntry = useMemo(() => {
    ...
    const inner = (
      ...
      >
        {transcribing
          ? t['com.notesgraph.audio.transcribing']()
          : t['com.notesgraph.audio.notes']()}
      </Button>
    );
    if (transcribing) {
      return (
        <Tooltip content={t['com.notesgraph.audio.transcribing']()}>
          {inner}
        </Tooltip>
      );
    }
