...
} from '@blocksuite/notesgraph/blocks/bookmark';
import { LinkedDocSlashMenuConfigIdentifier } from '@blocksuite/notesgraph/blocks/embed-doc';
import type { ServiceIdentifier } from '@blocksuite/notesgraph/global/di';
import {
...
import { ExternalLinksQuickSearchSession } from '@notesgraph/core/modules/quicksearch/impls/external-links';
import { JournalsQuickSearchSession } from '@notesgraph/core/modules/quicksearch/impls/journals';
    ...
    async openQuickSearch() {
      ...
      searchResult = await new Promise((resolve, reject) =>
        framework.get(QuickSearchService).quickSearch.show(
          ...
          {
            label: {
              i18nKey: 'com.notesgraph.cmdk.insert-links',
            },
            placeholder: {
              i18nKey: 'com.notesgraph.cmdk.docs.placeholder',
            },
          }
        )
      );
          ...
          item.action = ({ std }) => {
            ...
            insertedLinkType
              ?.then(type => {
                ...
                if (flavour === 'notesgraph:bookmark') {
                  track.doc.editor.slashMenu.bookmark();
                  return;
                }

                if (flavour === 'notesgraph:embed-linked-doc') {
