...
import { getSelectedBlocksCommand } from '@blocksuite/notesgraph/shared/commands';
import type { CommentProvider } from '@blocksuite/notesgraph/shared/services';
import { CommentProviderIdentifier } from '@blocksuite/notesgraph/shared/services';
import type { BlockStdScope } from '@blocksuite/notesgraph/std';
import { StdIdentifier } from '@blocksuite/notesgraph/std';
import type {
  BaseSelection,
  ExtensionType,
...
function getPreviewFromSelections(
  ...
  for (const selection of selections) {
    if (selection instanceof TextSelection) {
      ...
      if (block?.model) {
        const flavour = block.model.flavour.replace('notesgraph:', '');
      ...
      selection.elements.forEach(elementId => {
        ...
        if (model instanceof GfxPrimitiveElementModel) {
          const flavour = model.type.replace('notesgraph:', '');
          previews.push(`<${flavour}>`);
        } else if (model instanceof GfxBlockElementModel) {
          const flavour = model.flavour.replace('notesgraph:', '');
          previews.push(`<${flavour}>`);
        }
      });
    }
...
class NotesGraphCommentService implements CommentProvider {
...
export function NotesGraphCommentProvider(
  ...
  return {
    setup: (di: Container) => {
      di.addImpl(
        ...
        provider =>
          new NotesGraphCommentService(provider.get(StdIdentifier), framework)
      );
    },
  };
}
