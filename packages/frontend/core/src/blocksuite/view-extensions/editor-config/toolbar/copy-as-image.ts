...
import type { MenuContext } from '@blocksuite/notesgraph/components/toolbar';
import { Bound, getCommonBound } from '@blocksuite/notesgraph/global/gfx';
import type { BlockStdScope } from '@blocksuite/notesgraph/std';
import {
...
// Not from `blocksuite/ai/*`: this file is reached from the editor-config
// extension in the bootstrap-constructed view manager, and anything under
// `blocksuite/ai/` would drag the whole ~10MB AI chunk into the initial load.
import {
  isMindmapChild,
  isMindMapRoot,
...
const snapshotStyle = `
  notesgraph-edgeless-root .widgets-container,
...
export function copyAsImage(std: BlockStdScope) {
  if (!apis) {
    notify.error({
      title: I18n.t('com.notesgraph.copy.asImage.notAvailable.title'),
      message: I18n.t('com.notesgraph.copy.asImage.notAvailable.message'),
      actions: [
        {
          key: 'download',
          label: I18n.t('com.notesgraph.copy.asImage.notAvailable.action'),
          onClick: () => {
            window.open('https://notesgraph.com/download');
          },
        },
      ],
    });
  ...
  setTimeout(() => {
    ...
    try {
      ...
      const isFrameSelected =
        selected.length === 1 &&
        (selected[0] as GfxBlockElementModel).flavour === 'notesgraph:frame';
      ...
      apis.ui
        ...
        .then(() => {
          notify.success({
            title: I18n.t('com.notesgraph.copy.asImage.success'),
          });
        })
        .catch(e => {
          notify.error({
            title: I18n.t('com.notesgraph.copy.asImage.failed'),
            message: String(e),
          });
        })
        .finally(() => {
