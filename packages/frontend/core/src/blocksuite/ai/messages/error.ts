    ...
    return html` <div class="error-wrapper">
      ...
      <div class="action">
        ...
        >
          ${this.actionText}
          ${this.actionTooltip
            ? html`<notesgraph-tooltip tip-position="top">
                ${this.actionTooltip}
              </notesgraph-tooltip>`
            : nothing}
        </span>
...
const PaymentRequiredErrorRenderer = (host?: EditorHost | null) => html`
  <ai-error-wrapper
    .text=${"You've reached the current usage cap for NotesGraph AI. You can subscribe to NotesGraph AI(with free 7-day-trial) to continue the AI experience!"}
    .actionText=${'Upgrade'}
...
const LoginRequiredErrorRenderer = (host?: EditorHost | null) => html`
  <ai-error-wrapper
    .text=${'You need to login to NotesGraph Cloud to continue using NotesGraph AI.'}
    .actionText=${'Login'}
...
const GeneralErrorRenderer = (props: ErrorProps = {}) => {
  const onClick = () => {
    window.open('mailto:support@notesgraph.com', '_blank');
  };
  ...
  return html`<ai-error-wrapper
    ...
    .actionTooltip=${props.actionTooltip ?? 'support@notesgraph.com'}
    .onClick=${onClick}
  ></ai-error-wrapper>`;
};
