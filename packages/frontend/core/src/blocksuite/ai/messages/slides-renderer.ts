...
import { BlockStdScope, type EditorHost } from '@blocksuite/notesgraph/std';
import type { Store } from '@blocksuite/notesgraph/store';
import { WorkspaceImpl } from '@notesgraph/core/modules/workspace/impls/workspace';
...
import type { NotesGraphAIPanelWidgetConfig } from '../widgets/ai-panel/type';

export const createSlidesRenderer: (
  host: EditorHost,
  ctx: AIContext
) => NotesGraphAIPanelWidgetConfig['answerRenderer'] = (host, ctx) => {
  return (answer, state) => {
    if (state === 'generating') {
...
export class AISlidesRenderer extends WithDisposable(LitElement) {
  ...
  protected override render() {
    return html`<style>
        ...
        .edgeless-container notesgraph-edgeless-zoom-toolbar-widget,
        edgeless-toolbar {
          display: none;
        }
        ...
        * {
        ...
        .notesgraph-edgeless-surface-block-container {
          position: absolute;
          width: 100%;
          height: 100%;
        }
        ...
        .notesgraph-block-children-container.edgeless {
          padding-left: 0;
          position: relative;
          overflow: hidden;
          height: 100%;
        ...
        .notesgraph-edgeless-block-child {
          position: absolute;
          transform-origin: center;
          box-sizing: border-box;
          border: 2px solid var(--notesgraph-white-10);
          border-radius: 8px;
          box-shadow: var(--notesgraph-shadow-3);
          pointer-events: all;
        }

        notesgraph-edgeless-image .resizable-img,
        notesgraph-edgeless-image .resizable-img img {
          width: 100%;
          height: 100%;
        }

        .notesgraph-edgeless-layer {
          position: absolute;
          top: 0;
          left: 0;
      ...
      <div class="slides-container">
        <div
          class="edgeless-container notesgraph-edgeless-viewport"
  ...
  override connectedCallback(): void {
    ...
    doc.load(() => {
      const pageBlockId = doc.addBlock('notesgraph:page', {});
      doc.addBlock('notesgraph:surface', {}, pageBlockId);
    });
