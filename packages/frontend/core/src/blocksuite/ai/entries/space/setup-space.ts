...
export function setupSpaceAIEntry(panel: NotesGraphAIPanelWidget) {
  ...
  // In scenarios like (2), some browsers (see [ISSUE](https://github.com/notesgraph/notesgraph/issues/11541))
    ...
    if (hasAIRequestService() && isSpaceEvent(event)) {
      ...
      if (selection && selection.isCollapsed() && selection.from.index === 0) {
        ...
        if (
          ...
          block.model.flavour !== 'notesgraph:paragraph'
        )
          return;
