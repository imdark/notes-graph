...
import { CodeBlockHighlighter } from '@blocksuite/notesgraph/blocks/code';
import {
  SignalWatcher,
  WithDisposable,
...
> {
  static override styles = css`
    ...
    .code-artifact-preview > notesgraph-html-preview {
      height: 100%;
    }
  ...
  protected override getPreviewContent() {
    ...
    return html`<div class="code-artifact-preview">
      ${this.mode === 'preview'
        ? html`<notesgraph-html-preview
            .html=${htmlContent}
          ></notesgraph-html-preview>`
  ...
  protected override getPreviewControls() {
    ...
    const insertHtmlBlockToEnd = () => {
      try {
        ...
        if (!store) return;
        const notes = store.getBlocksByFlavour('notesgraph:note');
