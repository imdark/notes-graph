...
export class AIScrollableTextRenderer extends WithDisposable(
  ...
  @property({ attribute: false })
  accessor state: NotesGraphAIPanelState | undefined;
...
export const createAIScrollableTextRenderer: (
  ...
  maxHeight: number,
  autoScroll: boolean
) => NotesGraphAIPanelWidgetConfig['answerRenderer'] = (
  textRendererOptions,
  maxHeight,
  autoScroll
) => {
  return (answer: string, state: NotesGraphAIPanelState | undefined) => {
