...
export class AskAIIcon extends WithDisposable(LitElement) {
  ...
  static override styles = css`
    .ask-ai-icon-button {
      ...
      color: var(--notesgraph-brand-color);
    ...
    .ask-ai-icon-button.large {
      font-size: var(--notesgraph-font-md);
      svg {
        scale: 1.2;
      }
    }
    ...
    /* The star icon ships with a hardcoded blue fill; override it so the icon
     * matches the brand-purple "Ask AI" text. */
    .ask-ai-icon-button svg path {
      fill: var(--notesgraph-brand-color);
    }
  `;
