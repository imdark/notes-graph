...
import type { AIToolsConfigService } from '@notesgraph/core/modules/ai-button';
...
import { NotesGraphIcon } from '../../_common/icons';
import {
  AIAppEvents,
  type AIError,
  GeneralNetworkError,
...
export class AIChatMessages extends WithDisposable(ShadowlessElement) {
  static override styles = css`
    ...
    .down-indicator {
      ...
      width: 32px;
      height: 32px;
      border: 0.5px solid var(--notesgraph-border-color);
              ...
              >
                ${this.isHistoryLoading
                  ? html`<span data-testid="chat-panel-loading-state"
                      >NotesGraph AI is loading history...</span
                    >`
              ...
              (item, index) => {
                ...
                if (isChatMessage(item) && item.role === 'user') {
                  ...
                  return html`<chat-message-assistant
                    ...
                    .notesgraphFeatureFlagService=${this
