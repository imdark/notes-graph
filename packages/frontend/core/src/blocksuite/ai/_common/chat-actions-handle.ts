...
import { EdgelessCRUDIdentifier } from '@blocksuite/notesgraph/blocks/surface';
import {
  Bound,
...
import type { ImageSelection } from '@blocksuite/notesgraph/shared/selection';
import {
  DocModeProvider,
  EditPropsStore,
...
const ADD_TO_EDGELESS_AS_NOTE = {
  ...
  handler: async (host: EditorHost, content: string): Promise<boolean> => {
    ...
    const id = store.addBlock('notesgraph:note', props, store.root?.id);
...
export const SAVE_AS_DOC = {
  ...
  handler: (host: EditorHost, content: string) => {
    ...
    newDoc.load();
    const rootId = newDoc.addBlock('notesgraph:page');
    newDoc.addBlock('notesgraph:surface', {}, rootId);
    const noteId = newDoc.addBlock('notesgraph:note', {}, rootId);
