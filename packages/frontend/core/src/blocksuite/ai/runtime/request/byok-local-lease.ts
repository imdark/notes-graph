import { apis, type ClientHandler } from '@notesgraph/electron-api';
...
function isElectronBuild() {
  // BUILD_CONFIG only exists as compile-time member substitutions — a bare
  // `typeof BUILD_CONFIG` stays a runtime check that is always "undefined",
  // which silently made this return false even on desktop. try/catch covers
  // contexts where the constant truly doesn't exist.
  try {
