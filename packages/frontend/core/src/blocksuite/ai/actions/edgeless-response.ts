...
import { CodeBlockPreviewIdentifier } from '@blocksuite/notesgraph/blocks/code';
import { addImages } from '@blocksuite/notesgraph/blocks/image';
...
import { addTree } from '@blocksuite/notesgraph/gfx/mindmap';
import { fitContent } from '@blocksuite/notesgraph/gfx/shape';
import { createTemplateJob } from '@blocksuite/notesgraph/gfx/template';
...
import {
  ...
  type NotesGraphToolbarWidget,
} from '@blocksuite/notesgraph/widgets/toolbar';
...
import type { NotesGraphAIPanelWidget } from '../widgets/ai-panel/ai-panel';
...
type FinishConfig = Exclude<
  NotesGraphAIPanelWidget['config'],
  null
>['finishStateConfig'];
...
export function retry(panel: NotesGraphAIPanelWidget): AIItemConfig {
  return {
    name: 'Retry',
    icon: ResetIcon(),
    testId: 'answer-retry',
...
function createBlockAndInsert(
  ...
  doc.transact(() => {
    ...
    const surfaceBlock = doc.getBlocksByFlavour('notesgraph:surface')[0];
