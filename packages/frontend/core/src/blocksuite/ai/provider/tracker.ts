...
import type { GfxPrimitiveElementModel } from '@blocksuite/notesgraph/std/gfx';
import type { BlockModel } from '@blocksuite/notesgraph/store';
import { track, tracker } from '@notesgraph/track';
...
const inferPageMode = (host: EditorHost) => {
  return host.querySelector('notesgraph-page-root') ? 'doc' : 'edgeless';
};
...
function inferObjectType(event: AIRequestActionEvent) {
  ...
  if (!models) {
    ...
    if (flavour === 'notesgraph:note') {
      ...
      ['notesgraph:paragraph', 'notesgraph:list', 'notesgraph:code'].includes(
        flavour
      )
    ) {
      return 'text';
