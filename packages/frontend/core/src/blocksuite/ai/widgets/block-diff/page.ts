...
export const NOTESGRAPH_BLOCK_DIFF_WIDGET_FOR_PAGE =
  'notesgraph-block-diff-widget-for-page';

export class NotesGraphBlockDiffWidgetForPage extends WidgetComponent {
  ...
  _handleScroll(dir: 'prev' | 'next') {
    ...
    const diffWidgets = Array.from(
      this.std.host.querySelectorAll('notesgraph-block-diff-widget-for-block')
    );
...
export const blockDiffWidgetForPage = WidgetViewExtension(
  ...
  literal`${unsafeStatic(NOTESGRAPH_BLOCK_DIFF_WIDGET_FOR_PAGE)}`
);
