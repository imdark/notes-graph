...
export class AIPanelError extends WithDisposable(LitElement) {
  static override styles = css`
    ...
    .response-list-container ai-item-list {
      --item-padding: 4px;
      --item-icon-color: var(--notesgraph-icon-secondary);
      --item-icon-hover-color: var(--notesgraph-icon-color);
    }
  `;
    ...
    const errorTemplate = choose(
      ...
      [
        [
          ...
          () =>
            html` <div class="error-info">
                You need to login to NotesGraph Cloud to continue using
                NotesGraph AI.
              </div>
        ...
        [
          ...
          () =>
            html` <div class="error-info">
                You've reached the current usage cap for NotesGraph AI. You can
                subscribe to NotesGraph AI(with free 7-day-trial) to continue
                the AI experience!
              </div>
              <div class="action-button-group">
                <div @click=${this.config.cancel} class="action-button">
      ...
      () => {
        ...
        const error = tip
          ? html`<span class="error-tip">
              An error occurred
              <notesgraph-tooltip tip-position="bottom-start">
                ${tip}
              </notesgraph-tooltip>
            </span>`
          : 'An error occurred';
        return html`
          ...
          <div class="error-info">
            ...
            us know at
            <a href="mailto:support@notesgraph.com"> support@notesgraph.com </a>
          </div>
        `;
      }
    );
