...
import { RANGE_SYNC_EXCLUDE_ATTR } from '@blocksuite/notesgraph/std/inline';
import type { BaseSelection } from '@blocksuite/notesgraph/store';
import {
  NOTESGRAPH_VIEWPORT_OVERLAY_WIDGET,
  type NotesGraphViewportOverlayWidget,
} from '@blocksuite/notesgraph/widgets/viewport-overlay';
import {
  autoPlacement,
  autoUpdate,
  computePosition,
...
import type {
  AIActionAnswer,
  NotesGraphAIPanelState,
  NotesGraphAIPanelWidgetConfig,
} from './type.js';
import { mergeAIActionAnswer } from './utils';
export const NOTESGRAPH_AI_PANEL_WIDGET = 'notesgraph-ai-panel-widget';

export class NotesGraphAIPanelWidget extends WidgetComponent {
  static override styles = css`
    :host {
      ...
      border: 1px solid;
      border-color: ${unsafeCSSVar('--notesgraph-border-color')};
      ...
      --notesgraph-font-family: var(--notesgraph-font-sans-family);
    }
    ...
    :host([data-app-theme='light']) {
      ...
      border-color: ${unsafeCSS(
        lightCssVariables['--notesgraph-border-color']
      )};
    ...
    :host([data-app-theme='dark']) {
      ...
      border-color: ${unsafeCSS(darkCssVariables['--notesgraph-border-color'])};
  ...
  setState = (state: NotesGraphAIPanelState, reference: Element) => {
    this.state = state;
  ...
  get viewportOverlayWidget() {
    ...
    return rootId
      ? (this.host.view.getWidget(
          NOTESGRAPH_VIEWPORT_OVERLAY_WIDGET,
          rootId
        ) as NotesGraphViewportOverlayWidget)
      : null;
  }
  ...
  accessor config: NotesGraphAIPanelWidgetConfig | null = null;

  @query('ai-panel-generating')
  accessor generatingElement: AIPanelGenerating | null = null;

  @property()
  accessor state: NotesGraphAIPanelState = 'hidden';
...
export const aiPanelWidget = WidgetViewExtension(
  ...
  literal`${unsafeStatic(NOTESGRAPH_AI_PANEL_WIDGET)}`
);
