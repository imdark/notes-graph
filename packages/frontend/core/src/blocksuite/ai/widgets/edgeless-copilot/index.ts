import { EdgelessLegacySlotIdentifier } from '@blocksuite/notesgraph/blocks/surface';
import {
  Bound,
...
import { NOTESGRAPH_EDGELESS_COPILOT_WIDGET } from './constant.js';
...
export class EdgelessCopilotWidget extends WidgetComponent<RootBlockModel> {
  static override styles = css`
    .copilot-selection-rect {
      ...
      border-radius: 4px;
      border: 2px dashed var(--notesgraph-brand-color, #1e96eb);
    }
  `;
  ...
  private _showCopilotInput() {
    requestConnectedFrame(() => {
      ...
      if (input instanceof NotesGraphAIPanelWidget) {
  ...
  override render() {
    ...
    return html`<div class="notesgraph-edgeless-ai">
      <div
...
export const edgelessCopilotWidget = WidgetViewExtension(
  ...
  literal`${unsafeStatic(NOTESGRAPH_EDGELESS_COPILOT_WIDGET)}`
);
