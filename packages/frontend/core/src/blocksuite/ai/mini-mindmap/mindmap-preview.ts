import type { SurfaceBlockModel } from '@blocksuite/notesgraph/blocks/surface';
import {
  MindmapStyleFour,
  MindmapStyleOne,
  MindmapStyleThree,
...
import type { ServiceProvider } from '@blocksuite/notesgraph/global/di';
...
import { Schema, type Store } from '@blocksuite/notesgraph/store';
import { WorkspaceImpl } from '@notesgraph/core/modules/workspace/impls/workspace.js';
...
export class MiniMindmapPreview extends WithDisposable(LitElement) {
  static override styles = css`
    ...
    .template-item {
      ...
      border: 2px solid var(--notesgraph-border-color);
      border-radius: 4px;
      padding: 4px 6px;
    }
    ...
    .template-item:hover {
      border-color: var(--notesgraph-brand-color);
    }
  ...
  private _createTemporaryDoc() {
    ...
    const surfaceId = doc.addBlock('notesgraph:surface', {}, rootId);
