...
import type { Bound } from '@blocksuite/notesgraph/global/gfx';
import type { Color, ShapeElementModel } from '@blocksuite/notesgraph/model';
...
export class MindmapSurfaceBlock extends BlockComponent<SurfaceBlockModel> {
  ...
  override render() {
    return html`
      <style>
        .notesgraph-mini-mindmap-surface {
          width: 100%;
          height: 100%;
        }
      </style>
      <div class="notesgraph-mini-mindmap-surface">
        <!-- attach cavnas later in renderer -->
      </div>
    `;
  }

  @query('.notesgraph-mini-mindmap-surface')
