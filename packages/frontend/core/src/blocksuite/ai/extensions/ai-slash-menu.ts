...
export function AiSlashMenuConfigExtension() {
  ...
  const iconWrapper = (icon: AIItemConfig['icon']) => {
    return html`<div style="color: var(--notesgraph-primary-color)">
  ...
  const showWhenWrapper =
    ...
    ({ std }: SlashMenuContext) => {
      ...
      const notesgraphAIPanelWidget = std.view.getWidget(
        NOTESGRAPH_AI_PANEL_WIDGET,
        root.id
      );
      if (notesgraphAIPanelWidget === null) return false;
  ...
  const AIMenuItems: SlashMenuItem[] = [
    {
      ...
      action: ({ std }) => {
        ...
        const notesgraphAIPanelWidget = std.view.getWidget(
          NOTESGRAPH_AI_PANEL_WIDGET,
          root.id
        ) as NotesGraphAIPanelWidget;
        handleInlineAskAIAction(notesgraphAIPanelWidget.host);
      },
    },
    ...AIItems.filter(({ name }) =>
