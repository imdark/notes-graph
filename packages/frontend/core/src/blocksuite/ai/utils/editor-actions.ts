import { clipboardConfigs } from '@blocksuite/notesgraph/foundation/clipboard';
import { defaultImageProxyMiddleware } from '@blocksuite/notesgraph/shared/adapters';
import { replaceSelectedTextWithBlocksCommand } from '@blocksuite/notesgraph/shared/commands';
import { isInsideEdgelessEditor } from '@blocksuite/notesgraph/shared/utils';
import {
  type BlockComponent,
  BlockSelection,
  BlockStdScope,
...
const getNoteId = (blockElement: BlockComponent) => {
  ...
  while (element.flavour !== 'notesgraph:note') {
...
export const replace = async (
  ...
  if (textSelection) {
    ...
    try {
      ...
      const rootId = fragment.addBlock('notesgraph:page');
      fragment.addBlock('notesgraph:surface', {}, rootId);
      const noteId = fragment.addBlock('notesgraph:note', {}, rootId);
      ...
      if (snapshot) {
        const blockSnapshots = (
          snapshot.content[0].flavour === 'notesgraph:note'
...
export const copyTextAnswer = async (panel: NotesGraphAIPanelWidget) => {
  if (!panel.answer) {
    return false;
  }
...
export const copyText = async (text: string) => {
  ...
  const models = previewDoc
    .getBlocksByFlavour('notesgraph:note')
    .map(b => b.model)
