...
import { type Notification, notify } from '@notesgraph/component';
import {
  RouteLogic,
  useNavigateHelper,
} from '@notesgraph/core/components/hooks/use-navigate-helper';
...
export const AIOnboardingLocal = () => {
  ...
  const actions = useMemo(() => {
    const result: NonNullable<Notification['actions']> = [
      {
        key: 'learn-more',
        label: t['com.notesgraph.ai-onboarding.local.action-learn-more'](),
        onClick: () => {
          window.open('https://ai.notesgraph.com', '_blank', 'noreferrer');
        },
      },
    ];
    if (notSignedIn) {
      result.push({
        key: 'get-started',
        label: t['com.notesgraph.ai-onboarding.local.action-get-started'](),
        onClick: () => {
      ...
      const id = notify(
        {
          title: (
            <div className={styles.title}>
              {t['com.notesgraph.ai-onboarding.local.title']()}
            </div>
          ),
          message: t['com.notesgraph.ai-onboarding.local.message'](),
          icon: <AiIcon />,
