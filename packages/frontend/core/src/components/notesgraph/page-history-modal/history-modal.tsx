...
import type { Store, Workspace } from '@blocksuite/notesgraph/store';
import { Avatar, Loading, Scrollable } from '@notesgraph/component';
import { EditorLoading } from '@notesgraph/component/page-detail-skeleton';
import { Button, IconButton } from '@notesgraph/component/ui/button';
import { Modal, useConfirmModal } from '@notesgraph/component/ui/modal';
...
import { i18nTime, Trans, useI18n } from '@notesgraph/i18n';
...
import { NotesGraphErrorBoundary } from '../notesgraph-error-boundary';
import {
      ...
      <div className={styles.previewContent}>
        ...
        {snapshotPage ? (
          <NotesGraphErrorBoundary>
            <Scrollable.Root>
              <Scrollable.Viewport className="notesgraph-page-viewport">
                <BlockSuiteEditor
...
const PlanPrompt = () => {
      ...
      <div className={styles.planPromptTitle}>
        {
          isProWorkspace !== null
            ? !isProWorkspace
              ? t[
                  'com.notesgraph.history.confirm-restore-modal.plan-prompt.limited-title'
                ]()
              : t[
                  'com.notesgraph.history.confirm-restore-modal.plan-prompt.title'
                ]()
  ...
  const planDescription = useMemo(() => {
    if (!isProWorkspace) {
      return (
        <>
          <Trans i18nKey="com.notesgraph.history.confirm-restore-modal.free-plan-prompt.description">
          ...
          {isOwner ? (
            ...
            >
              {t[
                'com.notesgraph.history.confirm-restore-modal.pro-plan-prompt.upgrade'
              ]()}
            </span>
          ) : null}
        </>
      ...
      return (
        <Trans i18nKey="com.notesgraph.history.confirm-restore-modal.pro-plan-prompt.description">
  ...
  return (
    <div className={styles.historyList}>
      <div className={styles.historyListHeader}>
        {t['com.notesgraph.history.version-history']()}
      </div>
                    ...
                    return (
                      <Fragment key={history.timestamp}>
                        ...
                        {isLastGroup && isLastItem && onLoadMore ? (
                          ...
                          >
                            {t[
                              'com.notesgraph.history.confirm-restore-modal.load-more'
                            ]()}
                          </Button>
                        ) : null}
                      </Fragment>
  ...
  return (
    ...
    >
      ...
      <div className={styles.emptyHistoryPromptTitle}>
        {t['com.notesgraph.history.empty-prompt.title']()}
      </div>
      <div className={styles.emptyHistoryPromptDescription}>
        {t['com.notesgraph.history.empty-prompt.description']()}
      </div>
    </div>
  );
};
...
const PageHistoryManager = ({
  ...
  const onConfirmRestore = useCallback(() => {
    openConfirmModal({
      title: t['com.notesgraph.history.restore-current-version'](),
      description: t['com.notesgraph.history.confirm-restore-modal.hint'](),
      cancelText: t['Cancel'](),
      contentOptions: {
        ['data-testid' as string]: 'confirm-restore-history-modal',
        style: { padding: '20px 26px' },
      },
      confirmText: t['com.notesgraph.history.confirm-restore-modal.restore'](),
    ...
    <div className={styles.root}>
      ...
      <div className={styles.historyFooter}>
        <Button onClick={onClose}>
          {t['com.notesgraph.history.back-to-page']()}
        </Button>
        ...
        >
          {t['com.notesgraph.history.restore-current-version']()}
        </Button>
      </div>
    </div>
  );
