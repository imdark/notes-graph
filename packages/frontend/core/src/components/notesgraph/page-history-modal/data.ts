...
import { useDocMetaHelper } from '@notesgraph/core/components/hooks/use-block-suite-page-meta';
import { useDocCollectionPage } from '@notesgraph/core/components/hooks/use-block-suite-workspace-page';
import { FetchService, GraphQLService } from '@notesgraph/core/modules/cloud';
import {
...
import type { ListHistoryQuery } from '@notesgraph/graphql';
import { listHistoryQuery, recoverDocMutation } from '@notesgraph/graphql';
...
export const useSnapshotPage = (
  ...
  const notesgraphWorkspace = useService(WorkspaceService).workspace;
  ...
  const page = useMemo(() => {
    ...
    const historyShellWorkspace = getOrCreateShellWorkspace(
      docCollection.id,
      workspacesService.getWorkspaceFlavourProvider(notesgraphWorkspace.meta)
    );
