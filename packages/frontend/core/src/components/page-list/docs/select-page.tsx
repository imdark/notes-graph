...
import {
  ...
  type FilterParams,
} from '@notesgraph/core/modules/collection-rules';
import { ShareDocsListService } from '@notesgraph/core/modules/share-doc';
...
import { NotesGraphShapeIcon } from '..';
...
export const SelectPage = memo(function SelectPage({
  ...
  return (
    <SelectorLayout
      searchPlaceholder={t[
        'com.notesgraph.editCollection.search.placeholder'
      ]()}
        ...
        <div className={styles.pagesTabContent}>
          {header ?? (
            <div style={{ fontSize: 12, lineHeight: '20px', fontWeight: 600 }}>
              {t['com.notesgraph.selectPage.title']()}
            </div>
          )}
          {filters.length === 0 ? (
            <Menu
...
export const EmptyList = ({ search }: { search?: string }) => {
  ...
  return (
    ...
    >
      ...
      >
        {t['com.notesgraph.selectPage.empty']()}
      </div>
      {search ? (
        <div
        ...
        >
          <Trans
            i18nKey="com.notesgraph.selectPage.empty.tips"
            values={{ search }}
          >
