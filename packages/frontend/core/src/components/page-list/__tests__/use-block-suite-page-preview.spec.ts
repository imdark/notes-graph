...
beforeEach(async () => {
  ...
  const initPage = async (page: Store) => {
    ...
    const pageBlockId = page.addBlock('notesgraph:page', {
      title: new Text(''),
    });
    const frameId = page.addBlock('notesgraph:note', {}, pageBlockId);
    page.addBlock('notesgraph:paragraph', {}, frameId);
  };
...
describe('useBlockSuitePagePreview', () => {
  test('basic', async () => {
    ...
    const id = page.addBlock(
      ...
      page.getModelsByFlavour('notesgraph:note')[0].id
    );
