import { displayFlex, positionAbsolute, styled } from '@notesgraph/component';
...
export const StyledIsland = styled('div')<{
  ...
  return {
    ...
    borderRadius: '10px',
    background: spread
      ? 'var(--notesgraph-background-overlay-panel-color)'
      : 'var(--notesgraph-background-primary-color)',
    ':hover': {
      background: spread ? undefined : 'var(--notesgraph-white)',
      boxShadow: spread ? undefined : 'var(--notesgraph-menu-shadow)',
    },
    '::after': {
      content: '""',
      width: '36px',
      height: '1px',
      background: spread ? 'var(--notesgraph-border-color)' : 'transparent',
      ...positionAbsolute({
        left: 0,
        right: 0,
        bottom: '44px',
...
export const StyledTriggerWrapper = styled('div')<{
  ...
  return {
    ...
    ':hover': {
      backgroundColor: spread ? 'var(--notesgraph-hover-color)' : undefined,
    },
  };
});
