...
import { printToPdf } from '@blocksuite/notesgraph/shared/utils';
import type { BlockStdScope } from '@blocksuite/notesgraph/std';
import { type Store, Transformer } from '@blocksuite/notesgraph/store';
import {
  createAssetsArchive,
  download,
  HtmlTransformer,
...
import type { NotesGraphEditorContainer } from '@notesgraph/core/blocksuite/block-suite-editor/blocksuite-editor';
import { EditorService } from '@notesgraph/core/modules/editor';
import { getNotesGraphWorkspaceSchema } from '@notesgraph/core/modules/workspace/global-schema';
...
import { useAsyncCallback } from '../notesgraph-async-hooks';
  ...
  const onClickHandler = useAsyncCallback(
    async (type: ExportType) => {
      ...
      const originEditorContainer = (editorContainer as any)
        .origin as NotesGraphEditorContainer;
      ...
      try {
        ...
        if (!success) {
          notify.error({
            title: t['com.notesgraph.export.error.title'](),
            message: t['com.notesgraph.export.error.message'](),
          });
          return;
        }
        ...
        if (type === 'copy-markdown') {
          notify.success({
            title: t['com.notesgraph.export.copied-as-markdown'](),
          });
        } else {
          notify.success({
            title: t['com.notesgraph.export.success.title'](),
            message: t['com.notesgraph.export.success.message'](),
          });
        }
      } catch (err) {
        console.error(err);
