...
describe('use-navigate-helper utilities', () => {
  ...
  test('buildWorkspaceSettingsRedirectUri builds a settings redirect from a workspace page', () => {
    expect(
      buildWorkspaceSettingsRedirectUri(
        'https://app.notesgraph.com/workspace/workspace-1/all',
        {
          tab: 'workspace:integrations',
          scrollAnchor: 'integration-calendar',
        }
      ...
      'https://app.notesgraph.com/workspace/workspace-1/settings?tab=workspace%3Aintegrations&scrollAnchor=integration-calendar'
    );
  });
  ...
  test('buildWorkspaceSettingsRedirectUri preserves app subpaths before the workspace route', () => {
    expect(
      buildWorkspaceSettingsRedirectUri(
        'https://app.notesgraph.com/app/workspace/workspace-1/collection',
        {
          tab: 'workspace:integrations',
          scrollAnchor: 'integration-calendar',
        }
      ...
      'https://app.notesgraph.com/app/workspace/workspace-1/settings?tab=workspace%3Aintegrations&scrollAnchor=integration-calendar'
    );
  });

  test('buildWorkspaceSettingsRedirectUri falls back to the current url when no workspace route is present', () => {
    expect(
      buildWorkspaceSettingsRedirectUri('https://app.notesgraph.com/sign-in', {
        tab: 'workspace:integrations',
      })
    ).toBe('https://app.notesgraph.com/sign-in');
  });
});
