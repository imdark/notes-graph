...
import { DocIntegrationPropertiesTable } from '@notesgraph/core/modules/integration';
import {
  ViewService,
  WorkbenchService,
    ...
    <Collapsible.Trigger style={style} role="button" onClick={handleCollapse}>
      <div className={clsx(styles.tableHeader, className)}>
        <div className={clsx(!open ? styles.pageInfoDimmed : null)}>
          {t['com.notesgraph.page-properties.page-info']()}
        </div>
        <div
...
export const WorkspacePropertyRow = ({
  ...
  const { dropTargetRef, closestEdge } = useDropTarget<NotesGraphDNDData>(
    () => ({
      closestEdge: {
  ...
  // The page/edgeless default-mode property is meaningless with one mode.
  if (propertyInfo.type === 'docPrimaryMode' && docModes.length < 2)
    return null;
  // Edgeless theme is meaningless without the edgeless mode to apply it to.
  if (propertyInfo.type === 'edgelessTheme' && docModes.length < 2)
    return null;
        ...
        >
          ...
          <div className={styles.actionContainer}>
            ...
            {viewService ? (
              ...
              >
                {t['com.notesgraph.page-properties.config-properties']()}
              </Button>
            ) : null}
          </div>
        </PropertyCollapsibleContent>
