import { type MenuRef, PropertyValue } from '@notesgraph/component';
...
const CreatedUsernameTip = ({ userName }: { userName: string }) => {
  const i18n = useI18n();
  return i18n.t('com.notesgraph.page-properties.property.createdBy.tip', {
    userName,
  });
};
const UpdatedUsernameTip = ({ userName }: { userName: string }) => {
  const i18n = useI18n();
  return i18n.t('com.notesgraph.page-properties.property.updatedBy.tip', {
    userName,
  });
};
...
const NoRecordValue = () => {
  ...
  return (
    <span>
      {t['com.notesgraph.page-properties.property-user-avatar-no-record']()}
    </span>
  );
};
...
const LocalUserValue = () => {
  const t = useI18n();
  return <span>{t['com.notesgraph.page-properties.local-user']()}</span>;
};
