...
import { AuthService, CaptchaService } from '@notesgraph/core/modules/cloud';
import type { AuthSessionStatus } from '@notesgraph/core/modules/cloud/entities/session';
...
export const SignInWithEmailStep = ({
  ...
  useEffect(() => {
    if (loginStatus === 'authenticated') {
      notify.success({
        title: t['com.notesgraph.auth.toast.title.signed-in'](),
        message: t['com.notesgraph.auth.toast.message.signed-in'](),
      });
    }
  ...
  const onContinue = useAsyncCallback(async () => {
    ...
    if (otp.length !== 6 || !/[0-9]{6}/.test(otp)) {
      setOtpError(t['com.notesgraph.auth.sign.auth.code.invalid']());
      return;
    }
  ...
  return !verifyToken && needCaptcha ? (
    <>
      <AuthHeader title={t['com.notesgraph.auth.sign.in']()} />
    ...
    <AuthContainer>
      <AuthHeader
        title={t['com.notesgraph.auth.sign.in']()}
        subTitle={t['com.notesgraph.auth.sign.in.sent.email.subtitle']()}
      />
      <AuthContent>
        <p>
          <Trans
            i18nKey="com.notesgraph.auth.sign.auth.code.hint"
            values={{ email }}
        ...
        <AuthInput
          placeholder={t['com.notesgraph.auth.sign.auth.code']()}
        ...
        <Button
          ...
          loading={isVerifying}
        >
          {t['com.notesgraph.auth.sign.auth.code.continue']()}
        </Button>
        ...
        >
          {resendCountDown <= 0 ? (
            t['com.notesgraph.auth.sign.auth.code.resend']()
          ) : (
            <Trans
              i18nKey="com.notesgraph.auth.sign.auth.code.resend.hint"
      ...
      <AuthFooter>
        <div className={style.authMessage} style={{ marginTop: 20 }}>
          {t['com.notesgraph.auth.sign.auth.code.message']()}
          &nbsp;
          {state.hasPassword && (
            <Trans
              i18nKey="com.notesgraph.auth.sign.auth.code.message.password"
              components={{
                1: (
                  <span
                    className="link"
