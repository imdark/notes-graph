...
import { Loading } from '@notesgraph/component/ui/loading';
import { useSystemOnline } from '@notesgraph/core/components/hooks/use-system-online';
...
const useSyncEngineSyncProgress = (meta: WorkspaceMetadata) => {
  ...
  if (workspace.flavour === 'local') {
    ...
    content =
      `Syncing with NotesGraph Cloud` +
      (progress ? ` (${Math.floor(progress * 100)}%)` : '');
  } else {
    content = 'Synced with NotesGraph Cloud';
  }
...
export const WorkspaceCard = forwardRef<
  ...
  HTMLAttributes<HTMLDivElement> & {
    ...
    /** show the hover delete button (workspace list menu only) */
    showDelete?: boolean;
  }
>(
  (
    ...
    const doRemoveWorkspace = useAsyncCallback(async () => {
    ...
    const onRemoveWorkspace = useCatchEventCallback(() => {
      openConfirmModal({
        title: `Delete “${name}”?`,
        description:
          workspaceMetadata.flavour === 'local'
            ? 'This permanently deletes the workspace and all of its docs from this device. This cannot be undone.'
            : 'This permanently deletes the workspace and all of its docs for every member. This cannot be undone.',
        confirmText: 'Delete',
        cancelText: t['Cancel'](),
        confirmButtonOptions: { variant: 'error' },
        onConfirm: doRemoveWorkspace,
      });
    }, [openConfirmModal, name, workspaceMetadata.flavour, doRemoveWorkspace, t]);

    // deleting is allowed for local workspaces and cloud workspaces you own
    const canDelete =
      showDelete &&
      (workspaceMetadata.flavour === 'local' || information?.isOwner);
      ...
      >
        ...
        <div className={styles.suffixIcons}>
          {hideCollaborationIcon || information?.isOwner ? null : (
            <Tooltip
              content={t['com.notesgraph.settings.workspace.state.joined']()}
            >
          ...
          {hideTeamWorkspaceIcon || !information?.isTeam ? null : (
            <Tooltip
              content={t['com.notesgraph.settings.workspace.state.team']()}
            >
