...
import { IconButton, Menu, MenuItem } from '@notesgraph/component';
import { Divider } from '@notesgraph/component/ui/divider';
...
import type { AuthAccountInfo, Server } from '@notesgraph/core/modules/cloud';
import { AuthService, ServersService } from '@notesgraph/core/modules/cloud';
...
const WorkspaceServerInfo = ({
  ...
  const isNotesGraphCloud = server === 'notesgraph-cloud';
  const Icon = isNotesGraphCloud
    ? CloudWorkspaceIcon
    : isCloud
      ? SelfhostIcon
      : LocalWorkspaceIcon;
  ...
  const menuItems = useMemo(
    () =>
      [
        server !== 'notesgraph-cloud' && server !== 'local' && (
          <MenuItem
          ...
          >
            {t['com.notesgraph.server.delete']()}
          </MenuItem>
        ),
...
const LocalWorkspaces = ({
  ...
  return (
    <>
      <WorkspaceServerInfo
        server="local"
        name={t['com.notesgraph.workspaceList.workspaceListType.local']()}
      />
      <WorkspaceList
        items={workspaces}
        onClick={onClickWorkspace}
...
export const NotesGraphWorkspaceList = ({
  onEventEnd,
  onClickWorkspace,
  ...
  const notesgraphCloudServer = useMemo(
    () => servers.find(s => s.id === 'notesgraph-cloud') as Server,
    [servers]
  );
  const selfhostServers = useMemo(
    () => servers.filter(s => s.id !== 'notesgraph-cloud'),
    [servers]
  );
  ...
  return (
    <>
      {/* 1. notesgraph-cloud */}
      <FrameworkScope
        key={notesgraphCloudServer.id}
        scope={notesgraphCloudServer.scope}
      >
        <CloudWorkSpaceList
          server={notesgraphCloudServer}
          workspaces={cloudWorkspaces.filter(
            ({ flavour }) => flavour === notesgraphCloudServer.id
          )}
