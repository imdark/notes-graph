...
import type { DocSnapshot, Store } from '@blocksuite/notesgraph/store';
import {
  Avatar,
  IconButton,
  Loading,
...
import { AuthService } from '@notesgraph/core/modules/cloud/services/auth';
import { type DocCommentEntity } from '@notesgraph/core/modules/comment/entities/doc-comment';
import { CommentPanelService } from '@notesgraph/core/modules/comment/services/comment-panel-service';
import { DocCommentManagerService } from '@notesgraph/core/modules/comment/services/doc-comment-manager';
import type {
  CommentAttachment,
  DocComment,
  DocCommentReply,
} from '@notesgraph/core/modules/comment/types';
...
type CommentAccount = { id: string; label?: string; avatar?: string | null };

/**
 * The comment author identity. Cloud workspaces use the signed-in account;
 * local (offline) workspaces have no cloud account, so fall back to a local
 * identity — otherwise the comment/reply inputs (which require an account)
 * would never render on local.
 */
export const useCommentAccount = (): CommentAccount | null => {
  const session = useService(AuthService).session;
  const workspaceService = useService(WorkspaceService);
  const rawAccount = useLiveData(session.account$);
  const isLocal = workspaceService.workspace.flavour === 'local';
  return useMemo(
    () => rawAccount ?? (isLocal ? { id: 'local', label: 'You', avatar: null } : null),
    [rawAccount, isLocal]
  );
};
    ...
    <Menu
      ...
      items={
        <>
          ...
          >
            {t['com.notesgraph.comment.filter.show-resolved']()}
          </MenuItem>
          <MenuItem
          ...
          >
            {t['com.notesgraph.comment.filter.only-my-replies']()}
          </MenuItem>
          <MenuItem
          ...
          >
            {t['com.notesgraph.comment.filter.only-current-mode']()}
          </MenuItem>
        </>
      }
    >
        ...
        <>
          {canReply ? (
            <MenuItem onClick={onReply} disabled={!!disabled || !!resolved}>
              {t['com.notesgraph.comment.reply']()}
            </MenuItem>
          ) : null}
          {canCopyLink ? (
            <MenuItem onClick={onCopyLink} disabled={disabled}>
              {t['com.notesgraph.comment.copy-link']()}
            </MenuItem>
          ) : null}
          {canEdit ? (
...
export const CommentItem = ({
  ...
  const handleDelete = useAsyncCallback(
    async (e: React.MouseEvent) => {
      e.stopPropagation();
      openConfirmModal({
        title: t['com.notesgraph.comment.delete.confirm.title'](),
        description: t['com.notesgraph.comment.delete.confirm.description'](),
      ...
      >
        {canReply && (
          <IconButton
            ...
            icon={<AddCommentIcon />}
            disabled={isMutating}
            tooltip={t['com.notesgraph.comment.reply']()}
          />
        )}
        <ActionMenu
          open={menuOpen}
...
const ReplyItem = ({
  ...
  const handleDelete = useAsyncCallback(async () => {
    openConfirmModal({
      title: t['com.notesgraph.comment.reply.delete.confirm.title'](),
      description:
        t['com.notesgraph.comment.reply.delete.confirm.description'](),
      confirmText: t['Delete'](),
      cancelText: t['Cancel'](),
      confirmButtonOptions: {
        variant: 'error',
      ...
      <>
        ...
        >
          <div className={styles.collapsedRepliesTitle}>
            {t['com.notesgraph.comment.reply.show-more']({
