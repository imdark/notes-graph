...
import { getAttachmentFileIcon } from '@blocksuite/notesgraph/components/icons';
import {
  Button,
  Checkbox,
  Loading,
...
import { Pagination } from '@notesgraph/component/setting-components';
import { BlobManagementService } from '@notesgraph/core/modules/blob-management/services';
import { useI18n } from '@notesgraph/i18n';
import { useLiveData, useService } from '@notesgraph/infra';
import type { ListedBlobRecord } from '@notesgraph/nbstore';
...
const Empty = () => {
  ...
  return (
    <div className={styles.empty}>
      {t['com.notesgraph.settings.workspace.storage.unused-blobs.empty']()}
    </div>
  );
};
  ...
  const handleDeleteSelectedBlobs = useCallback(
    (e: React.MouseEvent) => {
      ...
      openConfirmModal({
        title:
          t[
            'com.notesgraph.settings.workspace.storage.unused-blobs.delete.title'
          ](),
        children:
          t[
            'com.notesgraph.settings.workspace.storage.unused-blobs.delete.warning'
          ](),
        onConfirm: async () => {
          setDeleting(true);
      ...
      {selectedBlobs.length > 0 ? (
        <div className={styles.blobManagementControls}>
          <div className={styles.blobManagementName}>
            {`${selectedBlobs.length} ${t['com.notesgraph.settings.workspace.storage.unused-blobs.selected']()}`}
          </div>
          <div className={styles.spacer} />
          {showSelectAll && (
            <Button onClick={handleSelectAll} variant="primary">
              {t['com.notesgraph.keyboardShortcuts.selectAll']()}
            </Button>
          )}
          <Button
            loading={deleting}
        ...
        <div className={styles.blobManagementNameInactive}>
          {`${t['com.notesgraph.settings.workspace.storage.unused-blobs']()} (${unusedBlobs.length})`}
        </div>
      )}
      {isEmpty ? (
        <Empty />
