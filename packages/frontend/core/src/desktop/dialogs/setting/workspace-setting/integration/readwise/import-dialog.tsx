...
import type { ReadwiseHighlight } from '@notesgraph/core/modules/integration/type';
...
export const ImportDialog = ({ onClose }: { onClose: () => void }) => {
  ...
  const handleConfirmImport = useCallback(
    (ids: string[]) => {
      ...
      readwise
        .highlightsToNotesGraphDocs(selectedHighlights.reverse(), books, {
          signal,
          ...
          onAbort: finished => {
            ...
            notify({
              ...
              title:
                t[
                  'com.notesgraph.integration.readwise.import.abort-notify-title'
                ](),
              message: t.t(
                'com.notesgraph.integration.readwise.import.abort-notify-desc',
                { finished }
              ),
            });
          },
    ...
    <>
      <header>
        <h3 className={styles.title}>
          {t['com.notesgraph.integration.readwise.import.title']()}
        </h3>
        <div className={styles.desc}>
          {lastImportedAt ? (
            <Trans
              i18nKey="com.notesgraph.integration.readwise.import.desc-from-last"
              values={{
            ...
            t['com.notesgraph.integration.readwise.import.desc-from-start']()
          )}
        </div>
    ...
    <div className={styles.table}>
      <div className={styles.tableHeadRow}>
        ...
        <div className={styles.tableCellTitle}>
          {t['com.notesgraph.integration.readwise.import.cell-h-content']()}
        </div>
        <div className={styles.tableCellTodo}>
          {t['com.notesgraph.integration.readwise.import.cell-h-todo']()}
        </div>
        <div className={styles.tableCellTime}>
          {t['com.notesgraph.integration.readwise.import.cell-h-time']()}
        </div>
      </div>
      <Virtuoso
              ...
              <div className={styles.tableCellTodo}>
                {action === 'new' ? (
                  <span className={styles.todoNew}>
                    {t['com.notesgraph.integration.readwise.import.todo-new']()}
                  </span>
                ) : action === 'skip' ? (
                  <span className={styles.todoSkip}>
                    {t[
                      'com.notesgraph.integration.readwise.import.todo-skip'
                    ]()}
                  </span>
                ) : (
                  <span className={styles.todoUpdate}>
                    {t[
                      'com.notesgraph.integration.readwise.import.todo-update'
                    ]()}
                  </span>
                )}
              </div>
...
const HighlightEmpty = () => {
  ...
  return (
    <div className={styles.empty}>
      {t['com.notesgraph.integration.readwise.import.empty']()}
    </div>
  );
};
    ...
    <>
      <header className={styles.importingHeader}>
        ...
        <h3 className={styles.importingTitle}>
          {t['com.notesgraph.integration.readwise.import.importing']()}
        </h3>
      </header>

      <main className={styles.importingDesc}>
        {t['com.notesgraph.integration.readwise.import.importing-desc']()}
      </main>

      <footer className={styles.importingFooter}>
        <Button variant="error" onClick={onStop}>
          {t['com.notesgraph.integration.readwise.import.importing-stop']()}
        </Button>
      </footer>
    </>
  );
