...
export const AgentEditor = ({
  ...
  const [maxSteps, setMaxSteps] = useState(agent?.maxSteps ?? DEFAULT_MAX_STEPS);
    ...
    >
      <div className={styles.editor}>
        ...
        <div className={styles.field}>
          ...
          <div className={styles.checkGrid}>
            {READ_TOOLS.map(({ name: tool, desc }) => (
              <label key={tool} className={styles.check} title={desc}>
                <Switch
          ...
          <span className={styles.hint}>
            Read-only. Anything an agent produces is shown for you to accept
            before it touches a note.
          </span>
        </div>

        <div className={styles.field}>
          <span className={styles.label}>Folder files</span>
          <div className={styles.checkGrid}>
            {FILE_TOOLS.map(({ name: tool, desc, mutates }) => (
              <label key={tool} className={styles.check} title={desc}>
                <Switch
                  checked={tools.includes(tool)}
                  onChange={() => setTools(prev => toggle(prev, tool))}
                />
                {mutates ? `${desc} (changes files)` : desc}
              </label>
            ))}
          </div>
          <span className={styles.hint}>
            Only works while a folder is bound to this workspace, and never
            reaches outside it. Overwrites and deletes keep a copy under
            .notesgraph/backups first.
          </span>
        </div>
        ...
        <div className={styles.field}>
          ...
          <Input
            ...
            onChange={value => {
              ...
              setMaxSteps(
                Number.isFinite(parsed) && parsed > 0 ? parsed : DEFAULT_MAX_STEPS
              );
            }}
