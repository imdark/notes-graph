...
export const SelfHostTeamCard = () => {
  ...
  const description = useMemo(() => {
    if (isTeam) {
      return (
        <div>
          <p>
            {t[
              'com.notesgraph.settings.workspace.license.self-host-team.team.description'
            ]({
          ...
          {isOneTimePurchase ? (
            <p>
              <Trans
                i18nKey="com.notesgraph.settings.workspace.license.self-host-team.team.license"
                components={{ 1: <strong /> }}
              />
            </p>
          ) : null}
    ...
    return t[
      'com.notesgraph.settings.workspace.license.self-host-team.free.description'
    ]({
    ...
    (license: string) => {
      ...
      selfhostLicenseService
        ...
        .then(() => {
          ...
          notify.success({
            title:
              t['com.notesgraph.settings.workspace.license.activate-success'](),
          });
        })
        .catch(e => {
          setLoading(false);
  ...
  const onDeactivate = useCallback(() => {
    ...
    selfhostLicenseService
      ...
      .then(() => {
        ...
        notify.success({
          title:
            t['com.notesgraph.settings.workspace.license.deactivate-success'](),
        });
      })
      .catch(e => {
        setLoading(false);
      ...
      <div className={styles.planCard}>
        <div className={styles.container}>
          <div className={styles.currentPlan}>
            <SettingRow
              spreadCol={false}
              name={t[
                `com.notesgraph.settings.workspace.license.self-host${isTeam ? '-team' : ''}`
              ]()}
              desc={description}
            />
          </div>
          ...
          >
            <span className={styles.seat}>
              {t[
                'com.notesgraph.settings.workspace.license.self-host-team.seats'
              ]()}
            </span>
            <span>
        ...
        >
          {!isTeam && !isLocalWorkspace ? (
            ...
            >
              {t[
                'com.notesgraph.settings.workspace.license.self-host-team.upload-license-file'
              ]()}
            </Button>
          ) : null}
          <Button
          ...
          >
            {t[
              `com.notesgraph.settings.workspace.license.self-host-team.${isTeam ? 'deactivate-license' : 'use-purchased-key'}`
            ]()}
          </Button>
        </div>
      </div>
...
const ActionModal = ({
  ...
  if (isTeam && isOneTimePurchase) {
    return (
      <ConfirmModal
        ...
        title={t[
          `com.notesgraph.settings.workspace.license.deactivate-modal.title`
        ]()}
        description={t[
          'com.notesgraph.settings.workspace.license.deactivate-modal.description-license'
        ]()}
        cancelText={t['Cancel']()}
        cancelButtonOptions={{
          variant: 'secondary',
  ...
  if (isTeam) {
    return (
      <Modal
        ...
        description={
          <Trans
            i18nKey="com.notesgraph.settings.workspace.license.deactivate-modal.description"
            components={{
              1: <strong />,
            }}
          />
      ...
      >
        <div className={styles.footer}>
          ...
          >
            {t[
              'com.notesgraph.settings.workspace.license.deactivate-modal.manage-payment'
            ]()}
          </Button>
  ...
  return (
    <ConfirmModal
      ...
      onOpenChange={handleOpenChange}
      title={t[
        'com.notesgraph.settings.workspace.license.activate-modal.title'
      ]()}
      description={t[
        'com.notesgraph.settings.workspace.license.activate-modal.description'
      ]()}
      cancelText={t['Cancel']()}
      cancelButtonOptions={{
        variant: 'secondary',
    ...
    >
      ...
      <span className={styles.tips}>
        <Trans
          i18nKey="com.notesgraph.settings.workspace.license.activate-modal.tips"
          components={{
            1: (
              <a
                href="https://notesgraph.com/pricing/?type=selfhost#table"
                target="_blank"
                rel="noreferrer"
