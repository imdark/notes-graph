import { Button, ErrorMessage, Skeleton } from '@notesgraph/component';
...
export const AIUsagePanel = ({
  ...
  if (loading) {
    if (loadError) {
      return (
        <SettingRow
          name={t['com.notesgraph.payment.ai.usage-title']()}
          desc={''}
          spreadCol={false}
        >
          {/* TODO(@catsjuice): i18n */}
  ...
  return (
    <SettingRow
      ...
      desc={
        aiSubscription
          ? t['com.notesgraph.payment.ai.usage-description-purchased']()
          : ''
      }
      name={t['com.notesgraph.payment.ai.usage-title']()}
    >
      {copilotActionLimit === 'unlimited' ? (
        hasPaymentFeature && aiSubscription?.canceledAt ? (
          ...
          <Button onClick={openBilling}>
            {t['com.notesgraph.payment.ai.usage.change-button-label']()}
          </Button>
        )
      ) : (
        <div className={styles.storageProgressContainer}>
          <div className={styles.storageProgressWrapper}>
            <div className="storage-progress-desc">
              <span>{t['com.notesgraph.payment.ai.usage.used-caption']()}</span>
              <span>
                {t['com.notesgraph.payment.ai.usage.used-detail']({
          ...
          {hasPaymentFeature && (
            <AISubscribe variant="primary">
              {t['com.notesgraph.payment.ai.usage.purchase-button-label']()}
            </AISubscribe>
          )}
        </div>
      )}
