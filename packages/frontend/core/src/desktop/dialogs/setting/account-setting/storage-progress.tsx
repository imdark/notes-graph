import { Button, ErrorMessage, Skeleton, Tooltip } from '@notesgraph/component';
    ...
    <div className={styles.storageProgressContainer}>
      <div className={styles.storageProgressWrapper}>
        <div className="storage-progress-desc">
          <span>{t['com.notesgraph.storage.used.hint']()}</span>
          <span>
            {usedFormatted}/{maxFormatted}
            {` (${quotaName} ${t['com.notesgraph.storage.plan']()})`}
          </span>
        </div>
      ...
      {hasPaymentFeature ? (
        <Tooltip
          ...
          content={
            isFreeUser
              ? t['com.notesgraph.storage.maximum-tips']()
              : t['com.notesgraph.storage.maximum-tips.pro']()
          }
        >
          <span tabIndex={0}>
            <Button variant={buttonType} onClick={onUpgrade}>
              {isFreeUser
                ? t['com.notesgraph.storage.upgrade']()
                : t['com.notesgraph.storage.change-plan']()}
            </Button>
          </span>
        </Tooltip>
      ) : null}
