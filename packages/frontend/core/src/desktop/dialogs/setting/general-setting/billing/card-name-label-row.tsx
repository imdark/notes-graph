...
const StatusLabel = ({ status }: { status: SubscriptionStatus }) => {
  ...
  const label = useMemo(() => {
    switch (status) {
      case SubscriptionStatus.Active:
        return t['com.notesgraph.payment.subscription-status.active']();
      case SubscriptionStatus.PastDue:
        return t['com.notesgraph.payment.subscription-status.past-due']();
      case SubscriptionStatus.Trialing:
        return t['com.notesgraph.payment.subscription-status.trialing']();
      default:
        return '';
    }
  }, [status, t]);
