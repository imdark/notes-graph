...
import { type I18nString, i18nTime, Trans, useI18n } from '@notesgraph/i18n';
...
const DescriptionI18NKey = {
  Basic: 'com.notesgraph.payment.billing-setting.current-plan.description',
  Monthly:
    'com.notesgraph.payment.billing-setting.current-plan.description.monthly',
  Yearly:
    'com.notesgraph.payment.billing-setting.current-plan.description.yearly',
  Lifetime:
    'com.notesgraph.payment.billing-setting.current-plan.description.lifetime',
      ...
      <div className={styles.currentPlan}>
        <SettingRow
          ...
          name={
            <CardNameLabelRow
              cardName={t[
                'com.notesgraph.payment.billing-setting.current-plan'
              ]()}
      ...
      <p className={styles.planPrice}>
        ...
        <span className={styles.billingFrequency}>
          ...
          {currentRecurring === SubscriptionRecurring.Monthly
            ? t['com.notesgraph.payment.billing-setting.month']()
            : t['com.notesgraph.payment.billing-setting.year']()}
        </span>
      </p>
    </div>
  );
...
const CloudExpirationInfo = () => {
  ...
  if (subscription?.status === SubscriptionStatus.PastDue) {
    text = t['com.notesgraph.payment.billing-tip.past-due']({
      due: i18nTime(subscription.nextBillAt, {
        absolute: { accuracy: 'day' },
      }),
    });
  } else if (subscription?.nextBillAt) {
    text = t['com.notesgraph.payment.billing-setting.renew-date.description']({
      renewDate: i18nTime(subscription.nextBillAt, {
        absolute: { accuracy: 'day' },
      }),
    });
  } else if (subscription?.end) {
    text = t['com.notesgraph.payment.billing-setting.due-date.description']({
    ...
    <div className={styles.planActionContainer}>
      ...
      >
        {plan === SubscriptionPlan.Pro
          ? t['com.notesgraph.payment.billing-setting.change-plan']()
          : t['com.notesgraph.payment.billing-setting.upgrade']()}
      </Button>
      {subscriptionStatus === SubscriptionStatus.PastDue ? (
