...
import { SubscriptionStatus } from '@notesgraph/graphql';
...
export const AIPlanCard = ({ onClick }: { onClick: () => void }) => {
  ...
  const priceReadable = price?.yearlyAmount
    ...
    : '?';
  const priceFrequency = t['com.notesgraph.payment.billing-setting.year']();

  const billingTip = useMemo(() => {
    if (subscription === undefined) {
      return (
        <Trans
          i18nKey={'com.notesgraph.payment.billing-setting.ai.free-desc'}
          components={{
    ...
    if (subscription?.status === SubscriptionStatus.PastDue) {
      return t['com.notesgraph.payment.billing-tip.past-due']({
    ...
    if (subscription?.nextBillAt) {
      return t['com.notesgraph.payment.ai.billing-tip.next-bill-at']({
        due: i18nTime(subscription.nextBillAt, {
          absolute: { accuracy: 'day' },
        }),
      });
    ...
    if (subscription?.canceledAt && subscription?.end) {
      return t['com.notesgraph.payment.ai.billing-tip.end-at']({
    ...
    <div className={styles.planCard} style={{ marginBottom: 24 }}>
      <div className={styles.currentPlan}>
        <SettingRow
          ...
          name={
            <CardNameLabelRow
              cardName={t['com.notesgraph.payment.billing-setting.ai-plan']()}
                ...
                {price?.yearlyAmount ? (
                  subscription ? (
                    ...
                    <AISubscribe className={styles.planAction}>
                      {t[
                        'com.notesgraph.payment.billing-setting.ai.start-free-trial'
                      ]()}
                    </AISubscribe>
                  )
                ) : null}
