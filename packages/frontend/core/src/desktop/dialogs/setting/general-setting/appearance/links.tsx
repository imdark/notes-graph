import { Menu, MenuItem, MenuTrigger } from '@notesgraph/component';
import {
  OpenInAppService,
  OpenLinkMode,
  ...
  const options = useMemo(
    () =>
      Object.values(OpenLinkMode).map(mode => ({
        label:
          t.t(`com.notesgraph.setting.appearance.open-in-app.${mode}`) ||
          `com.notesgraph.setting.appearance.open-in-app.${mode}`,
        value: mode,
      })),
    [t]
  );
