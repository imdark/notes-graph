...
import type { Block, Store } from '@blocksuite/notesgraph/store';
import { Skeleton } from '@notesgraph/component';
import { getViewManager } from '@notesgraph/core/blocksuite/manager/view';
import type { EditorSettingSchema } from '@notesgraph/core/modules/editor-setting';
  ...
  const renderEditor = useCallback(async () => {
    ...
    const disposable = editorHost.std.view.viewUpdated.subscribe(payload => {
      if (
        payload.type !== 'block' ||
        payload.method !== 'add' ||
        payload.view.model.flavour !== 'notesgraph:page'
      ) {
        return;
      }
      doc.readonly = false;
