...
import { Divider } from '@notesgraph/component';
...
export const SeeAllLink = () => {
  ...
  return (
    <a
      ...
      href="https://notesgraph.com/pricing"
      target="_blank"
      rel="noopener noreferrer"
    >
      {t['com.notesgraph.payment.see-all-plans']()}
...
export const CloudPlanLayout = ({
  ...
  return (
    <CollapsibleWrapper title={title} caption={caption}>
      <div className={styles.notesgraphCloudHeader}>
        <div>{select}</div>
        <div>{toggle}</div>
      </div>
      <ScrollArea.Root>
