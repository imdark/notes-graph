import { Button, Loading } from '@notesgraph/component';
import {
  Pagination,
  SettingRow,
  SettingWrapper,
} from '@notesgraph/component/setting-components';
import { InvoicesService } from '@notesgraph/core/modules/cloud';
import { UrlService } from '@notesgraph/core/modules/url';
import { UserFriendlyError } from '@notesgraph/error';
import { type InvoicesQuery, InvoiceStatus } from '@notesgraph/graphql';
      ...
      <div className={styles.historyContent}>
        {invoiceCount === 0 ? (
          <p className={styles.noInvoice}>
            {t['com.notesgraph.payment.billing-setting.no-invoice']()}
          </p>
        ) : (
    ...
    <SettingRow
      ...
      desc={`${
        invoice.status === InvoiceStatus.Paid
          ? t['com.notesgraph.payment.billing-setting.paid']()
          : ''
      } $${invoice.amount / 100}`}
    >
      <Button onClick={open}>
        {t['com.notesgraph.payment.billing-setting.view-invoice']()}
      </Button>
    </SettingRow>
  );
};
...
const BillingHistorySkeleton = () => {
  ...
  return (
    <SettingWrapper
      title={t['com.notesgraph.payment.billing-setting.history']()}
    >
