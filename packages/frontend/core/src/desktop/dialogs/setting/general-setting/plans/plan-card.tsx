...
import { Button, type ButtonProps } from '@notesgraph/component/ui/button';
import { Tooltip } from '@notesgraph/component/ui/tooltip';
import { generateSubscriptionCallbackLink } from '@notesgraph/core/components/hooks/notesgraph/use-subscription-notify';
...
const getSignUpText = (
  ...
  switch (plan) {
    case SubscriptionPlan.Free:
      return t['com.notesgraph.payment.sign-up-free']();
    case SubscriptionPlan.Team:
      return t['com.notesgraph.payment.upgrade']();
    default:
      return t['com.notesgraph.payment.buy-pro']();
  }
};
  ...
  if (isBeliever) {
    return (
      <Button className={styles.planAction} disabled>
        {t['com.notesgraph.payment.cloud.lifetime.included']()}
      </Button>
    );
  }
...
const CurrentPlan = () => {
  ...
  return (
    <Button className={styles.planAction}>
      {t['com.notesgraph.payment.current-plan']()}
    </Button>
  );
};
...
const Downgrade = ({ disabled }: { disabled?: boolean }) => {
  ...
  const tooltipContent = disabled
    ? t['com.notesgraph.payment.downgraded-tooltip']()
    : null;
      ...
      <Tooltip content={tooltipContent} rootOptions={{ delayDuration: 0 }}>
        <div className={styles.planAction}>
          <Button
            ...
            disabled={disabled}
          >
            {t['com.notesgraph.payment.downgrade']()}
          </Button>
        </div>
      </Tooltip>
    </CancelAction>
    ...
    <CheckoutSlot
      ...
      renderer={props => (
        <Button
          ...
          {...btnProps}
        >
          {children ?? t['com.notesgraph.payment.upgrade']()}
        </Button>
      )}
    />
  );
  ...
  return (
    <>
      <Button
        ...
        loading={isMutating}
      >
        {t['com.notesgraph.payment.change-to']({ to })}
      </Button>
    ...
    <ResumeAction open={open} onOpenChange={setOpen}>
      <Button className={styles.resumeAction} onClick={handleClick}>
        <span data-show-hover="true" className={clsx(styles.resumeContent)}>
          {t['com.notesgraph.payment.resume-renewal']()}
        </span>
