...
import { DocListItemComponent } from '@notesgraph/core/components/explorer/docs-view/docs-list';
import { Filters } from '@notesgraph/core/components/filter';
import { NotesGraphShapeIcon } from '@notesgraph/core/components/page-list';
import type { CollectionInfo } from '@notesgraph/core/modules/collection';
import { CollectionRulesService } from '@notesgraph/core/modules/collection-rules';
...
export const RulesMode = ({
        ...
        },
        {
          ...
          children: (
            <div className={styles.includeListTitle}>
              {t['com.notesgraph.editCollection.rules.include.title']()}
            </div>
          ),
  ...
  const tips = useMemo(
    () => (
      <Trans
        i18nKey="com.notesgraph.editCollection.rules.tips"
        values={{
          highlight: t['com.notesgraph.editCollection.rules.tips.highlight'](),
        }}
        components={{
    ...
    <>
                      ...
                      <div className={styles.includeItem} key={id}>
                        <div className={styles.includeItemContent}>
                          ...
                          >
                            ...
                            {t[
                              'com.notesgraph.editCollection.rules.include.page'
                            ]()}
                          </div>
                          <div className={styles.includeItemContentIs}>
                            {t[
                              'com.notesgraph.editCollection.rules.include.is'
                            ]()}
                          </div>
                          <DocTitle id={id} />
                        </div>
      ...
      <div className={styles.rulesBottom}>
        <div className={styles.bottomLeft}>
          ...
          >
            {t['com.notesgraph.editCollection.rules.preview']()}
          </Button>
          <Button variant="plain" onClick={reset}>
            {t['com.notesgraph.editCollection.rules.reset']()}
          </Button>
          <div className={styles.previewCountTips}>
            <Trans
              i18nKey="com.notesgraph.editCollection.rules.countTips"
              values={{
  ...
  return (
    ...
    >
      ...
      <strong style={{ fontSize: 20, lineHeight: '28px' }}>
        {noRules
          ? t['com.notesgraph.editCollection.rules.empty.noRules']()
          : t['com.notesgraph.editCollection.rules.empty.noResults']()}
      </strong>
      <div
        style={{
          width: '389px',
      ...
      >
        {noRules ? (
          <Trans i18nKey="com.notesgraph.editCollection.rules.empty.noRules.tips">
            Please <strong>add rules</strong> to save this collection or switch
            to <strong>Pages</strong>, use manual selection mode
          </Trans>
        ) : (
          t['com.notesgraph.editCollection.rules.empty.noResults.tips']()
        )}
      </div>
    </div>
  );
