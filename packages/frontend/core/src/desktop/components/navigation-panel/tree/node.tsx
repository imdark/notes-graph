...
import type { ExplorerType } from '@notesgraph/core/modules/explorer-icon/store/explorer-icon';
import type { DocPermissionActions } from '@notesgraph/core/modules/permissions';
...
export type NavigationPanelTreeNodeDropEffectData = {
  source: { data: NotesGraphDNDData['draggable'] };
...
export const NavigationPanelTreeNode = ({
  ...
  const { dragRef, dragging, CustomDragPreview } = useDraggable<
    NotesGraphDNDData & { draggable: { __cid: string } }
  >(
    () => ({
  ...
  const {
    ...
    draggedOver,
    draggedOverPosition,
  } = useDropTarget<NotesGraphDNDData & { draggable: { __cid: string } }>(
    () => ({
