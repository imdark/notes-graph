...
export const NavigationPanelOrganize = () => {
  ...
  const handleOnChildrenDrop = useCallback(
    (data: DropTargetDropEvent<NotesGraphDNDData>, node?: FolderNode) => {
      if (!node || !node.id) {
      ...
      if (
        ...
        if (data.source.data.entity?.type === 'folder') {
          ...
          toast(t['com.notesgraph.rootAppSidebar.organize.root-folder-only']());
        }
      } else {
    ...
    <CollapsibleSection
      ...
      actions={
        <IconButton
          ...
          size="16"
          tooltip={t[
            'com.notesgraph.rootAppSidebar.explorer.organize-section-add-tooltip'
          ]()}
        >
          <AddOrganizeIcon />
        </IconButton>
