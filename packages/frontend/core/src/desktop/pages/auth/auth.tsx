...
export const Component = () => {
  ...
  const onSendVerifyChangeEmail = useCallback(
    async (email: string) => {
      ...
      if (res?.sendVerifyChangeEmail) {
        notify.success({
          title: t['com.notesgraph.auth.sent.verify.email.hint'](),
        });
      } else {
        notify.error({
  ...
  switch (authType) {
    case 'onboarding':
      return (
        account && (
          <OnboardingPage user={account} onOpenNotesGraph={onOpenNotesGraph} />
        )
      );
    case 'signUp': {
      return (
        account && (
          <SignUpPage
            ...
            onOpenNotesGraph={onOpenNotesGraph}
          />
        )
      );
    }
    case 'signIn': {
      return <SignInSuccessPage onOpenNotesGraph={onOpenNotesGraph} />;
    }
    case 'changePassword': {
      return (
        <ChangePasswordPage
    ...
    case 'confirm-change-email': {
      return <ConfirmChangeEmail onOpenNotesGraph={onOpenNotesGraph} />;
    }
    case 'verify-email': {
      return <ConfirmVerifiedEmail onOpenNotesGraph={onOpenNotesGraph} />;
    }
  }
  return null;
};
