...
import type { DateCell } from '@notesgraph/component';
import {
  DatePicker,
  IconButton,
  Menu,
...
import { MoveToTrash } from '@notesgraph/core/components/page-list';
import { WorkspaceServerService } from '@notesgraph/core/modules/cloud';
import {
  type DocRecord,
  DocService,
  DocsService,
} from '@notesgraph/core/modules/doc';
      ...
      <div className={styles.pageItemLabel}>
        ...
        {duplicate ? (
          <div className={styles.duplicateTag}>
            {i18n[
              'com.notesgraph.page-properties.property.journal-duplicated'
            ]()}
          </div>
        ) : null}
      </div>
...
type DateDotType = 'journal' | 'event' | 'activity' | 'schedule';
...
export const EditorJournalPanel = () => {
  ...
  const monthScheduleOccurrences = useLiveData(
    useMemo(
      () =>
        scheduleService.occurrencesInRange$(
          calendarCursorMonthStart.format('YYYY-MM-DD'),
          calendarCursorMonthEnd.format('YYYY-MM-DD')
        ),
      [scheduleService, calendarCursorMonthStart, calendarCursorMonthEnd]
    )
  );
    ...
    () =>
      new Set(
        monthScheduleOccurrences
          .filter(o => o.recurring || o.blockId)
          .map(o => o.date)
      ),
    [monthScheduleOccurrences]
  );
  ...
  const getDotType = useCallback(
    (dateKey: string): DateDotType[] => {
      ...
      if (scheduleDates.has(dateKey)) {
        dotTypes.push('schedule');
      }
      return dotTypes;
    },
    [allJournalDates, docActivityDates, eventDates, scheduleDates]
  );
...
const DailyCountEmptyFallback = ({ name }: { name: NavItemName }) => {
  ...
  return (
    <div className={styles.dailyCountEmpty}>
      {name === 'createdToday'
        ? t['com.notesgraph.journal.daily-count-created-empty-tips']()
        : t['com.notesgraph.journal.daily-count-updated-empty-tips']()}
    </div>
  );
};
// Cap how many created/updated docs we render per day so a very active day
// (hundreds of docs, each a live-subscribed row) doesn't bog down the panel.
// Matches the "99+" cutoff shown in the count badge.
const MAX_DAILY_COUNT = 99;
...
const JournalDailyCountBlock = ({ date }: JournalBlockProps) => {
  ...
  const headerItems = useMemo<NavItem[]>(
    () => [
      {
        name: 'createdToday',
        label: t['com.notesgraph.journal.created-today'](),
        count: createdToday.length,
      },
      {
        name: 'updatedToday',
        label: t['com.notesgraph.journal.updated-today'](),
            ...
            <Scrollable.Root key={name} className={styles.dailyCountItem}>
              ...
              <Scrollable.Viewport>
                <div className={styles.dailyCountContent} ref={nodeRef}>
                  {renderList
                    .slice(0, MAX_DAILY_COUNT)
                    .map((pageRecord, index) => (
                      <PageItem
                  ...
                  {renderList.length > MAX_DAILY_COUNT ? (
                    <div className={styles.dailyCountTruncated}>
                      {`Showing the first ${MAX_DAILY_COUNT} of ${renderList.length}.`}
                    </div>
                  ) : null}
                </div>
              </Scrollable.Viewport>
                  ...
                  <>
                    <Guard docId={docRecord.id} permission="Doc_Update">
                      {canEdit => (
                        ...
                        >
                          {t[
                            'com.notesgraph.page-properties.property.journal-remove'
                          ]()}
                        </MenuItem>
                      )}
                    </Guard>
      ...
      {docs.length > MAX_CONFLICT_COUNT ? (
        ...
        >
          <div className={styles.journalConflictMoreTrigger}>
            {t['com.notesgraph.journal.conflict-show-more']({
