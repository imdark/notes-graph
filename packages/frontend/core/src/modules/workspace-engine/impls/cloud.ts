...
} from '@notesgraph/nbstore/idb';
import {
  IndexedDBV1BlobStorage,
  IndexedDBV1DocStorage,
} from '@notesgraph/nbstore/idb/v1';
import {
  SqliteBlobStorage,
...
const getCloudWorkspaceCacheKey = (serverId: string) => {
  if (serverId === 'notesgraph-cloud') {
    return 'cloud-workspace:'; // FOR BACKWARD COMPATIBILITY
  }
  return `selfhosted-workspace-${serverId}:`;
};

const logger = new DebugLogger('notesgraph:cloud-workspace-flavour-provider');
...
class CloudWorkspaceFlavourProvider implements WorkspaceFlavourProvider {
  ...
  IndexerStorageType =
    BUILD_CONFIG.isElectron || BUILD_CONFIG.isIOS || BUILD_CONFIG.isAndroid
      ? SqliteIndexerStorage
      : IndexedDBIndexerStorage;
  // NOTE: must stay Electron-only. The mobile native layer (NbStorePlugin.kt /
  // notesgraph_mobile_native uniffi) does NOT implement get/set/clearDocIndexedClock
  // — only Electron's napi binding does. Using SqliteIndexerSyncStorage on mobile
  // makes every indexer loop reject on the missing RPC, breaking search entirely.
  // Mobile therefore keeps the indexed-clock bookkeeping in IndexedDB.
  ...
  async getWorkspaceProfile(
    ...
    // we use notesgraph 'static' storage here, which use http protocol, no need to websocket.
  ...
  onWorkspaceInitialized(workspace: Workspace): void {
    // bind the workspace to the notesgraph cloud server
    ...
    return {
      ...
      remotes: {
        [`cloud:${this.flavour}`]: {
          ...
          // Always wire the remote indexer for cloud workspaces. CloudIndexerStorage
          // is read-only and makes NO network calls until a search query with
          // prefer:'remote' is issued — which only happens when the *live* server
          // feature check (isSupportServerIndexer) says Indexer is available.
          // Gating this on features AT ENGINE-BUILD TIME was a trap: if the engine
          // was constructed before the server advertised Indexer (config
          // revalidation not yet landed, or a stale/restored cached config), the
          // remote stayed permanently unwired even after the feature appeared, so
          // IndexerSyncImpl.remote was undefined and search silently fell back to
          // the local index (only opened docs, no network request). Wiring it
          // unconditionally lets the live feature check drive usage reactively.
          indexer: {
