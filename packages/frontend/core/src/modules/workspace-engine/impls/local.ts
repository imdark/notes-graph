...
export const LOCAL_WORKSPACE_LOCAL_STORAGE_KEY = 'notesgraph-local-workspace';
...
const LOCAL_WORKSPACE_CHANGED_BROADCAST_CHANNEL_KEY =
  'notesgraph-local-workspace-changed';
...
class LocalWorkspaceFlavourProvider implements WorkspaceFlavourProvider {
  ...
  IndexerStorageType =
    BUILD_CONFIG.isElectron || BUILD_CONFIG.isIOS || BUILD_CONFIG.isAndroid
      ? SqliteIndexerStorage
      : IndexedDBIndexerStorage;
  // Electron-only: mobile native lacks get/set/clearDocIndexedClock, so mobile
  // must keep indexed-clock bookkeeping in IndexedDB. See cloud.ts.
