...
import {
  type TagLike,
  TagsInlineEditor,
} from '@notesgraph/core/components/tags';
import { TagService } from '@notesgraph/core/modules/tag';
import {
  databaseTagColorToV2,
  notesgraphLabelToDatabaseTagColor,
} from '@notesgraph/core/modules/tag/entities/utils';
...
const BlocksuiteDatabaseSelector = ({
  ...
  const onCreateTag = useCallback(
    (name: string, color: string) => {
      // bs database uses --notesgraph-tag-xxx colors
      const newTag = {
        id: nanoid(),
        name: name,
        color: color,
  ...
  const tagColors = useMemo(() => {
    return tagService.tagColors.map(([name, color]) => ({
      id: name,
      value: notesgraphLabelToDatabaseTagColor(color),
      name,
    }));
  ...
  const onTagChange = useCallback(
    (tagId: string, property: string, value: string) => {
      adapter.updateTag(selectCell, dataSource, tagId, old => {
        if (property === 'color') {
          value = notesgraphLabelToDatabaseTagColor(value);
        }
        return {
          ...old,
          [property]: value,
