  ...
  public async writeDoc(options: {
    ...
    if (!docId) {
      ...
      if (updateStrategy === 'override') {
        ...
        const pageBlockId = doc.getBlocksByFlavour('notesgraph:page')[0]?.id;
        const blockId = doc.addBlock('notesgraph:note', {}, pageBlockId);
        await MarkdownTransformer.importMarkdownToBlock({
          doc,
          blockId,
          markdown: `---\n${markdown}`,
