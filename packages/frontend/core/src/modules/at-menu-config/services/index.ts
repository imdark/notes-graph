import { DateTimeIcon, UserIcon } from '@blocksuite/icons/lit';
import type { DocMode } from '@blocksuite/notesgraph/model';
import { DocModeProvider } from '@blocksuite/notesgraph/shared/services';
...
import type { DocMeta } from '@blocksuite/notesgraph/store';
import {
...
import { unsafeHTML } from '@blocksuite/notesgraph-shared/utils';
...
export class AtMenuConfigService extends Service {
  constructor(
    ...
    private readonly docModeRegistryService: DocModeRegistryService
  ) {
    super();
  }
  ...
  private insertDoc(inlineEditor: NotesGraphInlineEditor, id: string) {
  ...
  private newDocMenuGroup(
    ...
    // One "create" item per registered creatable mode (page is always present;
    // edgeless appears only while the edgeless plugin is enabled).
    const customNewDocItems: LinkedMenuItem[] =
      this.docModeRegistryService.creatableModes$.value.map(mode => ({
        key: `create:${mode.id}`,
        icon: mode.creationIcon?.() as LinkedMenuItem['icon'],
        name: I18n.t(`com.notesgraph.editor.at-menu.create-${mode.id}`, {
          name: query || I18n.t('Untitled'),
        }),
        action: () => {
          close();
          const page = createPage(mode.id);
          this.insertDoc(inlineEditor, page.id);
          track.doc.editor.atMenu.createDoc({ mode: mode.id });
        },
      }));
    const customImportItem: LinkedMenuItem = {
      ...importItem,
      name: I18n.t('com.notesgraph.editor.at-menu.import'),
      action: () => {
        close();
    ...
    return {
      ...
      name: I18n.t('com.notesgraph.editor.at-menu.new-doc'),
  ...
  private journalGroup(
    ...
    const items: LinkedMenuItem[] = [
      {
        ...
        name: I18n.t('com.notesgraph.editor.at-menu.date-picker'),
        action: () => {
          close();
    ...
    return {
      name: I18n.t('com.notesgraph.editor.at-menu.journal'),
      items,
    };
  }
  ...
  private memberGroup(
            ...
            .catch(error => {
              ...
              if (err.is(ErrorNames.MENTION_USER_DOC_ACCESS_DENIED)) {
                ...
                if (canUserManage) {
                  ...
                  notify.error({
                    title: I18n.t(
                      'com.notesgraph.editor.at-menu.access-needed'
                    ),
                    message: I18n[
                      'com.notesgraph.editor.at-menu.access-needed-message'
                    ]({
                      username,
                    }),
                    actions: [
                      {
                        ...
                        onClick: async () => {
                          ...
                          try {
                            ...
                            notify.success({
                              title: I18n.t(
                                'com.notesgraph.editor.at-menu.invited-and-notified'
                              ),
                            });
                          } catch (error) {
                            const err = UserFriendlyError.fromAny(error);
                  ...
                  notify.error({
                    title: I18n.t(
                      'com.notesgraph.editor.at-menu.member-not-notified'
                    ),
                    message:
                      I18n[
                        'com.notesgraph.editor.at-menu.member-not-notified-message'
                      ](),
                  });
                }
    ...
    return {
      name: I18n.t('com.notesgraph.editor.at-menu.mention-members'),
      items,
      ...
      overflowText: computed(() => {
        ...
        return I18n.t('com.notesgraph.editor.at-menu.more-members-hint', {
