  ...
  implements EditorSettingProvider
{
  ...
  constructor(
    ...
    const notesgraphCloudServer =
      this.serversService.server$('notesgraph-cloud').value; // TODO: support multiple servers
    if (!notesgraphCloudServer) {
      throw new Error('notesgraph-cloud server not found');
    }
    const userDBService = notesgraphCloudServer.scope.get(UserDBService);
