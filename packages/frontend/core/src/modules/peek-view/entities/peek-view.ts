import type { SurfaceRefBlockComponent } from '@blocksuite/notesgraph/blocks/surface-ref';
import { NotesGraphReference } from '@blocksuite/notesgraph/inlines/reference';
import type {
...
import type { BlockComponent, EditorHost } from '@blocksuite/notesgraph/std';
import type { Block, BlockModel } from '@blocksuite/notesgraph/store';
...
const isEmbedLinkedDocModel = (
  ...
  return blockModel.flavour === 'notesgraph:embed-linked-doc';
};

const isEmbedSyncedDocModel = (
  blockModel: BlockModel
): blockModel is EmbedSyncedDocModel => {
  return blockModel.flavour === 'notesgraph:embed-synced-doc';
};

const isImageBlockModel = (
  blockModel: BlockModel
): blockModel is ImageBlockModel => {
  return blockModel.flavour === 'notesgraph:image';
};

const isAttachmentBlockModel = (
  blockModel: BlockModel
): blockModel is AttachmentBlockModel => {
  return blockModel.flavour === 'notesgraph:attachment';
};

const isSurfaceRefModel = (
  blockModel: BlockModel
): blockModel is SurfaceRefBlockModel => {
  return blockModel.flavour === 'notesgraph:surface-ref';
};

const isAIChatBlockModel = (
  blockModel: BlockModel
): blockModel is AIChatBlockModel => {
  return blockModel.flavour === 'notesgraph:embed-ai-chat';
};
...
function resolvePeekInfoFromPeekTarget(
  ...
  if (element) {
    if (element instanceof NotesGraphReference) {
