...
// AI peek view pulls in the ~10MB blocksuite/ai bundle — load it only when an
// AI-chat peek is actually opened, not on every workspace load.
const AIChatBlockPeekView = lazy(() =>
  import('./ai-chat-block-peek-view').then(m => ({
    default: m.AIChatBlockPeekView,
  }))
);
import { AttachmentPreviewPeekView } from './attachment-preview';
// DocPeekPreview mounts the full BlockSuite editor (which also pulls AI). The
// peek-view module is configured at bootstrap, so a static import drags the
// editor + AI into the initial bundle. Load it only when a doc peek opens.
const DocPeekPreview = lazy(() =>
  import('./doc-preview').then(m => ({ default: m.DocPeekPreview }))
);
import {
...
function renderPeekView({ info }: ActivePeekView, animating?: boolean) {
  ...
  if (info.type === 'doc') {
    return (
      <Suspense fallback={null}>
        <DocPeekPreview docRef={info.docRef} animating={animating} />
      </Suspense>
    );
  }
  ...
  if (info.type === 'ai-chat-block') {
    return (
      <Suspense fallback={null}>
        <AIChatBlockPeekView model={info.model} host={info.host} />
      </Suspense>
    );
  }
