import { getDocCreatedByUpdatedByListQuery } from '@notesgraph/graphql';
import { Store, yjsGetPath } from '@notesgraph/infra';
...
export class DocCreatedByUpdatedBySyncStore extends Store {
  ...
  watchDocCreatedByUpdatedBySynced() {
    ...
    return yjsGetPath(
      rootYDoc.getMap('notesgraph:workspace-properties'),
  ...
  setDocCreatedByUpdatedBySynced(synced: boolean) {
    const rootYDoc = this.workspaceService.workspace.rootYDoc;
    rootYDoc
      .getMap('notesgraph:workspace-properties')
