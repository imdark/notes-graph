import type { RealtimeSubscriptionReady } from '@notesgraph/realtime';
...
/**
 * Realtime rejects subscriptions when the session isn't authenticated (local or
 * signed-out workspaces). For these cloud live queries that's an expected,
 * benign state, so it's swallowed instead of surfaced as an error.
 */
function isRealtimeUnauthenticated(error: unknown): boolean {
  return error instanceof Error && error.name === 'RealtimeUnauthenticated';
}
    ...
    this.subscription = this.options.subscribe().subscribe({
      ...
      error: error => {
        if (
          ...
          !isRealtimeUnauthenticated(error)
        ) {
