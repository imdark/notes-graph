...
const Options = ({
    ...
    async (userId: string, role: DocRole) => {
      ...
      try {
        ...
        if (res) {
          notify.success({
            title:
              t['com.notesgraph.share-menu.member-management.update-success'](),
          });
        } else {
          notify.error({
            title:
              t['com.notesgraph.share-menu.member-management.update-fail'](),
          });
        }
      } catch (error) {
  ...
  const openTransferOwnerModal = useCallback(() => {
    openConfirmModal({
      title:
        t[
          'com.notesgraph.share-menu.member-management.set-as-owner.confirm.title'
        ](),
      description:
        t[
          'com.notesgraph.share-menu.member-management.set-as-owner.confirm.description'
        ](),
  ...
  const operationButtonInfo = useMemo(() => {
    return [
      {
        label: t['com.notesgraph.share-menu.option.permission.can-manage'](),
        onClick: changeToManager,
        role: DocRole.Manager,
      },
      {
        label: t['com.notesgraph.share-menu.option.permission.can-edit'](),
        onClick: changeToEditor,
        role: DocRole.Editor,
        showPlanTag: hittingPaywall,
      },
      {
        label: t['com.notesgraph.share-menu.option.permission.can-read'](),
  ...
  return (
    <>
      ...
      <MenuItem onSelect={openTransferOwnerModal} disabled={!canTransferOwner}>
        {t['com.notesgraph.share-menu.member-management.set-as-owner']()}
      </MenuItem>
      <MenuSeparator />
      <MenuItem
        ...
        disabled={!canManageUsers}
      >
        {t['com.notesgraph.share-menu.member-management.remove']()}
      </MenuItem>
    </>
  );
};
