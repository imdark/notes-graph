...
import { type DeltaInsert, Text } from '@blocksuite/notesgraph/store';
import {
  TranscriptionBlockFlavour,
  type TranscriptionBlockModel,
} from '@notesgraph/core/blocksuite/ai/blocks/transcription-block/model';
import { insertFromMarkdown } from '@notesgraph/core/blocksuite/utils';
import { preprocessAudioBlobForTranscription } from '@notesgraph/core/utils/opus-encoding';
  ...
  private readonly fillTranscriptionResult = async (
    ...
    const fillTranscription = (segments: TranscriptionResult['segments']) => {
      ...
      for (const segment of segments) {
        ...
        const deltaInserts: DeltaInsert<NotesGraphTextAttributes>[] = [
          {
