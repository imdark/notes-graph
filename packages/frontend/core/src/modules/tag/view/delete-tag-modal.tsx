          ...
          if (state) {
            ...
            toast(
              tagIdsToDelete.length > 1
                ? t['com.notesgraph.delete-tags.count']({
                    count: tagIdsToDelete.length,
                  })
                : t['com.notesgraph.tags.delete-tags.toast']()
            );
          }
        }
      };
      openConfirmModal({
        title: t['com.notesgraph.delete-tags.confirm.title'](),
        description:
          tagIdsToDelete.length === 1 ? (
            <Trans
              i18nKey={'com.notesgraph.delete-tags.confirm.description'}
              values={{ tag: tagName }}
              components={{ 1: <strong /> }}
            />
          ) : (
            t['com.notesgraph.delete-tags.confirm.multi-tag-description']({
