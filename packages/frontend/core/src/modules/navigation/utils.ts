...
import { channelToScheme } from '@notesgraph/core/utils';
import { isNil, pick, pickBy } from 'lodash-es';
import type { ParsedQuery, ParseOptions } from 'query-string';
import queryString from 'query-string';

function maybeNotesGraphOrigin(origin: string, baseUrl: string) {
  return (
    origin.startsWith('assets://') ||
    origin.endsWith('notesgraph.com') || // stable/beta
    origin.endsWith('apple.notesgraph.com') || // stable/beta
    origin.endsWith('canary.notesgraph.com') || // canary
...
export const resolveRouteLinkMeta = (
  ...
  try {
    // if href is started with notesgraph protocol, we need to convert it to http protocol to may URL happy
    const notesgraphProtocol =
      channelToScheme[BUILD_CONFIG.appBuildType] + '://';

    if (href.startsWith(notesgraphProtocol)) {
      href = href.replace(notesgraphProtocol, 'http://');
    }

    const url = new URL(href, baseUrl);

    // check if origin is one of notesgraph's origins
    // check if origin is localhost or self-hosted

    if (!maybeNotesGraphOrigin(url.origin, baseUrl)) {
      return null;
    }
