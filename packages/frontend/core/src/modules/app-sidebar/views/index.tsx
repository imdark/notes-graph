import { type DropTargetGetFeedback, Skeleton } from '@notesgraph/component';
...
import { NavigateContext } from '@notesgraph/core/components/hooks/use-navigate-helper';
import { WorkspaceNavigator } from '@notesgraph/core/components/workspace-selector';
...
export function AppSidebar({ children }: PropsWithChildren) {
  ...
  const resizeHandleDropTargetOptions = useMemo(() => {
    return () => ({
      ...
      canDrop: (data: DropTargetGetFeedback<NotesGraphDNDData>) => {
        return (
...
const ResizeHandleTooltipContent = () => {
  ...
  return (
    <div>
      <div>
        {t['com.notesgraph.rootAppSidebar.resize-handle.tooltip.drag']()}
      </div>
      <div>
        {t['com.notesgraph.rootAppSidebar.resize-handle.tooltip.click']()}
      </div>
    </div>
  );
};
