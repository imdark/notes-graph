import { InlineCommentManager } from '@blocksuite/notesgraph/inlines/comment';
import type {
  BaseSelection,
  DocSnapshot,
  Store,
...
import { type CommentChangeAction, DocMode } from '@notesgraph/graphql';
import {
  effect,
  Entity,
  fromPromise,
...
export class DocCommentEntity extends Entity<{
  ...
  async addReply(commentId: string, reply?: DocCommentReply): Promise<string> {
    ...
    if (mention) {
      ...
      const paragraph = doc.getModelsByFlavour('notesgraph:paragraph').at(-1);
      if (paragraph) {
  ...
  /** A fresh empty editor store for composing a new comment. */
  async createDraftDoc(): Promise<Store | undefined> {
    return this.snapshotHelper.createStore();
  }

  /**
   * Create a page-level ("global") comment — one not anchored to any text
   * selection. It carries no `preview`, which is how the bottom
   * global-comments panel tells it apart from inline comments.
   */
  async addGlobalComment(
    doc: Store,
    attachments: CommentAttachment[] = []
  ): Promise<void> {
    const snapshot = this.snapshotHelper.getSnapshot(doc);
    ...
    this.comments$.setValue([...this.comments$.value, comment]);
    track.$.commentPanel.$.createComment({
      type: 'root',
      withAttachment: attachments.length > 0,
      withMention: mentions.length > 0,
  ...
  private localCommentsSubscription?: { unsubscribe: () => void };
  // The entity is shared (pooled) between the comment sidebar and the
  // bottom "global comments" panel; ref-count start/stop so one consumer
  // unmounting doesn't tear down the other's subscription.
  private startRefCount = 0;

  start(): void {
    this.startRefCount++;
    if (this.startRefCount > 1) {
      return;
    }
    this.revalidateCommentsInEditor();
    if (this.store.isLocal) {
      // Local workspaces have no comment server/realtime — drive the list
      // directly off the DB-backed watch (full snapshot on every change).
      this.startVersion++;
      this.loading$.setValue(true);
      this.localCommentsSubscription?.unsubscribe();
      this.localCommentsSubscription = this.store.watchComments().subscribe({
        next: comments => {
  ...
  stop(): void {
    ...
    this.startVersion++;
    this.localCommentsSubscription?.unsubscribe();
    this.localCommentsSubscription = undefined;
    this.liveQuery.stop();
    this.loading$.setValue(false);
  }
