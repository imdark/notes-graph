...
export class SnapshotHelper extends Service {
  ...
  getTransformer() {
    ...
    const schema = getNotesGraphWorkspaceSchema();
    // Local (offline) workspaces may have no server; the image-proxy base is
    // only used for remote images in comment content, so fall back to the
    // page origin rather than throwing on `serverService.baseUrl`.
    const baseUrl =
