...
interface DocDisplayIconOptions<T extends IconType> {
  ...
  /**
   * The doc has child notes (linked docs nested under it in the note tree).
   * When it would otherwise use the default page icon, show a folder icon so a
   * note that contains other notes reads as a container. User-set icons
   * (emoji/custom/alias/journal) still take precedence.
   */
  hasChildren?: boolean;
  /**
   * @default true
...
export class DocDisplayMetaService extends Service {
  ...
  icon$<T extends IconType = 'rc'>(
    ...
    return LiveData.computed(get => {
      ...
      // a note that contains other notes reads as a folder (below the explicit
      // user-set icons above, but ahead of the plain page/linked icons)
      if (options?.hasChildren) {
        return iconSet.FolderIcon;
      }
    ...
    return LiveData.computed(get => {
      ...
      if (!doc) {
        return this.i18nService.i18n.i18next.t(
          'com.notesgraph.notFoundPage.title',
          { lng }
        );
      }
