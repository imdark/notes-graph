...
const tabCanDrop =
  (
    tab?: TabStatus
  ): NonNullable<DropTargetOptions<NotesGraphDNDData>['canDrop']> =>
  ctx => {
    if (
...
const WorkbenchTab = ({
  ...
  const { dragRef } = useDraggable<NotesGraphDNDData>(() => {
    ...
    let entity: NotesGraphDNDData['draggable']['entity'];
...
export const AppTabsHeader = ({
  ...
  const onDrop = useAsyncCallback(
    async (data: DropTargetDropEvent<NotesGraphDNDData>, targetId?: string) => {
    ...
    >
      ...
      >
        <IconButton
          size={22.86}
          onClick={onAddTab}
          tooltip={t['com.notesgraph.multi-tab.new-tab']()}
