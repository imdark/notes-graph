import { fuzzyMatch } from '@notesgraph/core/utils/fuzzy-match';
...
export function suggestJournalDate(query: string): {
  ...
  if (query === '' || fuzzyMatch('today', query, true)) {
    return {
      ...
      alias: I18n.t('com.notesgraph.today'),
    };
  }
  ...
  if (fuzzyMatch('tomorrow', query, true)) {
    return {
      ...
      alias: I18n.t('com.notesgraph.tomorrow'),
    };
  }
  ...
  if (fuzzyMatch('yesterday', query, true)) {
    return {
      ...
      alias: I18n.t('com.notesgraph.yesterday'),
    };
  }
  ...
  const nextWeekDates = Array.from({ length: 7 }, (_, index) =>
    ...
    dateString: date,
    alias: I18n.t('com.notesgraph.next-week', {
  ...
  const lastWeekDates = Array.from({ length: 7 }, (_, index) =>
    ...
    dateString: date,
    alias: I18n.t('com.notesgraph.last-week', {
      weekday: weekDayNames[dayjs(date).day()],
    }),
  }));
