import { ResizePanel } from '@notesgraph/component/resize-panel';
import { NotesGraphErrorComponent } from '@notesgraph/core/components/notesgraph/notesgraph-error-boundary/notesgraph-error-fallback';
import { workbenchRoutes } from '@notesgraph/core/desktop/workbench-router';
import {
  appSettingAtom,
  FrameworkScope,
  useLiveData,
...
const routes: RouteObject[] = [
  {
    ...
    errorElement: <NotesGraphErrorComponent />,
...
export const WorkbenchRoot = memo(() => {
  ...
  // Only the active tab's views render side by side — other tabs stay off.
  const views = useLiveData(workbench.activeTabViews$);
  const indexOffset = useLiveData(workbench.activeTabStartIndex$);
