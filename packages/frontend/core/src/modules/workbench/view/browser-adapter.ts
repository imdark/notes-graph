...
export function useBindWorkbenchToBrowserRouter(
  ...
  // The view whose location the browser URL currently reflects. Lets the
  // effect below tell "the active view switched" apart from "the browser
  // location changed" — they need opposite sync directions.
  ...
  useEffect(() => {
    // The ACTIVE VIEW switched — a pane was focused, or a view was created
    // beside the current one (`workbench.open(..., { at: 'beside' })`, which
    // activates the new view). Here the browser must follow the view, never
    // the reverse. This effect used to fall straight through to the
    // browser→view push below with the still-unchanged URL, shoving the
    // current page into the newly active view: a view created with
    // defaultLocation '/chat' was overwritten on its first render into a
    // second copy of whatever page you were on — "clicking AI creates a
    // split of the current page" — and focusing another pane clobbered
    // that pane's location the same way.
    if (syncedViewRef.current !== view) {
      syncedViewRef.current = view;
      const viewLocation = view.location$.value;
      const target = viewLocationToBrowserLocation(viewLocation, basename);
      if (
        target.pathname !== browserLocation.pathname ||
        target.search !== browserLocation.search ||
        target.hash !== browserLocation.hash
      ) {
        // Tagged fromView so the re-run this navigation triggers is
        // recognised as our own echo (key match below) and ignored.
        navigate(target, {
          state: 'fromView,' + viewLocation.key,
          replace: true,
        });
      }
      return;
