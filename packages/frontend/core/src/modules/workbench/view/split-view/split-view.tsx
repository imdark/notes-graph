import { Checkbox, notify, useDndMonitor } from '@notesgraph/component';
...
export interface SplitViewProps extends HTMLAttributes<HTMLDivElement> {
  ...
  views: View[];
  /**
   * Index of `views[0]` within the workbench's full `views$` list. `views` is
   * the active tab's slice; workbench resize/move/createView take global
   * indices, so we add this offset before calling them.
   */
  indexOffset?: number;
...
export const SplitView = ({
  ...
  const onResizing = useCallback(
    (index: number, { x, y }: { x: number; y: number }) => {
      ...
      workbench.resize(index + indexOffset, percent);
    },
    [indexOffset, orientation, workbench]
  );

  const handleOnMove = useCallback(
    (from: number, to: number) => {
      onMove?.(from + indexOffset, to + indexOffset);
    },
    [indexOffset, onMove]
  );
  ...
  useDndMonitor<NotesGraphDNDData>(() => {
    return {
      canMonitor(data) {
        // allow dropping doc && tab view to split view panel (browser + desktop)
      ...
      onDrop(data) {
            ...
            notify.warning(
              {
                ...
                message: (
                  <div className={styles.folderWarningMessage}>
                    <p>
                      {t[
                        'com.notesgraph.split-view-folder-warning.description'
                      ]()}
                    </p>
                    <p>
                      <Checkbox
        ...
        const dropTarget =
          candidate.data as NotesGraphDNDData['draggable']['from'];
        const entity = data.source.data.entity;
        const from = data.source.data.from;
        ...
        if (
          ...
          // Global insertion index (the drop lands inside the active tab).
          const at = (() => {
            if (edge === 'left') {
              if (index === 0) {
                return indexOffset;
              ...
              if (index === views.length - 1) {
                return indexOffset + views.length;
              }
              return indexOffset + index + 1;
            } else {
              return indexOffset + views.length;
