...
export const SplitViewPanel = memo(function SplitViewPanel({
  ...
  const { dropTargetRef } = useDropTarget<NotesGraphDNDData>(() => {
    const handleDrag = (data: DropTargetDragEvent<NotesGraphDNDData>) => {
  ...
  const CloseItem =
    views.length > 1 ? (
      <MenuItem prefixIcon={<CloseIcon />} onClick={handleClose}>
        {t['com.notesgraph.workbench.split-view-menu.close']()}
      </MenuItem>
    ) : null;
  ...
  const MoveLeftItem =
    viewIndex > 0 && views.length > 1 ? (
      <MenuItem onClick={handleMoveLeft} prefixIcon={<InsertRightIcon />}>
        {t['com.notesgraph.workbench.split-view-menu.move-left']()}
      </MenuItem>
    ) : null;
  ...
  const FullScreenItem =
    views.length > 1 ? (
      <MenuItem onClick={handleCloseOthers} prefixIcon={<ExpandFullIcon />}>
        {t['com.notesgraph.workbench.split-view-menu.keep-this-one']()}
      </MenuItem>
    ) : null;
  ...
  const MoveRightItem =
    viewIndex < views.length - 1 ? (
      <MenuItem onClick={handleMoveRight} prefixIcon={<InsertLeftIcon />}>
        {t['com.notesgraph.workbench.split-view-menu.move-right']()}
      </MenuItem>
    ) : null;
  return (
    <>
