...
export class View extends Entity<{
  ...
  icon?: ViewIconName;
  /**
   * The tab this view belongs to. Views sharing a `tabId` are the split panes
   * of one tab and render side by side; different `tabId`s are separate tabs
   * (only the active tab's views render). Falls back to a fresh id so a view
   * created without one is its own tab.
   */
  tabId?: string;
}> {
  ...
  readonly tabId: string = this.props.tabId ?? nanoid();
