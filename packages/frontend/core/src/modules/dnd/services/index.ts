...
import type { SliceSnapshot } from '@blocksuite/notesgraph/store';
import type { DragBlockPayload } from '@blocksuite/notesgraph/widgets/drag-handle';
import {
...
export class DndService extends Service {
  ...
  private setupBlocksuiteAdapter() {
    /**
     * Migrate from notesgraph to blocksuite
     * For now, we only support doc
     */
    const notesgraphToBlocksuite = (args: MonitorGetFeedback<MixedDNDData>) => {
    ...
    /**
     * Migrate from blocksuite to notesgraph
     */
    const blocksuiteToNotesGraph = (args: MonitorGetFeedback<MixedDNDData>) => {
      const data = args.source.data;
      if (!data.entity && data.bsEntity) {
    ...
    function adaptDragEvent(args: MonitorGetFeedback<MixedDNDData>) {
      notesgraphToBlocksuite(args);
      blocksuiteToNotesGraph(args);
    }
    ...
    function getBSDropTarget(args: MonitorDragEvent<MixedDNDData>) {
      for (const target of args.location.current.dropTargets) {
        ...
        if (['NOTESGRAPH-EDGELESS-NOTE', 'NOTESGRAPH-NOTE'].includes(tagName))
          return 'note';
        if (tagName === 'NOTESGRAPH-EDGELESS-ROOT') return 'canvas';
      }
      return 'other';
    }
  ...
  ) => NotesGraphDNDData['draggable'] | null)[] = [];
  ...
  fromExternalData: fromExternalData<NotesGraphDNDData> = (
    ...
    let resolved: NotesGraphDNDData['draggable'] | null = null;
  ...
  toExternalData: toExternalData<NotesGraphDNDData> = (args, data) => {
  ...
  private readonly resolveBlockSnapshot = (
    ...
    for (const block of snapshot.content) {
      if (
        ['notesgraph:embed-linked-doc', 'notesgraph:embed-synced-doc'].includes(
          block.flavour
        )
      ) {
        return {
