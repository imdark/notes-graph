...
export const FolderCreateTip = ({
  ...
  const parent = parentName
    ? parentName
    : t['com.notesgraph.m.explorer.folder.root']();

  const tip = input
    ? t['com.notesgraph.m.explorer.folder.new-tip-not-empty']({
        value: input,
        parent,
      })
    : t['com.notesgraph.m.explorer.folder.new-tip-empty']({ parent });
...
export const FolderRenameSubMenu = ({
  ...
  const t = useI18n();
  const title = propsTitle || t['com.notesgraph.m.explorer.folder.rename']();
...
export const FolderRenameDialog = ({
  ...
  const t = useI18n();
  const title =
    propsTitle || t['com.notesgraph.m.explorer.folder.new-dialog-title']();
  const confirmText =
    propsConfirmText || t['com.notesgraph.m.explorer.folder.rename-confirm']();
