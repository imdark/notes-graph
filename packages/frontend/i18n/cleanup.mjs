...
const keys = Object.keys(enjson).filter(
  // exceptions
  key => !key.startsWith('com.notesgraph.payment.modal.')
);

const unusedKeys = keys.filter(key => {
  const regex1 = new RegExp(`[\`'"]${key.replace('.', '\\.')}[\`'"]`, 'g');
  // some place use i18n key like `t[`com.notesgraph.modal.${var}`]`
  // com.notesgraph.modal.confirm -> com.notesgraph.modal.
