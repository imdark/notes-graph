...
const i18nPkg = new Package('@notesgraph/i18n');
...
async function cleanupResources() {
  ...
  const templatePrefixRegex = /`[^`]*?(com\.notesgraph\.[^`]*?)\$\{/g;
  const concatPrefixRegex = /['"](com\.notesgraph\.[^'"]*?\.)['"]\s*\+/g;
  ...
  for (const resource of Object.values(resources)) {
    Object.keys(resource).forEach(key => {
      if (!key.startsWith('com.notesgraph.payment.modal.')) {
