...
import '@blocksuite/notesgraph/gfx/pointer';

import type { EdgelessRootBlockComponent } from '@blocksuite/notesgraph/blocks/root';
import { DefaultTool } from '@blocksuite/notesgraph/blocks/surface';
...
describe('theme service', () => {
  ...
  test('generates a color property', () => {
    ...
    expect(themeService.generateColorProperty('--notesgraph-hover-color')).toBe(
      'var(--notesgraph-hover-color)'
    );

    expect(themeService.generateColorProperty('--notesgraph-transparent')).toBe(
      'transparent'
    );
  ...
  test('gets a color value', () => {
    ...
    expect(themeService.getColorValue('--notesgraph-transparent')).toBe(
      '--notesgraph-transparent'
    );
    expect(
    ...
    expect(
      themeService.getColorValue('--notesgraph-tooltip', undefined, true)
    ).toBe('rgba(0, 0, 0, 1)');
