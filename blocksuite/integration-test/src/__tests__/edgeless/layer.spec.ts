...
import { ungroupCommand } from '@blocksuite/notesgraph/gfx/group';
import type {
  GroupElementModel,
  MindmapElementModel,
  NoteBlockModel,
} from '@blocksuite/notesgraph/model';
import { generateKeyBetween } from '@blocksuite/notesgraph/std/gfx';
  ...
  const assertBlocksContent = () => {
    const blocks = Array.from(
      document.querySelectorAll(
        'notesgraph-edgeless-root gfx-viewport > [data-block-id]'
      )
    );
...
test('block host z-index should update after reordering', async () => {
  ...
  const getBlockHost = (id: string) =>
    document.querySelector<HTMLElement>(
      `notesgraph-edgeless-root gfx-viewport > [data-block-id="${id}"]`
    );
...
test('the actual rendering z-index should satisfy the logic order of their indexes', async () => {
  ...
  blocks
    .filter(block => block.flavour !== 'notesgraph:surface')
    .forEach((block, index) => {
