...
describe('basic', () => {
  ...
  test('surface-ref should be rendered in page mode', async () => {
    ...
    expect(
      document.querySelector(
        `notesgraph-surface-ref[data-block-id="${surfaceRefId}"]`
      )
    ).instanceOf(Element);
  });
  ...
  test('content in frame should be rendered in the correct order', async () => {
    ...
    const refBlocks = Array.from(
      surfaceRef.querySelectorAll('notesgraph-edgeless-note')
    ) as HTMLElement[];
...
import type { FrameBlockComponent } from '@blocksuite/notesgraph/blocks/frame';
...
describe('clipboard', () => {
  test('import surface-ref snapshot should render content correctly', async () => {
    ...
    const surfaceRefs = newDoc!.getBlocksByFlavour('notesgraph:surface-ref');
