...
function addParagraph(doc: Store, noteId: string, content: string) {
  ...
  doc.addBlock('notesgraph:paragraph', props, noteId);
}
...
export function addSampleNotes(doc: Store, n: number) {
  ...
  const rootId = doc.getBlocksByFlavour('notesgraph:page')[0]?.id;
  ...
  for (let i = 0; i < n; i++) {
    ...
    const noteId = doc.addBlock('notesgraph:note', { xywh }, rootId);
