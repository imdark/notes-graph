import { LinkExtension } from '@blocksuite/notesgraph-inline-link';
...
import { createDefaultDoc } from '@blocksuite/notesgraph-shared/utils';
import { TextSelection } from '@blocksuite/std';
import type { InlineMarkdownMatch } from '@blocksuite/std/inline';
import { Text } from '@blocksuite/store';
import { beforeEach, describe, expect, test, vi } from 'vitest';

import { defaultSlashMenuConfig } from '../../../../notesgraph/widgets/slash-menu/src/config.js';
import type {
  SlashMenuActionItem,
  SlashMenuItem,
} from '../../../../notesgraph/widgets/slash-menu/src/types.js';
...
describe('markdown/list/paragraph/quote/code/link', () => {
  test('markdown list shortcut converts to todo list and keeps checked state', async () => {
    ...
    expect(model.flavour).toBe('notesgraph:list');
  ...
  test('markdown heading and quote shortcuts convert paragraph type', async () => {
    ...
    expect(headingModel.flavour).toBe('notesgraph:paragraph');
    ...
    expect(quoteModel.flavour).toBe('notesgraph:paragraph');
  ...
  test('markdown code shortcut converts paragraph to code block with language', async () => {
    ...
    expect(model.flavour).toBe('notesgraph:code');
  ...
  test('inline markdown converts style and link attributes', async () => {
    ...
    await triggerMarkdown(
      linkParagraphId,
      '[NotesGraph](https://notesgraph.com) ',
      'link'
    );
    const linkRichText = getRichTextByBlockId(linkParagraphId);
    expect(linkRichText.inlineEditor.yTextString).toBe('NotesGraph');
    expect(
...
describe('hotkey/bracket/linked-page', () => {
  ...
  test('consecutive linked-page reference nodes render as separate references', async () => {
    ...
    expect(richText.querySelectorAll('notesgraph-reference').length).toBe(2);
