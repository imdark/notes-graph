...
import { isStrictUrl } from '@blocksuite/notesgraph-shared/utils';
import type {
  BeforeinputHookCtx,
...
const autoIdentifyLink = (ctx: HookContext<NotesGraphTextAttributes>) => {
...
function handleExtendedAttributes(
  ctx:
    | BeforeinputHookCtx<NotesGraphTextAttributes>
    | CompositionEndHookCtx<NotesGraphTextAttributes>
) {
...
export const onVBeforeinput = (
  ctx: BeforeinputHookCtx<NotesGraphTextAttributes>
) => {
...
export const onVCompositionEnd = (
  ctx: CompositionEndHookCtx<NotesGraphTextAttributes>
) => {
  handleExtendedAttributes(ctx);
};
