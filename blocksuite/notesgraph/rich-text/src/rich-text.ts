...
export class RichText extends WithDisposable(ShadowlessElement) {
  ...
  private readonly _inlineEditor$ = signal<NotesGraphInlineEditor | null>(null);
  ...
  private _init() {
    ...
    // init inline editor
    this._inlineEditor$.value = new InlineEditor<NotesGraphTextAttributes>(
      this._yText,
      {
        ...
        inlineEditor
          ...
          .then(() => {
            ...
            if (verticalScrollContainer) {
              // A missing native range just means a queued Android input
              // apply hasn't re-seated the selection yet — not that the
              // caret actually left this block (same race documented on
              // `_isNativeSelectionInAnotherLiveRoot` in inline/services/
              // event.ts). Only skip the scroll when the native selection
              // demonstrably sits outside our own root; treat a missing
              // range, like the computed `range` above, as still ours. The
              // previous check compared containment backwards (parent
              // containing root, instead of root containing the native
              // range) and returned out of the whole effect on failure,
              // which also silently skipped the horizontal auto-scroll
              // below — this scopes the guard to vertical scroll only.
              const nativeRange = inlineEditor.getNativeRange();
              const movedToAnotherRoot =
                nativeRange &&
                inlineEditor.rootElement &&
                !inlineEditor.rootElement.contains(
                  nativeRange.commonAncestorContainer
                );

              if (!movedToAnotherRoot) {
                const containerRect =
                  verticalScrollContainer.getBoundingClientRect();
                const rangeRect = range.getBoundingClientRect();

                // Scroll the KNOWN container by just enough to reveal the
                // caret line (+ a small margin). Using
                // `this.scrollIntoView({block})` instead aligned the whole
                // block — for a tall/wrapped block (common on mobile, where
                // lines wrap more) that jumps the scroll far past the caret,
                // and it can target the wrong scrollable ancestor. A minimal
                // delta scroll avoids both.
                const MARGIN = 24;
                if (rangeRect.top < containerRect.top) {
                  verticalScrollContainer.scrollBy({
                    top: rangeRect.top - containerRect.top - MARGIN,
                  });
                } else if (rangeRect.bottom > containerRect.bottom) {
                  verticalScrollContainer.scrollBy({
                    top: rangeRect.bottom - containerRect.bottom + MARGIN,
                  });
                }
              }
            }
  ...
  accessor embedChecker: <
    TextAttributes extends NotesGraphTextAttributes = NotesGraphTextAttributes,
  >(
  ...
  @property({ attribute: false })
  accessor markdownMatches: InlineMarkdownMatch<NotesGraphTextAttributes>[] =
    [];
