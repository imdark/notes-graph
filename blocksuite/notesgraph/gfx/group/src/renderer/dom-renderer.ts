import { DomElementRendererExtension } from '@blocksuite/notesgraph-block-surface';
import {
  FontWeight,
...
export const GroupDomRendererExtension = DomElementRendererExtension(
  ...
  (model: GroupElementModel, domElement, renderer) => {
    ...
    const renderTitle = () => {
      ...
      titleElement.style.color = renderer.getPropertyValue('--notesgraph-blue');
    ...
    if (elements.includes(model.id)) {
      if (model.showTitle) {
        ...
        domElement.style.outlineColor =
          renderer.getPropertyValue('--notesgraph-blue');
