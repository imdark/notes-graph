...
function simplifyBlockForSnapshot(
  ...
  if (
    block.flavour === 'notesgraph:paragraph' ||
    block.flavour === 'notesgraph:list'
  ) {
  ...
  if (block.flavour === 'notesgraph:callout') {
    simplified.emoji = block.props.emoji;
  }

  if (block.flavour === 'notesgraph:attachment') {
  ...
  const children = (block.children ?? [])
    .filter(child => child.flavour !== 'notesgraph:surface')
...
`;
  ...
  test('link', async () => {
    ...
    const markdown = 'aaa [bbb](https://notesgraph.com/) ccc\n';
  ...
  test('inline link', async () => {
    ...
    const markdown = 'aaa https://notesgraph.com/  \n';
              ...
              delta: [
                {
                  insert: 'Hello',
                  attributes: {
                    color: 'var(--notesgraph-v2-text-highlight-fg-blue)',
                  },
                },
              ],
            },
...
`;
  ...
  test('inline link', async () => {
    const markdown = 'aaa https://notesgraph.com/ ccc\n';
    const blockSnapshot: BlockSnapshot = {
      type: 'block',
      id: 'matchesReplaceMap[0]',
      flavour: 'notesgraph:note',
