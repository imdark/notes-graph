...
import { FrameBlockModel } from '@blocksuite/notesgraph-model';
...
import type { NotesGraphFrameTitle } from './frame-title';
import {
  NOTESGRAPH_FRAME_TITLE_WIDGET,
  type NotesGraphFrameTitleWidget,
} from './notesgraph-frame-title-widget';
...
export class EdgelessFrameTitleEditor extends WithDisposable(
  ...
  override render() {
    ...
    const frameTitleWidget = this.edgeless.std.view.getWidget(
      NOTESGRAPH_FRAME_TITLE_WIDGET,
      this.frameModel.id
    ) as NotesGraphFrameTitleWidget | null;

    if (!frameTitleWidget) return nothing;

    const frameTitle = frameTitleWidget.querySelector<NotesGraphFrameTitle>(
      'notesgraph-frame-title'
    );

    const colors = frameTitle?.colors ?? {
      background: cssVarV2('edgeless/frame/background/white'),
      text: 'var(--notesgraph-text-primary-color)',
    };
