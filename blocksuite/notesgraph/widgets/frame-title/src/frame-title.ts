...
import { parseStringToRgba } from '@blocksuite/notesgraph-components/color-picker';
import {
  ColorScheme,
  FrameBlockModel,
  isTransparent,
...
export const NOTESGRAPH_FRAME_TITLE = 'notesgraph-frame-title';

@requiredProperties({
  model: PropTypes.instanceOf(FrameBlockModel),
})
export class NotesGraphFrameTitle extends SignalWatcher(
    ...
    _disposables.add(
      doc.slots.blockUpdated.subscribe(payload => {
        if (
          ...
          (payload.type === 'add' && payload.flavour === 'notesgraph:frame')
        ) {
          this._nestedFrame = this._isInsideFrame();
        }
