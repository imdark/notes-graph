  ...
  getPreviewStd = (blockIds: string[]) => {
    ...
    const extensions = [
      ...
      EditorSettingExtension({ setting$: settingSignal }),
      {
        setup(di) {
          di.override(
            BlockViewIdentifier('notesgraph:database'),
            () => literal`notesgraph-dnd-preview-database`
          );
        },
      } as ExtensionType,
      {
        setup(di) {
          di.override(BlockViewIdentifier('notesgraph:image'), () => {
            return (model: BlockModel) => {
              const parent = model.store.getParent(model.id);

              if (parent?.flavour === 'notesgraph:surface') {
                return literal`notesgraph-edgeless-placeholder-preview-image`;
              }

              return literal`notesgraph-placeholder-preview-image`;
            };
          });
        },
      } as ExtensionType,
    ...
    const noteBlock = this.widget.host.querySelector('notesgraph-note');
