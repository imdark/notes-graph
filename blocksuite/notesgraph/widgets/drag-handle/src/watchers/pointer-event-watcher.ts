...
import { captureEventTarget } from '@blocksuite/notesgraph-shared/utils';
import {
  BLOCK_ID_ATTR,
  type BlockComponent,
...
import { NOTESGRAPH_DRAG_HANDLE_WIDGET } from '../consts.js';
...
export class PointerEventWatcher {
  ...
  private readonly _clickHandler: UIEventHandler = ctx => {
    ...
    const insideDragHandle = !!element?.closest(NOTESGRAPH_DRAG_HANDLE_WIDGET);
  ...
  private readonly _pointerOutHandler: UIEventHandler = ctx => {
    ...
    const inPage = !!relatedElement?.closest('.notesgraph-page-viewport');
  ...
  private readonly _throttledPointerMoveHandler = throttle<UIEventHandler>(
    ctx => {
      ...
      if (
        element.closest('.notesgraph-drag-handle-container') ||
        element.closest('.notesgraph-add-block-widget-container')
      ) {
        return;
      }
