...
import { ParagraphBlockComponent } from '@blocksuite/notesgraph-block-paragraph';
...
import { surfaceRefToEmbed } from '@blocksuite/notesgraph-shared/adapters';
import {
...
export class DragEventWatcher {
    ...
    if (edge === 'right' && matchModels(dropModel, [ListBlockModel])) {
      ...
      if (
        snapshot.content.every(block =>
          schema.safeValidate(block.flavour, 'notesgraph:list')
        )
      ) {
      ...
      const placement =
        isDropOnNoteBlock &&
        schema.safeValidate(snapshot.content[0].flavour, 'notesgraph:note')
          ? 'in'
          : edge === 'top'
            ? 'before'
            : 'after';
  ...
  private readonly _onPageDrop = (
    ...
    if (
      ...
      snapshot.content.every(block => block.flavour === 'notesgraph:note')
    ) {
    ...
    if (
      ...
      if (dragPayload.from?.docId === this.widget.store.id) {
        ...
        const walk = (block: BlockSnapshot) => {
          if (block.flavour === 'notesgraph:surface') {
            Object.values(
  ...
  private readonly _rewriteSnapshotXYWH = (
    ...
    const rewrite = (block: BlockSnapshot) => {
      if (block.flavour === 'notesgraph:surface') {
        ...
        if (block.flavour === 'notesgraph:embed-iframe') {
          ...
          block.flavour === 'notesgraph:attachment' ||
          block.flavour === 'notesgraph:bookmark' ||
          block.flavour.startsWith('notesgraph:embed-')
        ) {
        ...
        if (block.flavour === 'notesgraph:image') {
  ...
  private readonly _dropAsGfxBlock = (
    ...
    const groupByParent = groupBy(snapshot.content, block =>
      schema.safeValidate(block.flavour, 'notesgraph:surface')
        ? 'notesgraph:surface'
        : schema.safeValidate(block.flavour, 'notesgraph:page')
          ? 'notesgraph:page'
    ...
    if (!groupByParent.empty?.length) {
      ...
      if (groupByParent['notesgraph:surface']) {
        const content = groupByParent['notesgraph:surface'];
            ...
            slices?.content.forEach((block, idx) => {
              if (block.id === content[idx].id) {
                if (block.flavour === 'notesgraph:embed-iframe') {
                  ...
                  block.flavour === 'notesgraph:image' ||
                  block.flavour === 'notesgraph:attachment' ||
                  block.flavour === 'notesgraph:bookmark' ||
                  block.flavour.startsWith('notesgraph:embed-')
                ) {
      ...
      if (groupByParent['notesgraph:page']) {
        const content = groupByParent['notesgraph:page'];
        const pageSnapshot = {
          ...snapshot,
          content,
        };
      ...
      const content = snapshot.content.filter(block =>
        schema.safeValidate(block.flavour, 'notesgraph:note')
      );
  ...
  private async _dropToModel(
    ...
    try {
      ...
      if (snapshot.content.length === 1) {
        const [first] = snapshot.content;
        if (first.flavour === 'notesgraph:embed-linked-doc') {
  ...
  private _makeDropTarget(view: BlockComponent) {
    ...
    if (
      // notesgraph:surface block can't be drop target in any modes
