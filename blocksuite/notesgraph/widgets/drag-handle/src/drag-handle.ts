...
import { EdgelessCRUDIdentifier } from '@blocksuite/notesgraph-block-surface';
...
import type { NOTESGRAPH_DRAG_HANDLE_WIDGET } from './consts.js';
...
export class NotesGraphDragHandleWidget extends WidgetComponent<RootBlockModel> {
  ...
  override render() {
    ...
    const classes = {
      'notesgraph-drag-handle-grabber': true,
      dots: isGfx ? true : false,
    };

    return html`
      <div class="notesgraph-drag-handle-widget">
        <div class="notesgraph-add-block-widget-container">
          <notesgraph-add-block-widget
            .visible=${this.showAddBlockWidget && this.mode === 'page'}
            @add-block=${this._handleAddBlock}
          ></notesgraph-add-block-widget>
        </div>
        <div class="notesgraph-drag-handle-container">
        ...
        <div class="notesgraph-drag-hover-rect" style=${hoverRectStyle}></div>
      </div>
    `;
  }

  @query('.notesgraph-drag-handle-container')
  accessor dragHandleContainer!: HTMLDivElement;

  @query('.notesgraph-drag-handle-grabber')
  accessor dragHandleGrabber!: HTMLDivElement;

  @query('.notesgraph-add-block-widget-container')
...
declare global {
  interface HTMLElementTagNameMap {
    [NOTESGRAPH_DRAG_HANDLE_WIDGET]: NotesGraphDragHandleWidget;
  }
}
