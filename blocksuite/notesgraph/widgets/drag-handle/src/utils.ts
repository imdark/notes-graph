...
import { type CalloutBlockComponent } from '@blocksuite/notesgraph-block-callout';
import {
  EdgelessNoteBackground,
  EdgelessNoteBlockComponent,
  NOTESGRAPH_EDGELESS_NOTE,
...
export const insideDatabaseTable = (element: Element) => {
  return !!element.closest('.notesgraph-database-block-table');
};
...
export const getParentNoteBlock = (blockComponent: BlockComponent) => {
  return (
    blockComponent.closest('notesgraph-note, notesgraph-edgeless-note') ?? null
  );
};
...
export const getClosestNoteBlock = (
  ...
  return isInsidePageEditor
    ? findClosestBlockComponent(rootComponent, point, 'notesgraph-note')
...
export const getClosestBlockByPoint = (
  ...
  if (
    !closestNoteBlock ||
    closestNoteBlock.closest('.notesgraph-surface-ref')
  ) {
    return null;
  }
  ...
  const blockSelector =
    '.notesgraph-note-block-container > .notesgraph-block-children-container > [data-block-id]';
  ...
  if (matchModels(closestBlock.model, [ParagraphBlockModel])) {
    const callout =
      closestBlock.closest<CalloutBlockComponent>('notesgraph-callout');
    if (callout) {
      return callout;
    }
  }
...
function getHoveringNote(point: Point) {
  ...
  for (const el of elements) {
    ...
    if (el instanceof EdgelessNoteBackground) {
      return el.closest(NOTESGRAPH_EDGELESS_NOTE) ?? null;
    }
  }
  return null;
}
