...
import { ViewportElementProvider } from '@blocksuite/notesgraph-shared/services';
import {
  autoScroll,
  getScrollContainer,
...
export const NOTESGRAPH_PAGE_DRAGGING_AREA_WIDGET =
  'notesgraph-page-dragging-area-widget';

export class NotesGraphPageDraggingAreaWidget extends WidgetComponent<RootBlockModel> {
  static excludeFlavours: string[] = ['notesgraph:note', 'notesgraph:surface'];
    ...
    const getAllNodeFromTree = (): BlockComponent[] => {
      ...
      this.host.view.walkThrough(node => {
        ...
        if (
          ...
          !NotesGraphPageDraggingAreaWidget.excludeFlavours.includes(
            view.model.flavour
          )
        ) {
          blocks.push(view);
  ...
  override render() {
    ...
    return html`
      ...
      <div class="notesgraph-page-dragging-area" style=${styleMap(style)}></div>
    `;
  }
...
export const pageDraggingAreaWidget = WidgetViewExtension(
  ...
  literal`${unsafeStatic(NOTESGRAPH_PAGE_DRAGGING_AREA_WIDGET)}`
);

declare global {
  interface HTMLElementTagNameMap {
    [NOTESGRAPH_PAGE_DRAGGING_AREA_WIDGET]: NotesGraphPageDraggingAreaWidget;
  }
}
