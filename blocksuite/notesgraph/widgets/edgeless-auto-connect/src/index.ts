...
import { SmallDocIcon } from '@blocksuite/notesgraph-components/icons';
import {
  FrameBlockModel,
  NoteBlockModel,
  NoteDisplayMode,
...
export const NOTESGRAPH_EDGELESS_AUTO_CONNECT_WIDGET =
  'notesgraph-edgeless-auto-connect-widget';
...
export class EdgelessAutoConnectWidget extends WidgetComponent<RootBlockModel> {
    ...
    return html`${repeat(
      ...
      note => {
        ...
        const style = styleMap({
          ...
          borderRadius: '50%',
          backgroundColor: 'var(--notesgraph-text-secondary-color)',
        ...
        return html`<div style=${style} class="edgeless-only-index-label">
          ...
          <notesgraph-tooltip tip-position="bottom">
  ...
  private _initLabels() {
    ...
    const surfaceRefs = this.block.std.store
      .getBlocksByFlavour('notesgraph:surface-ref')
    ...
    this._disposables.add(
      this.store.slots.blockUpdated.subscribe(payload => {
        if (payload.flavour === 'notesgraph:surface-ref') {
          switch (payload.type) {
            case 'add':
...
export const autoConnectWidget = WidgetViewExtension(
  ...
  literal`${unsafeStatic(NOTESGRAPH_EDGELESS_AUTO_CONNECT_WIDGET)}`
);

declare global {
  interface HTMLElementTagNameMap {
    'notesgraph-edgeless-auto-connect-widget': EdgelessAutoConnectWidget;
  }
}
