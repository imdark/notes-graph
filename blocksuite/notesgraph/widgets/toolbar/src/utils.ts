...
export const sideMap = new Map([
  ...
  ['notesgraph:surface:frame', { top: 28 }],
  // includes group element
  ['notesgraph:surface:group', { top: 20 }],
  // has only one shape element
  ['notesgraph:surface:shape', { top: 26, bottom: -26 }],
]);
...
export function autoUpdatePosition(
  ...
  const isInline = flavour === 'notesgraph:note';
...
export function renderToolbar(
  ...
  const actions = [
    ...
    `custom:${flavour}`,
    hasSurfaceScope
      ? ['notesgraph:surface:*', 'custom:notesgraph:surface:*']
      : [],
    'notesgraph:*',
    'custom:notesgraph:*',
  ]
  ...
  // Avoids shaking
  if (flavour === 'notesgraph:note' && context.std.range.value) {
