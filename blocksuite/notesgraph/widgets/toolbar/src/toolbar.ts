...
import { DatabaseSelection } from '@blocksuite/notesgraph-block-database';
import { EdgelessLegacySlotIdentifier } from '@blocksuite/notesgraph-block-surface';
import { TableSelection } from '@blocksuite/notesgraph-block-table';
import {
  darkToolbarStyles,
...
export const NOTESGRAPH_TOOLBAR_WIDGET = 'notesgraph-toolbar-widget';

const MOBILE_TOOLBAR_STYLE_ID = 'notesgraph-mobile-toolbar-style';

/**
 * The mobile toolbar is portaled to <body> (see the IS_MOBILE branch below),
 * so the widget's shadow-scoped rules no longer reach it — mirror the ones it
 * needs in a global sheet, scoped to the mobile variant.
 */
function ensureMobileToolbarStyles() {
  if (document.getElementById(MOBILE_TOOLBAR_STYLE_ID)) return;
  const style = document.createElement('style');
  style.id = MOBILE_TOOLBAR_STYLE_ID;
  style.textContent = `
    editor-toolbar[data-mobile='true'] {
      ...
      /* no overflow clip here — dropdown menus render above the bar and an
         auto overflow would cut them off */
      opacity: 0;
      display: none;
      width: max-content;
      backface-visibility: hidden;
      z-index: var(--notesgraph-z-index-popover, 1200);
    ...
    editor-toolbar[data-mobile='true'][data-open] {
      display: flex;
      opacity: 1;
      transition-timing-function: ease-in;
    }
    ${darkToolbarStyles("editor-toolbar[data-mobile='true']")}
    ${lightToolbarStyles("editor-toolbar[data-mobile='true']")}
  `;
  document.head.append(style);
}

export class NotesGraphToolbarWidget extends WidgetComponent {
  ...
  updateWithSurface(
    ...
    let flavour = 'notesgraph:surface';
    ...
    if (activated && surface) {
      ...
      if (hasLocked) {
        flavour = 'notesgraph:surface:locked';
      } else {
        if (paired.length === 1) {
          flavour = paired[0][0];
          if (
            flavour === 'notesgraph:surface:shape' &&
            paired[0][1].length === 1
          ) {
      ...
      if (!sideOptions) {
        ...
        if (flavours.has('notesgraph:surface:frame')) {
          sideOptions = sideMap.get('notesgraph:surface:frame') ?? null;
        } else if (flavours.has('notesgraph:surface:group')) {
          sideOptions = sideMap.get('notesgraph:surface:group') ?? null;
        }
      }
    }
  ...
  override connectedCallback() {
    ...
    if (IS_MOBILE) {
      ...
      // Portal to <body>: inside the widget a transformed ancestor both
      // demotes position:fixed to ancestor-relative AND traps the toolbar in
      // a stacking context below the app's bottom tab bar — no z-index can
      // escape that. The global sheet re-applies the shadow-scoped rules.
      ensureMobileToolbarStyles();
      ...
      updateMobilePosition = () => {
        ...
        const gap = Math.max(16, keyboardHeight + 16);
        toolbar.style.bottom = `${gap}px`;
        // A transformed ancestor demotes position:fixed to ancestor-relative,
        // which parks the toolbar just below the visible viewport (observed
        // on Android). Measure where it actually landed and translate the
        // difference away — converges in one frame and is a no-op when fixed
        // behaves normally.
        requestAnimationFrame(() => {
          const rect = toolbar.getBoundingClientRect();
          if (!rect.height) return;
          const targetBottom = vv.offsetTop + vv.height - gap;
          ...
          if (Math.abs(delta) > 1) {
            toolbar.style.transform = `translateX(-50%) translateY(${
              currentY + delta
            }px)`;
          }
        });
      ...
      const syncMobileTextSelection = () => {
        ...
        batch(() => {
          ...
          flavour$.value = 'notesgraph:note';
          placement$.value =
            toolbarRegistry.getModulePlacement('notesgraph:note');
      ...
      std.selection.filter$(BlockSelection).subscribe(selections => {
        ...
        batch(() => {
          ...
          placement$.value = toolbarRegistry.getModulePlacement(
            flavour,
            flavour === 'notesgraph:note' ? 'top' : 'top-start'
          );
