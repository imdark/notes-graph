...
export const NOTESGRAPH_VIEWPORT_OVERLAY_WIDGET =
  'notesgraph-viewport-overlay-widget';

export class NotesGraphViewportOverlayWidget extends WidgetComponent<RootBlockModel> {
  static override styles = css`
    .notesgraph-viewport-overlay-widget {
      ...
      pointer-events: none;
      z-index: calc(var(--notesgraph-z-index-popover) - 1);
    }

    .notesgraph-viewport-overlay-widget.lock {
      pointer-events: auto;
    }
  `;
  ...
  override render() {
    const classes = classMap({
      'notesgraph-viewport-overlay-widget': true,
...
export const viewportOverlayWidget = WidgetViewExtension(
  ...
  literal`${unsafeStatic(NOTESGRAPH_VIEWPORT_OVERLAY_WIDGET)}`
);

declare global {
  interface HTMLElementTagNameMap {
    [NOTESGRAPH_VIEWPORT_OVERLAY_WIDGET]: NotesGraphViewportOverlayWidget;
  }
}
