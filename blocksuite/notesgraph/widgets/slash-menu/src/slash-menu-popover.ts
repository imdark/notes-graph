...
import {
  NOTESGRAPH_SLASH_MENU_MAX_HEIGHT,
  NOTESGRAPH_SLASH_MENU_TOOLTIP_TIMEOUT,
  NOTESGRAPH_SLASH_MENU_TRIGGER_KEY,
} from './consts.js';
...
export class SlashMenu extends WithDisposable(LitElement) {
  ...
  private readonly _handleClickItem = (item: SlashMenuActionItem) => {
    ...
    cleanSpecifiedTail(
      this.context.std,
      this.context.model,
      NOTESGRAPH_SLASH_MENU_TRIGGER_KEY + (this._query || '')
    );
    this.inlineEditor
      .waitForUpdate()
      .then(() => {
        ...
        this._telemetry?.track('SelectSlashMenuItem', {
          ...
          segment:
            this.context.model.flavour === 'notesgraph:edgeless-text'
              ? 'edgeless-text'
              : 'doc',
          module: 'slash menu',
          control: item.name,
  ...
  constructor(
    private readonly inlineEditor: NotesGraphInlineEditor,
  ...
  override connectedCallback() {
    ...
    createKeydownObserver({
      ...
      interceptor: (event, next) => {
        ...
        if (key === NOTESGRAPH_SLASH_MENU_TRIGGER_KEY) {
  ...
  override render() {
    const slashMenuStyles = this._position
      ? {
          ...
          maxHeight: `${Math.min(this._position.height, NOTESGRAPH_SLASH_MENU_MAX_HEIGHT)}px`,
        }
      : {
          visibility: 'hidden',
        };
...
export class InnerSlashMenu extends WithDisposable(LitElement) {
  ...
  private readonly _openSubMenu = (item: SlashMenuSubMenu) => {
    ...
    subMenuElement.style.zIndex = `calc(var(--notesgraph-z-index-popover) + ${this.depth})`;
  ...
  private readonly _renderActionItem = (item: SlashMenuActionItem) => {
    ...
    >
      ...
      html`<notesgraph-tooltip
        ...
        .hoverOptions=${{
          enterDelay: NOTESGRAPH_SLASH_MENU_TOOLTIP_TIMEOUT,
          allowMultiple: false,
        }}
      >
