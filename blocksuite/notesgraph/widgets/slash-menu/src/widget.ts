...
import { getInlineEditorByModel } from '@blocksuite/notesgraph-rich-text';
...
import { NOTESGRAPH_SLASH_MENU_TRIGGER_KEY } from './consts';
...
export class NotesGraphSlashMenuWidget extends WidgetComponent {
  ...
  private readonly _handleInput = (
    ...
    inlineRangeApplyCallback(() => {
      ...
      if (!text.endsWith(NOTESGRAPH_SLASH_MENU_TRIGGER_KEY)) return;
  ...
  private readonly _onCompositionEnd = (ctx: UIEventStateContext) => {
    ...
    if (event.data !== NOTESGRAPH_SLASH_MENU_TRIGGER_KEY) return;
  ...
  private readonly _onBeforeInput = (ctx: UIEventStateContext) => {
    ...
    if (!event.data.includes(NOTESGRAPH_SLASH_MENU_TRIGGER_KEY)) return;
