...
export class NotesGraphLinkedDocWidget extends WidgetComponent<RootBlockModel> {
  ...
  private readonly _renderLinkedDocMenu = () => {
    ...
    return html`<notesgraph-mobile-linked-doc-menu
  ...
  private readonly _renderLinkedDocPopover = () => {
    return html`<notesgraph-linked-doc-popover
      .context=${this._context}
    ></notesgraph-linked-doc-popover>`;
  };
  ...
  get config(): LinkedWidgetConfig {
    return {
      triggerKeys: ['@', '[[', '【【'],
      ignoreBlockTypes: ['notesgraph:code'],
      ignoreSelector:
...
export const linkedDocWidget = WidgetViewExtension(
  'notesgraph:page',
  NOTESGRAPH_LINKED_DOC_WIDGET,
  literal`${unsafeStatic(NOTESGRAPH_LINKED_DOC_WIDGET)}`
);

declare global {
  interface HTMLElementTagNameMap {
    [NOTESGRAPH_LINKED_DOC_WIDGET]: NotesGraphLinkedDocWidget;
  }
}
