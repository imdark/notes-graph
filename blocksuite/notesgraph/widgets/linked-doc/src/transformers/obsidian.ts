import { FootNoteReferenceParamsSchema } from '@blocksuite/notesgraph-model';
import {
...
export const obsidianWikilinkToDeltaMatcher = MarkdownASTToDeltaExtension({
  ...
  toDelta: (ast, context) => {
    ...
    const deltas: DeltaInsert<NotesGraphTextAttributes>[] = [];
    let cursor = 0;
...
export type ImportObsidianVaultOptions = {
  ...
  /**
   * Optional hook to rewrite each markdown file before it is parsed — e.g. to
   * strip Logseq `key:: value` properties. Receives the raw text and the file's
   * path; returns the markdown to import.
   */
  preprocess?: (markdown: string, filePath: string) => string;
};

export type ImportObsidianVaultResult = {
  docIds: string[];
  docEmojis: Map<string, string>;
  /** doc id ↔ source file path, so callers can build a folder hierarchy. */
  docPaths: Array<{ fullPath: string; docId: string }>;
};
...
export async function importObsidianVault({
  ...
  const docIds: string[] = [];
  const docPaths: Array<{ fullPath: string; docId: string }> = [];
  ...
  for (const file of importedFiles) {
    ...
    if (file.name.endsWith('.md')) {
      ...
      const markdown = preprocess
        ? preprocess(rawMarkdown, filePath)
        : rawMarkdown;
    ...
    markdownBlobs.map(async markdownFile => {
      ...
      if (snapshot) {
        ...
        if (doc) {
          ...
          docIds.push(doc.id);
          docPaths.push({ fullPath, docId: doc.id });
        }
      }
    })
  );

  return { docIds, docEmojis, docPaths };
}
