...
import { addSiblingAttachmentBlocks } from '@blocksuite/notesgraph-block-attachment';
import { insertDatabaseBlockCommand } from '@blocksuite/notesgraph-block-database';
import { insertEmptyEmbedIframeCommand } from '@blocksuite/notesgraph-block-embed';
import { insertImagesCommand } from '@blocksuite/notesgraph-block-image';
import { insertLatexBlockCommand } from '@blocksuite/notesgraph-block-latex';
import {
  canDedentListCommand,
  canIndentListCommand,
  dedentListCommand,
  indentListCommand,
} from '@blocksuite/notesgraph-block-list';
import { updateBlockType } from '@blocksuite/notesgraph-block-note';
import {
  canDedentParagraphCommand,
  canIndentParagraphCommand,
  dedentParagraphCommand,
  indentParagraphCommand,
} from '@blocksuite/notesgraph-block-paragraph';
import {
  DefaultTool,
  getSurfaceBlock,
} from '@blocksuite/notesgraph-block-surface';
import { insertSurfaceRefBlockCommand } from '@blocksuite/notesgraph-block-surface-ref';
import { insertTableBlockCommand } from '@blocksuite/notesgraph-block-table';
import { toggleEmbedCardCreateModal } from '@blocksuite/notesgraph-components/embed-card-modal';
...
import { insertContent } from '@blocksuite/notesgraph-rich-text';
import {
...
import type { NotesGraphTextStyleAttributes } from '@blocksuite/notesgraph-shared/types';
import {
  createDefaultDoc,
...
import type { NotesGraphLinkedDocWidget } from '@blocksuite/notesgraph-widget-linked-doc';
import {
  type BlockComponent,
  type BlockStdScope,
...
const textToolActionItems: KeyboardToolbarActionItem[] = [
  {
    ...
    icon: TextIcon(),
    showWhen: ({ std }) =>
      std.store.schema.flavourSchemaMap.has('notesgraph:paragraph'),
    action: ({ std }) => {
  ...
  })),
  {
    name: 'CodeBlock',
    showWhen: ({ std }) =>
      std.store.schema.flavourSchemaMap.has('notesgraph:code'),
    icon: CodeBlockIcon(),
    action: ({ std }) => {
      std.command.exec(updateBlockType, {
  ...
  },
  {
    name: 'Equation',
    showWhen: ({ std }) =>
      std.store.schema.flavourSchemaMap.has('notesgraph:latex'),
    icon: TeXIcon(),
    action: ({ std }) => {
      std.command.exec(updateBlockType, {
        flavour: 'notesgraph:latex',
      });
    },
  },
  {
  ...
  },
  {
    ...
    icon: DividerIcon(),
    showWhen: ({ std }) =>
      std.store.schema.flavourSchemaMap.has('notesgraph:divider'),
    action: ({ std }) => {
      std.command.exec(updateBlockType, {
  ...
  },
  {
    ...
    showWhen: ({ std, rootComponent: { model } }) =>
      std.store.schema.flavourSchemaMap.has('notesgraph:table') &&
      !isInsideBlockByFlavour(std.store, model, 'notesgraph:edgeless-text'),
    action: ({ std }) => {
      std.command
        .chain()
  ...
  },
  {
    ...
    action: ({ rootComponent: { model }, std }) => {
      ...
      if (!calloutId) return;
      const paragraphId = store.addBlock('notesgraph:paragraph', {}, calloutId);
...
const listToolActionItems: KeyboardToolbarActionItem[] = [
  {
    ...
    showWhen: ({ std }) =>
      std.store.schema.flavourSchemaMap.has('notesgraph:list'),
    action: ({ std }) => {
      std.command.exec(updateBlockType, {
  ...
  items: [
    {
      ...
      icon: NewPageIcon(),
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('notesgraph:embed-linked-doc'),
      action: ({ std }) => {
        std.command
          .chain()
          .pipe(getSelectedModelsCommand)
    ...
    },
    {
      ...
      showWhen: ({ std, rootComponent }) => {
        const linkedDocWidget = std.view.getWidget(
          'notesgraph-linked-doc-widget',
          rootComponent.model.id
        );
        if (!linkedDocWidget) return false;

        return std.store.schema.flavourSchemaMap.has(
      ...
      action: ({ rootComponent, closeToolPanel }) => {
        ...
        assertType<NotesGraphLinkedDocWidget>(linkedDocWidget);
  ...
  items: [
    {
      ...
      icon: ImageIcon(),
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('notesgraph:image'),
      action: ({ std }) => {
        std.command
          .chain()
          .pipe(getSelectedModelsCommand)
    ...
    },
    {
      ...
      icon: LinkIcon(),
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('notesgraph:bookmark'),
      action: async ({ std }) => {
  ...
  items: [
    {
      ...
      showWhen: ({ std }) => {
        return std.store.schema.flavourSchemaMap.has('notesgraph:embed-iframe');
      },
      action: async ({ std }) => {
        std.command
          .chain()
    ...
    },
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('notesgraph:embed-youtube'),
      action: async ({ std }) => {
        const [_, { selectedModels }] = std.command.exec(
          getSelectedModelsCommand
        );
    ...
    },
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('notesgraph:embed-github'),
      action: async ({ std }) => {
        const [_, { selectedModels }] = std.command.exec(
          getSelectedModelsCommand
        );
    ...
    },
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('notesgraph:embed-figma'),
      action: async ({ std }) => {
        const [_, { selectedModels }] = std.command.exec(
          getSelectedModelsCommand
        );
    ...
    },
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('notesgraph:embed-loom'),
      action: async ({ std }) => {
        const [_, { selectedModels }] = std.command.exec(
          getSelectedModelsCommand
        );
...
const documentGroupFrameToolGroup: DynamicKeyboardToolPanelGroup = ({
  ...
  const frameModels = store
    .getBlocksByFlavour('notesgraph:frame')
  ...
  items: [
    {
      ...
      showWhen: ({ std }) =>
        std.store.schema.flavourSchemaMap.has('notesgraph:database'),
      action: ({ std }) => {
        std.command
          .chain()
          .pipe(getSelectedModelsCommand)
        ...
        ...(
          ...
          action: ({ std }) => {
            const payload = {
              ...
              } satisfies NotesGraphTextStyleAttributes,
            };
            std.command
              .chain()
              .try(chain => [
...
export const defaultKeyboardToolbarConfig: KeyboardToolbarConfig = {
  items: [
    moreToolPanel,
    // TODO(@L-Sun): add ai function in NotesGraph side
