...
export class ThemeService extends Extension {
   ...
   * `#fff`
   * `light-dark(#fff, #000)`
   * `var(--notesgraph-palette-shape-blue)`
   * ```
   */
  ...
  getCssVariableColor(property: string, theme = this.theme) {
    ...
    if (property.startsWith('--')) {
      ...
      const key = property as keyof NotesGraphCssVariables;
      // V1 theme
      const color =
