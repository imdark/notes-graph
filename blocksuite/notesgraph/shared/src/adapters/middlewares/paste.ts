...
class PasteTr {
  ...
  focusPasted = () => {
    ...
    const cursorBlock =
      this.pointState.model.flavour === 'notesgraph:code' || !this.lastSnapshot
  ...
  constructor(
    ...
    this.firstSnapshotIsPlainText =
      this.firstSnapshot.flavour === 'notesgraph:paragraph' &&
  ...
  private _transformLinkDelta(
    ...
    const newDelta = delta.map(op => {
      ...
      const reference: NotesGraphTextAttributes['reference'] = {
        pageId,
        type: 'LinkedPage',
      };
      // Title alias
  ...
  merge() {
    ...
    if (this.pointState.model.flavour === 'notesgraph:code') {
      this._mergeCode();
      return;
    }
...
function flatNote(snapshot: SliceSnapshot) {
  if (snapshot.content[0]?.flavour === 'notesgraph:note') {
