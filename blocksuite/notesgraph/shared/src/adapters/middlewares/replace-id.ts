...
export const replaceIdMiddleware =
  ...
  ({ slots, docCRUD, assetsManager }) => {
    ...
    const afterImportBlockSubscription = afterImportBlock$
      ...
      .subscribe(model => {
        // Each row is a child block whose id was regenerated; remap the cell
        // map keys to the new row ids.
        ...
        // Relation cells store references to other rows by id (e.g. the Gantt
        // parent / task-dependency links). Those ids must be remapped too, or
        // the relationships dangle after the rows are regenerated on duplicate,
        // paste, or cross-doc snapshot.
        const relationColumnIds = new Set(
          model.props.columns
            .filter(column => column.type === 'relation')
            .map(column => column.id)
        );
        if (relationColumnIds.size > 0) {
          Object.values(model.props.cells).forEach(row => {
            relationColumnIds.forEach(columnId => {
              const cell = row[columnId];
              if (!cell || !Array.isArray(cell.value)) return;
              cell.value = cell.value.map(ref =>
                typeof ref === 'string' && idMap.has(ref)
                  ? idMap.get(ref)!
                  : ref
              );
            });
    ...
    const replaceLinkedPageIdSubscription = afterImportBlock$
      ...
      .subscribe(model => {
        ...
        for (const d of model.props.text.toDelta()) {
          ...
          const originalPageId = reference?.pageId;
          if (reference && originalPageId) {
            let newId = idMap.get(originalPageId);
            if (!newId) {
              // The referenced doc hasn't been seen yet. During a multi-doc
              // import (e.g. a workspace snapshot zip) docs are processed in
              // arbitrary order, so the target may simply be imported later.
              // If it already exists in the collection under its original id,
              // leave the reference untouched; otherwise pre-assign a stable
              // new id and register it so the target doc reuses that same id
              // when it's imported (via `beforeImportPage`) and the link
              // survives. Mirrors the embed-linked-doc handling below —
              // previously this branch dropped the remap and left a dangling
              // id, silently breaking cross-note links on import.
              if (docCRUD.get(originalPageId)) {
                prev += d.insert?.length ?? 0;
                continue;
              }
              newId = idGenerator();
              idMap.set(originalPageId, newId);
            }
    ...
    const replaceSurfaceRefIdSubscription = afterImportBlock$
      ...
      .subscribe(model => {
        ...
        // Otherwise,
        // 1. If the reference is an notesgraph:frame not in doc, generate a new id.
        // 2. If the reference is graph, keep the original id.
        if (idMap.has(original)) {
          model.props.reference = idMap.get(original)!;
        } else if (
          model.props.refFlavour === 'notesgraph:frame' &&
    ...
    const beforeImportBlockSubscription = slots.beforeImport
      ...
      .subscribe(payload => {
        ...
        if (snapshot.flavour === 'notesgraph:page') {
          const index = snapshot.children.findIndex(
            c => c.flavour === 'notesgraph:surface'
          );
          if (index !== -1) {
        ...
        // Should be re-paired.
        if (
          ['notesgraph:attachment', 'notesgraph:image'].includes(
            snapshot.flavour
          )
        ) {
        ...
        if (snapshot.flavour === 'notesgraph:surface') {
