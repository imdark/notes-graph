...
const tagToFlavour: Record<string, string> = {
  ...
  'notesgraph-database': 'notesgraph:database',
...
export function createNotesGraphTemplate(
   ...
   *       <notesgraph-paragraph id="paragraph-1">Hello, world<anchor /></notesgraph-paragraph>
   *       <notesgraph-paragraph id="paragraph-2">Hello, world<focus /></notesgraph-paragraph>
   *     </notesgraph-note>
   *   </notesgraph-page>
   * `;
   * ```
   */
  function notesgraph(strings: TemplateStringsArray, ...values: any[]) {
   ...
   * Example:
   * ```
   * const block = block`<notesgraph-note />`
   * ```
   */
  function block(
    ...
    doc.load(() => {
      ...
      if (
        ...
        const pageId = store.addBlock('notesgraph:page', {});
        const noteId = store.addBlock('notesgraph:note', {}, pageId);
...
export const { notesgraph, block } = createNotesGraphTemplate();
