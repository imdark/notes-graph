...
/**
 * Android WebView backs `<input type=file>` picks with content:// URIs whose
 * read grant is revoked shortly after the picker activity closes — a lazy
 * read later (e.g. blobSync.set in a worker) throws NotReadableError. Copy
 * the bytes while the grant is still valid; keep the live handle if the copy
 * itself fails.
 */
async function materializeFiles(files: File[]): Promise<File[]> {
  return Promise.all(
...
export async function openFilesWith(
  ...
  return new Promise(resolve => {
    ...
    input.classList.add('notesgraph-upload-input');
    ...
    input.addEventListener('change', () => {
      ...
      resolve(input.files ? materializeFiles(Array.from(input.files)) : null);
    });
