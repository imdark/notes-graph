...
import { DatabaseBlockModel } from '@blocksuite/notesgraph-model';
...
export function getDropRectByPoint(
  ...
  if (isDatabase) {
    ...
    for (; i < len; i++) {
      ...
      if (e.classList.contains('notesgraph-database-block-row-cell-content')) {
      ...
      if (e.classList.contains('notesgraph-database-block-row')) {
  ...
  if (
    parent?.classList.contains('notesgraph-database-block-row-cell-content')
  ) {
    result.flag = DropFlags.Database;
...
function getDatabaseBlockTableElement(element: Element) {
  return element.querySelector('.notesgraph-database-block-table');
}
...
function getDatabaseBlockColumnHeaderElement(element: Element) {
  return element.querySelector('.notesgraph-database-column-header');
}
...
function getDatabaseBlockRowsElement(element: Element) {
  return element.querySelector('.notesgraph-database-block-rows');
}

function getCellRect(element: Element, bounds?: DOMRect) {
  if (!bounds) {
    const table = element.closest('.notesgraph-database-block-table');
    if (!table) {
      return null;
    }
    bounds = table.getBoundingClientRect();
  }
  // notesgraph-database-block-row-cell
