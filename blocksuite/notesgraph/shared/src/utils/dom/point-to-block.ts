...
import { NoteBlockModel, RootBlockModel } from '@blocksuite/notesgraph-model';
...
// margin-top: calc(var(--notesgraph-paragraph-space) + 24px);
...
function isImage({ tagName }: Element) {
  return tagName === 'NOTESGRAPH-IMAGE';
}

function isDatabase({ tagName }: Element) {
  return tagName === 'NOTESGRAPH-DATABASE';
}
 ...
 * In Chrome/Safari, `document.elementsFromPoint` does not include `notesgraph-image`.
 */
