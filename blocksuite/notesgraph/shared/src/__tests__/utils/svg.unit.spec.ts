...
import { containsUnsafeCss, sanitizeSvg } from '../../utils/svg.js';
...
describe('sanitizeSvg', () => {
  ...
  // `containsUnsafeCss` is the security boundary for keeping theme CSS. It's
  // tested directly because the surrounding `<style>`-preservation behavior
  // depends on a real browser's SVG-in-HTML parsing (see the mermaid preview
  // integration spec) which happy-dom does not reproduce.
  describe('containsUnsafeCss', () => {
    test('keeps same-document fragment url() refs (mermaid markers/theme)', () => {
      expect(containsUnsafeCss('.e { marker-end: url(#arrow); }')).toBe(false);
      expect(containsUnsafeCss('fill: url(#grad)')).toBe(false);
      expect(containsUnsafeCss(`fill: url("#grad")`)).toBe(false);
      expect(containsUnsafeCss('fill: url( #grad )')).toBe(false);
      expect(containsUnsafeCss('.node rect { fill: #1f2020; stroke: #ccc; }')).toBe(
        false
      );
    });

    test('flags external, protocol, and data url() refs', () => {
      expect(
        containsUnsafeCss('background: url(https://evil.example.com/x.png)')
      ).toBe(true);
      expect(containsUnsafeCss('fill: url(//evil.example.com/x)')).toBe(true);
      expect(containsUnsafeCss('fill: url(pattern.svg#x)')).toBe(true);
      expect(containsUnsafeCss('fill: url(data:image/png;base64,AAAA)')).toBe(
        true
      );
    });

    test('flags @import, javascript:, expression(), and -moz-binding', () => {
      expect(containsUnsafeCss('@import "https://x/y.css"')).toBe(true);
      expect(containsUnsafeCss('background: url(#a); x: javascript:alert(1)')).toBe(
        true
      );
      expect(containsUnsafeCss('width: expression(alert(1))')).toBe(true);
      expect(containsUnsafeCss('-moz-binding: url(#a)')).toBe(true);
    });

    test('a block mixing a safe and an external url() is unsafe', () => {
      expect(
        containsUnsafeCss(
          '.a { marker-end: url(#arrow); } .b { background: url(https://evil/x.png); }'
        )
      ).toBe(true);
    });
  });
