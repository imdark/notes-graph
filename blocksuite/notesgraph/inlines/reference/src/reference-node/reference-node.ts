...
export class NotesGraphReference extends WithDisposable(ShadowlessElement) {
  static override styles = css`
    .notesgraph-reference {
      white-space: normal;
    ...
    .notesgraph-reference:hover {
      background: var(--notesgraph-hover-color);
    }

    .notesgraph-reference[data-selected='true'] {
      background: var(--notesgraph-hover-color);
    }

    .notesgraph-reference-title {
      margin-left: 4px;
      border-bottom: 0.5px solid var(--notesgraph-divider-color);
      transition: border 0.2s ease-out;
    }
    .notesgraph-reference-title:hover {
      border-bottom: 0.5px solid var(--notesgraph-icon-color);
    }
  `;
  ...
  override render() {
    ...
    const style = notesgraphTextStyles(
      attributes,
      isDeleted
        ? {
            color: 'var(--notesgraph-text-disable-color)',
            textDecoration: 'line-through',
            fill: 'var(--notesgraph-text-disable-color)',
          }
        : {}
    );
    ...
    const content = this.customContent
      ...
      : html`${icon}<span
            ...
            class="notesgraph-reference-title"
            >${title}</span
          >`;
