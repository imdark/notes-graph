...
import type { NotesGraphReference } from './reference-node';

export interface ReferenceNodeConfig {
  customContent?: (reference: NotesGraphReference) => TemplateResult;
  interactable?: boolean;
  hidePopup?: boolean;
}

export const ReferenceNodeConfigExtension =
  ConfigExtensionFactory<ReferenceNodeConfig>('NotesGraphReferenceNodeConfig');

export class ReferenceNodeConfigProvider {
  private _customContent:
    | ((reference: NotesGraphReference) => TemplateResult)
    | undefined = undefined;
