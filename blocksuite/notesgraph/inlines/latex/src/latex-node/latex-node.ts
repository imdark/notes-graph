...
export class NotesGraphLatexNode extends SignalWatcher(
  WithDisposable(ShadowlessElement)
) {
  static override styles = css`
    notesgraph-latex-node {
      display: inline-block;
    }

    notesgraph-latex-node .notesgraph-latex {
      white-space: nowrap;
    ...
    notesgraph-latex-node .notesgraph-latex:hover {
      background: ${unsafeCSSVar('hoverColor')};
    }
    notesgraph-latex-node .notesgraph-latex[data-selected='true'] {
      background: ${unsafeCSSVar('hoverColor')};
    }

    notesgraph-latex-node .error-placeholder {
      display: flex;
      padding: 2px 4px;
    ...
    notesgraph-latex-node .placeholder {
      display: flex;
      padding: 2px 4px;
      justify-content: center;
      align-items: flex-start;
  ...
  override render() {
    return html`<span class="notesgraph-latex" data-selected=${this.selected}
  ...
  @property({ attribute: false })
  accessor editor!: InlineEditor<NotesGraphTextAttributes>;
