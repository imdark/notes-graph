...
import type { EditorIconButton } from '@blocksuite/notesgraph-components/toolbar';
...
export class LinkPopup extends WithDisposable(ShadowlessElement) {
  ...
  private readonly _createTemplate = () => {
    ...
    return html`
      <div class="notesgraph-link-popover create">
        <input
          id="link-input"
          class="notesgraph-link-popover-input"
          type="text"
          spellcheck="false"
  ...
  private readonly _editTemplate = () => {
    ...
    return html`
      <div class="notesgraph-link-edit-popover">
        <div class="notesgraph-edit-area text">
          <input
            class="notesgraph-edit-input"
            id="text-input"
            type="text"
            placeholder="Enter text"
            @input=${this._updateConfirmBtn}
          />
          <label class="notesgraph-edit-label" for="text-input">Text</label>
        </div>
        <div class="notesgraph-edit-area link">
          <input
            id="link-input"
            class="notesgraph-edit-input"
            type="text"
            ...
            @input=${this._updateConfirmBtn}
          />
          <label class="notesgraph-edit-label" for="link-input">Link</label>
        </div>
        ${this._confirmBtnTemplate()}
      </div>
    `;
  ...
  private _confirmBtnTemplate() {
    return html`
      <editor-icon-button
        class="notesgraph-confirm-button"
        .iconSize="${'24px'}"
        .disabled=${true}
  ...
  @query('.notesgraph-confirm-button')
  accessor confirmButton: EditorIconButton | null = null;

  @property({ attribute: false })
  accessor inlineEditor!: NotesGraphInlineEditor;
