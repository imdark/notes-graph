...
import { NotesGraphLink } from '../notesgraph-link';
  ...
  actions: [
    {
      ...
      content(cx) {
        ...
        if (!(target instanceof NotesGraphLink)) return null;
    ...
    },
    {
      ...
      actions: [
        ...
        },
        {
          ...
          when(ctx) {
            const target = ctx.message$.peek()?.element;
            if (!(target instanceof NotesGraphLink)) return false;
      ...
      when(ctx) {
        ...
        const bookmarkSchema = schema.flavourSchemaMap.get(
          'notesgraph:bookmark'
        );
