...
import { normalizeUrl } from '@blocksuite/notesgraph-shared/utils';
...
export class NotesGraphLink extends WithDisposable(ShadowlessElement) {
  static override styles = css`
    notesgraph-link a:hover [data-v-text='true'] {
  ...
  // see https://github.com/notesgraph/notesgraph/issues/1540
  ...
  override render() {
    ...
    if (this.delta.attributes && this.delta.attributes?.code) {
      const codeStyle = notesgraphTextStyles(this.delta.attributes);
    ...
    const style = this.delta.attributes
      ? notesgraphTextStyles(this.delta.attributes, linkStyle)
      : {};
