import { clearMarksOnDiscontinuousInput } from '@blocksuite/notesgraph-rich-text';
...
export const formatTextCommand: Command<{
  ...
  styles: NotesGraphTextStyleAttributes;
  mode?: 'replace' | 'merge';
}> = (ctx, next) => {
              ...
              Object.entries(styles).map(([key, value]) => {
                if (typeof value === 'boolean') {
                  return [
                    ...
                    (inlineEditor.marks &&
                      inlineEditor.marks[
                        key as keyof NotesGraphTextAttributes
                      ]) ||
                    (delta &&
                      delta.attributes &&
                      delta.attributes[key as keyof NotesGraphTextAttributes])
                      ? null
                      : value,
                  ];
                }
