...
function getCombinedFormatFromInlineEditors(
  inlineEditors: [NotesGraphInlineEditor, InlineRange | null][]
): NotesGraphTextAttributes {
  const formatArr: NotesGraphTextAttributes[] = [];
  ...
  return formatArr.reduce((acc, cur) => {
    const newFormat: NotesGraphTextAttributes = {};
    for (const key in acc) {
      const typedKey = key as keyof NotesGraphTextAttributes;
...
function getSelectedInlineEditors(
  ...
  filter: (
    inlineRoot: InlineRootElement<NotesGraphTextAttributes>
  ) => InlineEditor<NotesGraphTextAttributes> | []
) {
...
function handleCurrentSelection(
  ...
  handler: (
    ...
    inlineEditors: InlineEditor<NotesGraphTextAttributes>[]
  ) => { textAttributes: NotesGraphTextAttributes } | boolean | void
): Chain<InitCommandCtx & { textAttributes: NotesGraphTextAttributes }> {
  return chain.try(chain => [
    ...
    chain.pipe((ctx, next) => {
      const selectedInlineEditors = Array.from<InlineRootElement>(
        ...
        .map((el): NotesGraphInlineEditor => el.inlineEditor);
