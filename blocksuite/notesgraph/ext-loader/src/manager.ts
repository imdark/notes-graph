...
export class ExtensionManager<Scope extends string> {
  ...
  /**
   * Adds a provider after construction (e.g. one contributed by a plugin at
   * runtime). Idempotent. The next {@link get} call rebuilds from the provider
   * set, so the provider's extensions are included from then on.
   *
   * @param provider - The provider class to add
   */
  addProvider(provider: typeof BaseExtensionProvider<Scope>) {
    this._providers.add(provider);
  }

  /**
   * Removes a previously-added provider and drops its cached instance and
   * options, so its extensions disappear on the next {@link get} call.
   *
   * @param provider - The provider class to remove
   */
  removeProvider(provider: typeof BaseExtensionProvider<Scope>) {
