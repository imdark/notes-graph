...
const styles = css`
  ${unsafeCSS(dataViewCommonStyle('notesgraph-data-view-record-detail'))}
  notesgraph-data-view-record-detail {
    position: relative;
    display: flex;
    flex: 1;
...
export class RecordDetail extends SignalWatcher(
  ...
  override connectedCallback() {
    ...
    this.dataset.widgetId = 'notesgraph-detail-widget';
  }
  ...
  override render() {
    ...
    return html`
      ...
      <div style="flex:1;overflow-y: auto;overflow-x: hidden">
        <div
          style="width: 100%;max-width: var(--notesgraph-editor-width);display: flex;flex-direction: column;margin: 0 auto;box-sizing: border-box;"
        >
          ...
          ${repeat(
            ...
            property => {
              return keyed(
                this.rowId,
                html` <notesgraph-data-view-record-field
                  .view="${this.view}"
                  .column="${property}"
                  .rowId="${this.rowId}"
                  data-column-id="${property.id}"
                ></notesgraph-data-view-record-field>`
              );
            }
          )}
          ${!this.readonly
...
declare global {
  interface HTMLElementTagNameMap {
    'notesgraph-data-view-record-detail': RecordDetail;
  }
}
export const createRecordDetail = (ops: {
  ...
  return html` <notesgraph-data-view-record-detail
    .view=${ops.view}
    .rowId=${ops.rowId}
