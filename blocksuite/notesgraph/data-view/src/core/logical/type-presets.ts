...
export const SelectTagSchema = Zod.object({
  ...
  value: Zod.string(),
  /**
   * Kanban lane semantics when this option is a board lane: 'start'
   * (initial/todo), 'progress' (actively worked), 'blocked', or 'done'
   * (terminal). Optional — absent means inferred from the option's name.
   */
  role: Zod.enum(['start', 'progress', 'blocked', 'done']).optional(),
});
