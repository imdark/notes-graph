...
export interface DataSource {
  ...
  /**
   * True while `rows$` is still being populated, so a view can tell "nothing
   * matched" apart from "not finished looking yet" — without it an
   * asynchronous source (a query board resolving cross-doc hits) renders its
   * empty state first and only then pops the rows in. Sources whose rows are
   * synchronous leave this false.
   */
  rowsLoading$: ReadonlySignal<boolean>;
...
export abstract class DataSourceBase implements DataSource {
  ...
  // Synchronous by default; asynchronous sources override it.
  rowsLoading$: ReadonlySignal<boolean> = computed(() => false);
