...
export const inputConfig = (property: Property) => {
  return menu.input({
    prefix: html`
      <div class="notesgraph-database-column-type-menu-icon">
  ...
  return menu.group({
    items: [
      menu.subMenu({
        ...
        postfix: html` <div
          class="notesgraph-database-column-type-icon"
          style="color: var(--notesgraph-text-secondary-color);gap:4px;font-size: 14px;"
        >
          ${renderUniLit(property.icon)}
