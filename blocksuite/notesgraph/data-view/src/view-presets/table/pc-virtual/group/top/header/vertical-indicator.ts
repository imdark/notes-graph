...
export const getTableGroupRect = (ele: HTMLElement) => {
  const group = ele.closest('notesgraph-data-view-virtual-table-group');
  if (!group) {
    return;
  }
  ...
  const top =
    group
      .querySelector('.notesgraph-database-column-header')
      ?.getBoundingClientRect().top ?? groupRect.top;
  const bottom =
    group
      .querySelector('.notesgraph-database-block-rows')
