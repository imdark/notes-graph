...
export class TableSelectionController implements ReactiveController {
  ...
  focusToCell(position: 'left' | 'right' | 'up' | 'down') {
    ...
    const rows = Array.from(
      row
        ?.closest('.notesgraph-database-table-container')
    ...
    const container =
      groupKey != null
        ? this.tableContainer?.querySelector(
            `notesgraph-data-view-table-group[data-group-key="${groupKey}"]`
          )
        : this.tableContainer;
  ...
  startDrag(
    ...
    const groupKey = cell.closest<TableGroup>(
...
export class SelectionElement extends WithDisposable(ShadowlessElement) {
  ...
  updateFocusSelectionStyle(
    ...
    div.style.borderColor = 'var(--notesgraph-primary-color)';
