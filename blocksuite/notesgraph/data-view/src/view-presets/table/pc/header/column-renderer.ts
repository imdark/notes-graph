...
export class DataViewColumnPreview extends SignalWatcher(
  ...
  static override styles = css`
    notesgraph-data-view-column-preview {
      pointer-events: none;
      display: block;
      position: fixed;
  ...
  private renderGroup(rows: Row[]) {
    ...
    return html`
      <div
        style="background-color: var(--notesgraph-background-primary-color);border-top: 1px solid ${unsafeCSS(
...
declare global {
  interface HTMLElementTagNameMap {
    'notesgraph-data-view-column-preview': DataViewColumnPreview;
  }
}
