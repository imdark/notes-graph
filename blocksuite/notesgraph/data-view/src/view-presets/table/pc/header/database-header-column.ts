...
export class DatabaseHeaderColumn extends SignalWatcher(
  ...
  static override styles = css`
    ...
    .notesgraph-database-header-column-grabbing * {
      cursor: grabbing;
    }
  `;
              ...
              items: [
                ...
                ...numberFormats.map(format => {
                  ...
                  return menu.action({
                    ...
                    prefix: html`<span
                      style="font-size: var(--notesgraph-font-base); scale: 1.2;"
                      >${format.symbol}</span
                    >`,
                    name: format.label,
                    select: () => {
  ...
  override render() {
    ...
    const classes = classMap({
      'notesgraph-database-column-move': true,
      [this.grabStatus]: true,
    });
    return html`
      <div
        style=${style}
        class="notesgraph-database-column-content"
      ...
      >
        ...
        <div class="notesgraph-database-column-text ${column.type$.value}">
          <div
            class="notesgraph-database-column-type-icon dv-hover"
            @click="${this._clickTypeIcon}"
          >
            <uni-lit .uni="${column.icon}"></uni-lit>
          </div>
          <div class="notesgraph-database-column-text-content">
            <div class="notesgraph-database-column-text-input">
              ${column.name$.value}
            </div>
          </div>
        </div>
...
function numberFormatConfig(column: Property): MenuConfig {
  return () =>
    html` <notesgraph-database-number-format-bar
      .column="${column}"
    ></notesgraph-database-number-format-bar>`;
}

declare global {
  interface HTMLElementTagNameMap {
    'notesgraph-database-header-column': DatabaseHeaderColumn;
  }
}
