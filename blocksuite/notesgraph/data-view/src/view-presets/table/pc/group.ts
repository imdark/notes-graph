...
const styles = css`
  notesgraph-data-view-table-group:hover .group-header-op {
    visibility: visible;
    opacity: 1;
  }

  notesgraph-data-view-table-group {
    margin-top: 4px;
    padding-top: 4px;
  ...
  .notesgraph-data-view-table-group:hover svg {
...
export class TableGroup extends SignalWatcher(
  ...
  private renderRows(rows: Row[]) {
    return html`
      ...
      ></notesgraph-database-column-header>
      <div class="notesgraph-database-block-rows">
        ${repeat(
          rows,
          row => row.rowId,
          (row, idx) => {
        ...
        : html` <div
          ...
          >
            <div
              ...
              data-test-id="notesgraph-database-add-row-button"
              role="button"
            >
  ...
  @query('.notesgraph-database-block-rows')
...
declare global {
  interface HTMLElementTagNameMap {
    'notesgraph-data-view-table-group': TableGroup;
  }
}
