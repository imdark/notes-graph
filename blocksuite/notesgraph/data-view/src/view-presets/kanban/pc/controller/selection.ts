...
export class KanbanSelectionController implements ReactiveController {
  ...
  // Anchor for shift range-select: the last card clicked WITHOUT shift (a plain
  // click or a cmd/ctrl toggle-click). A shift-click then selects the whole
  // contiguous range from this anchor to the clicked card within the column.
  private _rangeAnchor?: KanbanCardSelectionCard;

  setRangeAnchor(cardId: string, groupKey: string) {
    this._rangeAnchor = { cardId, groupKey };
  }

  /**
   * Replace the selection with exactly these cards (e.g. "select all in a
   * column"). Clears the selection when the list is empty. Anchors the range at
   * the first card so a following shift-click extends from there.
   */
  selectCards(cards: KanbanCardSelectionCard[]) {
    if (!atLeastOne(cards)) {
      this.selection = undefined;
      return;
    }
    this._rangeAnchor = { ...cards[0] };
    this.selection = { selectionType: 'card', cards };
  }

  /**
   * Select every card in the currently-focused column (the group of the focused
   * cell, or of the last selected card). Returns false when there's nothing
   * focused to derive a column from. Backs the "select all in a column" ask
   * (cmd/ctrl+A and the group menu).
   */
  selectAllInCurrentGroup(): boolean {
    const selection = this.selection;
    let groupKey: string | undefined;
    if (selection?.selectionType === 'cell') {
      groupKey = selection.groupKey;
    } else if (selection?.selectionType === 'card') {
      groupKey = selection.cards[selection.cards.length - 1]?.groupKey;
    }
    if (!groupKey) return false;
    const group = this.logic.groups$.value?.find(g => g.key === groupKey);
    if (!group) return false;
    this.selectCards(
      group.rows.map(row => ({ cardId: row.rowId, groupKey }))
    );
    return true;
  }

  /**
   * Cmd/Ctrl-click: toggle a single card in/out of the multi-selection set
   * (discontiguous select). Also moves the range anchor to the clicked card.
   */
  toggleClickCard = (event: MouseEvent) => {
    ...
    const closestGroupKey = target.closest('notesgraph-data-view-kanban-group')
      ?.group.key;
    ...
    this._rangeAnchor = { cardId: closestCardId, groupKey: closestGroupKey };
  ...
  /**
   * Shift-click: select the contiguous range of cards between the anchor and
   * the clicked card, within the anchor's column. With no anchor in that column
   * it starts a fresh single-card selection anchored at the click. The anchor
   * does not move, so successive shift-clicks re-range from the same origin.
   */
  rangeClickCard = (event: MouseEvent) => {
    event.preventDefault();

    const target = event.target as HTMLElement;
    const groupEl = target.closest('notesgraph-data-view-kanban-group');
    const cardId = target.closest(
      'notesgraph-data-view-kanban-card'
    )?.cardId;
    const groupKey = groupEl?.group.key;
    if (!cardId || !groupKey || !groupEl) return;

    const anchor = this._rangeAnchor;
    if (!anchor || anchor.groupKey !== groupKey) {
      this._rangeAnchor = { cardId, groupKey };
      this.selection = {
        selectionType: 'card',
        cards: [{ cardId, groupKey }],
      };
      return;
    }

    const ids = Array.from(
      groupEl.querySelectorAll('notesgraph-data-view-kanban-card')
    ).map(card => card.cardId);
    const anchorIndex = ids.indexOf(anchor.cardId);
    const targetIndex = ids.indexOf(cardId);
    ...
    const range = ids
      .slice(lo, hi + 1)
      .map(id => ({ cardId: id, groupKey }));
    this.selection = atLeastOne(range)
      ? { selectionType: 'card', cards: range }
      : undefined;
  };
  ...
  focusIn() {
    ...
    if (selection.selectionType === 'card') {
      ...
      const cell = card?.querySelector('notesgraph-data-view-kanban-cell');
      if (card && cell) {
        this.selection = {
  ...
  focusNext(position: 'up' | 'down' | 'left' | 'right') {
    ...
    if (selection.selectionType === 'cell' && !selection.isEditing) {
      ...
      const group = this.host?.querySelector(
        `notesgraph-data-view-kanban-group[data-key="${selection.cards[0].groupKey}"]`
      );
  ...
  getNextFocusCard(
    ...
    const groups = Array.from(
      this.host?.querySelectorAll('notesgraph-data-view-kanban-group') ?? []
    );
  ...
  getNextFocusCell(
    ...
    const kanbanCells = getCardCellsBySelection(this.host, selection);
    const group = this.host.querySelector(
      `notesgraph-data-view-kanban-group[data-key="${selection.groupKey}"]`
    );
...
function getNextGroupFocusElement(
  ...
  const nextCards = Array.from(
    nextGroup.querySelectorAll('notesgraph-data-view-kanban-card')
  );
  ...
  const cells = Array.from(
    nextCard.querySelectorAll('notesgraph-data-view-kanban-cell')
  );
  const cellPos = cells
    .map((card, index) => {
      const targetRect = card.getBoundingClientRect();
...
function getCardCellsBySelection(
  ...
  return Array.from(
    card?.querySelectorAll('notesgraph-data-view-kanban-cell') ?? []
  );
}
...
function getSelectedCard(
  ...
  return group.querySelector<KanbanCard>(
    `notesgraph-data-view-kanban-card[data-card-id="${selection.cardId}"]`
  );
}
  ...
  const groups = groupKeys
    .map(key =>
      viewElement.querySelector(
        `notesgraph-data-view-kanban-group[data-key="${key}"]`
      )
    )
    ...
    .flatMap(group =>
      cardIds.map(id =>
        group.querySelector<KanbanCard>(
          `notesgraph-data-view-kanban-card[data-card-id="${id}"]`
        )
      )
    )
...
function getFocusCell(viewElement: Element, selection: KanbanCellSelection) {
  ...
  return card?.querySelector<KanbanCell>(
    `notesgraph-data-view-kanban-cell[data-column-id="${selection.columnId}"]`
  );
}
