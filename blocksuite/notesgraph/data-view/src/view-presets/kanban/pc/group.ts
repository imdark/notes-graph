...
const styles = css`
  notesgraph-data-view-kanban-group {
    width: 260px;
    flex-shrink: 0;
    border-radius: 8px;
    display: flex;
  ...
  notesgraph-data-view-kanban-group:hover .group-header-op {
    visibility: visible;
    opacity: 1;
  }
  ...
  notesgraph-data-view-kanban-group:hover .add-card {
    visibility: visible;
    opacity: 1;
  }

  notesgraph-data-view-kanban-group .add-card:hover {
...
export class KanbanGroup extends SignalWatcher(
  ...
  private readonly clickGroupOptions = (e: MouseEvent) => {
    ...
    popFilterableSimpleMenu(popupTargetFromElement(ele), [
      menu.action({
        name: 'Select all',
        hide: () => this.group.rows.length === 0,
        select: () => {
          this.kanbanViewLogic.selectionController.selectCards(
...
declare global {
  interface HTMLElementTagNameMap {
    'notesgraph-data-view-kanban-group': KanbanGroup;
  }
}
