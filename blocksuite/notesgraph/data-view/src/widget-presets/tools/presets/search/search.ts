...
const styles = css`
  .notesgraph-database-search-container {
    position: relative;
    display: flex;
    align-items: center;
    gap: 8px;
  ...
  .notesgraph-database-search-input-icon {
    position: absolute;
    left: 0;
    font-size: 20px;
    display: flex;
  ...
  .notesgraph-database-search-input-icon:hover {
    background: var(--notesgraph-hover-color);
  }

  .search-container-expand .notesgraph-database-search-input-icon {
    left: 4px;
    pointer-events: none;
  }

  .notesgraph-database-search-input {
    flex: 1;
    width: 100%;
  ...
  .notesgraph-database-search-input::placeholder {
...
> {
  ...
  override render() {
    const searchToolClassMap = classMap({
      'notesgraph-database-search-container': true,
      'search-container-expand': this.showSearch,
      active: this.showSearch,
    });
    return html`
      <label class="${searchToolClassMap}" @click="${this._clickSearch}">
        <div class="notesgraph-database-search-input-icon">${SearchIcon()}</div>
        <input
          placeholder="Search..."
          class="notesgraph-database-search-input"
  ...
  @query('.notesgraph-database-search-input')
