...
const styles = css`
  .notesgraph-database-toolbar-item.more-action {
    padding: 2px;
    border-radius: 4px;
    display: flex;
    align-items: center;
  ...
  .notesgraph-database-toolbar-item.more-action:hover {
...
export class DataViewHeaderToolsViewOptions extends WidgetBase {
  ...
  override render() {
    ...
    return html` <div
      class="notesgraph-database-toolbar-item more-action"
...
export const popViewOptions = (
  ...
  const isDesktopMenu = () =>
    menuHandler.menu.menuElement.tagName.toLowerCase() === 'notesgraph-menu';
    ...
    get postfix() {
      ...
      return () =>
        html`<div
          ...
          style="padding:4px 8px;font-size:12px;line-height:20px;font-weight:500;border-radius:4px;cursor:pointer;color:var(--notesgraph-primary-color);"
        ...
        items: [
          menuObj => {
            const viewTypeItems = menuObj.renderItems(
              view.manager.viewMetas.map<MenuConfig>(meta => {
                return menuObj => {
                  ...
                  return html`<notesgraph-menu-button
                    style="${containerStyle}"
                    .data="${buttonData}"
                    .menu="${menuObj}"
                  ></notesgraph-menu-button>`;
                };
              })
            );
            ...
            return html`
              ...
              >
                <div
                  style="display:flex;align-items:center;color:var(--notesgraph-icon-color);"
                >
                  ${LayoutIcon()}
                </div>
                <div
                  style="font-size:14px;line-height:22px;color:var(--notesgraph-text-secondary-color);"
                >
                  Layout
                </div>
              </div>
  ...
  if (isDesktopMenu()) {
    ...
    requestAnimationFrame(() => {
      const bodyEl = menuHandler.menu.menuElement.querySelector<HTMLElement>(
        '.notesgraph-menu-body'
      );
      if (bodyEl) {
