...
const playCheckAnimation = async (
  ...
  sparkingEl.classList.add('notesgraph-check-animation');
  if (size < 20) {
...
export class CheckboxCell extends BaseCellRenderer<boolean> {
  static override styles = css`
    notesgraph-database-checkbox-cell {
      display: block;
      width: 100%;
      cursor: pointer;
    }

    .notesgraph-database-checkbox-container {
      height: 100%;
    }

    .notesgraph-database-checkbox {
      display: flex;
      align-items: center;
  ...
  override render() {
    ...
    const icon = checked
      ...
      : CheckBoxUnIcon();
    return html` <div class="notesgraph-database-checkbox-container">
      <div
        class="notesgraph-database-checkbox checkbox ${checked
          ? 'checked'
          : ''}"
      >
        ${icon}
  ...
  @query('.notesgraph-database-checkbox')
