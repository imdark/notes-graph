...
import { changeNoteDisplayMode } from '@blocksuite/notesgraph-block-note';
...
import { focusTitle, matchModels } from '@blocksuite/notesgraph-shared/utils';
import {
  BlockSelection,
  ShadowlessElement,
  SurfaceSelection,
...
export const NOTESGRAPH_OUTLINE_PANEL_BODY = 'notesgraph-outline-panel-body';
...
export class OutlinePanelBody extends SignalWatcher(
  ...
  private _renderNoteCards(notes: NoteBlockModel[]) {
    return repeat(
      ...
      ({ id }) => id,
      (note, index) =>
        html`<notesgraph-outline-note-card
...
declare global {
  interface HTMLElementTagNameMap {
    [NOTESGRAPH_OUTLINE_PANEL_BODY]: OutlinePanelBody;
  }
}
