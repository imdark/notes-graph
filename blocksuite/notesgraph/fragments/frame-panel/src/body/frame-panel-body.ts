...
import { EdgelessFrameManager } from '@blocksuite/notesgraph-block-frame';
...
const styles = css`
  ...
  .no-frame-placeholder {
    ...
    width: 230px;
    height: 48px;
    color: var(--notesgraph-text-secondary-color, #8e8d91);
    text-align: center;
  ...
  .insert-indicator {
    height: 2px;
    border-radius: 1px;
    background-color: var(--notesgraph-blue-600);
    position: absolute;
    contain: layout size;
    width: 284px;
    left: 0;
...
export const NOTESGRAPH_FRAME_PANEL_BODY = 'notesgraph-frame-panel-body';
  ...
  private _setDocDisposables(doc: Store) {
    ...
    this._docDisposables.add(
      doc.slots.blockUpdated.subscribe(({ type, flavour }) => {
        if (flavour === 'notesgraph:frame' && type !== 'update') {
...
declare global {
  interface HTMLElementTagNameMap {
    [NOTESGRAPH_FRAME_PANEL_BODY]: FramePanelBody;
  }
}
