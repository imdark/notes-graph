...
import { on, once } from '@blocksuite/notesgraph-shared/utils';
...
const styles = css`
  ...
  .frame-card-container.dragging {
    ...
    top: 0;
    left: 0;
    z-index: calc(var(--notesgraph-z-index-popover, 0) + 3);
  }
  ...
  .frame-card-container.dragging .dragging-card-number {
    ...
    height: 24px;
    border-radius: 50%;
    background: var(--notesgraph-black);
...
export const NOTESGRAPH_FRAME_CARD = 'notesgraph-frame-card';
...
export class FrameCard extends WithDisposable(ShadowlessElement) {
  ...
  override render() {
    ...
    >
      ${this.status === 'dragging'
        ? nothing
        : html`<notesgraph-frame-card-title
            .cardIndex=${this.cardIndex}
            .frame=${this.frame}
          ></notesgraph-frame-card-title>`}
      <div
...
declare global {
  interface HTMLElementTagNameMap {
    [NOTESGRAPH_FRAME_CARD]: FrameCard;
  }
}
