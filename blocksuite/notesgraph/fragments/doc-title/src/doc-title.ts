...
export class DocTitle extends WithDisposable(ShadowlessElement) {
  static override styles = css`
    ...
    .doc-title-container {
      ...
      padding-left: var(
        --notesgraph-editor-side-padding,
  ...
  private _getOrCreateFirstPageVisibleNote() {
    ...
    const noteId = this.doc.addBlock('notesgraph:note', {}, this._rootModel, 0);
  ...
  private get _viewport() {
    return (
      this.closest<HTMLElement>('.notesgraph-page-viewport') ??
      this.closest<HTMLElement>('.notesgraph-edgeless-viewport')
    );
  }
