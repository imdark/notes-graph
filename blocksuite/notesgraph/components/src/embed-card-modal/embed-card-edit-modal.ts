...
import { fontSMStyle, fontXSStyle } from '@blocksuite/notesgraph-shared/styles';
...
export class EmbedCardEditModal extends SignalWatcher(
  ...
  static override styles = css`
    ...
    .embed-card-modal-wrapper {
      ...
      box-shadow: var(--notesgraph-overlay-shadow);
  ...
  private readonly _onSave = () => {
    ...
    // External links (bookmark / embed-iframe) can also edit the URL itself;
    // update it and refresh the preview when it changed.
    if (!this.isInternalEmbedModel) {
      const url = this.url$.value.trim();
      const currentUrl = (this.model.props as { url?: string }).url;
      if (url && url !== currentUrl) {
        this.model.store.updateBlock(this.model, { url });
        (blockComponent as { refreshData?: () => void }).refreshData?.();
      }
    }
  ...
  private readonly _updateUrl = (e: InputEvent) => {
    const target = e.target as HTMLInputElement;
    this.url$.value = target.value;
  };
  ...
  private _updateInfo() {
    ...
    this.url$.value = (this.model.props as { url?: string }).url ?? '';
  }
