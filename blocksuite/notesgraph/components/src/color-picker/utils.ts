...
import type { Color, ColorScheme } from '@blocksuite/notesgraph-model';
...
export const parseStringToRgba = (value: string) => {
  ...
  // Compatible old format: `--notesgraph-palette-transparent`
 ...
 *
 * ```json
 * { 'fillColor': '--notesgraph-palette-shape-yellow' }
 * { 'fillColor': '#ffffff' }
...
export const calcCustomButtonStyle = (
  ...
  if (color.startsWith('--')) {
    if (!color.endsWith('transparent')) {
      b = 'var(--notesgraph-background-overlay-panel-color)';
      c = keepColor(
        rgbaToHex8(
