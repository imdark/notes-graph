  ...
  implements MenuComponentInterface
{
  static override styles = css`
    ...
    .notesgraph-menu-search-container {
      border-radius: 4px;
      display: flex;
      align-items: center;
      padding: 4px 10px;
  ...
  override render() {
    ...
    return html`
      ...
      <div class="notesgraph-menu-body">
  ...
  renderSearch() {
    ...
    return html` <div
      style=${searchStyle}
      class="notesgraph-menu-search-container"
    >
      <div
        style="font-size:20px;display:flex;align-items:center;color: var(--notesgraph-text-secondary-color)"
      >
        ${SearchIcon()}
      </div>
      <input
        autocomplete="off"
        class="notesgraph-menu-search"
    ...
    return html`
      ...
      >
        ${title.onBack
          ...
          : nothing}
        <div
          style="flex:1;font-weight:500;font-size: 14px;line-height: 22px;color: var(--notesgraph-text-primary-color)"
        >
          ${title.text}
        </div>
        ${title.postfix?.()}
...
export const createModal = (container: HTMLElement = document.body) => {
  ...
  div.style.fontFamily = 'var(--notesgraph-font-family)';
