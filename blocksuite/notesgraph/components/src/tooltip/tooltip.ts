...
const styles = css`
  .notesgraph-tooltip {
    ...
    border-radius: 4px;
    padding: 6px 12px;
    color: var(--notesgraph-v2-tooltips-foreground, var(--notesgraph-white));
    background: var(
      --notesgraph-v2-tooltips-background,
      var(--notesgraph-tooltip)
    );
...
const TOOLTIP_ARROW_COLOR =
  'var(--notesgraph-v2-tooltips-background, var(--notesgraph-tooltip))';
 ...
 * // Simple usage
 * html`
 * <notesgraph-tooltip>Content</notesgraph-tooltip>
 * `
 * // With placement
 * html`
 * <notesgraph-tooltip tip-position="top">
 *   Content
 * </notesgraph-tooltip>
 * `
 *
 ...
 *   .autoFlip=${true}
 *   .arrow=${true}
 *   .tooltipStyle=${css`:host { z-index: 0; --notesgraph-tooltip: #fff; }`}
...
export class Tooltip extends LitElement {
  ...
  @property({ attribute: false })
  accessor zIndex: number | string = 'var(--notesgraph-z-index-popover)';
}

declare global {
  interface HTMLElementTagNameMap {
    'notesgraph-tooltip': Tooltip;
  }
}
