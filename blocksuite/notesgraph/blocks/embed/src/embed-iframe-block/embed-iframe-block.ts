...
import { SurfaceBlockModel } from '@blocksuite/notesgraph-block-surface';
import {
  CaptionedBlockComponent,
  SelectedStyle,
} from '@blocksuite/notesgraph-components/caption';
import { createLitPortal } from '@blocksuite/notesgraph-components/portal';
import type { EmbedIframeBlockModel } from '@blocksuite/notesgraph-model';
import {
...
export class EmbedIframeBlockComponent extends CaptionedBlockComponent<EmbedIframeBlockModel> {
  ...
  // Show the overlay (which captures pointer events) only while dragging or
  // resizing — not merely when unselected — so the embedded page stays
  // scrollable/interactive without having to select the block first.
  readonly showOverlay$ = computed(
    () =>
      this.isSuccess$.value &&
      (this.isDraggingOnHost$.value || this.isResizing$.value)
  );
  ...
  private readonly _renderIframe = () => {
    ...
    const title =
      this.model.props.title?.trim() || sourceHost || this.model.props.url;
    let faviconUrl: string | undefined;
    try {
      faviconUrl = new URL(this.model.props.url).origin + '/favicon.ico';
    } catch {
      faviconUrl = undefined;
    }
    return html`<div class="notesgraph-embed-iframe-frame">
        ${title
          ? html`<div class="notesgraph-embed-iframe-title" title=${title}>
              ${faviconUrl
                ? html`<img
                    class="notesgraph-embed-iframe-favicon"
                    src=${faviconUrl}
                    @error=${(e: Event) => {
                  ...
                  />`
                : nothing}
              <span class="notesgraph-embed-iframe-title-text">${title}</span>
            </div>`
          : nothing}
        <iframe
      ...
      ${sourceHost
        ? html`<div class="notesgraph-embed-iframe-source">${sourceHost}</div>`
        : nothing}`;
  };
  ...
  override renderBlock() {
    const containerClasses = classMap({
      'notesgraph-embed-iframe-block-container': true,
    ...
    const overlayClasses = classMap({
      'notesgraph-embed-iframe-block-overlay': true,
  ...
  @query('.notesgraph-embed-iframe-block-container')
