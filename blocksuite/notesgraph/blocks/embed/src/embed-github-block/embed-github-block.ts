import { LoadingIcon, OpenIcon } from '@blocksuite/notesgraph-components/icons';
import type {
  EmbedGithubModel,
  EmbedGithubStyles,
} from '@blocksuite/notesgraph-model';
import { ImageProxyService } from '@blocksuite/notesgraph-shared/adapters';
  ...
  override renderBlock() {
    ...
    return this.renderEmbed(
      () => html`
        <div
          class=${classMap({
            'notesgraph-embed-github-block': true,
            loading,
            [style]: true,
        ...
        >
          <div class="notesgraph-embed-github-content">
            <div class="notesgraph-embed-github-content-title">
              <div class="notesgraph-embed-github-content-title-icons">
                <div class="notesgraph-embed-github-content-title-site-icon">
                  ${titleIcon}
                </div>
                ...
                ${status && statusText
                  ? html`<div
                      class=${classMap({
                        'notesgraph-embed-github-content-title-status-icon': true,
                        [githubType]: true,
                        [status]: true,
              ...
              <div class="notesgraph-embed-github-content-title-text">
                ${titleText}
              </div>
            </div>

            <div class="notesgraph-embed-github-content-description">
              ${descriptionText}
            </div>

            ${githubType === 'issue' && assignees
              ? html`
                  <div class="notesgraph-embed-github-content-assignees">
                    <div
                      class="notesgraph-embed-github-content-assignees-text label"
                    >
                      Assignees
                    </div>

                    <div
                      class="notesgraph-embed-github-content-assignees-text users"
                    >
                      ${assignees.length === 0
                        ? html`<span
                            class="notesgraph-embed-github-content-assignees-text-users placeholder"
                            >No one</span
                          >`
                        : repeat(
                            assignees,
                            ...
                            (assignee, index) =>
                              html`<span
                                  class="notesgraph-embed-github-content-assignees-text-users user"
                                  @click=${() =>
            ...
            <div
              class="notesgraph-embed-github-content-url"
              @click=${this.open}
            >
              <span class="notesgraph-embed-github-content-repo"
                >${`${owner}/${repo} |`}</span
              >

              ${createdAt
                ? html`<span class="notesgraph-embed-github-content-date"
                    >${dateText} |</span
                  >`
                : nothing}
              <span>github.com</span>

              <div class="notesgraph-embed-github-content-url-icon">
                ${OpenIcon}
              </div>
            </div>
          </div>
          <div class="notesgraph-embed-github-banner">${bannerImage}</div>
        </div>
      `
    );
  }
