...
import { FileSizeLimitProvider } from '@blocksuite/notesgraph-shared/services';
import {
  readImageSize,
  transformModel,
  withTempBlobData,
...
export const AttachmentEmbedConfigIdentifier =
  createIdentifier<AttachmentEmbedConfig>(
    'NotesGraphAttachmentEmbedConfigIdentifier'
  );
...
>('NotesGraphAttachmentEmbedConfigMapIdentifier');

export const AttachmentEmbedProvider = createIdentifier<AttachmentEmbedService>(
  'NotesGraphAttachmentEmbedProvider'
);
    ...
    render: (_, blobUrl) => {
      ...
      return html`
        <iframe
          ...
          credentialless
        ></iframe>
        <div class="notesgraph-attachment-embed-event-mask"></div>
      `;
    },
  },
  {
...
async function turnIntoImageBlock(model: AttachmentBlockModel) {
  if (!model.store.schema.flavourSchemaMap.has('notesgraph:image')) {
  ...
  transformModel(model, 'notesgraph:image', imageProp);
}
