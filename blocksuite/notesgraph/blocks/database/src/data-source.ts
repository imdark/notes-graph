...
export class DatabaseBlockDataSource extends DataSourceBase {
  ...
  protected getModelById(rowId: string): BlockModel | undefined {
  ...
  cellValueGet(rowId: string, propertyId: string): unknown {
    ...
    if (type === 'formula') {
      return this.formulaValueGet(rowId, propertyId);
    }
  ...
  /** Guards formula chains against circular references, per row+column. */
  private readonly formulaStack = new Set<string>();

  /**
   * A referenced column's value coerced into the formula value space:
   * select ids become option labels, Text becomes string, dates stay
   * epoch-ms numbers — so expressions read columns the way users see them.
   */
  private formulaRefValue(rowId: string, propertyId: string): FormulaValue {
    const type = this.propertyTypeGet(propertyId);
    const raw = this.cellValueGet(rowId, propertyId);
    ...
    switch (type) {
      ...
      case 'multi-select': {
        const options = this.propertyDataGet(propertyId).options;
        if (!Array.isArray(raw) || !Array.isArray(options)) return null;
        return raw
      ...
      default: {
        ...
        // Text and other stringables (title, rich-text)
        return String(raw);
      }
    }
  }

  protected formulaValueGet(rowId: string, propertyId: string): unknown {
    const key = `${rowId}:${propertyId}`;
    if (this.formulaStack.has(key)) return '#ERR: circular reference';
    const expression = this.propertyDataGet(propertyId).expression;
    ...
    try {
      return evaluateFormula(expression, name => {
        ...
        const target = this.properties$.value.find(
          id => this.propertyNameGet(id).trim().toLowerCase() === wanted
        );
        if (target === undefined) {
          throw new Error(`unknown column '${name}'`);
        }
        return this.formulaRefValue(rowId, target);
      });
    } finally {
  ...
  rowAdd(insertPosition: InsertToPosition | number): string {
    ...
    return this.doc.addBlock('notesgraph:paragraph', {}, this._model.id, index);
  }
...
export const convertToDatabase = (host: EditorHost, viewType: string) => {
  ...
  // A database block can only live under a note (see its schema's `parent`
  // list). If firstModel is a nested list item, climb to the top-level
  // ancestor and insert there instead — otherwise addBlock throws
  // "Block cannot have parent: notesgraph:list". The selected models
  // themselves are moved into the database unchanged below, regardless of
  ...
  const id = host.store.addBlock(
    ...
    {},
    parentModel,
    parentModel.children.indexOf(topAncestor)
  );
  ...
  // The database can land at note level, away from a nested selection (or
  // outside a narrowed focus scope) — bring it into view so the conversion
