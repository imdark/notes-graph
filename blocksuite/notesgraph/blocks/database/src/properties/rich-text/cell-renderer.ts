...
function toggleStyle(
  inlineEditor: NotesGraphInlineEditor | null,
  attrs: NotesGraphTextAttributes
): void {
  ...
  let oldAttributes: NotesGraphTextAttributes = {};
...
export class RichTextCell extends BaseCellRenderer<Text, string> {
  ...
  get topContenteditableElement() {
    const databaseBlock = this.closest<DatabaseBlockComponent>(
      'notesgraph-database'
    );
  ...
  insertDelta = (delta: DeltaInsert<NotesGraphTextAttributes>) => {
    const inlineEditor = this.inlineEditor$.value;
...
declare global {
  interface HTMLElementTagNameMap {
    'notesgraph-database-rich-text-cell': RichTextCell;
  }
}
