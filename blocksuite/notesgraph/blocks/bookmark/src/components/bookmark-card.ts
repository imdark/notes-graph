...
import { getEmbedCardIcons } from '@blocksuite/notesgraph-block-embed';
import {
  LoadingIcon,
  WebIcon16,
...
import { getHostName } from '@blocksuite/notesgraph-shared/utils';
...
export class BookmarkCard extends SignalWatcher(
  ...
  override render() {
    ...
    return html`
      <div
        class="notesgraph-bookmark-card ${cardClassMap}"
        @click=${this.bookmark.handleClick}
        @dblclick=${this.bookmark.handleDoubleClick}
      >
        <div class="notesgraph-bookmark-content">
          <div class="notesgraph-bookmark-content-title">
            <div class="notesgraph-bookmark-content-title-icon">
              ${titleIcon}
            </div>
            <div class="notesgraph-bookmark-content-title-text">
              ${titleText}
            </div>
          </div>
          <div class="notesgraph-bookmark-content-description">
            ${descriptionText}
          </div>
          <div class="notesgraph-bookmark-content-url-wrapper">
            <div
              class="notesgraph-bookmark-content-url"
              @click=${this.bookmark.open}
            >
              <span>${getHostName(url)}</span>
              <div class="notesgraph-bookmark-content-url-icon">
                ${OpenInNewIcon({ width: '12', height: '12' })}
              </div>
            </div>
          </div>
        </div>
        <div class="notesgraph-bookmark-banner">${bannerImage}</div>
      </div>
    `;
  }
