import { insertEmbedIframeWithUrlCommand } from '@blocksuite/notesgraph-block-embed';
import {
  type InsertedLinkType,
  insertEmbedLinkedDocCommand,
  insertEmbedSyncedDocCommand,
  type LinkableFlavour,
} from '@blocksuite/notesgraph-block-embed-doc';
import {
  DocModeProvider,
  ...
  const insertedLinkType: Promise<InsertedLinkType> = quickSearchService
    ...
    .then(result => {
      ...
      if ('docId' in result) {
        ...
        let flavour: LinkableFlavour = 'notesgraph:embed-linked-doc';
