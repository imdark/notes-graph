...
import type { BlockCaptionEditor } from '@blocksuite/notesgraph-components/caption';
import { LoadingIcon } from '@blocksuite/notesgraph-components/icons';
import { Peekable } from '@blocksuite/notesgraph-components/peek';
import { ResourceController } from '@blocksuite/notesgraph-components/resource';
import {
  type ImageBlockModel,
  ImageBlockSchema,
} from '@blocksuite/notesgraph-model';
import { cssVarV2, unsafeCSSVarV2 } from '@blocksuite/notesgraph-shared/theme';
import { formatSize } from '@blocksuite/notesgraph-shared/utils';
...
export class ImageEdgelessBlockComponent extends GfxBlockComponent<ImageBlockModel> {
  ...
  static override styles = css`
    ...
    notesgraph-edgeless-image .loading {
      display: flex;
      align-items: center;
    ...
    notesgraph-edgeless-image .notesgraph-image-status {
      position: absolute;
      left: 18px;
      bottom: 18px;
    }
  ...
  override renderGfxBlock() {
    ...
    return html`
      <div class="notesgraph-image-container" style=${containerStyleMap}>
        ${when(
          blobUrl,
          () => html`
            ...
            ${when(
              ...
              () =>
                html`<notesgraph-resource-status
                  class="notesgraph-image-status"
                  .message=${description}
          ...
          `,
          () =>
            html`<notesgraph-image-fallback-card
              .state=${resovledState}
            ></notesgraph-image-fallback-card>`
        )}
        <notesgraph-block-selection .block=${this}></notesgraph-block-selection>
      </div>
...
declare global {
  interface HTMLElementTagNameMap {
    'notesgraph-edgeless-image': ImageEdgelessBlockComponent;
  }
}
