...
import { autoResizeElementsCommand } from '@blocksuite/notesgraph-block-surface';
...
const DEFAULT_ATTACHMENT_NAME = 'notesgraph-attachment';
...
export async function turnImageIntoCardView(
  ...
  if (!doc.schema.flavourSchemaMap.has('notesgraph:attachment')) {
  ...
  transformModel(model, 'notesgraph:attachment', attachmentProp);
}
...
// A storage failure (e.g. NotReadableError on a revoked Android content URI)
// must surface to the user — otherwise the pick appears to succeed and the
// image just never shows up.
async function buildAllPropsWith(std: BlockStdScope, files: File[]) {
  try {
    return await Promise.all(files.map(file => buildPropsWith(std, file)));
  } catch (err) {
    console.error(err);
    if (!(err instanceof Error && err.message === 'Failed to read image size')) {
      toast(std.host, 'Failed to read the selected image, please try again');
    }
    return null;
  }
}
...
export async function addSiblingImageBlocks(
  ...
  const propsArray = await buildAllPropsWith(std, files);
