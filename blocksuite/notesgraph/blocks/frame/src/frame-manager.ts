...
import type { SurfaceBlockModel } from '@blocksuite/notesgraph-block-surface';
import { Overlay } from '@blocksuite/notesgraph-block-surface';
...
export class EdgelessFrameManager extends GfxExtension {
  ...
  private _addFrameBlock(bound: Bound) {
    ...
    const id = this.gfx.doc.addBlock('notesgraph:frame', props, surfaceModel);
...
export function isFrameBlock(element: unknown): element is FrameBlockModel {
  return !!element && (element as BlockModel).flavour === 'notesgraph:frame';
}
