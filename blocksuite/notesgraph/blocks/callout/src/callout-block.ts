...
// Copy of renderUniLit and UniLit from notesgraph-data-view
...
const getIcon = (icon?: IconData) => {
  ...
  if (icon.type === IconType.NotesGraphIcon) {
    return (
...
export class CalloutBlockComponent extends CaptionedBlockComponent<CalloutBlockModel> {
  ...
  private _getEmojiMarginTop(): string {
    ...
    const marginTopMap: Record<string, string> = {
      'notesgraph:paragraph:h1': '23px',
      'notesgraph:paragraph:h2': '20px',
      'notesgraph:paragraph:h3': '16px',
      'notesgraph:paragraph:h4': '15px',
      'notesgraph:paragraph:h5': '14px',
      'notesgraph:paragraph:h6': '13px',
    };
  ...
  private _toggleIconPicker(event: MouseEvent) {
    ...
    wrapper.style.boxShadow = 'var(--notesgraph-menu-shadow)';
  ...
  private readonly _handleBlockClick = (event: MouseEvent) => {
    ...
    if (
      target.closest('.notesgraph-callout-emoji-container') ||
      target.classList.contains('notesgraph-callout-emoji')
    ) {
      return;
    }
