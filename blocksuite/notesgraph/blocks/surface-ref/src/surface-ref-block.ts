...
import { FrameBlockComponent } from '@blocksuite/notesgraph-block-frame';
import {
  EdgelessCRUDIdentifier,
  getSurfaceBlock,
} from '@blocksuite/notesgraph-block-surface';
import type { BlockCaptionEditor } from '@blocksuite/notesgraph-components/caption';
import { whenHover } from '@blocksuite/notesgraph-components/hover';
...
export class SurfaceRefBlockComponent extends BlockComponent<SurfaceRefBlockModel> {
  static override styles = css`
    ...
    notesgraph-surface-ref:not(:hover)
      notesgraph-surface-ref-toolbar:not([data-open-menu-display='show']) {
      display: none;
    }
    ...
    .notesgraph-surface-ref.focused {
      border-color: ${unsafeCSSVarV2('edgeless/frame/border/active')};
    }

    .notesgraph-surface-ref.comment-highlighted {
  ...
  private get _shouldRender() {
    return (
      ...
      !this.parentComponent?.closest('notesgraph-surface-ref')
    );
  }
  ...
  @query('.notesgraph-surface-ref')
  accessor hoverableContainer!: HTMLDivElement;

  @query('notesgraph-surface-ref > block-caption-editor')
...
declare global {
  interface HTMLElementTagNameMap {
    'notesgraph-surface-ref': SurfaceRefBlockComponent;
  }
}
