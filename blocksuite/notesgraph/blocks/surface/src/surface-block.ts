...
import type { Color } from '@blocksuite/notesgraph-model';
import {
  FeatureFlagService,
  ThemeProvider,
...
export class SurfaceBlockComponent extends BlockComponent<SurfaceBlockModel> {
  ...
  override render() {
    return html`
      <div class="notesgraph-edgeless-surface-block-container">
        <!-- attach canvas later in renderer -->
      </div>
    `;
  }

  @query('.notesgraph-edgeless-surface-block-container')
  private accessor _surfaceContainer!: HTMLElement;
}

declare global {
  interface HTMLElementTagNameMap {
    'notesgraph-surface': SurfaceBlockComponent;
  }
}
