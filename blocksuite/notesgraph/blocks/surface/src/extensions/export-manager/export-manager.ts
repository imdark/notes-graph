...
import { FetchUtils } from '@blocksuite/notesgraph-shared/adapters';
import {
  CANVAS_EXPORT_IGNORE_TAGS,
  DEFAULT_IMAGE_PROXY_ENDPOINT,
} from '@blocksuite/notesgraph-shared/consts';
import type { Viewport } from '@blocksuite/notesgraph-shared/types';
import {
  isInsidePageEditor,
  matchModels,
    ...
    const html2canvasOption = {
      ignoreElements: function (element: Element) {
        if (
          ...
          // the close and expand buttons in notesgraph-doc-meta-data is not needed to be showed
          return true;
        } else {
          return false;
        }
  ...
  private async _html2canvas(
    ...
    const html2canvasOption = {
      ...
      onclone: async (documentClone: Document, element: HTMLElement) => {
        ...
        const layer = element.classList.contains('.notesgraph-edgeless-layer')
          ? element
          : null;
