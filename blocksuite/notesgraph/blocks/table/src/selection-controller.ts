...
export class SelectionController implements ReactiveController {
        ...
        ${cells
          ...
          <tr>
            ${row
              .map(
                cell => `
              <td style="border: 1px solid var(--notesgraph-border-color); padding: 8px 12px; min-width: ${DefaultColumnWidth}px; min-height: 22px;">${cell}</td>
            `
              )
              .join('')}
          </tr>
  ...
  onDragStart(event: MouseEvent) {
    ...
    let selected = false;
    const initCell = target.closest('notesgraph-table-cell');
    if (!initCell) {
      selected = true;
    }
    const onMove = (event: MouseEvent) => {
      const target = event.target;
      if (target instanceof HTMLElement) {
        const cell = target.closest('notesgraph-table-cell');
