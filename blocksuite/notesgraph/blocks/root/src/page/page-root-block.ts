...
import { appendParagraphCommand } from '@blocksuite/notesgraph-block-paragraph';
import {
  CodeBlockModel,
  ListBlockModel,
  NoteBlockModel,
...
export class PageRootBlockComponent extends BlockComponent<RootBlockModel> {
  static override styles = css`
    editor-host:has(> notesgraph-page-root, * > notesgraph-page-root) {
      display: block;
      height: 100%;
    }
    ...
    .notesgraph-page-root-block-container {
      ...
      font-weight: 400;
      max-width: var(--notesgraph-editor-width);
      margin: 0 auto;

      /* Leave a place for drag-handle */
      /* Do not use prettier format this style, or it will be broken */
      /* prettier-ignore */
      padding-left: var(--notesgraph-editor-side-padding, ${DOC_BLOCK_CHILD_PADDING}px);
      /* prettier-ignore */
      padding-right: var(--notesgraph-editor-side-padding, ${DOC_BLOCK_CHILD_PADDING}px);
      /* prettier-ignore */
      padding-bottom: var(--notesgraph-editor-bottom-padding, ${DOC_BOTTOM_PADDING}px);
    }
  ...
  private _createDefaultNoteBlock() {
    ...
    const noteId = store.addBlock('notesgraph:note', {}, store.root?.id);
  ...
  override connectedCallback() {
    ...
    this.bindHotKey({
      ...
      ArrowUp: () => {
        ...
        while (current && !model) {
          if (current.flavour === 'notesgraph:note') {
            model = current;
          } else {
        ...
        if (!prevNote || prevNote.flavour !== 'notesgraph:note') {
        ...
        const notes = this.store.getModelsByFlavour('notesgraph:note');
  ...
  @query('.notesgraph-page-root-block-container')
