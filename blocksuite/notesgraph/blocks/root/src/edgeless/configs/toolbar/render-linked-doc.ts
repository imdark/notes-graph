import { isFrameBlock } from '@blocksuite/notesgraph-block-frame';
import {
  getSurfaceBlock,
  isNoteBlock,
...
export function createLinkedDocFromNote(
  ...
  linkedDoc.load(() => {
    const rootId = linkedDoc.addBlock('notesgraph:page', {
      title: new Text(docTitle),
    });
    linkedDoc.addBlock('notesgraph:surface', {}, rootId);
    transformer
...
export function createLinkedDocFromEdgelessElements(
  ...
  linkedDoc.load(() => {
    ...
    const surfaceId = linkedDoc.addBlock('notesgraph:surface', {}, rootId);
