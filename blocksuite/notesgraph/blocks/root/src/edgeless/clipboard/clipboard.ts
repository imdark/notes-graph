...
import { EdgelessFrameManagerIdentifier } from '@blocksuite/notesgraph-block-frame';
...
import { splitIntoLines } from '@blocksuite/notesgraph-gfx-text';
import type {
  EmbedCardStyle,
  ShapeElementModel,
...
export class EdgelessClipboardController extends PageClipboard {
  static override key = 'notesgraph-edgeless-clipboard';
  ...
  private readonly _onPaste = async (_context: UIEventStateContext) => {
    ...
    if (isUrlInClipboard(data)) {
      ...
      // try to interpret url as notesgraph doc url
  ...
  private async _edgelessToCanvas(
    ...
    const html2canvasOption = {
      ...
      onclone: async function (documentClone: Document, element: HTMLElement) {
        ...
        const layer = documentClone.querySelector('.notesgraph-edgeless-layer');
  ...
  private async _pasteTextContentAsNote(content: BlockSnapshot[] | string) {
    ...
    if (typeof content === 'string') {
      ...
      for (let index = 0; index < content.length; index++) {
        ...
        if (blockSnapshot.flavour === 'notesgraph:note') {
