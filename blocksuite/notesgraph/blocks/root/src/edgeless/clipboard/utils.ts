...
import type { FrameBlockProps } from '@blocksuite/notesgraph-model';
import { encodeClipboardBlobs } from '@blocksuite/notesgraph-shared/adapters';
...
export function createNewPresentationIndexes(
  ...
  const frames = raw
    .filter((block): block is FrameSnapshot => {
      ...
      return data?.flavour === 'notesgraph:frame';
    })
