...
import { TextUtils } from '@blocksuite/notesgraph-block-surface';
import { formatBlockCommand } from '@blocksuite/notesgraph-inline-preset';
import {
  DefaultTheme,
...
export class EdgelessTextBlockComponent extends GfxBlockComponent<EdgelessTextBlockModel> {
  static override styles = css`
    ...
    .edgeless-text-block-container notesgraph-paragraph,
    notesgraph-list {
  ...
  override renderGfxBlock() {
    ...
    const containerStyle: StyleInfo = {
      ...
      border: `1px solid ${editing ? 'var(--notesgraph—primary—color, #1e96eb)' : 'transparent'}`,
      borderRadius: '4px',
      boxSizing: 'border-box',
      boxShadow: editing ? '0px 0px 0px 2px rgba(30, 150, 235, 0.3)' : 'none',
      fontWeight: '400',
      lineHeight: 'var(--notesgraph-line-height)',
  ...
  override renderPageContent() {
    ...
    const style = styleMap({
      ...
      '--notesgraph-list-margin': '0',
      '--notesgraph-paragraph-margin': '0',
    });

    return html`
      <div style=${style} class="notesgraph-block-children-container">
  ...
  tryFocusEnd() {
    const paragraphOrLists = Array.from(
      this.querySelectorAll<BlockComponent>(
        'notesgraph-paragraph, notesgraph-list'
      )
    );
  ...
  @query('.notesgraph-block-children-container')
  accessor childrenContainer!: HTMLDivElement;
}

declare global {
  interface HTMLElementTagNameMap {
    'notesgraph-edgeless-text': EdgelessTextBlockComponent;
  }
}
              ...
              view.updateComplete
                .then(() => {
                  ...
                  if (model.children.length === 0) {
                    ...
                    const rect = view
                      .querySelector('.notesgraph-block-children-container')
