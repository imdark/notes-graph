import '@blocksuite/notesgraph-shared/commands';

import { CaptionedBlockComponent } from '@blocksuite/notesgraph-components/caption';
import { playCheckAnimation } from '@blocksuite/notesgraph-components/icons';
import { TOGGLE_BUTTON_PARENT_CLASS } from '@blocksuite/notesgraph-components/toggle-button';
...
import { getViewportElement } from '@blocksuite/notesgraph-shared/utils';
...
export class ListBlockComponent extends CaptionedBlockComponent<ListBlockModel> {
  ...
  private readonly _onClickIcon = (e: MouseEvent) => {
    ...
    if (this.model.props.type === 'toggle') {
      ...
      if (this.model.props.checked) {
        const checkEl = this.querySelector(
  ...
  override renderBlock(): TemplateResult<1> {
    ...
    const children = html`<div
      class="notesgraph-block-children-container"
      style=${styleMap({
    ...
    return html`
      <div class=${'notesgraph-list-block-container'} style="${textAlignStyle}">
        <div
          class=${classMap({
            'notesgraph-list-rich-text-wrapper': true,
  ...
  override accessor blockContainerStyles = {
    margin: 'var(--notesgraph-list-margin, 10px 0)',
  };
}
