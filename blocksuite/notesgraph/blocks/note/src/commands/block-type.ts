...
> = (ctx, next) => {
  ...
  const mergeToCode: Command<{}, { updatedBlocks: BlockModel[] }> = (
    ...
    if (flavour !== 'notesgraph:code') return;
  ...
  const appendDivider: Command<{}, { updatedBlocks: BlockModel[] }> = (
    ...
    const id = doc.addBlock('notesgraph:divider', {}, parent, index + 1);
    if (!nextSibling) {
      nextSiblingId = doc.addBlock('notesgraph:paragraph', {}, parent);
    }
  ...
  const transformToLatex: Command<{}, { updatedBlocks: BlockModel[] }> = (
    ...
    if (flavour !== 'notesgraph:latex') return;
    ...
    blockModels.forEach(model => {
      ...
      const newId = transformModel(model, 'notesgraph:latex', { latex });
      if (!newId) {
        return;
      }
  ...
  const [result, resultCtx] = std.command
    ...
    .try(chain => [
      chain
        ...
        .pipe(selectBlocks),
      chain.pipe((_, next) => {
        if (['notesgraph:code', 'notesgraph:divider'].includes(flavour)) {
          return next();
        }
        return false;
      }),
