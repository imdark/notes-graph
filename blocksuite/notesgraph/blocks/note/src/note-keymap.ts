...
class NoteKeymap {
                ...
                [key]: ctx => {
                  ...
                  const [result] = this._std.command
                    ...
                    .pipe((ctx, next) => {
                      ...
                      if (item.flavour !== 'notesgraph:code') {
                        return;
                      }
  ...
  /**
   * Cut-line via the literal Ctrl+X chord (the platform cut combo — Cmd+X
   * on macOS, Ctrl+X elsewhere — never reaches keymaps and is handled by
   * the clipboard's cut path instead; see PageClipboard.onPageCut).
   */
  private readonly _onCutLine = (ctx: UIEventStateContext) => {
    if (!deleteCurrentLine(this.std)) return;
