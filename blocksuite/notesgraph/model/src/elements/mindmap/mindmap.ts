...
export class MindmapElementModel extends GfxGroupLikeElementModel<MindmapElementProps> {
  ...
  buildTree() {
    ...
    // The element may be removed, or — for partially imported / corrupt data —
    // `children` may not be a usable Y.Map. Bail out gracefully instead of
    // throwing, so one malformed mindmap can't break the whole surface.
    if (!(nodesMap instanceof Y.Map) || nodesMap.size === 0) {
