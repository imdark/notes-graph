...
::: info
🌐 This documentation has a [Chinese translation](https://insider.notesgraph.com/share/af3478a2-9c9c-4d16-864d-bffa1eb10eb6/-3bEQPBoOEkNH13ULW9Ed).
:::
...
const props = {};
const noteId = doc.addBlock('notesgraph:note', props, rootId);

// You can also provide an optional `parentIndex`
const paragraphId = doc.addBlock('notesgraph:paragraph', props, noteId, 0);
...
::: info
The block tree hierarchy is specific to the preset editors. At the framework level, `@blocksuite/store` does **NOT** treat the "first-party" `notesgraph:*` blocks with any special way. Feel free to add blocks from different namespaces for the block tree!
:::
...
```ts
const rootService = host.spec.getService('notesgraph:page');
...
@customElement('notesgraph-embed-github-block')
export class EmbedGithubBlock extends EmbedBlockComponent<EmbedGithubModel> {
  ...
  override render() {
    return this.renderEmbed(() => {
      return html`
        <div class="notesgraph-embed-github-block">
          <h3>GitHub Card</h3>
...
export const EmbedGithubBlockSpec = createEmbedBlock({
  ...
  view: {
    component: literal`notesgraph-embed-github-block`,
  },
});
```
...
```ts
const props = {
  owner: 'toeverything', // The company behind BlockSuite and NotesGraph 🤫
  repo: 'https://github.com/toeverything/blocksuite',
};

// The 'notesgraph' prefix is kept by default, but you can also override it.
doc.addBlock('notesgraph:embed-github', props, parentId);
```
