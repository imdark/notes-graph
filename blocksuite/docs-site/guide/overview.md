...
BlockSuite originated from the [NotesGraph](https://github.com/notesgraph/notesgraph) knowledge base, with design goals including:
...
During the development of NotesGraph, it became clear that BlockSuite was advancing beyond merely being an in-house editor and evolving into a versatile framework. That's why we chose to open source and maintain BlockSuite independently.

<!-- ## Examples -->

## Features

With BlockSuite editors, you can selectively reuse all the editing features in [NotesGraph](https://notesgraph.com/):

[![notesgraph-demo](../images/notesgraph-demo.jpg)](https://notesgraph.com)
...
The relationship between BlockSuite and NotesGraph is similar to that between the [Monaco Editor](https://github.com/microsoft/monaco-editor) and [VSCode](https://code.visualstudio.com/), but with one major difference: BlockSuite is not automatically generated based on the NotesGraph codebase, but is maintained independently with a different tech stack — NotesGraph uses React while BlockSuite uses [web components](https://developer.mozilla.org/en-US/docs/Web/API/Web_components).

This difference has led BlockSuite to set clear boundaries between packages, ensuring:

- Both NotesGraph and other projects should equally reuse and extend BlockSuite through components, without any privileges.
