...
- The BlockSuite `canary` versions are released daily based on the master branch, which is also used in production in [NotesGraph](https://github.com/notesgraph/notesgraph).
...
(async () => {
  ...
  const paragraphs = doc.getBlockByFlavour('notesgraph:paragraph');
