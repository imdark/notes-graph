...
import { TestNotesGraphEditorContainer } from '@blocksuite/integration-test';
import { PresentTool } from '@blocksuite/notesgraph/blocks/frame';
import { ExportManager } from '@blocksuite/notesgraph/blocks/surface';
import { toast } from '@blocksuite/notesgraph/components/toast';
import { StoreExtensionManagerIdentifier } from '@blocksuite/notesgraph/ext-loader';
import {
  BlockSuiteError,
  ErrorCode,
} from '@blocksuite/notesgraph/global/exceptions';
import type { SerializedXYWH } from '@blocksuite/notesgraph/global/gfx';
import { ColorScheme, type DocMode } from '@blocksuite/notesgraph/model';
import {
...
import { NotionHtmlAdapter } from '@blocksuite/notesgraph-shared/adapters';
...
function initStyleDebugMenu(
  ...
  fontFamilyFolder
    ...
    .on('change', e => {
      writer.setProperty('--notesgraph-font-family', e.value);
    });
...
export class StarterDebugMenu extends ShadowlessElement {
  static override styles = css`
    :root {
      --sl-font-size-medium: var(--notesgraph-font-xs);
      --sl-input-font-size-small: var(--notesgraph-font-xs);
    }
  ...
  private _addNote() {
    ...
    const noteId = this.doc.addBlock('notesgraph:note', { xywh }, rootId);
    this.doc.addBlock('notesgraph:paragraph', {}, noteId);
  }
    ...
    input.onchange = async () => {
      ...
      try {
        ...
        for (const doc of docs) {
          if (doc) {
            const noteBlock = window.doc.getModelsByFlavour('notesgraph:note');
            window.doc.addBlock(
    ...
    if (app) {
      ...
      if (currentEditorCount === 1) {
        ...
        app.childNodes.forEach(child => {
          if (child instanceof TestNotesGraphEditorContainer) {
            child.style.flex = '1';
          }
        });
        ...
        const secondEditor = app.querySelectorAll(
          'notesgraph-editor-container'
        )[1];
        if (secondEditor) {
          secondEditor.remove();
