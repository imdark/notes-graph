...
export const synced: InitFn = (collection: Workspace, id: string) => {
  ...
  docSyncedPage.load(() => {
    ...
    const rootId = docSyncedPage.addBlock('notesgraph:page', {
      title: new Text('Synced - Page View'),
    });

    docSyncedPage.addBlock('notesgraph:surface', {}, rootId);

    // Add note block inside root block
    const noteId = docSyncedPage.addBlock('notesgraph:note', {}, rootId);
  ...
  docSyncedEdgeless.load(() => {
    // Add root block and surface block at root level
    const rootId = docSyncedEdgeless.addBlock('notesgraph:page', {
      title: new Text('Synced - Edgeless View'),
    });

    docSyncedEdgeless.addBlock('notesgraph:surface', {}, rootId);

    // Add note block inside root block
    const noteId = docSyncedEdgeless.addBlock('notesgraph:note', {}, rootId);
  ...
  docMain.load(() => {
    // Add root block and surface block at root level
    const rootId = docMain.addBlock('notesgraph:page', {
      title: new Text('Home doc, having synced blocks'),
    });

    const surfaceId = docMain.addBlock('notesgraph:surface', {}, rootId);
    const noteId = docMain.addBlock('notesgraph:note', {}, rootId);
