 ...
 * Used for text field
 */
export const SURFACE_TEXT_UNIQ_IDENTIFIER = 'notesgraph:surface:text';
/**
 * Used for field that use Y.Map. E.g. group children field
 */
export const SURFACE_YMAP_UNIQ_IDENTIFIER = 'notesgraph:surface:ymap';
...
export class SurfaceBlockModel extends BlockModel<SurfaceBlockProps> {
  ...
  private _initElementModels() {
    ...
    const onElementsMapChange = (
      ...
      addedElements.forEach(({ mount, model }) => {
        try {
          ...
          console.error(
            `Failed to mount surface element ${model.id} (type: ${model.type})`,
            error
          );
        }
    ...
    this._elementModels.forEach(({ mount, model }) => {
      ...
      // A single malformed element (e.g. from partially imported / corrupt
      // data) must not abort initialization of the rest of the surface.
      try {
        mount();
      } catch (error) {
        console.error(
