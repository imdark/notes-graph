...
describe('basic', () => {
  ...
  it('doc ready lifecycle', () => {
    ...
    store.load(() => {
      ...
      store.addBlock('notesgraph:note', {}, rootId);
    });
  ...
  it('collection docs with yjs applyUpdate', () => {
    ...
    doc.load(() => {
      store.addBlock('notesgraph:page', {
        title: new Text(),
      });
    });
    {
...
describe('addBlock', () => {
  ...
  it('can add model with props', () => {
    const doc = createTestDoc();
    doc.addBlock('notesgraph:page', { title: new Text('hello') });
  ...
  it('can add multi models', () => {
    ...
    doc.addBlocks(
      [
        { flavour: 'notesgraph:paragraph', blockProps: { type: 'h1' } },
        { flavour: 'notesgraph:paragraph', blockProps: { type: 'h2' } },
      ],
      noteId
    );
  ...
  it('can observe slot events', async () => {
    ...
    assert.equal(block.flavour, 'notesgraph:page');
  });
  ...
  it('can add block to root', async () => {
    ...
    queueMicrotask(() => {
      const rootId = doc.addBlock('notesgraph:page');
      noteId = doc.addBlock('notesgraph:note', {}, rootId);
    });
    await waitOnce(doc.slots.rootAdded);
    const { root } = doc;
    if (!root) throw new Error('root is null');

    assert.equal(root.flavour, 'notesgraph:page');

    doc.addBlock('notesgraph:paragraph', {}, noteId!);
    assert.equal(root.children[0].flavour, 'notesgraph:note');
    assert.equal(root.children[0].children[0].flavour, 'notesgraph:paragraph');
  ...
  it('can add and remove multi docs', async () => {
    ...
    store0.addBlock('notesgraph:page', {
      title: new Text(),
    });
...
describe('deleteBlock', () => {
  ...
  it('bring children to parent', () => {
    ...
    const rootId = doc.addBlock('notesgraph:page', {});
    const noteId = doc.addBlock('notesgraph:note', {}, rootId);
    const p1 = doc.addBlock('notesgraph:paragraph', {}, noteId);
    doc.addBlock('notesgraph:paragraph', {}, p1);
    doc.addBlock('notesgraph:paragraph', {}, p1);
  ...
  it('bring children to other block', () => {
    ...
    const noteId = doc.addBlock('notesgraph:note', {}, rootId);
    const p1 = doc.addBlock('notesgraph:paragraph', {}, noteId);
    const p2 = doc.addBlock('notesgraph:paragraph', {}, noteId);
    doc.addBlock('notesgraph:paragraph', {}, p1);
    doc.addBlock('notesgraph:paragraph', {}, p1);
    doc.addBlock('notesgraph:paragraph', {}, p2);
  ...
  it('can delete model with parent', () => {
    ...
    const noteId = doc.addBlock('notesgraph:note', {}, rootModel.id);
...
describe('getBlock', () => {
  it('can get block by id', () => {
    ...
    assert.equal(text.flavour, 'notesgraph:paragraph');
