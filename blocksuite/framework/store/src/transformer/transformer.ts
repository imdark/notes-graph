...
export class Transformer {
  ...
  snapshotToSlice = async (
    ...
    try {
      ...
      if (!blockTree) {
        console.warn(
          'Skipped importing slice: no convertible blocks in its content'
        );
        return undefined;
      }
  ...
  ): DraftBlockTreeNode | null {
    // Every block in the subtree failed to convert (e.g. unknown flavour in
    // corrupt/legacy data) — nothing to rebuild.
  ...
  private async _snapshotToBlock(
    ...
    if (!blockTree) {
      console.warn(
        `Skipped importing block ${snapshot.id} (${snapshot.flavour}): no convertible blocks in its subtree`
      );
      return null;
    }
