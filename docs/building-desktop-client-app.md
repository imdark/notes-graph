# Building NotesGraph Desktop Client App
...
Before you start building NotesGraph Desktop Client Application, please following the same steps in [BUILDING#Prerequisites](./BUILDING.md#prerequisites) to install Node.js and Rust.
...
> repos/NotesGraph/.github/workflows/release-desktop.yml contains real order to build the desktop client app, but here we will explain the steps in a more detailed way. Up-to date.
...
```shell
BUILD_TYPE=canary yarn notesgraph @notesgraph/electron build

BUILD_TYPE=canary yarn notesgraph @notesgraph/electron generate-assets
```
...
```shell
BUILD_TYPE=canary SKIP_WEB_BUILD=1 HOIST_NODE_MODULES=1 yarn notesgraph @notesgraph/electron make
```
...
yarn notesgraph @notesgraph/electron package
yarn notesgraph @notesgraph/electron make-squirrel
yarn notesgraph @notesgraph/electron make-nsis
```
...
Finished 2 bundles at:
  › Artifacts available at: <notesgraph-repo>/packages/frontend/apps/electron/out/canary/make
```
