import { Path } from '@notesgraph-tools/utils/path';
import { execAsync } from '@notesgraph-tools/utils/process';
import type { Package, PackageName } from '@notesgraph-tools/utils/workspace';
...
export class RunCommand extends PackageCommand {
  ...
  static override usage = PackageCommand.Usage({
    description: 'NotesGraph Monorepo scripts',
    details: `
      \`notesgraph web <script>\`    Run any script defined in package's package.json

      \`notesgraph init\`            Generate the required files if there are any package added or removed

      \`notesgraph clean\`           Clean the output files of ts, cargo, bundler outputs, etc.

      \`notesgraph bundle\`          Bundle the packages

      \`notesgraph build\`           A proxy for <-p package>'s \`build\` script

      \`notesgraph dev\`             A proxy for <-p package>'s \`dev\` script
    `,
    examples: [
      [`See detail of each command`, '$0 -h'],
      [
        `Run custom 'xxx' script defined in @notesgraph/web's package.json`,
        '$0 web xxx',
      ],
      ...
      [`Run 'build' script for @notesgraph/web`, '$0 build -p web'],
      [
        `Run 'build' script for @notesgraph/web with all deps prebuild before`,
        '$0 build -p web --deps',
      ],
    ],
  });
  ...
  async runScript(
    ...
    const isNotesGraphCommand = args[0] === 'notesgraph';
    if (isNotesGraphCommand) {
      // remove 'notesgraph' from 'notesgraph xxx' command
      args.shift();
