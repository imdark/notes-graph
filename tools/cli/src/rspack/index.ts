...
import { Path, ProjectRoot } from '@notesgraph-tools/utils/path';
...
export function createHTMLTargetConfig(
  ...
  const hasTailwind =
    ...
    pkg.name === '@notesgraph/media-capture-playground';
  ...
  const config: RspackConfiguration = {
    ...
    // On-device AI libs (transformers.js, onnxruntime-web, diffusers.js) ship
    // bundled code referencing Node globals (`import.meta`, `__dirname`,
    // `__filename`) that are dead/mocked in the browser — harmless warnings.
    ignoreWarnings: [
      /Accessing import\.meta/,
      /(__dirname|__filename)" is used and has been mocked/,
    ...
    // no source maps for mobile: they ship inside the app bundle (APK/IPA)
    // and add tens of MB to the install size without being usable there.
    // NG_SOURCEMAP=1 forces them back on for symbolicating device stacks.
    devtool: buildConfig.debug
      ? 'cheap-module-source-map'
      : (buildConfig.distribution === 'android' ||
            buildConfig.distribution === 'ios') &&
          !process.env.NG_SOURCEMAP
        ? false
        : 'source-map',
    resolve: {
      symlinks: true,
      ...
      alias: {
        ...
        // diffusers.js (on-device image gen) bundles an old onnxruntime fork
        // that breaks on modern browsers (`WebAssembly.Function`); point it at
        // the current onnxruntime-web instead.
        '@aislamov/onnxruntime-web64': ProjectRoot.join(
          'node_modules',
          'onnxruntime-web'
        ).value,
        ...
        '@notesgraph/core/modules/code-block-preview-renderer/platform-backend':
          ...
          patterns: [
            {
              ...
              from: new Package('@notesgraph/core').join('public').value,
            },
          ],
        }),
      // onnxruntime-web (used by diffusers.js for on-device image gen) loads
      // its WebGPU runtime glue (`…jsep.mjs` + its `.wasm`) via a runtime
      // dynamic import, resolving it against the bundle's `/js/` chunk dir —
      // which rspack never emits. Copy the JSEP runtime there so the import
      // resolves locally (offline-friendly). Explicit file paths (no glob) so
      // there's no chance of a non-match silently emitting nothing.
      new rspack.CopyRspackPlugin({
        patterns: [
          {
            from: ProjectRoot.join(
              'node_modules',
              'onnxruntime-web',
              'dist',
...
export function createWorkerTargetConfig(
  ...
  return {
    ...
    mode: buildConfig.debug ? 'development' : 'production',
    // same as the HTML target: no source maps inside mobile app bundles
    devtool: buildConfig.debug
      ? 'cheap-module-source-map'
      : buildConfig.distribution === 'android' ||
          buildConfig.distribution === 'ios'
