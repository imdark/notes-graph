...
import type { Path } from '@notesgraph-tools/utils/path';
import {
  type Package,
  Workspace,
  yarnList,
} from '@notesgraph-tools/utils/workspace';
...
export class InitCommand extends Command {
  ...
  async generateWorkspaceFiles() {
    ...
    const filesToGenerate: [
      ...
      [
        this.workspace
          .getPackage('@notesgraph-tools/utils')
  ...
  genPackageTsConfig = (pkg: Package, prev: string) => {
    ...
    if (pkg.name === '@notesgraph/electron-api') {
      return prev;
    }
