...
Compare and Select Flights`
    ...
    const todos = await answer.locator('notesgraph-list').all();
    ...
    await Promise.all(
      todos.map(async (todo, index) => {
        await expect(
          todo.locator('.notesgraph-list-block__todo-prefix')
        ).toBeVisible();
...
Compare and Select Flights`
    ...
    const todos = await panelAnswer.locator('notesgraph-list').all();
