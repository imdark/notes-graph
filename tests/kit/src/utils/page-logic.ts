...
export const focusInlineEditor = async (page: Page) => {
  await page
    .locator(
      `.notesgraph-paragraph-rich-text-wrapper:has(.visible):has-text("Type '/' for commands")`
    )
...
export const addDatabase = async (page: Page, title?: string) => {
  ...
  if (title) {
    await page.locator('notesgraph-database-title').click();
    await page
      .locator(
        'notesgraph-database-title textarea[data-block-is-database-title="true"]'
      )
      .fill(title);
    await page
      .locator(
