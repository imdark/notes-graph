...
const server = new Package('@notesgraph/server');
...
export async function switchDefaultChatModel(model: string) {
  await runPrisma(async client => {
    const prompt = await client.aiPrompt.findFirst({
      where: { name: 'Chat With NotesGraph AI' },
      select: { id: true },
    });
    if (!prompt) return;
...
export async function enableCloudWorkspace(page: Page) {
  ...
  await page.getByTestId('publish-enable-notesgraph-cloud-button').click();
  await page.getByTestId('confirm-enable-notesgraph-cloud-button').click();
...
export async function enableCloudWorkspaceFromShareButton(page: Page) {
  ...
  await page.getByTestId('share-menu-enable-notesgraph-cloud-button').click();
  await page.getByTestId('confirm-enable-notesgraph-cloud-button').click();
  // wait for upload and delete local workspace
  await page.waitForTimeout(2000);
  await waitForEditorLoad(page);
