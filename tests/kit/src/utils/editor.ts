...
import type { IVec, XYWH } from '@blocksuite/notesgraph/global/gfx';
...
const PARAGRAPH_BLOCK_LOCATOR = 'notesgraph-paragraph';
const CODE_BLOCK_LOCATOR = 'notesgraph-code';
...
export function locateEditorContainer(page: Page, editorIndex = 0) {
  return page.locator('[data-notesgraph-editor-container]').nth(editorIndex);
}
...
export async function getEdgelessSelectedIds(page: Page, editorIndex = 0) {
  ...
  return container.evaluate(container => {
    const root = container.querySelector('notesgraph-edgeless-root');
    if (!root) {
