...
test('drag handle should be shown when a note is activated in default mode or hidden in other modes', async ({
  ...
  await expect(page.locator('.notesgraph-drag-handle-container')).toBeHidden();
  await page.mouse.dblclick(x, y);
  await waitNextFrame(page);
  await page.mouse.move(x, y);

  await expect(page.locator('.notesgraph-drag-handle-container')).toBeVisible();
...
test('should keep relative order of new note when a block is dragged from note to canvas', async ({
  ...
  const notes = page.locator('notesgraph-edgeless-note');
...
test('drag handle should work when hover on the background of a selected edgeless note', async ({
  ...
  const noteRect = await page.locator('notesgraph-edgeless-note').boundingBox();
  assertRectExist(noteRect);
  ...
  const paragraphRect = await page
    .locator('notesgraph-paragraph')
    .boundingBox();
