import type { EdgelessTextBlockComponent } from '@blocksuite/notesgraph/blocks/edgeless-text';
...
test.describe('edgeless text block', () => {
  ...
  test('add text block in default mode', async ({ page }) => {
    ...
    await assertBlockFlavour(page, 4, 'notesgraph:edgeless-text');
    await assertBlockFlavour(page, 5, 'notesgraph:paragraph');
    await assertBlockFlavour(page, 6, 'notesgraph:paragraph');
  ...
  test('When creating edgeless text, if the input is empty, it will be automatically deleted', async ({
    ...
    await waitNextFrame(page);
    let block = page.locator('notesgraph-edgeless-text[data-block-id="4"]');
    expect(await block.isVisible()).toBe(true);
    await page.mouse.click(0, 0);
    expect(await block.isVisible()).toBe(false);

    block = page.locator('notesgraph-edgeless-text[data-block-id="6"]');
  ...
  test.fixme('edgeless text max width', async ({ page }) => {
    ...
    let textRect = await page
      .locator('notesgraph-edgeless-text[data-block-id="4"]')
      .boundingBox();
  ...
  test('cut edgeless text', async ({ page }) => {
    ...
    const edgelessText = page.locator('notesgraph-edgeless-text');
    const paragraph = page.locator(
      'notesgraph-edgeless-text notesgraph-paragraph'
    );
  ...
  test('latex in edgeless text', async ({ page }) => {
    ...
    await page.locator('notesgraph-latex-node').click();
    await waitNextFrame(page);
    await type(page, 'ccc');
...
test('press backspace at the start of first line when edgeless text exist', async ({
  ...
  await page.locator('.notesgraph-page-root-block-container').click();
  expect(await getPageSnapshot(page, true)).toMatchSnapshot(
