...
test.describe('embed-synced-doc toolbar', () => {
  ...
  test('can change embed synced doc to card view', async ({ page }) => {
    ...
    const syncedDoc = page.locator('notesgraph-embed-synced-doc-block');
    ...
    const toolbar = page.locator(
      ...
      'notesgraph-page-root > div > notesgraph-toolbar-widget editor-toolbar'
    );
    await expect(toolbar).toBeVisible();
    ...
    const embedSyncedBlock = page.locator('notesgraph-embed-linked-doc-block');
    expect(await embedSyncedBlock.count()).toBe(1);
  });
});
