...
test.describe('embed-synced-doc render', () => {
  ...
  test('nested embed synced doc should be rendered as card when depth >=1', async ({
    ...
    expect(
      await page.locator('notesgraph-embed-synced-doc-block').count()
    ).toBe(2);
    expect(await page.locator('notesgraph-paragraph').count()).toBe(2);
    expect(await page.locator('notesgraph-embed-synced-doc-card').count()).toBe(
      1
    );
  ...
  test('synced doc should be readonly', async ({ page }) => {
    ...
    const locator = page.locator('notesgraph-embed-synced-doc-block');
    ...
    await page.evaluate(async embedDocId => {
      ...
      const [noteBlock] = doc2!.getBlocksByFlavour('notesgraph:note');
    ...
    const databaseFirstCell = page.locator(
      '.notesgraph-database-column-header.database-row'
    );
    await databaseFirstCell.click({ force: true });
    const selectedCount = await page
      .locator('.notesgraph-embed-synced-doc-container.selected')
      .count();
