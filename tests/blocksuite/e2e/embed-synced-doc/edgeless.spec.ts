...
import type { EmbedSyncedDocBlockProps } from '@blocksuite/notesgraph/model';
...
test.describe('Embed synced doc in edgeless mode', () => {
  ...
  test.fixme('drag embed synced doc to whiteboard should fit in height', async ({
    ...
    const noteBlock = page.locator('notesgraph-edgeless-note');
  ...
  test('new edgeless embed synced doc should fit in height', async ({
    ...
    const paragraphHeight = (
      await page
        .locator('notesgraph-embed-synced-doc-block notesgraph-paragraph')
        .boundingBox()
    )?.height;
    if (!paragraphHeight) {
      test.fail();
    ...
    const createEmbedDocWithHeight = async (height: number) => {
      await page.evaluate(
        ({ embedDocId, height }) => {
          ...
          const surface = std.store.getModelsByFlavour('notesgraph:surface')[0];
          std.store.addBlock(
  ...
  test.describe('edgeless element toolbar', () => {
    ...
    const locateToolbar = (page: Page) => {
      return page.locator(
        ...
        'notesgraph-edgeless-root > .widgets-container notesgraph-toolbar-widget editor-toolbar'
      );
    };
    ...
    test('should render a reference node and all content of embed-synced-doc after click "Duplicate as note" button', async ({
      ...
      const edgelessNotes = page.locator('notesgraph-edgeless-note');
    ...
    test('edgeless note duplicated from embed-synced-doc should be above other elements', async ({
      ...
      await page.locator('notesgraph-embed-edgeless-synced-doc-block').click();
      const toolbar = locateToolbar(page);
      await toolbar.getByLabel('Duplicate as note').click();
