...
async function clickListIcon(page: Page, i = 0) {
  const locator = page.locator('.notesgraph-list-block__prefix').nth(i);
    ...
    },
    {
      beforeMouseUp: async () => {
        const rect = await page
          .locator('.notesgraph-page-dragging-area')
...
test('should not clear selected rects when scrolling the wheel', async ({
  ...
  const count2 = await page.evaluate(() => {
    ...
    const rects = viewport.querySelectorAll('notesgraph-block-selection');
...
test('should show toolbar of image on block selection', async ({ page }) => {
  ...
  const imageRect = await page.locator('notesgraph-image').boundingBox();
  if (!imageRect) {
    throw new Error();
  }
  ...
  await expect(
    page.locator('notesgraph-block-selection').locator('visible=true')
  ).toHaveCount(1);
});
...
test('should not select parent block when dragging area only intersects with child', async ({
  ...
  rects = page.locator('notesgraph-block-selection').locator('visible=true');
