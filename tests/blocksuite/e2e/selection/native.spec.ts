...
test('select text in the same line with dragging leftward and move outside the notesgraph-note', async ({
  page,
}) => {
  await enterPlaygroundRoom(page);
  await initEmptyParagraphState(page);
  ...
  const noteLeft = await page.evaluate(() => {
    const note = document.querySelector('notesgraph-note');
    if (!note) {
      throw new Error();
    }
...
test('select text in the same line with dragging leftward and move outside the notesgraph-note by forwardDelete', async ({
  page,
}) => {
  await enterPlaygroundRoom(page);
  await initEmptyParagraphState(page);
...
test('select text in the same line with dragging rightward and move outside the notesgraph-note', async ({
  page,
}) => {
  await enterPlaygroundRoom(page);
  await initEmptyParagraphState(page);
...
test('select text in the same line with dragging rightward and move outside the notesgraph-note by forwardDelete', async ({
  page,
}) => {
  await enterPlaygroundRoom(page);
  await initEmptyParagraphState(page);
...
test('should not crash when mouse over the left side of the list block prefix', async ({
  ...
  const prefixIconRect = await page.evaluate(() => {
    ...
    const prefixIcon = block.querySelector('.notesgraph-list-block__prefix ');
    if (!prefixIcon) {
      throw new Error();
    }
...
test('should set the last block to end the range after when leaving the notesgraph-note', async ({
  page,
}) => {
  await enterPlaygroundRoom(page);
  await initEmptyParagraphState(page);
...
test('should set the first block to start the range before when leaving the notesgraph-note-block-container', async ({
  page,
}) => {
  await enterPlaygroundRoom(page);
  await initEmptyParagraphState(page);
...
test('should select full text of the first block when leaving the notesgraph-note-block-container in edgeless mode', async ({
  page,
}) => {
  await enterPlaygroundRoom(page);
...
test('should add a new line when clicking the bottom of the last non-text block', async ({
  ...
  const locator = page.locator('notesgraph-code');
...
test('should clear native selection before block selection', async ({
  ...
  const rects = page
    .locator('notesgraph-block-selection')
...
test('should keep native range selection when scrolling backward with the scroll wheel', async ({
  ...
  const blockHeight = await page.evaluate(() => {
    ...
    const container = viewport.querySelector(
      'notesgraph-note .notesgraph-block-children-container'
    );
    if (!container) {
      throw new Error();
    }
...
test('should keep native range selection when scrolling forward with the scroll wheel', async ({
  ...
  await page.evaluate(() => {
    document.querySelector('.notesgraph-page-viewport')?.scrollTo(0, 0);
  });
  await page.mouse.move(0, 0);
...
test('click to select divided', async ({ page }) => {
  ...
  await page.click('notesgraph-divider');
...
test('abnormal cursor jumping', async ({ page }) => {
  ...
  await pressEnter(page);
  await page
    .locator('notesgraph-image block-zero-width .block-zero-width')
    .click();
  await pressArrowUp(page);
  await pressTab(page);
  await pressArrowDown(page);
  ...
  const image = page.locator('notesgraph-image');
