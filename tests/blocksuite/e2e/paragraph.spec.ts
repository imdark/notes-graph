...
test('switch between paragraph types', async ({ page }) => {
  ...
  const selector = '.notesgraph-paragraph-rich-text-wrapper';

  await updateBlockType(page, 'notesgraph:paragraph', 'h1');
  await assertClassName(page, selector, /h1/);

  await updateBlockType(page, 'notesgraph:paragraph', 'h2');
  await assertClassName(page, selector, /h2/);

  await updateBlockType(page, 'notesgraph:paragraph', 'h3');
...
test('delete at start of paragraph immediately following list', async ({
  ...
  // text -> numbered
  await focusRichText(page, 1);
  await updateBlockType(page, 'notesgraph:list', 'numbered');
  await assertBlockType(page, '2', 'text');
  ...
  // text -> todo
  await focusRichText(page, 1);
  await updateBlockType(page, 'notesgraph:list', 'todo');
  await assertBlockType(page, '2', 'text');
...
test('press arrow down in indent line should not move caret to the start of line', async ({
  ...
  await page.evaluate(() => {
    ...
    const note = doc.addBlock('notesgraph:note', {}, rootId);
    const p1 = doc.addBlock('notesgraph:paragraph', {}, note);
    const p2 = doc.addBlock('notesgraph:paragraph', {}, p1);
...
test('should placeholder works', async ({ page }) => {
  ...
  await initEmptyParagraphState(page);
  await focusRichText(page);
  const placeholder = page.locator('.notesgraph-paragraph-placeholder.visible');
  ...
  await updateBlockType(page, 'notesgraph:paragraph', 'text');
  await focusRichText(page, 0);
  await expect(placeholder).toBeVisible();
  await expect(placeholder).toContainText("Type '/' for commands");
