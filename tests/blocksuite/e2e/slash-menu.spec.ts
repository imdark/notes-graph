...
test.describe('slash menu should show and hide correctly', () => {
  ...
  test('should open tooltip when hover on item', async ({ page }) => {
    ...
    const slashItems = slashMenu.locator('icon-button');
    const tooltip = page.locator('.notesgraph-tooltip');
...
test('should insert database', async ({ page }) => {
  ...
  const titleColumn = page.locator('.notesgraph-database-column').nth(0);
  expect(await titleColumn.innerText()).toBe('Title');
  const defaultRows = page.locator('.notesgraph-database-block-row');
...
test('should slash menu can trigger linked doc popover', async ({ page }) => {
  ...
  await type(page, 'doc');
  await pressEnter(page);
  await expect(page.locator('notesgraph-reference')).toBeVisible();
  await expect(
    page.locator('notesgraph-reference .notesgraph-reference-title')
  ).toHaveText('doc');
});
