...
import type { BookmarkBlockComponent } from '@blocksuite/notesgraph/blocks/bookmark';
...
test(scoped`covert bookmark block to link text`, async ({ page }, testInfo) => {
  await createBookmarkBlockBySlashMenu(page);
  const bookmark = page.locator('notesgraph-bookmark');
...
test.describe('embed card toolbar', () => {
  ...
  // TODO(@fundon): should move to notesgraph side
...
test('indent bookmark block to paragraph', async ({ page }) => {
  ...
  await assertBlockFlavour(page, '1', 'notesgraph:note');
  await assertBlockFlavour(page, '2', 'notesgraph:paragraph');
  await assertBlockFlavour(page, '4', 'notesgraph:bookmark');
...
test('indent bookmark block to list', async ({ page }) => {
  ...
  await assertBlockFlavour(page, '1', 'notesgraph:note');
  await assertBlockFlavour(page, '3', 'notesgraph:list');
  await assertBlockFlavour(page, '5', 'notesgraph:bookmark');
...
test('bookmark can be dragged from note to surface top level block', async ({
  ...
  await waitNextFrame(page);
  await assertParentBlockFlavour(page, '4', 'notesgraph:surface');
});
...
test('bookmark card should show banner in edgeless mode', async ({ page }) => {
  ...
  const url = 'https://github.com/notesgraph/notesgraph/pull/11796';

  await page.locator('edgeless-link-tool-button').click();
  await page.locator('.embed-card-modal-input').fill(url);
  await pressEnter(page);

  const banner = page.locator('.notesgraph-bookmark-banner');
...
test.describe('embed youtube card', () => {
  ...
  test(scoped`change youtube card style`, async ({ page }) => {
    ...
    await createBookmarkBlockBySlashMenu(page, YOUTUBE_URL);
    const youtube = page.locator('notesgraph-embed-youtube-block');
    await youtube.click();
    await page.waitForTimeout(100);

    // change to card view
    const embedToolbar = page.locator(
      'notesgraph-toolbar-widget editor-toolbar'
    );
...
test.describe('embed figma card', () => {
  ...
  test(scoped`change figma card style`, async ({ page }) => {
    ...
    await createBookmarkBlockBySlashMenu(page, FIGMA_URL);
    const youtube = page.locator('notesgraph-embed-figma-block');
    await youtube.click();
    await page.waitForTimeout(100);
...
test.describe('embed github card', () => {
  test('github embed card should show banner in edgeless mode', async ({
    ...
    const banner = page.locator('.notesgraph-embed-github-banner');
    await expect(banner).toBeVisible();
  });
});
...
test('drag a card from canvas to note should not change the style of the card', async ({
  ...
  const url = 'https://github.com/notesgraph/notesgraph/pull/12660';
  ...
  const edgelessBookmark = page.locator('notesgraph-edgeless-bookmark');
  await edgelessBookmark.click();
  await waitNextFrame(page);
  const dragHandle = page.locator('.notesgraph-drag-handle-container');
  await dragHandle.dragTo(page.locator('notesgraph-edgeless-note'));

  const noteBookmark = page.locator('notesgraph-bookmark');
