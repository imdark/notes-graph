...
function getAttachment(page: Page) {
  const attachment = page.locator('notesgraph-attachment');
  const loading = attachment.locator('.notesgraph-attachment-card.loading');
  ...
  const renameInput = page.locator(
    '.notesgraph-attachment-rename-container input'
  );
  ...
  const getName = () =>
    attachment.locator('.notesgraph-attachment-content-title-text').innerText();
  ...
  return {
    ...
    getName,
    getSize: () =>
      attachment.locator('.notesgraph-attachment-content-info').innerText(),
...
test('should undo/redo works for attachment', async ({ page }, testInfo) => {
  ...
  await page.locator('notesgraph-attachment').waitFor({ state: 'detached' });
...
test(`support dragging attachment block directly`, async ({
  ...
  const attachmentBlock = page.locator('notesgraph-attachment');
  ...
  const rects = page.locator('.notesgraph-attachment-container.focused');
...
test('indent attachment block to paragraph', async ({ page }) => {
  ...
  await assertBlockFlavour(page, '4', 'notesgraph:attachment');
...
test('indent attachment block to list', async ({ page }) => {
  ...
  await assertBlockFlavour(page, '5', 'notesgraph:attachment');
