...
test.describe('linked page popover', () => {
  ...
  test('should multiple paste query not works', async ({ page }) => {
    ...
    await waitNextFrame(page, 200);
    await cutByKeyboard(page);
    const note = page.locator('notesgraph-note');
...
test('add reference node before the other reference node', async ({ page }) => {
  ...
  const firstRefNode = page.locator('notesgraph-reference').nth(0);
...
test('linked doc can be dragged from note to surface top level block', async ({
  ...
  await waitNextFrame(page);
  await assertParentBlockFlavour(page, '9', 'notesgraph:surface');
});

// TODO(@fundon): should move to notesgraph
// Aliases
test.describe.skip('Customize linked doc title and description', () => {
  ...
  test('should set a custom title and description for card link', async ({
    ...
    await expect(linkedDocBlockTitle).toHaveText('page0-title0');
    await expect(
      page.locator('.notesgraph-embed-linked-doc-content-note.alias')
  ...
  test('should automatically switch to card view and set a custom title and description', async ({
    ...
    const referencePopup = page.locator(
      '.notesgraph-reference-popover-container'
    );
    ...
    await waitNextFrame(page, 200);
    const cardToolbar = page.locator('notesgraph-embed-card-toolbar');
    ...
    await waitNextFrame(page, 200);
    const syncedDocBlock = page.locator('notesgraph-embed-synced-doc-block');
    ...
    await expect(linkedDocBlockTitle).toHaveText('page0-title0');
    await expect(
      linkedDocBlock.locator('.notesgraph-embed-linked-doc-content-note.alias')
    ).toHaveText('This is a new description');
  });
