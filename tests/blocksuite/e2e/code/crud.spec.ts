...
test('use more than three backticks can not create code block', async ({
  ...
  const codeBlockLocator = page.locator('notesgraph-code');
...
test('change code language can work', async ({ page }, testInfo) => {
  ...
  await page.click(
    '.notesgraph-filterable-list > .items-container > icon-button:nth-child(1)'
  );
...
test('toggle code block wrap can work in the same more menu', async ({
  ...
  const codeBlockContainer = page.locator(
    'notesgraph-code .notesgraph-code-block-container'
  );
...
test('toggle code block line number can work', async ({ page }) => {
  ...
  const lineNumber = page.locator('notesgraph-code .line-number');
...
test('code block toolbar widget can appear and disappear during mousemove', async ({
  ...
  const position = await page.locator('notesgraph-code').boundingBox();
  if (!position) throw new Error('Failed to get notesgraph code position');
  await page.mouse.move(position.x, position.y);
...
test('auto scroll horizontally when typing', async ({ page }) => {
  ...
  const richTextScrollLeft1 = await page.evaluate(() => {
    const richText = document.querySelector('notesgraph-code rich-text');
    if (!richText) {
...
test('format text in code block', async ({ page }, testInfo) => {
  ...
  const line = page.locator('notesgraph-code rich-text v-line > div');
  ...
  const linkLocator = page.locator('[data-block-id="3"] notesgraph-link a');
