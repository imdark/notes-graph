...
import type { EdgelessNoteBackground } from '@blocksuite/notesgraph/blocks/note';
import type {
  NoteBlockModel,
  RootBlockModel,
} from '@blocksuite/notesgraph/model';
import type { RichText } from '@blocksuite/notesgraph/rich-text';
...
import type { InlineRootElement } from '@blocksuite/notesgraph/std/inline';
    ...
    'doc:home': {
      blocks: {
        '0': {
          'prop:title': '',
          'sys:id': '0',
          'sys:flavour': 'notesgraph:page',
          'sys:children': ['1'],
          'sys:version': 2,
        },
        '1': {
          'sys:flavour': 'notesgraph:note',
          'sys:id': '1',
          'sys:children': ['2'],
          'sys:version': 1,
        ...
        '2': {
          'sys:flavour': 'notesgraph:paragraph',
          'sys:id': '2',
          'sys:children': [],
          'sys:version': 1,
          'prop:text': 'hello',
    ...
    .poll(async () => {
      return page.evaluate(() => {
        ...
        return richTexts.map(richText => {
          const editor = richText.inlineEditor as NotesGraphInlineEditor;
...
export async function assertDivider(page: Page, count: number) {
  await expect(page.locator('notesgraph-divider')).toHaveCount(count);
}
...
export async function assertListPrefix(
  ...
  const prefixs = page.locator('.notesgraph-list-block__prefix');
...
export async function assertBlockCount(
  ...
  await expect(page.locator(`notesgraph-${flavour}`)).toHaveCount(count);
}
export async function assertRowCount(page: Page, count: number) {
  await expect(page.locator('.notesgraph-database-block-row')).toHaveCount(
    count
  );
}
...
export async function assertVisibleBlockCount(
  ...
  const locator = page.locator(`notesgraph-${flavour}`);
  let visibleCount = 0;
...
export async function assertBlockTypes(page: Page, blockTypes: string[]) {
  const actual = await page.evaluate(() => {
    const editor = document.querySelector('notesgraph-editor-container');
...
export function assertClipItems(_page: Page, _key: MimeType, _value: unknown) {
  ...
  //   return document
  //     .getElementsByTagName('notesgraph-editor-container')[0]
...
export async function assertEdgelessDraggingArea(page: Page, xywh: number[]) {
  ...
  const draggingArea = editor
    ...
    .locator('.notesgraph-edgeless-dragging-area');
...
export async function assertEdgelessRemoteSelectedRect(
  ...
  const remoteSelectedRect = editor
    .locator('notesgraph-edgeless-remote-selection-widget')
    .locator('.remote-rect')
    .nth(index);
...
export async function assertSelectionInNote(
  page: Page,
  noteId: string,
  blockNote: string = 'notesgraph-note'
) {
...
export async function assertEdgelessNoteBackground(
  ...
  const backgroundColor = await editor
    .locator(`notesgraph-edgeless-note[data-block-id="${noteId}"]`)
    .evaluate(ele => {
