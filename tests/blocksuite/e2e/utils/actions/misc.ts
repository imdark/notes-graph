...
import type {
  InlineRange,
  InlineRootElement,
} from '@blocksuite/notesgraph/std/inline';
import type { BlockModel } from '@blocksuite/notesgraph/store';
import { uuidv4 } from '@blocksuite/notesgraph/store';
...
export const getEditorLocator = (page: Page) => {
  return page.locator('notesgraph-editor-container').nth(currentEditorIndex);
};
...
export async function enterPlaygroundRoom(
  ...
  const locator = page.locator('notesgraph-editor-container');
  await locator.isVisible();
  await page.evaluate(async () => {
    const dom = document.querySelector<TestNotesGraphEditorContainer>(
...
export async function waitDefaultPageLoaded(page: Page) {
  await page.waitForSelector('notesgraph-page-root[data-block-id="0"]');
}
...
export async function initEmptyParagraphState(page: Page, rootId?: string) {
  const ids = await page.evaluate(rootId => {
    ...
    doc.captureSync();
    if (!rootId) {
      rootId = doc.addBlock('notesgraph:page', {
        title: new window.$blocksuite.store.Text(),
      });
    }
    ...
    // doc.addBlock('notesgraph:surface', {}, rootId);
    doc.captureSync();
...
export async function initDatabaseDynamicRowWithData(
  ...
  const lastRow = editorHost.locator('.notesgraph-database-block-row').last();
...
export async function focusDatabaseTitle(page: Page) {
  ...
  await page.evaluate(() => {
    const dbTitle = document.querySelector(
      'notesgraph-database-title textarea'
...
export async function assertDatabaseColumnOrder(page: Page, order: string[]) {
  const columns = await page
    .locator('notesgraph-database-column-header')
    .locator('notesgraph-database-header-column')
    .all();
...
export async function initEmptyCodeBlockState(
  ...
  const ids = await page.evaluate(codeBlockProps => {
    ...
    const rootId = doc.addBlock('notesgraph:page');
    const noteId = doc.addBlock('notesgraph:note', {}, rootId);
    const codeBlockId = doc.addBlock('notesgraph:code', codeBlockProps, noteId);
    doc.captureSync();
...
export async function focusRichText(
  ...
  // need to set `force` to true when clicking on `notesgraph-selected-blocks`
...
export async function getCurrentEditorDocId(page: Page) {
  return page.evaluate(index => {
    const editor = document.querySelectorAll('notesgraph-editor-container')[
      index
    ];
    if (!editor) throw new Error("Can't find notesgraph-editor-container");
...
export async function getCurrentEditorTheme(page: Page) {
  const mode = await page
    .locator('notesgraph-editor-container')
    .first()
    .evaluate(() =>
      window
        .getComputedStyle(document.documentElement)
        .getPropertyValue('--notesgraph-theme-mode')
        .trim()
    );
  return mode;
}
...
export async function scrollToTop(page: Page) {
  const scrollContainer = page.locator('.notesgraph-page-viewport');
