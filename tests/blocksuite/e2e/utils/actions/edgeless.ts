...
import { ConnectorTool } from '@blocksuite/notesgraph/gfx/connector';
import { ShapeTool } from '@blocksuite/notesgraph/gfx/shape';
import type { IPoint, IVec } from '@blocksuite/notesgraph/global/gfx';
import { sleep } from '@blocksuite/notesgraph/global/utils';
import type {
  ConnectorElementModel,
  NoteBlockModel,
  NoteDisplayMode,
} from '@blocksuite/notesgraph/model';
import type { ToolOptions } from '@blocksuite/notesgraph/std/gfx';
...
export async function getNoteRect(page: Page, noteId: string) {
  const xywh: string | null = await page.evaluate(
    ([noteId]) => {
      ...
      if (block?.flavour === 'notesgraph:note') {
...
export async function switchMultipleEditorsMode(page: Page) {
  await page.evaluate(() => {
    const containers = document.querySelectorAll('notesgraph-editor-container');
...
export function locatorEdgelessComponentToolButton(
  ...
  const button = page
    .locator('notesgraph-toolbar-widget editor-toolbar editor-icon-button')
    .filter({
      hasText: text,
    });
...
export async function assertEdgelessShapeType(page: Page, type: ShapeName) {
  const curType = await page.evaluate(() => {
    const container = document.querySelector('notesgraph-edgeless-root');
    if (!container) {
...
export async function getEdgelessBlockChild(page: Page) {
  const block = page.locator('notesgraph-edgeless-note');
...
export async function getEdgelessSelectedRect(page: Page) {
  const selectedBox = await page.evaluate(() => {
    const selected = document
      ...
      ?.shadowRoot?.querySelector('.notesgraph-edgeless-selected-rect');
    if (!selected) {
...
export async function rotateElementByHandle(
  ...
  const rect = await page
    .locator('.notesgraph-edgeless-selected-rect')
    .boundingBox();
...
export async function pickColorAtPoints(page: Page, points: number[][]) {
  const pickedColors: `#${string}`[] = await page.evaluate(points => {
    const node = document.querySelector(
      '.notesgraph-edgeless-surface-block-container canvas'
...
export async function getNoteBoundBoxInEdgeless(page: Page, noteId: string) {
  ...
  const note = editor.locator(
    `notesgraph-edgeless-note[data-block-id="${noteId}"]`
  );
...
export async function getAllNoteIds(page: Page) {
  return page.evaluate(() => {
    return Array.from(document.querySelectorAll('notesgraph-note')).map(
      note => note.model.id
    );
  });
}
...
export async function getAllEdgelessNoteIds(page: Page) {
  return page.evaluate(() => {
    return Array.from(
      document.querySelectorAll('notesgraph-edgeless-note')
    ).map(note => note.model.id);
  });
}
...
export async function getAllEdgelessTextIds(page: Page) {
  return page.evaluate(() => {
    return Array.from(
      document.querySelectorAll('notesgraph-edgeless-text')
    ).map(text => text.model.id);
  });
}
...
export function locatorScalePanelButton(page: Page, scale: number) {
  return page
    .locator('notesgraph-size-dropdown-menu')
    .getByLabel(String(scale));
}
...
export async function openComponentToolbarMoreMenu(page: Page) {
  const btn = page
    .locator('notesgraph-toolbar-widget editor-toolbar')
...
export async function multiTouchDown(page: Page, points: Point[]) {
  await page.evaluate(points => {
    const target = document.querySelector('notesgraph-edgeless-root');
    if (!target) {
      throw new Error('Missing edgeless page');
    }
...
export function locatorComponentToolbar(page: Page) {
  return page.locator('notesgraph-toolbar-widget editor-toolbar');
}
...
export async function resizeConnectorByStartCapitalHandler(
  ...
  const handler = page.locator(
    '.notesgraph-edgeless-selected-rect .line-controller.line-start'
  );
...
export function getFrameTitle(page: Page, frame: string) {
  return page.locator(`notesgraph-frame-title[data-id="${frame}"]`);
}

export async function selectElementInEdgeless(page: Page, elements: string[]) {
  await page.evaluate(
    ({ elements }) => {
      const edgelessBlock = document.querySelector('notesgraph-edgeless-root');
      if (!edgelessBlock) {
