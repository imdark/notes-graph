...
export function getLinkedDocPopover(page: Page) {
  ...
  const refNode = page.locator('notesgraph-reference');
  ...
  const findRefNode = async (title: string) => {
    const refNode = page.locator(`notesgraph-reference`, {
      has: page.locator(`.notesgraph-reference-title[data-title="${title}"]`),
    });
  ...
  const assertExistRefText = async (text: string) => {
    await expect(refNode).toBeVisible();
    const refTitleNode = refNode.locator('.notesgraph-reference-title');
