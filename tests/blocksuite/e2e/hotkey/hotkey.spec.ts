...
test('should support ctrl/cmd+shift+l convert to linked doc', async ({
  ...
  const linkedDocCard = page.locator('notesgraph-embed-linked-doc-block');
  await expect(linkedDocCard).toBeVisible();

  const title = page.locator('.notesgraph-embed-linked-doc-content-title-text');
  expect(await title.innerText()).toBe('Untitled');

  const noteContent = page.locator('.notesgraph-embed-linked-doc-content-note');
