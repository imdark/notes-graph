...
test(scoped`pasting internal url`, async ({ page }) => {
  ...
  await expect(page.locator('notesgraph-reference')).toContainText('test page');
});
...
test(
  ...
  async ({ page }) => {
    ...
    await pasteContent(page, {
      'text/plain': 'https://notesgraph.com/',
    });
    await expect(page.locator('notesgraph-link')).toContainText('alias');
  }
);
...
test(
  ...
  async ({ page }) => {
    ...
    await expect(page.locator('notesgraph-reference')).toContainText('alias');
  }
);
