...
test('basic link', async ({ page }, testInfo) => {
  ...
  const createLinkPopoverLocator = page.locator(
  ...
  const editLinkPopoverLocator = page.locator('.notesgraph-link-edit-popover');
  ...
  await page.keyboard.press('Tab');
  await pressEnter(page);
  const link2Locator = page.locator('notesgraph-link a');
...
test('link bar should not be appear when the range is collapsed', async ({
  ...
  await pressCreateLinkShortCut(page);
  const linkPopoverLocator = page.locator('.notesgraph-link-popover');
...
test('create link with paste', async ({ page }, testInfo) => {
  ...
  const confirmBtn = createLinkPopoverLocator.locator(
  ...
  await type(page, 'notesgraph.com');
