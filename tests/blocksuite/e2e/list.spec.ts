...
test('add new todo list', async ({ page }) => {
  ...
  const checkBox = page.locator('.notesgraph-list-block__prefix');
  await expect(page.locator('.notesgraph-list--checked')).toHaveCount(0);
  await checkBox.click();
  await expect(page.locator('.notesgraph-list--checked')).toHaveCount(1);
  await checkBox.click();
  await expect(page.locator('.notesgraph-list--checked')).toHaveCount(0);
});
...
test('add new toggle list', async ({ page }) => {
  ...
  await focusRichText(page, 0);
  await updateBlockType(page, 'notesgraph:list', 'toggle');
  await focusRichText(page, 0);
  await type(page, 'top');
  await pressTab(page);
  await pressEnter(page);
...
test('convert to numbered list block', async ({ page }) => {
  ...
  await updateBlockType(page, 'notesgraph:list', 'bulleted'); // replaced 2 to 3
  await waitNextFrame(page);
  await updateBlockType(page, 'notesgraph:list', 'numbered');
  await focusRichText(page, 0);

  const listSelector = '.notesgraph-list-rich-text-wrapper';
...
test.describe('toggle list', () => {
  test('click toggle icon should collapsed list', async ({
    ...
    const prefixes = page.locator('.notesgraph-list-block__prefix');
    const listChildren = page
      .locator('[data-block-id="4"] .notesgraph-block-children-container')
      .nth(0);
...
test.describe('readonly', () => {
  ...
  test('should render collapsed list correctly', async ({ page }) => {
    ...
    const toggleIcon = page.locator('.toggle-icon');
    const listChildren = page
      .locator('[data-block-id="5"] .notesgraph-block-children-container')
      .nth(0);
