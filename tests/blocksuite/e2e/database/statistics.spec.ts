...
const insertRightColumn = async (page: Page, index = 0) => {
  await waitNextFrame(page);
  await page.locator('notesgraph-database-header-column').nth(index).click();
  await waitNextFrame(page, 200);
  await pressKey(page, 'Escape');
  const menu = page.locator('.notesgraph-menu-button', {
...
test.describe('title', () => {
  test('empty count', async ({ page }) => {
    ...
    await addRow(page, 3);
    const statCell = page
      .locator('notesgraph-database-column-stats-cell')
      .nth(0);
