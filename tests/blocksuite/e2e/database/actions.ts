import type { RichTextCell } from '@blocksuite/notesgraph/blocks/database';
...
export async function initDatabaseColumn(page: Page, title = '') {
  ...
  await editor.locator('notesgraph-data-view-table-group').first().hover();
...
export const renameColumn = async (page: Page, name: string) => {
  const column = page.locator('notesgraph-database-header-column', {
    hasText: name,
  });
  await column.click();
};
...
export async function performColumnAction(
  ...
  const actionMenu = page.locator(`.notesgraph-menu-button`, {
    hasText: action,
  });
...
export function clickColumnType(page: Page, columnType: string) {
  const typeMenu = page.locator(`.notesgraph-menu-button`, {
...
export function getDatabaseBodyRows(page: Page) {
  const rowContainer = page.locator('.notesgraph-database-block-rows');
...
export async function performSelectColumnTagAction(
  ...
  await page
    .locator('.notesgraph-menu-button', { hasText: new RegExp(name) })
    .click();
}
...
export async function assertDatabaseCellRichTexts(
  ...
  const cell = cellContainer.locator('notesgraph-database-rich-text-cell');
...
export async function assertDatabaseCellNumber(
  ...
  const actualText = await page
    .locator('.notesgraph-database-block-rows')
...
export async function assertDatabaseCellLink(
  ...
  const actualTexts = await page.evaluate(
    ({ rowIndex, columnIndex }) => {
      const rows = document.querySelector('.notesgraph-database-block-rows');
...
export async function focusDatabaseSearch(page: Page) {
  ...
  const count = await searchExpand.count();
  if (count === 1) {
    const input = page.locator('.notesgraph-database-search-input');
    await input.click();
  } else {
    const searchIcon = page.locator('.notesgraph-database-search-input-icon');
...
export async function focusDatabaseHeader(page: Page, columnIndex = 0) {
  const column = page.locator('.notesgraph-database-column').nth(columnIndex);
...
export async function getDatabaseHeaderColumn(page: Page, index = 0) {
  const column = page.locator('.notesgraph-database-column').nth(index);
  const box = await getBoundingBox(column);
  const textElement = column.locator('.notesgraph-database-column-text-input');
  const text = await textElement.innerText();
  const typeIcon = column.locator('.notesgraph-database-column-type-icon');
...
export async function assertKanbanCellSelected(
  ...
  const border = await page.evaluate(
    ({ groupIndex, cardIndex, cellIndex }) => {
      const group = document.querySelector(
        `notesgraph-data-view-kanban-group:nth-child(${groupIndex + 1})`
      );
      const card = group?.querySelector(
        `notesgraph-data-view-kanban-card:nth-child(${cardIndex + 1})`
      );
  ...
  expect(border).toEqual('1px solid var(--notesgraph-primary-color)');
}
...
export function getKanbanCard(
  ...
  const group = page
    ...
    .nth(groupIndex);
  const card = group.locator('notesgraph-data-view-kanban-card').nth(cardIndex);
  return card;
}
...
export const changeColumnType = async (
  ...
  await waitNextFrame(page);
  await page.locator('notesgraph-database-header-column').nth(column).click();
  await waitNextFrame(page, 200);
