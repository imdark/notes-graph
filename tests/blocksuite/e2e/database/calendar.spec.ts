  ...
  return page.evaluate(
    ({
      ...
      if (linkedDocId && linkedDocTitle) {
        ...
        linkedDoc.load();
        const linkedRootId = linkedDoc.addBlock('notesgraph:page', {
          title: new window.$blocksuite.store.Text(linkedDocTitle),
        });
        linkedDoc.addBlock('notesgraph:surface', {}, linkedRootId);
        linkedDoc.addBlock('notesgraph:note', {}, linkedRootId);
      }
...
test(scoped`database calendar setup and row interactions`, async ({ page }) => {
  ...
  await expect(
    page.locator('notesgraph-data-view-record-detail')
  ).toBeVisible();
  ...
  async ({ page }) => {
    ...
    await page.evaluate(({ databaseId }) => {
      const database = document.querySelector(
        `notesgraph-database[data-block-id="${databaseId}"]`
      ) as any;
