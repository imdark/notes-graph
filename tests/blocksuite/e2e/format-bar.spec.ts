...
test('should format quick bar show when select text', async ({ page }) => {
  ...
  const noteEl = page.locator('notesgraph-note');
...
test('should format quick bar show when clicking drag handle', async ({
  ...
  const locator = page.locator('notesgraph-paragraph').first();
  await locator.hover();
  const dragHandle = page.locator('.notesgraph-drag-handle-grabber');
...
test('should format quick bar be able to change background color', async ({
  ...
  await assertRichTextInlineDeltas(
    ...
    [
      {
        attributes: {
          color: 'var(--notesgraph-text-highlight-foreground-red)',
        },
        insert: '456',
      },
    ],
  ...
  // await expect(highlight.highlightBtn).toHaveAttribute(
  //   'data-last-used',
  //   'var(--notesgraph-text-highlight-foreground-red)'
  // );
...
test('should format quick bar be able to link text', async ({
  ...
  const linkPopoverInput = page.locator('.notesgraph-link-popover-input');
  ...
  const linkLocator = page.locator('notesgraph-link a');
...
test('should format bar follow scroll', async ({ page }) => {
  ...
  await scrollToTop(page);
  await assertLocatorVisible(page, formatBar);
  await updateBlockType(page, 'notesgraph:list', 'bulleted');
  await scrollToBottom(page);
  await assertLocatorVisible(page, formatBar, false);
});
...
test('should format quick bar position correct at the start of second line', async ({
  ...
  await page.evaluate(() => {
    ...
    const paragraphId = doc.addBlock('notesgraph:paragraph', { text }, note);
    return paragraphId;
  });
  // await focusRichText(page);
  const editorHost = getEditorHostLocator(page);
...
test('should format bar style active correctly', async ({ page }) => {
  ...
  await page.evaluate(() => {
    ...
    doc.addBlock('notesgraph:paragraph', { text }, note);
  });
...
test('should the database action icon show correctly', async ({ page }) => {
  ...
  await pressEnter(page);
  await updateBlockType(page, 'notesgraph:code');
  const codeBlock = page.locator('notesgraph-code');
...
test('should convert to database work', async ({ page }) => {
  ...
  const rows = page.locator('.notesgraph-database-block-row');
...
test('create linked doc from block selection with format bar', async ({
  ...
  const linkedDocBlock = page.locator('notesgraph-embed-linked-doc-block');
