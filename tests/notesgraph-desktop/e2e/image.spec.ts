...
import { pasteByKeyboard } from '@notesgraph-test/kit/utils/keyboard';
import {
  clickNewPageButton,
...
test('should be able to insert SVG images', async ({ page }) => {
  ...
  await importImage(page, 'notesgraph.svg');

  const svg = page.locator('notesgraph-image').first();
...
test('should paste it as PNG after copying SVG', async ({ page }) => {
  ...
  const png = page.locator('notesgraph-image').nth(1);
