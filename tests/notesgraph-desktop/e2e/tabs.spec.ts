import { test } from '@notesgraph-test/kit/electron';
import {
  closeTab,
  expectActiveTab,
  expectTabCount,
  expectTabTitle,
} from '@notesgraph-test/kit/utils/app-tabs';
import {
  clickNewPageButton,
  createLinkedPage,
  dragTo,
...
test('open new tab via cmd+click page link', async ({ page }) => {
  ...
  await page
    .locator('.notesgraph-reference-title:has-text("hi from another page")')
    .click({
...
test('reorder tabs', async ({ page }) => {
  ...
  await page
    .locator(`.notesgraph-reference-title:has-text("${titles[0]}")`)
    .click({
      modifiers: ['ControlOrMeta'],
    });
  await page
    .locator(`.notesgraph-reference-title:has-text("${titles[1]}")`)
    .click({
      modifiers: ['ControlOrMeta'],
    });
