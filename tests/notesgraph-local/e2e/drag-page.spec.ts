...
import {
  ...
  getDocIdFromUrl,
} from '@notesgraph-test/kit/utils/url';
...
test('drag a page link in editor to favourites', async ({ page }) => {
  ...
  const pageReference = page.locator('a').filter({
    has: page.locator(
      '.notesgraph-reference-title:has-text("hi from another page")'
    ),
  });
...
test('drag a page card block to another page', async ({ page }) => {
  ...
  await dragToFavourites(
    page,
    page.locator('.notesgraph-drag-handle-container'),
    pageId
  );
});
...
test('drag a favourite page into note on page mode', async ({ page }) => {
  ...
  await dragTo(
    page,
    item,
    page.locator('.notesgraph-paragraph-block-container').first()
  );

  await expect(page.locator('notesgraph-embed-linked-doc-block')).toContainText(
    'hi from page'
  );
});
...
test('drag a favourite page into canvas on edgeless mode', async ({ page }) => {
  ...
  // drag item into blocksuite editor
  await dragTo(page, item, page.locator('notesgraph-edgeless-root').first());

  const embedDoc = page.locator('notesgraph-embed-edgeless-synced-doc-block');
  ...
  const cardDoc = page.locator('notesgraph-embed-edgeless-linked-doc-block');
...
test('drag a favourite page into note on edgeless mode', async ({ page }) => {
  ...
  const edgelessNote = page.locator('notesgraph-edgeless-note');
  await dragTo(page, item, edgelessNote);

  const embedDocInNote = edgelessNote.locator(
    'notesgraph-embed-synced-doc-block'
  );
  const cardDocInNote = edgelessNote.locator(
