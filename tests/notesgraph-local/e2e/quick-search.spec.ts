...
import { clickEdgelessModeButton } from '@notesgraph-test/kit/utils/editor';
import {
  copyByKeyboard,
  pasteByKeyboard,
  selectAllByKeyboard,
...
import { clickSideBarAllPageButton } from '@notesgraph-test/kit/utils/sidebar';
...
async function assertTitle(page: Page, text: string) {
  const edgeless = page.locator('notesgraph-edgeless-root');
  if (!edgeless) {
...
test('assert the recent browse pages are on the recent list', async ({
  ...
  await waitForEditorLoad(page);
  {
    ...
    await title.click();
    await page.waitForTimeout(200);
    await title.pressSequentially('notesgraph is the best', { delay: 100 });
    await expect(title).toHaveText('notesgraph is the best', { timeout: 500 });
  }
  await page.waitForTimeout(1000);
  await openQuickSearchByShortcut(page);
  {
    ...
    await expect(quickSearchItems.nth(0)).toHaveText('notesgraph is the best');
  }
});
...
test('can use cmdk to search page content and scroll to it, then the block will be selected', async ({
  ...
  const textBlock = page
    .locator('[data-notesgraph-editor-container]')
    .getByText('123456');
  ...
  const isVisitable = await checkElementIsInView(
    page,
    page.locator('[data-notesgraph-editor-container]').getByText('123456')
  );
  expect(isVisitable).toBe(true);
  const selectionElement = page.locator(
    'notesgraph-scroll-anchoring-widget div.highlight'
  );
...
test('disable quick search when the link-popup is visitable', async ({
  ...
  await withCtrlOrMeta(page, () => page.keyboard.press('k'));
  const linkPopup = page.locator('.notesgraph-link-popover');
...
test('can use @ to open quick search to search for doc and insert into canvas', async ({
  ...
  await page.locator('notesgraph-edgeless-root').press('@');
  ...
  await page.keyboard.press('Enter');
  await expect(
    page.locator('notesgraph-embed-edgeless-synced-doc-block')
  ).toBeVisible();
  await expect(
...
test('can paste a doc link to create link reference', async ({ page }) => {
  ...
  await expect(
    page.locator('notesgraph-reference:has-text("Getting Started")')
  ).toBeVisible();
  ...
  await expect(
    page.locator(`notesgraph-link:has-text("${url}")`)
  ).toBeVisible();
});
...
test('can use slash menu to insert an external link', async ({ page }) => {
  ...
  const link = 'notesgraph.com';
  await page.locator('[cmdk-input]').fill(link);

  const insertLinkBtn = page.locator(
    '[cmdk-item] [data-value="external-link:notesgraph.com"]'
  );
  ...
  await expect(page.locator('notesgraph-bookmark')).toBeVisible();
  await expect(page.locator('.notesgraph-bookmark-content-url')).toContainText(
    link
  );
});
