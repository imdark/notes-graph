...
test('image able to copy to clipboard', async ({ page }) => {
  ...
  await expect(
    page.locator(
      '[data-testid=notesgraph-toast]:has-text("Copied to clipboard.")'
    )
  ).toBeVisible();
});
...
test('tooltips for all buttons should be visible when hovering', async ({
  ...
  let blobId: string;
  {
    ...
    await page.keyboard.press('Enter');
    await importImage(page, 'large-image.png');
    await page.locator('notesgraph-page-image').first().click();
    await page.keyboard.press('Enter');
    await importImage(page, 'large-image.png');
    await page.locator('notesgraph-page-image').first().dblclick();
    await page.waitForTimeout(500);
...
test('caption should be visible and different styles were applied if image zoomed larger than viewport', async ({
  ...
  const sampleCaption = 'notesgraph owns me and all';
  await openHomePage(page);
  await waitForEditorLoad(page);
  await clickNewPageButton(page);
  const title = getBlockSuiteEditorTitle(page);
  ...
  await page.keyboard.press('Enter');
  await importImage(page, 'large-image.png');
  await page.locator('notesgraph-page-image').first().hover();
