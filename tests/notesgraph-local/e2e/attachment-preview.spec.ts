...
test('attachment preview should be shown', async ({ page }) => {
  ...
  await page.locator('notesgraph-attachment').first().dblclick();
...
test('should preview PDF in embed view', async ({ page }) => {
  ...
  // Title alias
  {
    ...
    const inlineLink = page.locator('notesgraph-reference').nth(0);
    const inlineTitle = inlineLink.locator('.notesgraph-reference-title');
  ...
  {
    const inlineLink = page.locator('notesgraph-reference').nth(1);
    const inlineTitle = inlineLink.locator('.notesgraph-reference-title');
...
test('should sync name in pdf embed view', async ({ page }) => {
  ...
  const input = page
    .locator('.notesgraph-attachment-rename-input-wrapper')
    .locator('input');
...
test('should enable pointer event in pdf viewer', async ({ page }) => {
  ...
  const attachmentSelection = attachment.locator(
    '.notesgraph-attachment-container.focused'
  );
  ...
  await attachment
    .locator('.notesgraph-attachment-embed-container')
