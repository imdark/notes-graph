...
import {
  openHomePage,
  openJournalsPage,
} from '@notesgraph-test/kit/utils/load-page';
import {
  addDatabase,
  addDatabaseRow,
  clickNewPageButton,
...
} from '@notesgraph-test/kit/utils/properties';
...
test('can show database backlink info', async ({ page }) => {
  ...
  await expect(page.locator('notesgraph-database-title')).toContainText(
    databaseTitle
  );

  await expect(
    page.locator(`notesgraph-database-title:has-text("${databaseTitle}")`)
  ).toBeVisible();
  ...
  await page
    .locator('.notesgraph-reference-title:has-text("linked page")')
    .click();
  ...
  await expect(
    linkedDatabaseSection.locator(
      `.notesgraph-reference-title:has-text("${pageTitle}")`
    )
  ).toBeVisible();
