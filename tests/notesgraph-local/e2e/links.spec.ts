import { toolbarButtons } from '@notesgraph-test/kit/bs/linked-toolbar';
import { waitNextFrame } from '@notesgraph-test/kit/bs/misc';
...
} from '@notesgraph-test/kit/utils/keyboard';
import { coreUrl, openHomePage } from '@notesgraph-test/kit/utils/load-page';
import {
  clickNewPageButton,
  createLinkedPage,
  createTodayPage,
...
test('not allowed to switch to embed view when linking to the same document', async ({
  ...
  // Inline
  const inlineLink = page.locator('notesgraph-reference');
  ...
  // Card
  const cardLink = page.locator('notesgraph-embed-linked-doc-block');
...
test('not allowed to switch to embed view when linking to block', async ({
  ...
  const otherCardLink = page
    .locator('notesgraph-embed-linked-doc-block')
    .nth(1);
...
test('allow switching to embed view when linking to the other document without mode', async ({
  ...
  // Embed
  const embedLink = page.locator('notesgraph-embed-synced-doc-block');
  ...
  await switchViewBtn.click();
  await expect(
    page.locator('.notesgraph-embed-synced-doc-container.page')
  ).toBeVisible();
...
test('allow switching to embed view when linking to the other document with mode', async ({
  ...
  await switchViewBtn.click();
  await expect(
    page.locator('.notesgraph-embed-synced-doc-container.edgeless')
  ).toBeVisible();
...
test('@ popover should show today menu item', async ({ page }) => {
  ...
  // a notesgraph-reference should be created with name date
  await expect(
    page.locator('notesgraph-reference:has-text("' + date + '")')
  ).toBeVisible();
});
...
test('@ popover with input=tmr', async ({ page }) => {
  ...
  // a notesgraph-reference should be created with name date
  await expect(
    page.locator('notesgraph-reference:has-text("' + textContent + '")')
  ).toBeVisible();
});
...
test('linked doc should show markdown preview in the backlink section', async ({
  ...
  await createLinkedPage(page, 'Test Page');
  await page.locator('notesgraph-reference:has-text("Test Page")').click();
...
test('the viewport should be fit when the linked document is with edgeless mode', async ({
  ...
  await page.waitForSelector('notesgraph-edgeless-text');
  ...
  // Inline
  await page.locator('notesgraph-reference').hover();
  await page.getByLabel('Switch view').click();
  await page.getByTestId('link-to-embed').click();

  const viewport = await page
    .locator('notesgraph-embed-synced-doc-block')
    .boundingBox();
  expect(viewport).not.toBeNull();
  if (!viewport) return;

  const edgelessText = await page
    .locator('notesgraph-embed-synced-doc-block notesgraph-edgeless-text')
    .boundingBox();
...
test('should show edgeless content when switching card view of linked mode doc in edgeless', async ({
  ...
  await page.locator('notesgraph-edgeless-text').waitFor({ state: 'visible' });
  await page.keyboard.type('Edgeless Text');
  ...
  // Inline
  const embed = page.locator('notesgraph-embed-edgeless-linked-doc-block');
  await expect(embed).toBeVisible();
  await page.getByLabel('Switch view').click();
  await page.getByTestId('link-to-embed').click();

  const viewport = await page
    .locator('notesgraph-embed-edgeless-synced-doc-block')
    .boundingBox();
  expect(viewport).not.toBeNull();
  if (!viewport) return;
  ...
  const edgelessText = await page
    .locator(
      'notesgraph-embed-edgeless-synced-doc-block notesgraph-edgeless-text'
    )
    .boundingBox();
  expect(edgelessText).not.toBeNull();
  if (!edgelessText) return;
...
test.describe('Customize linked doc title and description', () => {
  ...
  test('should set a custom title for inline link', async ({ page }) => {
    ...
    const link = page.locator('notesgraph-reference');
    const title = link.locator('.notesgraph-reference-title');
  ...
  test('should set a custom title and description for card link', async ({
    ...
    const cardTitle = cardLink.locator(
      '.notesgraph-embed-linked-doc-content-title-text'
    );
    const cardDescription = cardLink.locator(
      '.notesgraph-embed-linked-doc-content-note.alias'
    );
    ...
    // Switches to inline view
    {
      ...
      const title = inlineLink.locator('.notesgraph-reference-title');
  ...
  //   await expect(a.locator('.notesgraph-reference-title')).toHaveText('hello');
  // });
  ...
  //     await expect(a.locator('svg')).toBeHidden();
  //     await expect(a.locator('.notesgraph-reference-title')).toContainText(year);
  //   });
});
...
test('should show full email address', async ({ page }) => {
  ...
  await writeTextToClipboard(page, 'dev@notesgraph.com');

  const inlineLink = page.locator('notesgraph-link');

  await expect(inlineLink).toHaveText('dev@notesgraph.com');
  ...
  await expect(toolbar.locator('notesgraph-link-preview')).toHaveText(
    'dev@notesgraph.com'
  );
});
...
test('should not show view toggle button when protocol of link is not http(s)', async ({
  ...
  await writeTextToClipboard(page, 'ftp://notesgraph.com/blocksuite.pdf');

  const inlineLink = page.locator('notesgraph-link');

  await expect(inlineLink).toHaveText('ftp://notesgraph.com/blocksuite.pdf');
...
test('should reach target block when clicking notesgraph-link multiple times', async ({
  page,
}) => {
  await page.keyboard.press('Enter');
  ...
  const paragraph = page.locator('notesgraph-paragraph').nth(0);
  ...
  await page.waitForSelector('.notesgraph-link-popover');

  await pasteByKeyboard(page);

  await page.locator('.notesgraph-confirm-button').click();

  const scrollAnchoringWidget = page.locator(
    'notesgraph-scroll-anchoring-widget'
  );
...
test('should display date as the original title of journal', async ({
  ...
  const linkedDocTitle = toolbar.locator('notesgraph-linked-doc-title .label');
...
test('should add HTTP protocol into link automatically', async ({ page }) => {
  ...
  await page.keyboard.type('/');
  await page.keyboard.type('notesgraph');

  await page.keyboard.press('Space');

  const link = 'https://github.com/notesgraph/notesgraph';
  ...
  const linkPreview = toolbar.locator('notesgraph-link-preview');
  ...
  const cardLink = page.locator('notesgraph-bookmark');
...
test('should open link in new tab when middle clicking on link', async ({
  ...
  // external link
  {
    ...
    const input = page.locator('.notesgraph-link-popover-input');
    ...
    await page.locator('notesgraph-link').click({ button: 'middle' });
  ...
  // reference doc
  {
    ...
    await page.locator('notesgraph-reference').click({ button: 'middle' });
