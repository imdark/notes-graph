import type { EdgelessRootBlockComponent } from '@blocksuite/notesgraph/blocks/root';
import type { IVec } from '@blocksuite/notesgraph/global/gfx';
...
import { clickLocatorByRatio } from '@notesgraph-test/kit/utils/utils';
...
test.describe('edgeless page block', () => {
  ...
  test('should shrink note block when clicking on the toggle button', async ({
    ...
    const noteBox = await page
      .locator('notesgraph-edgeless-note')
      .boundingBox();
  ...
  test('info button should hidden in peek view', async ({ page }) => {
    ...
    const reference = page.locator('notesgraph-reference');
  ...
  test('caret on focusing', async ({ page }) => {
    ...
    const firstParagraph = note.locator('notesgraph-paragraph:first-child');
  ...
  test('page title should be editable', async ({ page }) => {
    ...
    const paragraphs = note.locator('notesgraph-paragraph v-line');
...
test.describe('edgeless note element toolbar', () => {
  ...
  test('the toolbar of note block should contains auto-height button and display in page button', async ({
    ...
    await createEdgelessNoteBlock(page, [100, 100]);
    await page.waitForSelector('.notesgraph-paragraph-placeholder.visible');
  ...
  test('display in page button', async ({ page }) => {
    ...
    const notes = editorContainer.locator('notesgraph-note');
    ...
    const toc = page.locator('notesgraph-outline-panel');
    await toc.waitFor({ state: 'visible' });
    const highlightNoteCards = toc.locator(
      'notesgraph-outline-note-card > [data-status="selected"]'
    );
  ...
  test('note edgeless styles', async ({ page }) => {
    ...
    await noteStylePanel.getByTestId('notesgraph-note-shadow-film').click();
    ...
    expect(await getNoteEdgelessProps(page, noteId)).toEqual({
      style: {
        ...
        borderSize: 4,
        borderStyle: 'none',
        shadowType: '--notesgraph-note-shadow-film',
      },
    });

    const borderStylePanel = noteStylePanel.getByTestId(
      'notesgraph-note-border-style-panel'
    );
    await borderStylePanel.locator('.mode-solid').click();
    await borderStylePanel.locator('notesgraph-slider').getByLabel('8').click();
    ...
    const cornerPanel = noteStylePanel.getByTestId(
      'notesgraph-note-corner-radius-panel'
    );
    await cornerPanel.locator('notesgraph-slider').getByLabel('24').click();
...
test('should convert note block to linked doc when clicking turn into linked doc button', async ({
  ...
  const syncedDoc = page.locator('notesgraph-embed-edgeless-synced-doc-block');
  await expect(syncedDoc).toBeVisible();

  const noteInSyncedDoc = syncedDoc.locator('notesgraph-note');
  await expect(noteInSyncedDoc).toBeVisible();

  const paragraphs = noteInSyncedDoc.locator('notesgraph-paragraph');
