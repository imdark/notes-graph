import type { CodeBlockComponent } from '@blocksuite/notesgraph-block-code';
import type { ParagraphBlockComponent } from '@blocksuite/notesgraph-block-paragraph';
import type { PageRootBlockComponent } from '@blocksuite/notesgraph-block-root';
import type { BlockComponent } from '@blocksuite/std';
import { test } from '@notesgraph-test/kit/playwright';
import { importFile } from '@notesgraph-test/kit/utils/attachment';
import { pasteContent } from '@notesgraph-test/kit/utils/clipboard';
import {
...
import { setSelection } from '@notesgraph-test/kit/utils/selection';
import { expect, type Page } from '@playwright/test';

const paragraphLocator = 'notesgraph-note notesgraph-paragraph';
const codeBlockLocator = 'notesgraph-note notesgraph-code';
...
test.describe('surface-ref block', () => {
  async function setupSurfaceRefBlock(page: Page) {
    ...
    const frameTitle = page.locator('notesgraph-frame-title');
  ...
  test('paste surface-ref block to another doc as embed-linked-doc block', async ({
    ...
    const surfaceRefBlock = page.locator('notesgraph-surface-ref');
    await surfaceRefBlock.click();
    await page.waitForSelector('notesgraph-surface-ref .focused');
    await copyByKeyboard(page);
    ...
    const embedLinkedDocBlock = page.locator(
...
test('should copy single image from edgeless and paste to page', async ({
  ...
  const image = page.locator('notesgraph-edgeless-image').first();
  await image.click();
  ...
  await expect(page.locator('notesgraph-page-image')).toBeVisible();
});
