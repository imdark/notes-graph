...
test.describe('split list', () => {
  test('split at the start of the list', async ({ page }) => {
    ...
    const listLocator = page.locator('notesgraph-list');
    ...
    await expect(
      listLocator.nth(0).locator('.notesgraph-list-block__numbered')
    ).toHaveText(['1.', 'a.']);
    await expect(listLocator.nth(2).locator('rich-text')).toHaveText('ccc');
    await expect(
      listLocator.nth(2).locator('.notesgraph-list-block__numbered')
    ).toHaveText('2.');
    await expect(listLocator.nth(3).locator('rich-text')).toHaveText('ddd');
    await expect(
      listLocator.nth(3).locator('.notesgraph-list-block__numbered')
    ).toHaveText('3.');
    ...
    await expect(
      listLocator.nth(1).locator('.notesgraph-list-block__numbered')
    ).toHaveText(['2.', 'a.']);
    ...
    await expect(
      listLocator.nth(4).locator('.notesgraph-list-block__numbered')
    ).toHaveText('4.');
  });
  ...
  test('tab in list should not propagate out of editor', async ({ page }) => {
    ...
    const list = page.locator('notesgraph-list');
    const card = page.locator('notesgraph-embed-linked-doc-block');
