...
import { importImage } from '@notesgraph-test/kit/utils/image';
import {
  selectAllByKeyboard,
...
test.describe('Formatting', () => {
  test('should change text color', async ({ page }) => {
    ...
    const toolbar = locateToolbar(page);
    const highlightButton = toolbar.locator(
    ...
    const fgColor = await fgGreenButton
      .locator('notesgraph-text-duotone-icon')
      .evaluate(e => window.getComputedStyle(e).getPropertyValue('--color'));

    const paragraph = page.locator('notesgraph-paragraph');
    const textSpan = paragraph
      .locator('notesgraph-text:has-text("rld")')
      .locator('span')
      .first();
...
test('should not show inner toolbar of surface-ref in note under edgeless', async ({
  ...
  const surfaceRef = page.locator('notesgraph-surface-ref');
  ...
  const edgelessSurfaceRef = note.locator('notesgraph-edgeless-surface-ref');
  await edgelessSurfaceRef.hover();

  await expect(toolbar).toBeHidden();

  const dragHandler = page.locator('.notesgraph-drag-handle-grabber');
  await dragHandler.hover();
  await dragHandler.click();

  await expect(
    edgelessSurfaceRef.locator('.notesgraph-edgeless-surface-ref-container')
  ).toHaveClass(/focused$/);
...
test('should show toolbar when inline link is preceded by image or surface-ref', async ({
  ...
  await importImage(page, 'notesgraph-preview.png');
...
test('should focus on input of popover on toolbar', async ({ page }) => {
  ...
  const note = page.locator('notesgraph-edgeless-note').first();
...
test.describe('Toolbar More Actions', () => {
  ...
  test('should duplicate block', async ({ page }) => {
    ...
    const images = page.locator('notesgraph-page-image');
