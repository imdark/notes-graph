import type { ParagraphBlockComponent } from '@blocksuite/notesgraph-block-paragraph';
import { test } from '@notesgraph-test/kit/playwright';
import { locateToolbar } from '@notesgraph-test/kit/utils/editor';
import {
  pressEnter,
  pressShiftTab,
  pressTab,
...
test('heading icon should be updated after change heading level', async ({
  ...
  const paragraph = page.locator('notesgraph-note notesgraph-paragraph').nth(0);
...
test('basic heading collapsed', async ({ page }) => {
  ...
  const a = page.locator('notesgraph-note notesgraph-paragraph').nth(0);
  const b = page.locator('notesgraph-note notesgraph-paragraph').nth(1);
  const c = page.locator('notesgraph-note notesgraph-paragraph').nth(2);
  const d = page.locator('notesgraph-note notesgraph-paragraph').nth(3);
  const e = page.locator('notesgraph-note notesgraph-paragraph').nth(4);
  const f = page.locator('notesgraph-note notesgraph-paragraph').nth(5);
  const g = page.locator('notesgraph-note notesgraph-paragraph').nth(6);
  const h = page.locator('notesgraph-note notesgraph-paragraph').nth(7);
  const i = page.locator('notesgraph-note notesgraph-paragraph').nth(8);
  const j = page.locator('notesgraph-note notesgraph-paragraph').nth(9);
...
test('add new heading when press enter at the end of collapsed heading', async ({
  ...
  const paragraph = page.locator('notesgraph-note notesgraph-paragraph');
...
test('also move children when dedent collapsed heading', async ({ page }) => {
  ...
  const paragraph = page.locator('notesgraph-note notesgraph-paragraph');
  const subParagraph = paragraph.nth(0).locator('notesgraph-paragraph');
...
test('also move collapsed siblings when indent collapsed heading', async ({
  ...
  const paragraph = page.locator('notesgraph-note notesgraph-paragraph');
  expect(await paragraph.nth(0).locator('notesgraph-paragraph').count()).toBe(
    0
  );
  await pressTab(page);
  expect(await paragraph.nth(0).locator('notesgraph-paragraph').count()).toBe(
...
test('unfold collapsed heading when its other blocks indented to be its sibling', async ({
  ...
  await expectParagraphVisibility(paragraph, 2, false);
  await expect
    .poll(() => paragraph.nth(0).locator('notesgraph-paragraph').count())
    .toBe(2);
  await pressTab(page);
  await expect
    .poll(() => paragraph.nth(0).locator('notesgraph-paragraph').count())
