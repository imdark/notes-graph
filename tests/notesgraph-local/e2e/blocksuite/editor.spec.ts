...
import { locateEditorContainer } from '@notesgraph-test/kit/utils/editor';
...
test('database is useable', async ({ page }) => {
  ...
  await page.keyboard.press('Enter');
  await addDatabase(page);
  const database2 = page.locator('notesgraph-database');
...
test('link page is useable', async ({ page }) => {
  ...
  await page.click('.notesgraph-reference');
  await page.waitForTimeout(500);
