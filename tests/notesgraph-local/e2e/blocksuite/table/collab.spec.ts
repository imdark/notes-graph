import {
  ...
  getCellText,
  inputToCell,
} from '@notesgraph-test/kit/bs/table';
...
test('should table collab work', async ({ page: pageA, context }) => {
  ...
  const cellsInA = pageA.locator('notesgraph-table-cell');
  ...
  const tableInB = pageB.locator('notesgraph-table');
  await expect(tableInB).toBeVisible();
  const cellsInB = tableInB.locator('notesgraph-table-cell');
