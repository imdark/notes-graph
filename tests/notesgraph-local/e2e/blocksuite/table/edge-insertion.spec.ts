...
import { createTable, getCellText } from '@notesgraph-test/kit/bs/table';
...
test.describe('Table edge case insertion operations', () => {
  test('should correctly insert at edges', async ({ page }) => {
    ...
    const table = page.locator('notesgraph-table');
    ...
    const cells = table.locator('notesgraph-table-cell');
    ...
    const menu = page.locator('notesgraph-menu');
    await expect(menu).toBeVisible();
    await menu.getByText('Insert Left').click();
    await waitNextFrame(page);

    // Verify: Now we should have 6 cells (3x2 table)
    const cellsAfterLeftEdgeInsert = table.locator('notesgraph-table-cell');
    ...
    const cellsAfterAboveEdgeInsert = table.locator('notesgraph-table-cell');
    ...
    const cellsAfterRightEdgeInsert = table.locator('notesgraph-table-cell');
    ...
    const cellsAfterBelowEdgeInsert = table.locator('notesgraph-table-cell');
