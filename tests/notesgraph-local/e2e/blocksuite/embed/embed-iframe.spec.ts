...
const TEST_NOTESGRAPH_URL = 'https://app.notesgraph.com/';

const EMBED_IFRAME_BLOCK = 'notesgraph-embed-iframe-block';
const EMBED_EDGELESS_IFRAME_BLOCK = 'notesgraph-embed-edgeless-iframe-block';
...
test.beforeEach(async ({ page }) => {
  ...
  await page.locator('notesgraph-paragraph v-line div').click();
});
...
test.describe('embed iframe block', () => {
  ...
  test('add bookmark when the url cannot be embedded', async ({ page }) => {
    ...
    await input.fill(TEST_NOTESGRAPH_URL);
    await input.press('Enter');
    ...
    const bookmarkBlock = page.locator('notesgraph-bookmark');
  ...
  test.describe('conversions', () => {
    ...
    test('embed iframe block to link text', async ({ page }) => {
      ...
      const notesgraphLink = page.locator('notesgraph-link');
      await expect(notesgraphLink).toBeVisible();

      // hover notesgraph link
      await notesgraphLink.hover();
      await page.waitForTimeout(100);
      ...
      await expect(notesgraphLink).toHaveCount(0);
    });
  ...
  test.describe('edgeless mode', () => {
    ...
    test('convert between embed view and card view', async ({ page }) => {
      ...
      // expect the bookmark block count is 1
      const bookmarkEdgelessBlock = page.locator(
