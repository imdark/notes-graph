...
import { getPagesCount } from '@notesgraph-test/kit/utils/filter';
...
} from '@notesgraph-test/kit/utils/workspace';
...
test.skip('create multi workspace in the workspace list', async ({
  ...
  await page.evaluate(async () => {
    await new Promise(resolve => {
      window.addEventListener('notesgraph:workspace:change', resolve, {
        once: true,
      });
    });
  });
