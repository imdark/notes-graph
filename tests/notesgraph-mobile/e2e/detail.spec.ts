import { test } from '@notesgraph-test/kit/mobile';
...
test.beforeEach(async ({ page }) => {
  ...
  await expect(page.locator('.notesgraph-page-viewport')).toBeVisible();
});
...
test('switch to edgeless mode', async ({ page }) => {
  ...
  await page
    ...
    .click();
  await expect(page.locator('.notesgraph-edgeless-viewport')).toBeVisible();
});
