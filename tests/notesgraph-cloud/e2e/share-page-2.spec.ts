...
import { importAttachment } from '@notesgraph-test/kit/utils/attachment';
import {
  createRandomUser,
...
import { copyByKeyboard } from '@notesgraph-test/kit/utils/keyboard';
import {
  clickNewPageButton,
...
test('The reference links in the shared page should be accessible normally and can go back and forward', async ({
  ...
  // check share page is accessible
  {
    ...
    // check linked page
    const link = page2.locator('.notesgraph-reference');
...
test('Inline latex modal should be not shown in shared mode when clicking', async ({
  ...
  const latexLocator = page.locator('notesgraph-latex-node');
  ...
  // check share page is accessible
  {
    ...
    // click the latex node
    const latexLocator = page2.locator('notesgraph-latex-node');
    await latexLocator.click();
...
test('should enable opening peek view with pdf viewer in readonly and sharing modes', async ({
  ...
  // check share page is accessible
  {
    ...
    const attachment = page2.locator('notesgraph-attachment');
