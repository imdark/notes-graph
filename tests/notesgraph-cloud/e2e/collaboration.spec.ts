import { Path, skipOnboarding, test } from '@notesgraph-test/kit/playwright';
import {
  addUserToWorkspace,
  createRandomUser,
...
import { clickUserInfoCard } from '@notesgraph-test/kit/utils/setting';
import { clickSideBarSettingButton } from '@notesgraph-test/kit/utils/sidebar';
...
test('can sync svg between different browsers', async ({ page, browser }) => {
  ...
  // get the image src under "notesgraph-image img"
  const src1 = await page.locator('notesgraph-image img').getAttribute('src');
...

  {
    ...
    // get the image src under "notesgraph-image img"
    const src2 = await page2
      .locator('notesgraph-image img')
      .getAttribute('src');
