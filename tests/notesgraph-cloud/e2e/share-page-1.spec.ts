import { skipOnboarding, test } from '@notesgraph-test/kit/playwright';
import {
  createRandomUser,
...
test('can enable share page', async ({ page, browser }) => {
  ...
  // check share page is accessible
  {
    ...
    await expect(page2.locator('notesgraph-paragraph').first()).toContainText(
      'TEST CONTENT'
    );
  }
});
...
test('share page should have toc', async ({ page, browser }) => {
  ...
  // check share page is accessible
  {
    ...
    const tocIndicators = page2.locator(
      'notesgraph-outline-viewer .outline-viewer-indicator'
    );
    ...
    const viewer = page2.locator('notesgraph-outline-viewer');
...
test('append paragraph should be disabled in shared mode', async ({
...

  {
    ...
    const paragraph = page2.locator('notesgraph-paragraph');
...
test('share page with default edgeless', async ({ page, browser }) => {
  ...
  await expect(page.locator('notesgraph-edgeless-root')).toBeVisible({
    timeout: 1000,
  });
  ...
  // check share page is accessible
  {
    ...
    await waitForEditorLoad(page2);
    await expect(page2.locator('notesgraph-edgeless-root')).toBeVisible({
      timeout: 1000,
    });
    await expect(page2.locator('notesgraph-paragraph').first()).toContainText(
      'TEST CONTENT'
...
test('image preview should be shown', async ({ page, browser }) => {
  ...
  // check share page is accessible
  {
    ...
    await page.locator('notesgraph-page-image').first().dblclick();
